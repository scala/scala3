...
object Build {
  ...
  def withCompilerClasspath(compilerProject: ProjectReference): Def.Initialize[Task[Seq[String]]] = Def.task {
    ...
    val asm = findArtifactPath(externalDeps, "scala-asm", conv)
  ...
  lazy val `scala3-nonbootstrapped` = project.in(file("."))
    ...
    .settings(
      ...
      buildQuick := Def.uncached {
        ...
        implicit val conv: FileConverter = fileConverter.value
        val cp = (`scala3-repl` / Compile / fullClasspath).value.map(f => toFile(f).getAbsolutePath).mkString(File.pathSeparator)
    ...
    .settings(
      ...
      Test / javaOptions ++= Def.uncached {
        ...
        Seq(
          ...
          s"-Ddotty.tests.classes.scalaAsm=${findArtifactPath(externalDeps, "scala-asm", conv)}",
