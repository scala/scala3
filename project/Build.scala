...
object Build {
  ...
  lazy val `stdlib-bootstrapped` = project.in(file("stdlib-bootstrapped")).
    ...
    settings(stdlibBootstrappedSettings)

  /** Scala 2 library compiled by dotty using the latest published sources of the library.
   *  This version of the library contains overrides that introduce capture checking into the library.
   *
   *  This version of the library is not (yet) TASTy/binary compatible with the Scala 2 compiled library (see stdlib-bootstrapped).
   */
  lazy val `stdlib-bootstrapped-cc` = project.in(file("stdlib-bootstrapped-cc")).
    withCommonSettings(Bootstrapped).
    dependsOn(dottyCompiler(Bootstrapped) % "provided; compile->runtime; test->test").
    settings(commonBootstrappedSettings).
    settings(stdlibBootstrappedSettings).
    settings(
      scalacOptions -= "-Ycheck:all", // checked in stdlib-bootstrapped
      Compile / packageBin / mappings ~= { _.filterNot(_._1.getName.endsWith(".class")) },
    )

  lazy val stdlibBootstrappedSettings =
    Seq(
      ...
      scalacOptions += "-Ycheck:all", // To check generated code with -Yscala2-stdlib
      ...
      tastyMiMaReportIssues := tastyMiMaReportIssues.dependsOn(Def.task {
        ...
        streams.value.log.warn(
          s"""To allow TASTy-MiMa to read TASTy files generated by this vesion of the compile you must:
             ...
             | * Run $projectName/tastyMiMaReportIssues
             |""".stripMargin)
      }).value,
          ...
          case _ =>
            ...
            println(
              s"""Usage:
                ...
                |  -- lists all files that are not overriden in $projectName/src
                |
                |> $projectName/run clone <sources>*
                |  -- clones the specified sources from the $projectName/src
                |  -- example: $projectName/run clone scala/Option.scala
                |
                |> $projectName/run overwrite <sources>*
                |  -- (danger) overwrites the specified sources from the $projectName/src
                |""".stripMargin)
        }
      }
    )
  ...
  lazy val `stdlib-bootstrapped-tasty-tests` = project.in(file("stdlib-bootstrapped-tasty-tests")).
    ...
    settings(commonBootstrappedSettings).
    settings(stdlibBootstrappedTestsSettings)

  lazy val `stdlib-bootstrapped-cc-tests` = project.in(file("stdlib-bootstrapped-cc-tests")).
    withCommonSettings(Bootstrapped).
    dependsOn(dottyCompiler(Bootstrapped) % "compile->compile").
    dependsOn(`scala3-tasty-inspector` % "test->test").
    dependsOn(`stdlib-bootstrapped-cc`).
    settings(commonBootstrappedSettings).
    settings(stdlibBootstrappedTestsSettings)

  lazy val stdlibBootstrappedTestsSettings =
    Seq(
      javaOptions := (`scala3-compiler-bootstrapped` / javaOptions).value,
      ...
      Compile / compile / dependencyClasspath := {
        // make sure that the scala2-library (tasty of `stdlib-bootstrapped-tasty` or `stdlib-bootstrapped-cc`) is listed before the scala-library (classfiles)
