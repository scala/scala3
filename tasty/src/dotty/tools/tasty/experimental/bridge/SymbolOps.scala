...
trait SymbolOps extends Core with
  ...
  given SymbolOps: (sym: Symbol) with
    ...
    def owner: Symbol = internal.Symbol_owner(sym)
    ...
    def fullName(given Context): sym.ThisName = internal.Symbol_fullName(sym)
    ...
    def exists: Boolean = internal.Symbol_exists(sym)
