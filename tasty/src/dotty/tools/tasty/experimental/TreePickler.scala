...
class TreePickler[T <: Tasty](val tasty: T)(val pickler: TastyPickler[tasty.type]) {
  ...
  private def pickleNameAndSig(name: Name, sig: Signature): Unit =
    pickleName(if sig.isNotAMethod then name else SignedName(name.toTermName, sig))
  ...
  def pickleConstant(c: Constant)(implicit ctx: Context): Unit = c.tag match {
    ...
    case ByteTag =>
      writeByte(BYTEconst)
      writeInt(c.byteValue.toInt)
    case ShortTag =>
      writeByte(SHORTconst)
      writeInt(c.shortValue.toInt)
    case CharTag =>
      writeByte(CHARconst)
      writeNat(c.charValue.toInt)
    case IntTag =>
      writeByte(INTconst)
      writeInt(c.intValue)
    case LongTag =>
  ...
  private def pickleNewType(tpe: Type, richTypes: Boolean)(implicit ctx: Context): Unit = tpe match {
    ...
    case tpe: NamedType =>
      ...
      def pickleExternalRef(sym: Symbol) = {
        ...
        if (sym.isPrivate || isShadowedRef) {
    ...
    case tpe: ThisType =>
      val cls = tpe.cls
      if (cls.isPackage && !cls.isEffectiveRoot) {
        writeByte(TERMREFpkg)
        pickleName(cls.fullName)
      }
      else {
        writeByte(THIS)
        pickleType(tpe.tref)
    ...
    if (addr != currentAddr) {
      ...
      try tree match {
          ...
          if (qual.isEmpty) pickleType(tree.tpe)
            ...
            tree.tpe match {
              case thistpe: ThisType =>
                pickleTree(qual.withType(thistpe.tref))
              case _ =>
            }
          }
        ...
        case Literal(const1) =>
          pickleConstant {
            tree.tpe match {
              case tpe: ConstantType => tpe.value
              case _ => const1
            }
          }
        case Super(qual, mix) =>
          ...
          withLength {
            ...
            if (!mix.isEmpty) {
              tree.tpe match {
                case tpe: SuperType => tpe.supertpe match {
                  case mixinType: TypeRef =>
        ...
        case tree: Template =>
          registerDef(tree.symbol)
          writeByte(TEMPLATE)
          val (params, rest) = tree.decomposeBody
          withLength {
            pickleParams(params)
            tree.parents.foreach(pickleTree)
            tree.symbol.owner.info match {
