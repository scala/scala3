  ...
  def apply(
    ...
    val precomputedSourcePackages = sourcePathMode match
      ...
      case SourcePathMode.PRUNED | SourcePathMode.FULL =>
        val sourcePathFiles = sourcePath.get().nn.asScala.toSeq
        val logicalSourcePath = sourcePathFiles.mkString(File.pathSeparator.nn)
      ...
      case SourcePathMode.MBT =>
        Some(ParsedLogicalPackage.fromMbtIndex(semanticdbFileManager.listAllPackages().nn))
