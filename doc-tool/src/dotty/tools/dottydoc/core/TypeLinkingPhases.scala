...
class LinkReturnTypes extends DocMiniPhase with TypeLinker {
  override def transformDef(implicit ctx: ContextRenamed) = { case df: DefImpl =>
    val returnValue = linkReference(df, df.returnValue, ctx.docbase.packages)
    df.copy(returnValue = returnValue) :: Nil
  }

  override def transformVal(implicit ctx: ContextRenamed) = { case vl: ValImpl =>
    val returnValue = linkReference(vl, vl.returnValue, ctx.docbase.packages)
    vl.copy(returnValue = returnValue) :: Nil
  }

  override def transformTypeAlias(implicit ctx: ContextRenamed) = { case ta: TypeAliasImpl =>
    ta.alias.map { alias =>
...
class LinkSuperTypes extends DocMiniPhase with TypeLinker {
  def linkSuperTypes(ent: Entity with SuperTypes)(implicit ctx: ContextRenamed): List[MaterializableLink] =
  ...
  override def transformTrait(implicit ctx: ContextRenamed) = { case trt: TraitImpl =>
    trt.copy(superTypes = linkSuperTypes(trt)) :: Nil
  }

  override def transformObject(implicit ctx: ContextRenamed) = { case obj: ObjectImpl =>
    obj.copy(superTypes = linkSuperTypes(obj)) :: Nil
  }
}

class LinkImplicitlyAddedTypes extends DocMiniPhase with TypeLinker {
  override def transformDef(implicit ctx: ContextRenamed) = {
  ...
  override def transformVal(implicit ctx: ContextRenamed) = {
