...
object tags {
  ...
  sealed trait ParamConverter {
    ...
    def baseurl(implicit ctx: ContextRenamed): String = {
      if (_baseurl eq null) {
        _baseurl =
  ...
  /** Renders a `MaterializableLink` into a HTML anchor tag. If the link is
    ...
    */
  final case class RenderLink(params: Map[String, AnyRef])(implicit ctx: ContextRenamed)
  ...
  private[this] def renderLink(baseurl: String, link: MaterializableLink)(implicit ctx: ContextRenamed): String =
    link match {
  ...
  final case class RenderReference(params: Map[String, AnyRef])(implicit ctx: ContextRenamed)
  ...
  case class ResourceInclude(params: Map[String, AnyRef], includes: Map[String, Include])(implicit ctx: ContextRenamed)
  extends Tag("include") {
  ...
  /** Can be used to render the `sidebar.yml` entries, represented here as
    ...
    */
  case class RenderTitle(params: Map[String, AnyRef])(implicit ctx: ContextRenamed)
