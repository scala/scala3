...
The following pages introduce a redesign of contextual abstractions in Scala. They introduce four fundamental changes:
 ...
 3. [Default Imports](./default-imports.md) are a new class of imports that specifically import defaults and nothing else. Defaults _must be_ imported with defaults imports, a plain import will no longer bring them into scope.

 4. [Implicit Conversions](./conversions.md) are now expressed as defaults of a standard `Conversion` class. All other forms of implicit conversions will be phased out.
