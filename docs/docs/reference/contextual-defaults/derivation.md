...
Type class derivation is a way to automatically generate default instances for type classes which satisfy some simple
...
The `derives` clause generates the following default instances for the `Eq`, `Ordering` and `Show` type classes in the
companion object of `Tree`,

```scala
default [T: Eq]       for Eq[Tree[T]]    = Eq.derived
default [T: Ordering] for Ordering[Tree] = Ordering.derived
default [T: Show]     for Show[Tree]     = Show.derived
```
...
All data types can have a `derives` clause. This document focuses primarily on data types which also have a default
of the `Mirror` type class available. Defaults of the `Mirror` type class are generated automatically by the compiler
for,
...
That is, the `derived` method takes an implicit parameter of (some subtype of) type `Mirror` which defines the shape of
...
we need to implement a method `Eq.derived` on the companion object of `Eq` that produces a default for `Eq[T]` given
a `Mirror[T]`. Here is a possible implementation,

```scala
inline default derived[T](given m: Mirror.Of[T]) for Eq[T] = {
...
Note that `derived` is defined as an `inline` default. This means that the `derived` method will be expanded at
...
```scala
default derived$Eq[T] for (eqT: Eq[T]) => Eq[Opt[T]] =
...
```scala
default eqSum[A](given inst: => K0.CoproductInstances[Eq, A]) for Eq[A] {
  def eqv(x: A, y: A): Boolean = inst.fold2(x, y)(false)(
    [t] => (eqt: Eq[t], t0: t, t1: t) => eqt.eqv(t0, t1)
  )
}

default eqProduct[A](given inst: K0.ProductInstances[Eq, A]) for Eq[A] {
...
```scala
default [T: Ordering] for Ordering[Option[T]] = Ordering.derived
```
