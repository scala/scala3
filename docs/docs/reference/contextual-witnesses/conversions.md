...
Implicit conversions are defined by witnesses of the `scala.Conversion` class.
...
```scala
witness of Conversion[String, Token] {
  def apply(str: String): Token = new KeyWord(str)
}
```
Using an alias this can be expressed more concisely as:
```scala
witness of Conversion[String, Token] = new KeyWord(_)
```
...
In the first case, the compiler looks for a `scala.Conversion` witness that maps
an argument of type `T` to type `S`. In the second and third
case, it looks for a `scala.Conversion` witness that maps an argument of type `T`
to a type that defines a member `m` which can be applied to `args` if present.
If such a witness `C` is found, the expression `e` is replaced by `C.apply(e)`.
...
```scala
witness int2Integer of Conversion[Int, java.lang.Integer] =
...
object Completions {
  ...
  object CompletionArg {
    ...
    witness fromString     of Conversion[String, CompletionArg]               = Error(_)
    witness fromFuture     of Conversion[Future[HttpResponse], CompletionArg] = Response(_)
    witness fromStatusCode of Conversion[Future[StatusCode], CompletionArg]   = Status(_)
  }
