...
For example, with the [witnesses](./witnesses.md) defined previously,
...
witness ctx  of global.Context { type Symbol = ...; type Kind = ... }
witness sym  of ctx.Symbol
witness kind of ctx.Kind
```
...
The method `summon` in `Predef` returns the witness of a specific type. For example,
the witness of `Ord[List[Int]]` is produced by
```scala
