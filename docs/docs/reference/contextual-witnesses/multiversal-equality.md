...
class T derives Eql
```
Alternatively, one can also provide an `Eql` witness directly, like this:
```scala
witness of Eql[T, T] = Eql.derived
```
...
One can have several `Eql` witnesses for a type. For example, the four
definitions below make values of type `A` and type `B` comparable with
each other, but not comparable to anything else:

```scala
witness of Eql[A, A] = Eql.derived
witness of Eql[B, B] = Eql.derived
witness of Eql[A, B] = Eql.derived
witness of Eql[B, A] = Eql.derived
```
The `scala.Eql` object defines a number of `Eql` witnesses that together
...
over all types that do not themselves have an `Eql` witness.  `eqlAny` is defined as follows:

```scala
def eqlAny[L, R]: Eql[L, R] = Eql.derived
```

Even though `eqlAny` is not declared a witness, the compiler will still construct an `eqlAny` instance as answer to an implicit search for the
type `Eql[L, R]`, unless `L` or `R` have `Eql` witnesses
...
## Deriving Eql Witnesses

Instead of defining `Eql` witnesses directly, it is often more convenient to derive them. Example:
```scala
class Box[T](x: T) derives Eql
```
By the usual rules of [typeclass derivation](./derivation.md),
this generates the following `Eql` witness in the companion object of `Box`:
```scala
witness [T, U](given Eql[T, U]) of Eql[Box[T], Box[U]] = Eql.derived
```
...
is legal if there is a witness for `Eql[T, U]`.
...
also legal if
 ...
 3. neither `T` nor `U` have a _reflexive_ `Eql` witness.
...
Explanations:
 ...
 - a type `T` has a _reflexive_ `Eql` witness if the implicit search for `Eql[T, T]`
   succeeds.

## Predefined Eql Instances

The `Eql` object defines witnesses for comparing
...
Witnesses are defined so that every one of these types has a _reflexive_ `Eql` witness, and the following holds:
...
unusable for all types that have not yet declared an `Eql1` witness, including all
