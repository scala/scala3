...
The following pages introduce a redesign of contextual abstractions in Scala. They introduce four fundamental changes:
 ...
 3. [Delegate Imports](./import-implied.html) are new form of import that specifically imports delegates and nothing else. Delegates _must be_ imported with  `import delegate`, a plain import will no longer bring them into scope.

 4. [Implicit Conversions](./conversions.html) are now expressed as delegates for a standard `Conversion` class. All other forms of implicit conversions will be phased out.
...
This section also contains pages describing other language features that are related to context abstraction. These are:
 ...
 - [Relationship with Scala 2 Implicits](./relationship-implicits.html) discusses the relationship between old-style implicits and
 new-style delegates and given clauses and how to migrate from one to the other.
