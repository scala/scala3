...
e.g. `A$$foo`, `B$$foo` and the `override def foo` in `C` will delegate to one of these methods. 
Super calls to non-overridden methods are also supported.
...
### When does trait inlining happen?
Trait inlining must happen before pickling to get its benefit across compilation units. 
Otherwise with the following under separate compilation we will induce boxing.
...
```
 ...
 This happens because when `B.scala` is compiled separately against the interface of `A` (derived from the pickled A.tasty) 
 it will appear that `A` only supports the generic `T` interface (erasing to `Object` and so boxed), 
 whereas it actually also has a specialized `Int` interface from inline trait inlining. 
 If instead, trait inlining occurs before pickling, the generated interface is present in the pickle. 
 There is precedent for inlining before pickling with transparent inlines.
 In addition, because inline traits are optional, code duplication should be expected alongside an increase in tasty file.
...
| Self types                          | ✅                                           |
| Inheritance (of inline traits)      | ✳️                                           |
| Opaque types                        | ✅                                           |

✳️ Only allowed by classes, inline traits, traits if the inline trait has no parameters
...
### Non-local private members
Non-local private members members which are labelled private and accessed from within the class on other instances of the class.
Local private members (members with the same access patterns as the former `private[this]`) are allowed.

### Inner classes
Inner classes and traits are supported with same behaviour as in normal traits. In particular, the following is completely fine, and will be inlined into B.
```scala
inline trait A[T](val x: T):
...
We also need to replace members accessed on inline receivers with the corresponding inlined symbols, 
(see member reference replacements above). 
We opt however to do this in erasure instead of its own phase because it needs to be coordinated with erasure of specialized traits with the following constraints:
...
removing their definitions and handling name mangling of private members. 
