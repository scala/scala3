...
object Test:
  ...
  def f2[T](x: T)(using erased s: Singleton { type Self = T}): Wrap[T] = Wrap(x)
  val x2 = f2(1)
  val _: Wrap[1] = x2
  ...
  def f4[T](x: T)(using erased s: T is Singleton): Wrap[T] = Wrap(x)
  val x4 = f4(1)
  val _: Wrap[1] = x4
  ...
  class C2[T](x: T)(using erased s: T is Singleton):
    def fld: T = x
  val y2 = C2("hi")
  val _: "hi" = y2.fld
