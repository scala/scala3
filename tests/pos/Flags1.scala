...
object Flags {
  ...
  delegate FlagOps {
    ...
    def (x: FlagSet)
        & (y: FlagSet): FlagSet = FlagSet(x.bits & y.bits)
     ...
     */
    def (x: FlagSet)
        is(flag: Flag, butNot: FlagSet): Boolean = x.is(flag) && !x.isOneOf(butNot)
    ...
    */
   def (x: FlagSet)
       isOneOf(flags: FlagSet, butNot: FlagSet): Boolean = x.isOneOf(flags) && !x.isOneOf(butNot)
     ...
     */
    def (x: FlagSet)
        isAllOf(flags: FlagSet, butNot: FlagSet): Boolean = x.isAllOf(flags) && !x.isOneOf(butNot)
    ...
    def (x: FlagSet)
        toTypeFlags: FlagSet = if (x.bits == 0) x else FlagSet(x.bits & ~KINDFLAGS | TYPES)

    /** The given flag set with all flags transposed to be term flags */
    def (x: FlagSet)
        toTermFlags: FlagSet = if (x.bits == 0) x else FlagSet(x.bits & ~KINDFLAGS | TERMS)

    /** The given flag set with all flags transposed to be common flags */
    def (x: FlagSet)
        toCommonFlags: FlagSet = if (x.bits == 0) x else FlagSet(x.bits | KINDFLAGS)

    /** The number of non-kind flags in the given flag set */
    def (x: FlagSet)
        numFlags: Int = java.lang.Long.bitCount(x.bits & ~KINDFLAGS)

    /** The lowest non-kind bit set in the given flag set */
    def (x: FlagSet)
        firstBit: Int = java.lang.Long.numberOfTrailingZeros(x.bits & ~KINDFLAGS)

    /** The  list of non-empty names of flags with given index idx that are set in the given flag set */
    private def (x: FlagSet)
        flagString(idx: Int): List[String] =
    ...
    def (x: FlagSet)
        flagStrings(privateWithin: String): Seq[String] = {
    ...
    def (x: FlagSet)
        flagsString: String = x.flagStrings("").mkString(" ")
  }
