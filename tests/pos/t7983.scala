...
class DivergenceTest {
  ...
  implicit final def columnBaseShape[Level >: Flat <: ShapeLevel, T, C <: ColumnBase[?]]
  ...
  implicit final def tuple2Shape[Level <: ShapeLevel, M1,M2, U1,U2]
                                (implicit u1: Shape2[? <: Level, M1, U1],
                                          u2: Shape2[? <: Level, M2, U2]
                                ): Shape2[Level, (M1,M2), (U1,U2)] = ???

  def foo: Unit = {
    class Coffees extends ColumnBase[Int]

    def map1[F, T](f: F)(implicit shape: Shape2[? <: Flat, F, T]) = ???
