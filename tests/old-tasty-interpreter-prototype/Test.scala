...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val allTastyFiles = dotty.tools.io.Path(classpath).walkFilter(_.ext.toLowerCase == "tasty").map(_.toString).toList
...
//    compileAndInterpret("traitInit.scala")
  ...
  def compileAndInterpret(testFileName: String) = {
    ...
    // Artefact of the current test infrastructure
    // TODO improve infrastructure to avoid needing this code on each test
    val allTastyFiles = dotty.tools.io.Path(out).walkFilter(_.ext.toLowerCase == "tasty").map(_.toString).toList
