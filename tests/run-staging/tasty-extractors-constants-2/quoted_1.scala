...
object Macros {
  ...
  def impl(using s: Scope): s.Expr[Unit] = {
    given Toolbox = Toolbox.make(getClass.getClassLoader)
    // 2 is a lifted constant
    val show1 = usingNewScope(power('{2}, '{3.0}).show)
    val run1  = run(power('{2}, '{3.0}))

    // n is a lifted constant
    val n = 2
    val show2 = usingNewScope(power(Expr(n), '{4.0}).show)
    val run2  = run(power(Expr(n), '{4.0}))

    // n is a constant in a quote
    val show3 = usingNewScope(power('{2}, '{5.0}).show)
    val run3  = run(power('{2}, '{5.0}))

    // n2 is not a constant
    def n2(using s: Scope) = '{ println("foo"); 2 }
    val show4 = usingNewScope(power(n2, '{6.0}).show)
  ...
  def power(using s: Scope)(n: s.Expr[Int], x: s.Expr[Double]): s.Expr[Double] = {
    n.unlift match {
