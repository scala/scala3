...
object Test {
  ...
  def main(args: Array[String]): Unit = withQuoteContext {
    ...
    implicit def ValueOfExprInt: ValueOfExpr[Int] = new {
      def apply(n: Expr[Int]) with QuoteContext : Option[Int] = n match {
        case '{ 0 } => Some(0)
        case '{ 1 } => Some(1)
        case '{ 2 } => Some(1)
        case _ => None
    ...
    implicit def ValueOfExprBoolean: ValueOfExpr[Boolean] = new ValueOfExpr[Boolean] {
      def apply(b: Expr[Boolean]) with QuoteContext : Option[Boolean] = b match {
        case '{ true } => Some(true)
    ...
    implicit def ValueOfExprList[T: ValueOfExpr: Type]: ValueOfExpr[List[T]] = new {
      def apply(xs: Expr[List[T]]) with QuoteContext : Option[List[T]] = (xs: Expr[Any]) match {
        case '{ ($xs1: List[T]).::($x) } =>
    ...
    implicit def ValueOfExprOption[T: ValueOfExpr: Type]: ValueOfExpr[Option[T]] = new {
      def apply(expr: Expr[Option[T]]) with QuoteContext : Option[Option[T]] = expr match {
