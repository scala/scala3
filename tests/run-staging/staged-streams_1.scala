...
object Test {
  ...
  case class Stream[A: Type](stream: StagedStream[Expr[A]]) {
    ...
    /** Selects all elements of this stream which satisfy a predicate.
      ...
      *             predicate `pred`.
      */
    def filter(pred: (Expr[A] => Expr[Boolean])) with QuoteContext : Stream[A] = {
    ...
    /** The nested stream receives the same variable reference; thus all streams decrement the same global count.
      ...
      */
    private def takeRaw[A: Type](n: Expr[Int], stream: StagedStream[A]) with QuoteContext : StagedStream[A] = {
      stream match {
    ...
    def take(n: Expr[Int]) with QuoteContext : Stream[A] = Stream(takeRaw[Expr[A]](n, stream))

    private def zipRaw[A: Type, B: Type](stream1: StagedStream[Expr[A]], stream2: StagedStream[B]) with QuoteContext : StagedStream[(Expr[A], B)] = {
    ...
    /** Make a stream linear
      ...
      * @return
      */
    private def makeLinear[A: Type](stream: StagedStream[Expr[A]]) with QuoteContext : Producer[Expr[A]] = {
      stream match {
    ...
    private def pushLinear[A, B, C](producer: Producer[A], nestedProducer: Producer[B], nestedf: (B => StagedStream[C])) with QuoteContext : StagedStream[(A, C)] = {
    ...
    /** zip **/
    def zip[B: Type, C: Type](f: (Expr[A] => Expr[B] => Expr[C]), stream2: Stream[B]) with QuoteContext : Stream[C] = {
  ...
  object Stream {
    def of[A: Type](arr: Expr[Array[A]]) with QuoteContext : Stream[A] = {
