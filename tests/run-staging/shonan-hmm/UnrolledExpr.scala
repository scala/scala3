...
object UnrolledExpr {
  ...
  def block[T: Type](stats: Iterable[Expr[_]], expr: Expr[T]) with QuoteContext : Expr[T] = {
...
class UnrolledExpr[T: Liftable, It <: Iterable[T]](xs: It) {
  ...
  def foreach[U](f: T => Expr[U]) with QuoteContext : Expr[Unit] = block(xs.map(f), '{})

  def withFilter(f: T => Boolean) with QuoteContext : UnrolledExpr[T, Iterable[T]] = new UnrolledExpr(xs.filter(f))

  def foldLeft[U](acc: Expr[U])(f: (Expr[U], T) => Expr[U]) with QuoteContext : Expr[U] =
