...
class RingIntExpr with QuoteContext extends Ring[Expr[Int]] {
  val zero = '{0}
  val one  = '{1}
  val add  = (x, y) => '{$x + $y}
  val sub  = (x, y) => '{$x - $y}
...
case class RingPV[U: Liftable](staRing: Ring[U], dynRing: Ring[Expr[U]]) with QuoteContext extends Ring[PV[U]] {
  type T = PV[U]
...
class RingIntPExpr with QuoteContext extends RingPV(RingInt, new RingIntExpr)

class RingIntOPExpr with QuoteContext extends RingIntPExpr {
