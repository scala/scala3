...
object Lifters {
  implicit def LiftedClassTag[T: Type: ClassTag] with QuoteContext : Expr[ClassTag[T]] = {
  ...
  private def initArray[T : Liftable : Type](arr: Array[T], array: Expr[Array[T]]) with QuoteContext : Expr[Array[T]] = {
    UnrolledExpr.block(
