...
package liftable {
  ...
  object Lets {
    def letVal[T, U: Staged](expr: Expr[T])(body: Expr[T] => Expr[U])(implicit t: Staged[T], qctx: QuoteContext): Expr[U] =
      '{ val letVal: $t = $expr; ${ body('letVal) } }
    def letLazyVal[T, U: Staged](expr: Expr[T])(body: Expr[T] => Expr[U])(implicit t: Staged[T], qctx: QuoteContext): Expr[U] =
      '{ lazy val letLazyVal: $t = $expr; ${ body('letLazyVal) } }
    def letDef[T, U: Staged](expr: Expr[T])(body: Expr[T] => Expr[U])(implicit t: Staged[T], qctx: QuoteContext): Expr[U] =
  ...
  object Lists {
    ...
    implicit class LiftedOps[T: Liftable](list: Expr[List[T]])(implicit t: Staged[T]) {
      def foldLeft[U](acc: Expr[U])(f: Expr[(U, T) => U])(implicit u: Staged[U], qctx: QuoteContext): Expr[U] =
        '{ ($list).foldLeft[$u]($acc)($f) }
      def foreach(f: Expr[T => Unit])(using QuoteContext): Expr[Unit] =
        '{ ($list).foreach($f) }
    }

    implicit class UnrolledOps[T: Liftable](list: List[T])(implicit t: Staged[T], qctx: QuoteContext) {
      def unrolledFoldLeft[U](acc: Expr[U])(f: Expr[(U, T) => U])(implicit u: Staged[U]): Expr[U] = list match {
