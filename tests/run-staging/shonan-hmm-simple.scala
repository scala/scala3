...
sealed trait PV[T]
  def expr with (Liftable[T], QuoteContext) : Expr[T]

case class Sta[T](x: T) extends PV[T]
  def expr with (Liftable[T], QuoteContext) : Expr[T] = x

case class Dyn[T](x: Expr[T]) extends PV[T]
  def expr with (Liftable[T], QuoteContext) : Expr[T] = x

class RingPV[U: Liftable](u: Ring[U], eu: Ring[Expr[U]]) with QuoteContext extends Ring[PV[U]] {
...
class ExprVecOps[T: Type] with QuoteContext extends VecOps[Expr[Int], Expr[T]]
...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def blasStaticIntExpr with QuoteContext = new Blas1(new RingIntExpr, new StaticVecOps)
    def resCode1 with QuoteContext = blasStaticIntExpr.dot(
      vec1.map(Expr(_)),
      vec2.map(Expr(_))
    )
    ...
    def blasExprIntExpr with QuoteContext = new Blas1(new RingIntExpr, new ExprVecOps)
    def resCode2 with QuoteContext : Expr[(Array[Int], Array[Int]) => Int] = '{
      (arr1, arr2) =>
    ...
    def blasStaticIntPVExpr with QuoteContext = new Blas1(new RingPV[Int](new RingInt, new RingIntExpr), new StaticVecOps)
    def resCode3 with QuoteContext = blasStaticIntPVExpr.dot(
      vec1.map(i => Dyn(i)),
      vec2.map(i => Sta(i))
    ).expr
    ...
    def blasExprIntPVExpr with QuoteContext = new Blas1(new RingPV[Int](new RingInt, new RingIntExpr), new StaticVecOps)
    def resCode4 with QuoteContext : Expr[Array[Int] => Int] = '{
      arr =>
    ...
    def blasExprComplexPVInt with QuoteContext = new Blas1[Int, Complex[PV[Int]]](new RingComplex(new RingPV[Int](new RingInt, new RingIntExpr)), new StaticVecOps)
    def resCode5 with QuoteContext : Expr[Array[Complex[Int]] => Complex[Int]] = '{
      arr =>
    ...
    def RingPVInt with QuoteContext = new RingPV[Int](new RingInt, new RingIntExpr)
    // Staged loop of dot product on vectors of Int or Expr[Int]
    def dotIntOptExpr with QuoteContext = new Blas1(RingPVInt, new StaticVecOps).dot
    // will generate the code '{ ((arr: scala.Array[scala.Int]) => arr.apply(1).+(arr.apply(3))) }
    def staticVec with QuoteContext = Vec[Int, PV[Int]](5, i => Sta((i % 2)))
    def code with QuoteContext = '{(arr: Array[Int]) => ${dotIntOptExpr(Vec(5, i => Dyn('{arr(${i})})), staticVec).expr} }
