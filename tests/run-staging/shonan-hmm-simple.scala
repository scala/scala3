...
class RingPV[U: Liftable](u: Ring[U], eu: Ring[Expr[U]])(using QuoteContext) extends Ring[PV[U]]:
...
object Test:
  ...
  def main(args: Array[String]): Unit =
    ...
    def blasStaticIntExpr(using QuoteContext) = new Blas1(new RingIntExpr, new StaticVecOps)
    def resCode1(using QuoteContext) = blasStaticIntExpr.dot(
      vec1.map(Expr(_)),
      vec2.map(Expr(_))
    )
    ...
    def blasExprIntExpr(using QuoteContext) = new Blas1(new RingIntExpr, new ExprVecOps)
    def resCode2(using QuoteContext): Expr[(Array[Int], Array[Int]) => Int] = '{
      (arr1, arr2) =>
    ...
    def RingPVInt(using QuoteContext) = new RingPV[Int](new RingInt, new RingIntExpr)
