...
object Test {
  ...
  def compile(e: Exp, env: Map[String, Expr[Int]], keepLets: Boolean) with QuoteContext : Expr[Int] = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def res1 with QuoteContext = '{ (x: Int) => ${compile(exp, Map("x" -> 'x), false)} }
    ...
    def res2 with QuoteContext = compile(letExp, Map(), false)
    println(withQuoteContext(res2.show))
    println(run(res2))

    println("---")

    def res3 with QuoteContext = compile(letExp, Map(), true)
