...
{ self =>
  ...
  protected override def newPrepended[B >: A](that: scala.collection.GenTraversable[B]): TransformedM[B] = new AbstractTransformedM[B] with PrependedM[B] { lazy protected[this] val fst = that }
