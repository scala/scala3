...
/**
  ...
  */
private[impl] final class CompletionLatch[sealed T] extends AbstractQueuedSynchronizer with (Try[T] -> Unit) {
...
private[concurrent] object Promise {
  ...
  class DefaultPromise[sealed T] private[this] (initial: AnyRef) extends AtomicReference[AnyRef](initial) with scala.concurrent.Promise[T] with scala.concurrent.Future[T] with (Try[T] -> Unit) { this: DefaultPromise[T]^ =>
    /**
    ...
    override final def transform[S](f: Try[T] => Try[S])(implicit executor: ExecutionContext): Future[S]^{this, f} =
      dispatchOrAddCallbacks(get(), new Transformation[T, S @uncheckedCaptures](Xform_transform, f, executor))

    override final def transformWith[S](f: Try[T] => Future[S]^)(implicit executor: ExecutionContext): Future[S]^{this, f} =
      dispatchOrAddCallbacks(get(), new Transformation[T, S @uncheckedCaptures](Xform_transformWith, f, executor))

    override final def zipWith[sealed U, sealed R](that: Future[U]^)(f: (T, U) => R)(implicit executor: ExecutionContext): Future[R]^{this, that, f} = {
      val state = get()
    ...
    override final def flatMap[S](f: T => Future[S]^)(implicit executor: ExecutionContext): Future[S]^{this, f} = {
      val state = get()
      if (!state.isInstanceOf[Failure[_]]) dispatchOrAddCallbacks(state, new Transformation[T, S @uncheckedCaptures](Xform_flatMap, f, executor))
      else this.asInstanceOf[Future[S]]
    }

    override final def map[S](f: T => S)(implicit executor: ExecutionContext): Future[S]^{this, f} = {
      val state = get()
      if (!state.isInstanceOf[Failure[_]]) dispatchOrAddCallbacks(state, new Transformation[T, S @uncheckedCaptures](Xform_map, f, executor))
      else this.asInstanceOf[Future[S]]
    }
    ...
    override final def collect[S](pf: PartialFunction[T, S]^)(implicit executor: ExecutionContext): Future[S]^{this, pf} = {
      val state = get()
      if (!state.isInstanceOf[Failure[_]]) dispatchOrAddCallbacks(state, new Transformation[T, S @uncheckedCaptures](Xform_collect, pf, executor)) // Short-circuit if we get a Success
      else this.asInstanceOf[Future[S]]
    }
    ...
    override final def recoverWith[U >: T](pf: PartialFunction[Throwable, Future[U]^]^)(implicit executor: ExecutionContext): Future[U]^{this, pf} = {
      val state = get()
      if (!state.isInstanceOf[Success[_]]) dispatchOrAddCallbacks(state, new Transformation[T, U @uncheckedCaptures](Xform_recoverWith, pf, executor)) // Short-circuit if we get a Failure
    ...
    override final def recover[U >: T](pf: PartialFunction[Throwable, U]^)(implicit executor: ExecutionContext): Future[U]^{this, pf} = {
      val state = get()
      if (!state.isInstanceOf[Success[_]]) dispatchOrAddCallbacks(state, new Transformation[T, U @uncheckedCaptures](Xform_recover, pf, executor)) // Short-circuit if we get a Failure
      else this.asInstanceOf[Future[U]]
    }
   ...
   **/
  final class Transformation[-F, sealed T] private[this] (
