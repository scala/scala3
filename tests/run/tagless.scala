...
object Test extends App {
  ...
  object ExpSyntax {
    ...
    def add[T](l: T, r: T)(using e: Exp[T]): T = e.add(l, r)
  }
  ...
  def fromTree[T](t: Tree)(using Exp[T]): Maybe[T] = t match {
  ...
  given Exp[Wrapped]:
    ...
    def neg(t: Wrapped) = new Wrapped {
      def value[T](using e: Exp[T]): T = e.neg(t.value)
    }
  ...
  def fromTreeExt[T](recur: => Tree => Maybe[T])(using Exp[T]): Tree => Maybe[T] = {
    case Node("Lit", Leaf(n)) => lit(readInt(n))
  ...
  def fromTreeExt2[T](recur: => Tree => Maybe[T])(using Exp[T], Mult[T]): Tree => Maybe[T] = {
