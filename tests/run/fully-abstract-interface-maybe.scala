...
abstract class Arithmetic {
  ...
  abstract class AppliedOpExtractor {
    ...
    def unapply(x: AppliedOp): (Op, Number, Number)?
  }
...
object CaseClassImplementation extends Arithmetic {
  ...
  object Constant extends ConstantExtractor {
    ...
    def unapply(x: Constant): Int? = x.n
  }
  ...
  object AppliedOp extends AppliedOpExtractor {
    ...
    def unapply(app: AppliedOp): (Op, Number, Number)? = (app.op, app.x, app.y)
  }
...
object ListImplementation extends Arithmetic {
  ...
  object Constant extends ConstantExtractor {
    ...
    def unapply(x: Constant): Int? = ConstantDeco(x).num
  }
  ...
  object AppliedOp extends AppliedOpExtractor {
    ...
    def unapply(app: AppliedOp): (Op, Number, Number)? = {
      val app2 = AppliedOpDeco(app)
      (app2.op, app2.lhs, app2.rhs)
    }
  }
