...
object CaseClassImplementation extends Numbers {
  ...
  val Succ: SuccExtractor = new SuccExtractor {
    ...
    def unapply(succ: Succ): Nat? = succ.n // checked by class tag before calling the unapply
  }
...
object IntImplementation extends Numbers {
  ...
  val Succ: SuccExtractor = new SuccExtractor {
    ...
    def unapply(succ: Succ): Nat? = succ - 1 // checked by class tag before calling the unapply
  }
