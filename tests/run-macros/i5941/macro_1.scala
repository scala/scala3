  ...
  def impl[S: Type, T: Type](getter: Expr[S => T])(using qctx: QuoteContext) : Expr[Lens[S, T]] = {
    ...
    getter.asReflectTree match {
      case Function(param :: Nil, Path(o, parts)) if o.symbol == param.symbol =>
        '{
          val setter = (t: T) => (s: S) => ${ setterBody(('s).asReflectTree, ('t).asReflectTree, parts).asExprOf[S] }
          apply($getter)(setter)
        }
      case _ =>
...
object Iso {
  ...
  def impl[S: Type, A: Type](using qctx: QuoteContext) : Expr[Iso[S, A]] = {
    ...
    if (!(fieldTp =:= tpA)) {
      ...
      // (p: S) => p._1
      val to = (p: S) =>  ${ Select.unique(('p).asReflectTree, "_1").asExprOf[A] }
      // (p: A) => S(p)
      val from = (p: A) =>  ${ Select.overloaded(Ident(companion), "apply", Nil, ('p).asReflectTree :: Nil).asExprOf[S] }
      apply(from)(to)
    }
  }
