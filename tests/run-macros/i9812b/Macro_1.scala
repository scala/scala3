...
trait Liftable[T] {
  ...
  def toExpr(x: T): (s: Scope) ?=> s.Expr[T]
}

object Lift:
  def apply[T](t: T)(using s: Scope)(ev: s.Liftable[T]): Expr[T] = ev.toExpr(t)
...
given LiftFoo as Liftable[Foo.type]:
  def toExpr(x: Foo.type): (s: Scope) ?=> s.Expr[Foo.type] = '{Foo}

given LiftBar[S <: SomeEnum: Type: Liftable] as Liftable[Bar[S]]:
  def toExpr(x: Bar[S]): (s: Scope) ?=> s.Expr[Bar[S]] = '{new Bar(${Lift(x.s)})}
...
given IntLiftable[T <: Int] as Liftable[T]:
  def toExpr(x: T): (s: Scope) ?=> s.Expr[T] = qctx ?=> {
    import qctx.tasty._
...
given LiftLst[T: Type: Liftable](using ev1: => Liftable[CONS[T]], ev2: => Liftable[NIL.type]) as Liftable[Lst[T]]:
  def toExpr(xs: Lst[T]): (s: Scope) ?=> s.Expr[Lst[T]] = xs match
...
given LiftCONS[T: Type: Liftable](using Liftable[Lst[T]]) as Liftable[CONS[T]]:
  def toExpr(x: CONS[T]): (s: Scope) ?=> s.Expr[CONS[T]] = '{CONS(${Lift(x.head)}, ${Lift(x.tail)})}

given LiftNIL as Liftable[NIL.type]:
  def toExpr(x: NIL.type): (s: Scope) ?=> s.Expr[NIL.type] = '{NIL}

def mkLst[T](ts: T*) = ts.foldRight(NIL: Lst[T])(CONS(_,_))

def quote123: (s: Scope) ?=> s.Expr[Lst[Int]] = Lift(mkLst(1,2,3))
