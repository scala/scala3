...
object Optional {
  ...
  /*private*/ def getOrElseImpl[T >: Null : Type](opt: Expr[Optional[T]], alt: Expr[T]) with QuoteContext : Expr[T] = '{
    if ($opt.isEmpty) $alt else $opt.value
  }

  // FIXME fix issue #5097 and enable private
  /*private*/ def mapImpl[A >: Null : Type, B >: Null : Type](opt: Expr[Optional[A]], f: Expr[A => B]) with QuoteContext : Expr[Optional[B]] = '{
