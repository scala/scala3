...
object Optional {
  ...
  /*private*/ def getOrElseImpl[T >: Null : TypeTag](opt: Expr[Optional[T]], alt: Expr[T])(given QuoteContext): Expr[T] = '{
    if ($opt.isEmpty) $alt else $opt.value
  }

  // FIXME fix issue #5097 and enable private
  /*private*/ def mapImpl[A >: Null : TypeTag, B >: Null : TypeTag](opt: Expr[Optional[A]], f: Expr[A => B])(given QuoteContext): Expr[Optional[B]] = '{
