...
object Macro {
  ...
  def impl(x: Expr[Any])(using Quotes): Expr[Any] = {
    x match {
      case '{ (x: Int) => $body(x): Int } => UnsafeExpr.open(body) { (body, close) => close(body)(Value(2)) }
      case '{ (x1: Int, x2: Int) => $body(x1, x2): Int } => UnsafeExpr.open(body) { (body, close) => close(body)(Value(2), Value(3)) }
      case '{ (x1: Int, x2: Int, x3: Int) => $body(x1, x2, x3): Int } => UnsafeExpr.open(body) { (body, close) => close(body)(Value(2), Value(3), Value(4)) }
    }
  }
