...
object Macros {
  ...
  def macro1Impl given QuoteContext = '{3}

  inline def macro2(inline p: Boolean) = ${ macro2Impl(p) }
  def macro2Impl(p: Boolean) given QuoteContext = if (p) '{3} else '{4}

  inline def macro3(n: Int) = ${ macro3Impl('n) }
  def macro3Impl(p: Expr[Int]) given QuoteContext = '{ 2 + $p }

  inline def macro4(i: Int)(j: Int) = ${ macro4Impl('i)('j) }
  def macro4Impl(i: Expr[Int])(j: Expr[Int]) given QuoteContext = '{ $i + $j }

  inline def macro5(i: Int, j: Int) = ${ macro5Impl(j = 'j, i = 'i) }
  def macro5Impl(i: Expr[Int], j: Expr[Int]) given QuoteContext = '{ $i + $j }
