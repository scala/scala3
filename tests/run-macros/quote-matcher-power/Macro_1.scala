...
object Macros {
  ...
  def power_s(x: Expr[Double], n: Expr[Int]) (using QuoteContext): Expr[Double] =
    power_s(x, n.value)

  def power_s(x: Expr[Double], n: Int) (using QuoteContext): Expr[Double] =
    if (n == 0) '{1.0}
  ...
  def rewrite(expr: Expr[Double]) (using QuoteContext): Expr[Double] = {
    val res = expr match {
      // product rule
