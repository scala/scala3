...
object TypeToolbox {
  ...
  private def tpEqImpl[A, B](a: TypeTag[A], b: TypeTag[B])(given qctx: QuoteContext): Expr[Boolean] = {
  ...
  private def tpLEqImpl[A, B](a: TypeTag[A], b: TypeTag[B])(given qctx: QuoteContext): Expr[Boolean] = {
    import qctx.tasty.{_, given}
    Expr(a.unseal.tpe <:< b.unseal.tpe)
  }
  ...
  private def typeOfImpl(a: Expr[_], expected: TypeTag[_])(given qctx: QuoteContext): Expr[Boolean] = {
    import qctx.tasty.{_, given}
  ...
  private def isCaseClassImpl(tp: TypeTag[_])(given qctx: QuoteContext): Expr[Boolean] = {
    import qctx.tasty.{_, given}
  ...
  private def caseFieldsImpl(tp: TypeTag[_])(given qctx: QuoteContext): Expr[List[String]] = {
    import qctx.tasty.{_, given}
  ...
  private def fieldInImpl(t: TypeTag[_], mem: String)(given qctx: QuoteContext): Expr[String] = {
    import qctx.tasty.{_, given}
  ...
  private def fieldsInImpl(t: TypeTag[_])(given qctx: QuoteContext): Expr[Seq[String]] = {
    import qctx.tasty.{_, given}
  ...
  private def methodInImpl(t: TypeTag[_], mem: String)(given qctx: QuoteContext): Expr[Seq[String]] = {
    import qctx.tasty.{_, given}
    Expr(t.unseal.symbol.classMethod(mem).map(_.name))
  }

  inline def methodsIn[T]: Seq[String] = ${methodsInImpl('[T])}
  private def methodsInImpl(t: TypeTag[_])(given qctx: QuoteContext): Expr[Seq[String]] = {
    import qctx.tasty.{_, given}
    Expr(t.unseal.symbol.classMethods.map(_.name))
  }

  inline def method[T](inline mem: String): Seq[String] = ${methodImpl('[T], mem)}
  private def methodImpl(t: TypeTag[_], mem: String)(given qctx: QuoteContext): Expr[Seq[String]] = {
    import qctx.tasty.{_, given}
    Expr(t.unseal.symbol.method(mem).map(_.name))
  }

  inline def methods[T]: Seq[String] = ${methodsImpl('[T])}
  private def methodsImpl(t: TypeTag[_])(given qctx: QuoteContext): Expr[Seq[String]] = {
    import qctx.tasty.{_, given}
    Expr(t.unseal.symbol.methods.map(_.name))
  }

  inline def typeTag[T](x: T): String = ${typeTagImpl('[T])}
  private def typeTagImpl(tp: TypeTag[_])(given qctx: QuoteContext): Expr[String] = {
    import qctx.tasty.{_, given}
  ...
  private def companionImpl(t1: TypeTag[_], t2: TypeTag[_])(given qctx: QuoteContext): Expr[Boolean] = {
    import qctx.tasty.{_, given}
  ...
  private def companionNameImpl(tp: TypeTag[_])(given qctx: QuoteContext): Expr[String] = {
    import qctx.tasty.{_, given}
    val sym = tp.unseal.symbol
    val companionClass =
