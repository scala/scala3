...
object TypeToolbox {
  ...
  private def tpEqImpl[A, B](a: Type[A], b: Type[B]) with (qctx: QuoteContext) : Expr[Boolean] = {
  ...
  private def tpLEqImpl[A, B](a: Type[A], b: Type[B]) with (qctx: QuoteContext) : Expr[Boolean] = {
    import qctx.tasty.{_, given _}
    Expr(a.unseal.tpe <:< b.unseal.tpe)
  }
  ...
  private def typeOfImpl(a: Expr[_], expected: Type[_]) with (qctx: QuoteContext) : Expr[Boolean] = {
    import qctx.tasty.{_, given _}
  ...
  private def isCaseClassImpl(tp: Type[_]) with (qctx: QuoteContext) : Expr[Boolean] = {
    import qctx.tasty.{_, given _}
  ...
  private def caseFieldsImpl(tp: Type[_]) with (qctx: QuoteContext) : Expr[List[String]] = {
    import qctx.tasty.{_, given _}
  ...
  private def fieldsInImpl(t: Type[_]) with (qctx: QuoteContext) : Expr[Seq[String]] = {
    import qctx.tasty.{_, given _}
  ...
  private def methodsInImpl(t: Type[_]) with (qctx: QuoteContext) : Expr[Seq[String]] = {
    import qctx.tasty.{_, given _}
  ...
  private def methodsImpl(t: Type[_]) with (qctx: QuoteContext) : Expr[Seq[String]] = {
    import qctx.tasty.{_, given _}
    Expr(t.unseal.symbol.methods.map(_.name))
  }

  inline def typeTag[T](x: T): String = ${typeTagImpl('[T])}
  private def typeTagImpl(tp: Type[_]) with (qctx: QuoteContext) : Expr[String] = {
    import qctx.tasty.{_, given _}
  ...
  private def companionImpl(t1: Type[_], t2: Type[_]) with (qctx: QuoteContext) : Expr[Boolean] = {
    import qctx.tasty.{_, given _}
  ...
  private def companionNameImpl(tp: Type[_]) with (qctx: QuoteContext) : Expr[String] = {
    import qctx.tasty.{_, given _}
    val sym = tp.unseal.symbol
    val companionClass =
