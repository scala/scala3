...
class RingIntExpr(using val s: Scope) extends Ring[s.Expr[Int]]:
  val zero = '{0}
  val one  = '{1}
  val add  = (x, y) => '{$x + $y}
  val sub  = (x, y) => '{$x - $y}
...
sealed trait PV[T]:
  def expr(using s: Scope)(using s.Liftable[T]): s.Expr[T]

case class Sta[T](x: T) extends PV[T]:
  def expr(using s: Scope)(using s.Liftable[T]): s.Expr[T] = Expr(x)

case class Dyn[T](x: Expr[T]) extends PV[T]:
  def expr(using s: Scope)(using s.Liftable[T]): s.Expr[T] = x

class RingPV[U](using s: Scope)(u: Ring[U], eu: Ring[Expr[U]])(using s.Liftable[U]) extends Ring[PV[U]]:
...
object Complex:
  implicit def isLiftable[T](using s: Scope)(using s.Type[T], s.Liftable[T]): s.Liftable[Complex[T]] = new s.Liftable[Complex[T]]:
    def toExpr(comp: Complex[T]) = '{Complex(${s.Expr(comp.re)}, ${s.Expr(comp.im)})}
...
class ExprVecOps[T](using val s: Scope)(using s.Type[T]) extends VecOps[s.Expr[Int], s.Expr[T]]:
  val reduce: ((s.Expr[T], s.Expr[T]) => s.Expr[T], s.Expr[T], Vec[s.Expr[Int], s.Expr[T]]) => s.Expr[T] = (plus, zero, vec) => '{
    var sum = $zero
    var i = 0
    while i < ${vec.size} do
      sum = ${ plus('sum, vec.get('i)) }
...
object Test:
  ...
  def main(args: Array[String]): Unit =
    ...
    def blasStaticIntExpr(using Scope) = new Blas1(new RingIntExpr, new StaticVecOps)
    def resCode1(using s: Scope) = blasStaticIntExpr.dot(
      vec1.map(s.Expr(_)),
      vec2.map(s.Expr(_))
    )
    println(usingNewScope(resCode1.show))
    println(run(resCode1))
    println()

    def blasExprIntExpr(using s: Scope) = new Blas1(new RingIntExpr, new ExprVecOps)
    def resCode2(using s: Scope): s.Expr[(Array[Int], Array[Int]) => Int] = '{
      (arr1: Array[Int], arr2: Array[Int]) =>
    ...
    def blasStaticIntPVExpr(using Scope) = new Blas1(new RingPV[Int](new RingInt, new RingIntExpr), new StaticVecOps)
    def resCode3(using Scope) = blasStaticIntPVExpr.dot(
      vec1.map(i => Dyn(s.Expr(i))),
      vec2.map(i => Sta(i))
    ).expr
    ...
    def blasExprIntPVExpr(using Scope) = new Blas1(new RingPV[Int](new RingInt, new RingIntExpr), new StaticVecOps)
    def resCode4(using Scope): Expr[Array[Int] => Int] = '{
      (arr: Array[Int]) =>
    ...
    println(usingNewScope(resCode4.show))
    println(run(resCode4).apply(arr1))
    println()

    import Complex.isLiftable
    def blasExprComplexPVInt(using Scope) = new Blas1[Int, Complex[PV[Int]]](new RingComplex(new RingPV[Int](new RingInt, new RingIntExpr)), new StaticVecOps)
    def resCode5(using Scope): s.Expr[Array[Complex[Int]] => Complex[Int]] = '{
      (arr: Array[Complex[Int]]) =>
    ...
    println(usingNewScope(resCode5.show))
    println(run(resCode5).apply(cmpxArr1))
    println()

    def RingPVInt(using Scope) = new RingPV[Int](new RingInt, new RingIntExpr)
    // Staged loop of dot product on vectors of Int or Expr[Int]
    def dotIntOptExpr(using Scope) = new Blas1(RingPVInt, new StaticVecOps).dot
    // will generate the code '{ ((arr: scala.Array[scala.Int]) => arr.apply(1).+(arr.apply(3))) }
    def staticVec(using Scope) = Vec[Int, PV[Int]](5, i => Sta((i % 2)))
    def code(using Scope) = '{(arr: Array[Int]) => ${dotIntOptExpr(Vec(5, i => Dyn('{arr(${Expr(i)})})), staticVec).expr} }
