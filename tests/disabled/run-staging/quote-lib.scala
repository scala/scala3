...
object Test {
  ...
  def main(args: Array[String]): Unit = usingNewScope { s ?=>
    ...
    letVal('{1})(s ?=> a => '{ $a + 1 }).show
    letLazyVal('{1})(s ?=> a => '{ $a + 1 }).show
    letDef('{1})(s ?=> a => '{ $a + 1 }).show
...
package liftable {
  ...
  object Exprs {
    implicit class LiftExprOps[T](x: T) extends AnyVal {
      def toExpr(using s: Scope)(using s.Liftable[T]): s.Expr[T] =
        summon[s.Liftable[T]].toExpr(x)
    }
  }

  object Units {
    implicit def UnitIsLiftable(using s: Scope): s.Liftable[Unit] = new s.Liftable[Unit] {
      def toExpr(x: Unit) = '{}
    }
  }

  object Lets {
    def letVal[T, U](using s: Scope)(expr: s.Expr[T])(body: (s2: s.Nested) ?=> s2.Expr[T] => s2.Expr[U])(using s.Type[T], s.Type[U]): s.Expr[U] =
      '{ val letVal: T = $expr; ${ body('letVal) } }
    def letLazyVal[T, U](using s: Scope)(expr: Expr[T])(body: (s2: s.Nested) ?=> s2.Expr[T] => s2.Expr[U])(using s.Type[T], s.Type[U]): s.Expr[U] =
      '{ lazy val letLazyVal: T = $expr; ${ body('letLazyVal) } }
    def letDef[T, U](using s: Scope)(expr: s.Expr[T])(body: (s2: s.Nested) ?=> (s2: s.Nested) ?=> s2.Expr[T] => s2.Expr[U])(using s.Type[T], s.Type[U]): s.Expr[U] =
      '{ def letDef: T = $expr; ${ body('letDef) } }
  }

  object Loops {
    def liftedWhile(using s: Scope)(cond: s.Expr[Boolean])(body: s.Expr[Unit]): s.Expr[Unit] = '{ while ($cond) $body }
    def liftedDoWhile(using s: Scope)(body: s.Expr[Unit])(cond: s.Expr[Boolean]): s.Expr[Unit] = '{ while { $body ; $cond } do () }
  }


  object Lists {

    implicit class LiftedOps[T](using val s: Scope)(list: s.Expr[List[T]])(using s.Type[T], s.Liftable[T]) {
      def foldLeft[U](acc: s.Expr[U])(f: s.Expr[(U, T) => U])(using s.Type[U]): s.Expr[U] =
        '{ ($list).foldLeft[U]($acc)($f) }
      def foreach(f: s.Expr[T => Unit]): s.Expr[Unit] =
        '{ ($list).foreach($f) }
    }

    implicit class UnrolledOps[T](using val s: Scope)(list: List[T])(using s.Type[T], s.Liftable[T]) {
      def unrolledFoldLeft[U](acc: s.Expr[U])(f: s.Expr[(U, T) => U])(using s.Type[U]): s.Expr[U] = list match {
        case x :: xs => xs.unrolledFoldLeft('{ ($f).apply($acc, ${Expr(x)}) })(f)
        case Nil => acc
      }
       def unrolledForeach(f: s.Expr[T => Unit]): s.Expr[Unit] = list match {
         case x :: xs => '{ ($f).apply(${s.Expr(x)}); ${ xs.unrolledForeach(f) } }
         case Nil => '{}
       }
    }
