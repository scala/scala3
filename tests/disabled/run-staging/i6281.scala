...
object Test extends App {
  ...
  trait Effects[L <: HList] {
    def reify[A](using s: Scope)(using s.Type[A]): STM[A, L] => s.Expr[Stm[A, L]]
    def reflect[A](using s: Scope)(using s.Type[A]): s.Expr[Stm[A, L]] => STM[A, L]
  }
  given empty as Effects[HNil] {
    def reify[A](using s: Scope)(using s.Type[A]) = m => m
    def reflect[A](using s: Scope)(using s.Type[A]) = m => m
  }
  // for reify, we need type tags for E and also strangely for L.
  implicit def cons [E, L <: HList](using s: Scope)(using Effects[L])(using s.Type[E])(using s.Type[L]): Effects[E :: L] =  new Effects[E :: L] {
  ...
  def m(using s: Scope): STM[Int, RS] = k => k('{42})
