...
class RingIntExpr(using s: Scope) extends Ring[s.Expr[Int]] {
  val zero = '{0}
  val one  = '{1}
  val add  = (x, y) => '{$x + $y}
  val sub  = (x, y) => '{$x - $y}
...
case class RingPV[U](using s: Scope)(staRing: Ring[U], dynRing: Ring[s.Expr[U]])(using s.Liftable[U]) extends Ring[PV[U]] {
  type T = PV[U]
...
class RingIntPExpr(using s: Scope) extends RingPV(RingInt, new RingIntExpr)

class RingIntOPExpr(using s: Scope) extends RingIntPExpr {
