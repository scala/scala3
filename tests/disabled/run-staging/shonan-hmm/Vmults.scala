...
object Vmults {
  ...
  def vmultCA(using s: Scope): s.Expr[(Array[Complex[Int]], Array[Complex[Int]], Array[Complex[Int]]) => Unit] = '{
    (vout: Array[Complex[Int]], v1: Array[Complex[Int]], v2: Array[Complex[Int]]) => {
      val n = vout.length
      ${
        val vout_ = OVec[s.Expr[Int], Complex[s.Expr[Int]], s.Expr[Unit]]('n, (i, v) => '{vout($i) = ${Complex.of_expr_complex(v)}})
        val v1_ = Vec ('n, i => Complex.of_complex_expr('{v1($i)}))
        val v2_ = Vec ('n, i => Complex.of_complex_expr('{v2($i)}))

        val V = new Vmult[s.Expr[Int], Complex[s.Expr[Int]], s.Expr[Unit]](RingComplex(new RingIntExpr), new VecDyn)
        V.vmult(vout_, v1_, v2_)
      }
    }
  }
