...
object Appender {
  implicit def lowLevelAppender[L1 <: HList, L2 <: HList, O <: HList] with (erased p: PhantomAppender.Aux[L1, L2, O]) : Appender[L1, L2] { type Out = O } =
    new Appender[L1, L2] {
      type Out = O
...
object PhantomAppender {
  ...
  implicit erased def caseHCons[H, T <: HList, L <: HList, O <: HList] with (erased p: Aux[T, L, O]) : Aux[H :: T, L, H :: O] = ???
}
