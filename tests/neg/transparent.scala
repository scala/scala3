...
object Foo {
  dependent def foo(b: Boolean): { if (b) 1 else 2 } =
    if (b) 1 else 2
...
object NullTests {
  dependent def f(x: String): Boolean = x == null
  ...
  dependent def g(x: String): Boolean = x.isInstanceOf[String]
  val b1: true  = g("aa")
  ...
  dependent def h(x: String): String = x + "A"
  val x: { h(null) } = h(null)
  val y = h(null)

  dependent def hTest: Unit = {
    val y = h(null)
    val z: { h(null) } = y
  }
}
