...
object Rewrites {
   ...
   */
  def patch(source: SourceFile, span: Span, replacement: String)(using Ctx, CState): Unit =
  ...
  def patch(span: Span, replacement: String)(using Ctx, CState): Unit =
    patch(ctx.compilationUnit.source, span, replacement)

  /** Does `span` overlap with a patch region of `source`? */
  def overlapsPatch(source: SourceFile, span: Span)(using Ctx, CState): Boolean =
   ...
   */
  def writeBack()(using Ctx, CState): Unit =
