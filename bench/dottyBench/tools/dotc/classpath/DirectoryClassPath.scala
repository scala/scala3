...
import dottyBench.tools.io.{AbstractFile, PlainFile, ClassPath, ClassRepresentation}
import FileUtils._
...
trait DirectoryLookup[FileEntryType <: ClassRepresentation] extends ClassPath {
  ...
  override private[dottyBench] def hasPackage(pkg: String): Boolean = getDirectory(pkg).isDefined

  private[dottyBench] def packages(inPackage: String): Seq[PackageEntry] = {
  ...
  private[dottyBench] def list(inPackage: String): ClassPathEntries = {
    val dirForPackage = getDirectory(inPackage)
    val files: Array[F] = dirForPackage match {
      case None => emptyFiles
...
trait JFileDirectoryLookup[FileEntryType <: ClassRepresentation] extends DirectoryLookup[FileEntryType] {
  ...
  protected def toAbstractFile(f: JFile): AbstractFile = new PlainFile(new dottyBench.tools.io.File(f.toPath))
...
final class JrtClassPath(fs: java.nio.file.FileSystem) extends ClassPath with NoSourcePaths {
  ...
  override private[dottyBench] def hasPackage(pkg: String): Boolean = packageToModuleBases.contains(pkg)

  override private[dottyBench] def packages(inPackage: String): Seq[PackageEntry] = {
  ...
  private[dottyBench] def classes(inPackage: String): Seq[ClassFileEntry] =
    if (inPackage == "") Nil
    else
      packageToModuleBases.getOrElse(inPackage, Nil).flatMap(x =>
        ...
        ClassFileEntryImpl(new PlainFile(new dottyBench.tools.io.File(x)))).toVector

  override private[dottyBench] def list(inPackage: String): ClassPathEntries =
  ...
  def findClassFile(className: String): Option[AbstractFile] =
    if (!className.contains(".")) None
      ...
      packageToModuleBases.getOrElse(inPackage, Nil).iterator.flatMap{x =>
        ...
        if (Files.exists(file)) new PlainFile(new dottyBench.tools.io.File(file)) :: Nil else Nil
...
case class DirectoryClassPath(dir: JFile) extends JFileDirectoryLookup[ClassFileEntryImpl] with NoSourcePaths {
  ...
  def findClassFile(className: String): Option[AbstractFile] = {
    ...
    if (classFile.exists) {
      val wrappedClassFile = new dottyBench.tools.io.File(classFile.toPath)
  ...
  private[dottyBench] def classes(inPackage: String): Seq[ClassFileEntry] = files(inPackage)
}
...
case class DirectorySourcePath(dir: JFile) extends JFileDirectoryLookup[SourceFileEntryImpl] with NoClassPaths {
  ...
  private def findSourceFile(className: String): Option[AbstractFile] = {
    ...
    sourceFile.map { file =>
      val wrappedSourceFile = new dottyBench.tools.io.File(file.toPath)
      val abstractSourceFile = new PlainFile(wrappedSourceFile)
      abstractSourceFile
    }
  }

  private[dottyBench] def sources(inPackage: String): Seq[SourceFileEntry] = files(inPackage)
}
