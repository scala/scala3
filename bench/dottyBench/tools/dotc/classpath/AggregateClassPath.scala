...
import dottyBench.tools.io.{ AbstractFile, ClassPath, ClassRepresentation }
...
case class AggregateClassPath(aggregates: Seq[ClassPath]) extends ClassPath {
  ...
  override private[dottyBench] def classes(inPackage: String): Seq[ClassFileEntry] =
    getDistinctEntries(_.classes(inPackage))

  override private[dottyBench] def sources(inPackage: String): Seq[SourceFileEntry] =
    getDistinctEntries(_.sources(inPackage))

  override private[dottyBench] def hasPackage(pkg: String): Boolean = aggregates.exists(_.hasPackage(pkg))
