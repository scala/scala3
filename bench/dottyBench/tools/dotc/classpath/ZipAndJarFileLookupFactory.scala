...
import dottyBench.tools.io.{AbstractFile, ClassPath, ClassRepresentation, FileZipArchive, ManifestResources}
...
sealed trait ZipAndJarFileLookupFactory {
  ...
  def create(zipFile: AbstractFile)(using Ctx, CState): ClassPath =
...
object ZipAndJarClassPathFactory extends ZipAndJarFileLookupFactory {
  private case class ZipArchiveClassPath(zipFile: File)
    ...
    override private[dottyBench] def classes(inPackage: String): Seq[ClassFileEntry] = files(inPackage)
  ...
  private case class ManifestResourcesClassPath(file: ManifestResources) extends ClassPath with NoSourcePaths {
    ...
    override private[dottyBench] def packages(inPackage: String): Seq[PackageEntry] = cachedPackages.get(inPackage) match {
      case None => Seq.empty
    ...
    override private[dottyBench] def classes(inPackage: String): Seq[ClassFileEntry] = cachedPackages.get(inPackage) match {
      case None => Seq.empty
      case Some(PackageFileInfo(pkg, _)) =>
        (for (file <- pkg if file.isClass) yield ClassFileEntryImpl(file)).toSeq
    }

    override private[dottyBench] def hasPackage(pkg: String) = cachedPackages.contains(pkg)
    override private[dottyBench] def list(inPackage: String): ClassPathEntries = ClassPathEntries(packages(inPackage), classes(inPackage))
  }
...
object ZipAndJarSourcePathFactory extends ZipAndJarFileLookupFactory {
  private case class ZipArchiveSourcePath(zipFile: File)
    ...
    override private[dottyBench] def sources(inPackage: String): Seq[SourceFileEntry] = files(inPackage)
