...
trait Plugins {
   ...
   */
  protected def loadRoughPluginsList(using ctx: Context): List[Plugin] = {
   ...
   */
  protected def loadPlugins(using ctx: Context): List[Plugin] = {
    given CState = ctx.cstate
  ...
  def addPluginPhases(plan: List[List[Phase]])(using ctx: Context): List[List[Phase]] = {
    given CState = ctx.cstate
