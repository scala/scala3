...
object VarianceChecker {
  ...
  def check(tree: tpd.Tree)(using Ctx, CState): Unit =
   ...
   */
  def checkLambda(tree: tpd.LambdaTypeTree, bounds: TypeBounds)(using Ctx, CState): Unit =
...
class VarianceChecker(using Ctx, CState) {
  ...
  private object Traverser extends TreeTraverser {
    ...
    override def traverse(tree: Tree)(using Ctx, CState) = {
      def sym = tree.symbol
