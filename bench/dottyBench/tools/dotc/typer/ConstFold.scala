...
object ConstFold {
  ...
  def apply[T <: Tree](tree: T)(using Ctx, CState): T = finish(tree) {
    tree match {
   ...
   *  the conversion.
   */
  def apply[T <: Tree](tree: T, pt: Type)(using Ctx, CState): T =
    finish(apply(tree)) {
  ...
  inline private def finish[T <: Tree](tree: T)(compX: => Constant)(using Ctx, CState): T =
    try {
      val x = compX
