...
object ProtoTypes {
  ...
  trait Compatibility {
    ...
    def viewExists(tp: Type, pt: Type)(using Ctx, CState): Boolean
     ...
     */
    def isCompatible(tp: Type, pt: Type)(using Ctx, CState): Boolean =
      (tp.widenExpr relaxed_<:< pt.widenExpr) || viewExists(tp, pt)

    /** Test compatibility after normalization.
     *  Do this in a fresh typerstate unless `keepConstraint` is true.
     */
    def normalizedCompatible(tp: Type, pt: Type, keepConstraint: Boolean)(using Ctx, CState): Boolean = {
      def testCompat(using Ctx, CState): Boolean = {
    ...
    private def disregardProto(pt: Type)(using Ctx, CState): Boolean =
      pt.dealias.isRef(defn.UnitClass)

    /** Check that the result type of the current method
     *  fits the given expected result type.
     */
    def constrainResult(mt: Type, pt: Type)(using Ctx, CState): Boolean =
     ...
     */
    def constrainResult(meth: Symbol, mt: Type, pt: Type)(using Ctx, CState): Boolean =
  ...
  object NoViewsAllowed extends Compatibility {
    override def viewExists(tp: Type, pt: Type)(using Ctx, CState): Boolean = false
  }

  /** A trait for prototypes that match all types */
  trait MatchAlways extends ProtoType {
    def isMatchedBy(tp1: Type, keepConstraint: Boolean)(using Ctx, CState): Boolean = true
    def map(tm: TypeMap)(using Ctx, CState): ProtoType = this
    def fold[T](x: T, ta: TypeAccumulator[T])(using Ctx, CState): T = x
  ...
  case class IgnoredProto(ignored: Type) extends UncachedGroundType with MatchAlways:
    ...
    override def deepenProto(using Ctx, CState): Type = ignored
  ...
  extends CachedProxyType with ProtoType with ValueTypeOrProto {
     ...
     */
    private def hasUnknownMembers(tp: Type)(using Ctx, CState): Boolean = tp match {
    ...
    override def isMatchedBy(tp1: Type, keepConstraint: Boolean)(using Ctx, CState): Boolean =
    ...
    def underlying(using Ctx, CState): Type = WildcardType

    def derivedSelectionProto(name: Name, memberProto: Type, compat: Compatibility)(using Ctx, CState): SelectionProto =
    ...
    def map(tm: TypeMap)(using Ctx, CState): SelectionProto = derivedSelectionProto(name, tm(memberProto), compat)
    def fold[T](x: T, ta: TypeAccumulator[T])(using Ctx, CState): T = ta(x, memberProto)

    override def deepenProto(using Ctx, CState): SelectionProto = derivedSelectionProto(name, memberProto.deepenProto, compat)
  ...
  object SelectionProto {
    def apply(name: Name, memberProto: Type, compat: Compatibility, privateOK: Boolean)(using Ctx, CState): SelectionProto = {
   ...
   */
  def selectionProto(name: Name, tp: Type, typer: Typer)(using Ctx, CState): TermType =
  ...
  case class FunProto(args: List[untpd.Tree], resType: Type)(typer: Typer,
    override val applyKind: ApplyKind, state: FunProtoState = new FunProtoState)(using protoCtx: Ctx, protoCS: CState)
  extends UncachedGroundType with ApplyingProto with FunOrPolyProto {
    override def resultType(using Ctx, CState): Type = resType

    def isMatchedBy(tp: Type, keepConstraint: Boolean)(using Ctx, CState): Boolean = {
      val args = typedArgs()
     ...
     */
    def allArgTypesAreCurrent()(using Ctx, CState): Boolean =
    ...
    private def cacheTypedArg(arg: untpd.Tree, typerFn: untpd.Tree => Tree, force: Boolean)(using Ctx, CState): Tree = {
     ...
     */
    def typedArgs(norm: (untpd.Tree, Int) => untpd.Tree = sameTree)(using Ctx, CState): List[Tree] =
      if (state.typedArgs.size == args.length) state.typedArgs
      else {
        val prevConstraint = protoCtx.typerState.constraint

        try
          inCtxAndState(protoCtx, protoCS) {
     ...
     */
    def typedArg(arg: untpd.Tree, formal: Type)(using Ctx, CState): Tree = {
     ...
     */
    def typeOfArg(arg: untpd.Tree)(using Ctx, CState): Type = {
    ...
    override def isErroneous(using Ctx, CState): Boolean =
      state.typedArgs.tpes.exists(_.isErroneous)

    override def toString: String = s"FunProto(${args mkString ","} => $resultType)"

    def map(tm: TypeMap)(using Ctx, CState): FunProto =
      derivedFunProto(args, tm(resultType), typer)

    def fold[T](x: T, ta: TypeAccumulator[T])(using Ctx, CState): T =
      ta(ta.foldOver(x, typedArgs().tpes), resultType)

    override def deepenProto(using Ctx, CState): FunProto = derivedFunProto(args, resultType.deepenProto, typer)

    override def withContext(newCtx: Ctx, newCS: CState): ProtoType =
      if (newCtx eq protoCtx) && (newCS == protoCS) then this
      else new FunProto(args, resType)(typer, applyKind, state)(using newCtx, newCS)
  }
   ...
   */
  class FunProtoTyped(args: List[tpd.Tree], resultType: Type)(typer: Typer, applyKind: ApplyKind)(using Ctx, CState)
  extends FunProto(args, resultType)(typer, applyKind):
    override def typedArgs(norm: (untpd.Tree, Int) => untpd.Tree)(using Ctx, CState): List[tpd.Tree] = args
    override def typedArg(arg: untpd.Tree, formal: Type)(using Ctx, CState): tpd.Tree = arg.asInstanceOf[tpd.Tree]
    override def allArgTypesAreCurrent()(using Ctx, CState): Boolean = true
    override def withContext(newCtx: Ctx, newCS: CState): FunProtoTyped = this
  ...
  extends CachedGroundType with ApplyingProto {
    ...
    def derivedViewProto(argType: Type, resultType: Type)(using Ctx, CState): ViewProto =
      if ((argType eq this.argType) && (resultType eq this.resultType)) this
      else ViewProto(argType, resultType)

    def map(tm: TypeMap)(using Ctx, CState): ViewProto = derivedViewProto(tm(argType), tm(resultType))

    def fold[T](x: T, ta: TypeAccumulator[T])(using Ctx, CState): T =
      ta(ta(x, argType), resultType)

    override def deepenProto(using Ctx, CState): ViewProto = derivedViewProto(argType, resultType.deepenProto)
  }
  ...
  object ViewProto {
    def apply(argType: Type, resultType: Type)(using Ctx, CState): ViewProto =
      unique(new CachedViewProto(argType, resultType))
  }

  class UnapplyFunProto(argType: Type, typer: Typer)(using Ctx, CState) extends FunProto(
  ...
  case class PolyProto(targs: List[Tree], resType: Type) extends UncachedGroundType with FunOrPolyProto {
    ...
    def canInstantiate(tp: Type)(using Ctx, CState) = tp.widen match
      case tp: PolyType => tp.paramNames.length == targs.length
      case _ => false

    override def isMatchedBy(tp: Type, keepConstraint: Boolean)(using Ctx, CState): Boolean =
    ...
    def map(tm: TypeMap)(using Ctx, CState): PolyProto =
      derivedPolyProto(targs, tm(resultType))

    def fold[T](x: T, ta: TypeAccumulator[T])(using Ctx, CState): T =
      ta(ta.foldOver(x, targs.tpes), resultType)

    override def deepenProto(using Ctx, CState): PolyProto = derivedPolyProto(targs, resultType.deepenProto)
  }
   ...
   */
  def constrained(tl: TypeLambda, owningTree: untpd.Tree, alwaysAddTypeVars: Boolean)(using Ctx, CState): (TypeLambda, List[TypeTree]) = {
  ...
  def constrained(tl: TypeLambda, owningTree: untpd.Tree)(using Ctx, CState): (TypeLambda, List[TypeTree]) =
  ...
  def constrained(tl: TypeLambda)(using Ctx, CState): TypeLambda =
    constrained(tl, EmptyTree)._1

  def newTypeVar(bounds: TypeBounds)(using Ctx, CState): TypeVar = {
  ...
  def newDepTypeVar(tp: Type)(using Ctx, CState): TypeVar =
   ...
   */
  def resultTypeApprox(mt: MethodType, wildcardOnly: Boolean = false)(using Ctx, CState): Type =
   ...
   */
  def normalize(tp: Type, pt: Type)(using Ctx, CState): Type = {
   ...
   *  by wildcard types.
   */
  private def wildApprox(tp: Type, theMap: WildApproxMap, seen: Set[TypeParamRef], internal: Set[TypeLambda])(using Ctx, CState): Type = tp match {
  ...
  final def wildApprox(tp: Type)(using Ctx, CState): Type = wildApprox(tp, null, Set.empty, Set.empty)

  @sharable object AssignProto extends UncachedGroundType with MatchAlways

  private[ProtoTypes] class WildApproxMap(val seen: Set[TypeParamRef], val internal: Set[TypeLambda])(using Ctx, CState) extends TypeMap {
