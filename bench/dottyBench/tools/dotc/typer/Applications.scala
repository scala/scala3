...
object Applications {
  ...
  def extractorMember(tp: Type, name: Name)(using Ctx, CState): SingleDenotation =
    tp.member(name).suchThat(sym => sym.info.isParameterless && sym.info.widenExpr.isValueType)

  def extractorMemberType(tp: Type, name: Name, errorPos: SourcePosition)(using Ctx, CState): Type = {
   ...
   */
  def isProductMatch(tp: Type, numArgs: Int, errorPos: SourcePosition = NoSourcePosition)(using Ctx, CState): Boolean =
   ...
   */
  def isProductSeqMatch(tp: Type, numArgs: Int, errorPos: SourcePosition = NoSourcePosition)(using Ctx, CState): Boolean = {
   ...
   */
  def isGetMatch(tp: Type, errorPos: SourcePosition = NoSourcePosition)(using Ctx, CState): Boolean =
  ...
  /** If `getType` is of the form:
    ...
    */
  def unapplySeqTypeElemTp(getTp: Type)(using Ctx, CState): Type = {
  ...
  def productSelectorTypes(tp: Type, errorPos: SourcePosition)(using Ctx, CState): List[Type] = {
  ...
  def productArity(tp: Type, errorPos: SourcePosition = NoSourcePosition)(using Ctx, CState): Int =
    if (defn.isProductSubType(tp)) productSelectorTypes(tp, errorPos).size else -1

  def productSelectors(tp: Type)(using Ctx, CState): List[Symbol] = {
    val sels = for (n <- Iterator.from(0)) yield
      tp.member(nme.selectorName(n)).suchThat(_.info.isParameterless).symbol
    sels.takeWhile(_.exists).toList
  }

  def getUnapplySelectors(tp: Type, args: List[untpd.Tree], pos: SourcePosition)(using Ctx, CState): List[Type] =
  ...
  def productSeqSelectors(tp: Type, argsNum: Int, pos: SourcePosition)(using Ctx, CState): List[Type] = {
  ...
  def unapplyArgs(unapplyResult: Type, unapplyFn: Tree, args: List[untpd.Tree], pos: SourcePosition)(using Ctx, CState): List[Type] = {
  ...
  def wrapDefs(defs: mutable.ListBuffer[Tree], tree: Tree)(using Ctx, CState): Tree =
  ...
  object IntegratedTypeArgs {
    def apply(app: Tree)(using Ctx, CState) = new IntegratedTypeArgs(app)
    def unapply(tree: Tree)(using Ctx, CState): Option[Tree] = tree match {
...
trait Applications extends Compatibility {
   ...
   */
  abstract class Application[Arg](methRef: TermRef, funType: Type, args: List[Arg], resultType: Type)(using Ctx, CState) {
     ...
     */
    def findDefaultGetter(n: Int)(using Ctx, CState): Tree = {
   ...
   */
  abstract class TestApplication[Arg](methRef: TermRef, funType: Type, args: List[Arg], resultType: Type)(using Ctx, CState)
   ...
   *  argument trees.
   */
  class ApplicableToTrees(methRef: TermRef, args: List[Tree], resultType: Type)(using Ctx, CState)
   ...
   * argument trees.
   */
  class ApplicableToTreesDirectly(methRef: TermRef, args: List[Tree], resultType: Type)(using Ctx, CState)
  ...
  class ApplicableToTypes(methRef: TermRef, args: List[Type], resultType: Type)(using Ctx, CState)
  ...
  abstract class TypedApply[T >: Untyped](
    ...
    override val applyKind: ApplyKind)(using Ctx, CState)
  extends Application(methRef, fun.tpe, args, resultType) {
    ...
    private def lifter(using Ctx, CState) =
  ...
  class ApplyToUntyped(
    ...
    resultType: Type)(using Ctx, CState)
  ...
  class ApplyToTyped(
    ...
    resultType: Type, applyKind: ApplyKind)(using Ctx, CState)
   ...
   */
  def argCtx(app: untpd.Tree)(using Ctx, CState): Ctx =
   ...
   */
  def typedFunPart(fn: untpd.Tree, pt: Type)(using Ctx, CState): Tree =
    tryEither {
      typedExpr(fn, pt)
    } { (result, tstate) =>
   ...
   *  Block node.
   */
  def typedApply(tree: untpd.Apply, pt: Type)(using Ctx, CState): Tree = {

    def realApply(using Ctx, CState): Tree = {
      val originalProto =
      ...
      def simpleApply(fun1: Tree, proto: FunProto)(using Ctx, CState): Tree =
       ...
       */
      def tryWithImplicitOnQualifier(fun1: Tree, proto: FunProto)(using Ctx, CState): Option[Tree] =
     ...
     *     { val xs = es; e' = e' + args }
     */
    def typedOpAssign(using Ctx, CState): Tree = {
  ...
  def ApplyTo(app: untpd.Apply, fun: tpd.Tree, methRef: TermRef, proto: FunProto, resultType: Type)(using Ctx, CState): tpd.Tree =
    val typer = ctx.typer
  ...
  protected def handleUnexpectedFunType(tree: untpd.Apply, fun: Tree)(using Ctx, CState): Tree =
    if ctx.reporter.errorsReported then
      throw TypeError(i"unexpected function type: ${methPart(fun).tpe}")
    else
      throw Error(i"unexpected type.\n  fun = $fun,\n  methPart(fun) = ${methPart(fun)},\n  methPart(fun).tpe = ${methPart(fun).tpe},\n  tpe = ${fun.tpe}")

  def typedNamedArgs(args: List[untpd.Tree])(using Ctx, CState): List[NamedArg] =
    for (arg @ NamedArg(id, argtpt) <- args) yield {
      val argtpt1 = typedType(argtpt)
      cpy.NamedArg(arg)(id, argtpt1).withType(argtpt1.tpe)
    }

  def typedTypeApply(tree: untpd.TypeApply, pt: Type)(using Ctx, CState): Tree = {
   ...
   */
  def convertNewGenericArray(tree: Tree)(using Ctx, CState):  Tree = tree match {
   ...
   */
  def saysNotFound(state: TyperState, memberName: Name)(using Ctx, CState): Boolean =
  ...
  def typedUnApply(tree: untpd.Apply, selType: Type)(using Ctx, CState): Tree = {
    record("typedUnApply")
    ...
    def trySelectUnapply(qual: untpd.Tree)(fallBack: (Tree, TyperState) => Tree): Tree = {
      ...
      def tryWithName(name: TermName)(fallBack: (Tree, TyperState) => Tree)(using Ctx, CState): Tree = {
        def tryWithProto(pt: Type)(using Ctx, CState) = {
   ...
   *  and pattern matcher.
   */
  def typedUnApply(tree: untpd.UnApply, selType: Type)(using Ctx, CState): UnApply =
   ...
   *  @param  resultType   The expected result type of the application
   */
  def isApplicableMethodRef(methRef: TermRef, args: List[Tree], resultType: Type, keepConstraint: Boolean)(using Ctx, CState): Boolean = {
    def isApp(using Ctx, CState): Boolean =
  ...
  /** Is given method reference applicable to argument trees `args` without inferring views?
    *  @param  resultType   The expected result type of the application
    */
  def isDirectlyApplicableMethodRef(methRef: TermRef, args: List[Tree], resultType: Type)(using Ctx, CState): Boolean =
    explore(new ApplicableToTreesDirectly(methRef, args, resultType).success)

  /** Is given method reference applicable to argument types `args`?
   *  @param  resultType   The expected result type of the application
   */
  def isApplicableMethodRef(methRef: TermRef, args: List[Type], resultType: Type)(using Ctx, CState): Boolean =
   ...
   *  @param  resultType   The expected result type of the application
   */
  def isApplicableType(tp: Type, args: List[Tree], resultType: Type, keepConstraint: Boolean)(using Ctx, CState): Boolean =
   ...
   *  @param  resultType   The expected result type of the application
   */
  def isApplicableType(tp: Type, args: List[Type], resultType: Type)(using Ctx, CState): Boolean =
    onMethod(tp, args.nonEmpty) {
      isApplicableMethodRef(_, args, resultType)
    }

  private def onMethod(tp: Type, followApply: Boolean)(p: TermRef => Boolean)(using Ctx, CState): Boolean = tp match {
   ...
   */
  def hasExtensionMethod(tp: Type, name: TermName, argType: Type, resultType: Type)(using Ctx, CState) = {
  ...
  def stripImplicit(tp: Type)(using Ctx, CState): Type = tp match {
  ...
  /** Compare owner inheritance level.
    ...
    */
  def compareOwner(sym1: Symbol, sym2: Symbol)(using Ctx, CState): Int =
    if sym1 == sym2 then 0
   ...
   *  that takes fewer.
   */
  def compare(alt1: TermRef, alt2: TermRef)(using Ctx, CState): Int = trace(i"compare($alt1, $alt2)", overload) {
    record("compare")
    ...
    def isAsSpecific(alt1: TermRef, tp1: Type, alt2: TermRef, tp2: Type): Boolean = trace(i"isAsSpecific $tp1 $tp2", overload) { tp1 match {
      ...
      case tp1: PolyType => // (2)
        inCtx(ctx.fresh.setExploreTyperState()) {
     ...
     */
    def isAsSpecificValueType(tp1: Type, tp2: Type)(using Ctx, CState) =
  ...
  def narrowMostSpecific(alts: List[TermRef])(using Ctx, CState): List[TermRef] = {
   ...
   */
  def resolveOverloaded(alts: List[TermRef], pt: Type)(using Ctx, CState): List[TermRef] =
     ...
     */
    def resultConforms(altSym: Symbol, altType: Type, resultType: Type)(using Ctx, CState): Boolean =
   ...
   */
  private def resolveOverloaded1(alts: List[TermRef], pt: Type)(using Ctx, CState): List[TermRef] =
   ...
   */
  def resolveMapped(alts: List[TermRef], f: TermRef => Type, pt: Type)(using Ctx, CState): List[TermRef] =
   ...
   */
  private def pretypeArgs(alts: List[TermRef], pt: FunProto)(using Ctx, CState): Unit = {
  ...
  private def harmonizeWith[T <: AnyRef](ts: List[T])(tpe: T => Type, adapt: (T, Type) => T)(using Ctx, CState): List[T] = {
   ...
   */
  def harmonize(trees: List[Tree])(using Ctx, CState): List[Tree] = {
   ...
   */
  def harmonic[T](harmonize: List[T] => List[T], pt: Type)(op: => List[T])(using Ctx, CState): List[T] =
   ...
   */
  private def harmonizeTypes(tpes: List[Type])(using Ctx, CState): List[Type] =
   ...
   */
  def extMethodApply(methodRef: untpd.Tree, receiver: Tree, pt: Type)(using Ctx, CState): Tree = {
