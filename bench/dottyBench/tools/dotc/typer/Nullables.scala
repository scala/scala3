...
object Nullables:
  ...
  object CompareNull:
     ...
     */
    def unapply(tree: Tree)(using Ctx, CState): Option[(Tree, Boolean)] = tree match
    ...
    private def testSym(sym: Symbol, operand: Tree)(using Ctx, CState) =
  ...
  object TrackedRef:
    def unapply(tree: Tree)(using Ctx, CState): Option[TermRef] = tree.typeOpt match
   ...
   */
  def isTracked(ref: TermRef)(using Ctx, CState) =
    ref.isStable
    || { val sym = ref.symbol
         !ref.usedOutOfOrder
         && sym.span.exists
   ...
   */
  def afterPatternContext(sel: Tree, pat: Tree)(using Ctx, CState) = (sel, pat) match
   ...
   */
  def caseContext(sel: Tree, pat: Tree)(using Ctx, CState): Ctx = sel match
    case TrackedRef(ref) if matchesNotNull(pat) => ctx.addNotNullRefs(Set(ref))
    case _ => ctx

  private def matchesNotNull(pat: Tree)(using Ctx, CState): Boolean = pat match
  ...
  given notNullInfoOps as AnyRef:
    extension (infos: List[NotNullInfo]):
      ...
      */
      ...
      def retractMutables(using Ctx, CState) =
  ...
  given refOps as AnyRef:
    extension (ref: TermRef):
      ...
      *  ```
      */
      def usedOutOfOrder(using Ctx, CState): Boolean =
  ...
  given treeOps as AnyRef:
    extension (tree: Tree):
      ...
      def notNullInfo(using Ctx, CState): NotNullInfo =
      ...
      def notNullInfoIf(c: Boolean)(using Ctx, CState): NotNullInfo =
      ...
      *  a condition.
      */
      def notNullConditional(using Ctx, CState): NotNullConditional =
      ...
      def nullableContext(using Ctx, CState): Ctx =
      ...
      *  the value of `c`.
      */
      def nullableContextIf(c: Boolean)(using Ctx, CState): Ctx =
      ...
      */
      def nullableInArgContext(using Ctx, CState): Ctx = tree match
      ...
      *  2. Boolean &&, ||, !
      */
      def computeNullable()(using Ctx, CState): tree.type =
      ...
      def computeNullableDeeply()(using Ctx, CState): Unit =
        new TreeTraverser {
          def traverse(tree: Tree)(using Ctx, CState) =
  ...
  given assignOps as AnyRef:
    extension (tree: Assign):
      def computeAssignNullable()(using Ctx, CState): tree.type = tree.lhs match
        case TrackedRef(ref) =>
   ...
   */
  def assignmentSpans(using Ctx, CState): Map[Int, List[Span]] =
    import ast.untpd._
   ...
   *       xs = xs.next
   */
  def whileContext(whileSpan: Span)(using Ctx, CState): Ctx =
   ...
   */
  def postProcessByNameArgs(fn: TermRef, app: Tree)(using Ctx, CState): Tree =
    fn.widen match
      case mt: MethodType
        ...
        app match
          case Apply(fn, args) =>
            val dropNotNull = new TreeMap:
              override def transform(t: Tree)(using Ctx, CState) = t match
            ...
            object retyper extends ReTyper:
              override def typedUnadapted(t: untpd.Tree, pt: Type, locked: TypeVars)(using Ctx, CState): Tree = t match
