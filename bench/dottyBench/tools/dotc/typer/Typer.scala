...
object Typer {
  ...
  def assertPositioned(tree: untpd.Tree)(using Ctx, CState): Unit =
...
class Typer extends Namer
   ...
   */
  def findRef(name: Name, pt: Type, required: FlagSet, posd: Positioned)(using Ctx, CState): Type = {
    val refctx = ctx
     ...
     */
    def findRefRecur(previous: Type, prevPrec: BindingPrec, prevCtx: Ctx)(using Ctx, CState): Type = {
       ...
       */
      def checkNewOrShadowed(found: Type, newPrec: BindingPrec, scala2pkg: Boolean = false)(using Ctx, CState): Type =
       ...
       */
      def recurAndCheckNewOrShadowed(previous: Type, prevPrec: BindingPrec, prevCtx: Ctx)(using Ctx, CState): Type =
       ...
       */
      def namedImportRef(imp: ImportInfo)(using Ctx, CState): Type = {
       ...
       */
      def wildImportRef(imp: ImportInfo)(using Ctx, CState): Type =
       ...
       */
      def isDefinedInCurrentUnit(denot: Denotation)(using Ctx, CState): Boolean = denot match {
      ...
      def isSelfDenot(denot: Denotation)(using Ctx, CState) = denot match {
        case denot: SymDenotation => denot.is(SelfName)
        case _ => false
      }

      /** Would import of kind `prec` be not shadowed by a nested higher-precedence definition? */
      def isPossibleImport(prec: BindingPrec)(using Ctx, CState) =
        !noImports &&
        (prevPrec.ordinal < prec.ordinal || prevPrec == prec && (prevCtx.scope eq ctx.scope))

      @tailrec def loop(lastCtx: Ctx)(using Ctx): Type =
        if (ctx.scope == null) previous
          ...
          def checkNoOuterDefs(denot: Denotation, last: Ctx, prevCtx: Ctx): Unit =
   ...
   */
  def toNotNullTermRef(tree: Tree, pt: Type)(using Ctx, CState): Tree = tree.tpe match
   ...
   */
  def typedIdent(tree: untpd.Ident, pt: Type)(using Ctx, CState): Tree =
    record("typedIdent")
    val name = tree.name
   ...
   */
  private def checkStableIdentPattern(tree: Tree, pt: Type)(using Ctx, CState): Unit =
    if ctx.mode.is(Mode.Pattern)
       && !tree.isType
  ...
  def typedSelect(tree: untpd.Select, pt: Type, qual: Tree)(using Ctx, CState): Tree = qual match {
  ...
  def typedSelect(tree: untpd.Select, pt: Type)(using Ctx, CState): Tree = {
    record("typedSelect")

    def typeSelectOnTerm(using Ctx, CState): Tree =
      typedSelect(tree, pt, typedExpr(tree.qualifier, selectionProto(tree.name, pt, this)))
        .computeNullable()

    def typeSelectOnType(qual: untpd.Tree)(using Ctx, CState) =
      typedSelect(untpd.cpy.Select(tree)(qual, tree.name.toTypeName), pt)

    def tryJavaSelectOnType(using Ctx, CState): Tree = tree.qualifier match {
    ...
    def selectWithFallback(fallBack: (Ctx, CState) ?=> Tree) =
  ...
  def typedThis(tree: untpd.This)(using Ctx, CState): Tree = {
    record("typedThis")
    assignType(tree)
  }

  def typedSuper(tree: untpd.Super, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedNumber(tree: untpd.Number, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedLiteral(tree: untpd.Literal)(using Ctx, CState): Tree = {
  ...
  def typedNew(tree: untpd.New, pt: Type)(using Ctx, CState): Tree =
    tree.tpt match {
  ...
  def typedTyped(tree: untpd.Typed, pt: Type)(using Ctx, CState): Tree = {
   ...
   */
  def tryWithClassTag(tree: Typed, pt: Type)(using Ctx, CState): Tree = tree.tpt.tpe.dealias match {
  ...
  def typedNamedArg(tree: untpd.NamedArg, pt: Type)(using Ctx, CState): NamedArg = {
    val arg1 = typed(tree.arg, pt)
    assignType(cpy.NamedArg(tree)(tree.name, arg1), arg1)
  }

  def typedAssign(tree: untpd.Assign, pt: Type)(using Ctx, CState): Tree =
    tree.lhs match {
  ...
  def typedBlockStats(stats: List[untpd.Tree])(using Ctx, CState): (List[tpd.Tree], Ctx) =
    index(stats)
    typedStats(stats, ctx.owner)

  def typedBlock(tree: untpd.Block, pt: Type)(using Ctx, CState): Tree = {
  ...
  def escapingRefs(block: Tree, localSyms: => List[Symbol])(using Ctx, CState): collection.Set[NamedType] = {
   ...
   */
  protected def ensureNoLocalRefs(tree: Tree, pt: Type, localSyms: => List[Symbol])(using Ctx, CState): Tree = {
  ...
  def typedIf(tree: untpd.If, pt: Type)(using Ctx, CState): Tree =
   ...
   *     "abc" flatMap double
   */
  private def decomposeProtoFunction(pt: Type, defaultArity: Int, tree: untpd.Tree)(using Ctx, CState): (List[Type], untpd.Tree) = {
  ...
  def typedFunction(tree: untpd.Function, pt: Type)(using Ctx, CState): Tree =
    if (ctx.mode is Mode.Type) typedFunctionType(tree, pt)
    else typedFunctionValue(tree, pt)

  def typedFunctionType(tree: untpd.Function, pt: Type)(using Ctx, CState): Tree = {
    ...
    def typedDependent(params: List[untpd.ValDef])(using Ctx, CState): Tree =
      val fixThis = new untpd.UntypedTreeMap:
        ...
        override def transform(tree: untpd.Tree)(using Ctx, CState): untpd.Tree = tree match
  ...
  def typedFunctionValue(tree: untpd.Function, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedClosure(tree: untpd.Closure, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedMatch(tree: untpd.Match, pt: Type)(using Ctx, CState): Tree =
    tree.selector match {
      case EmptyTree =>
        if (tree.isInline) {
          ...
          && tree.cases
            ...
            .forall {
              case (pat: untpd.Typed, pt) =>
                ...
                val gadtCtx: Ctx = ctx.fresh.setFreshGADTBounds
   ...
   */
  def typedDependentMatchFinish(tree: untpd.Match, sel: Tree, wideSelType: Type, cases: List[untpd.CaseDef], pt: MatchType)(using Ctx, CState): Tree = {
    var caseCtx = ctx
  ...
  def typedMatchFinish(tree: untpd.Match, sel: Tree, wideSelType: Type, cases: List[untpd.CaseDef], pt: Type)(using Ctx, CState): Tree = {
    val cases1 = harmonic(harmonize, pt)(typedCases(cases, sel, wideSelType, pt.dropIfProto))
      .asInstanceOf[List[CaseDef]]
    assignType(cpy.Match(tree)(sel, cases1), sel, cases1)
  }

  def typedCases(cases: List[untpd.CaseDef], sel: Tree, wideSelType: Type, pt: Type)(using Ctx, CState): List[CaseDef] =
    var caseCtx = ctx
  ...
  /** - strip all instantiated TypeVars from pattern types.
    ...
    */
  private def indexPattern(cdef: untpd.CaseDef)(using Ctx, CState) = new TreeMap {
    val stripTypeVars = new TypeMap {
      def apply(t: Type) = mapOver(t)
    }
    override def transform(trt: Tree)(using Ctx, CState) =
   ...
   */
  def instantiateMatchTypeProto(pat: Tree, pt: Type)(using Ctx, CState) = pt match {
  ...
  /** Type a case. */
  def typedCase(tree: untpd.CaseDef, sel: Tree, wideSelType: Type, pt: Type)(using Ctx, CState): CaseDef = {
    val originalCtx = ctx
    val gadtCtx: Ctx = ctx.fresh.setFreshGADTBounds

    def caseRest(pat: Tree)(using Ctx, CState) = {
  ...
  def typedLabeled(tree: untpd.Labeled)(using Ctx, CState): Labeled = {
  ...
  def typedTypeCase(cdef: untpd.CaseDef, selType: Type, pt: Type)(using Ctx, CState): CaseDef = {
    def caseRest(using Ctx, CState) = {
  ...
  def typedReturn(tree: untpd.Return)(using Ctx, CState): Return = {
    ...
    def enclMethInfo(cx: Ctx): (Tree, Type) = {
      val owner = cx.owner
      if (owner.isType) {
  ...
  def typedWhileDo(tree: untpd.WhileDo)(using Ctx, CState): Tree =
    inCtx(Nullables.whileContext(tree.span)) {
      val cond1 =
  ...
  def typedTry(tree: untpd.Try, pt: Type)(using Ctx, CState): Try = {
  ...
  def typedTry(tree: untpd.ParsedTry, pt: Type)(using Ctx, CState): Try =
  ...
  def typedThrow(tree: untpd.Throw)(using Ctx, CState): Tree = {
    val expr1 = typed(tree.expr, defn.ThrowableType)
    Throw(expr1).withSpan(tree.span)
  }

  def typedSeqLiteral(tree: untpd.SeqLiteral, pt: Type)(using Ctx, CState): SeqLiteral = {
  ...
  def typedInlined(tree: untpd.Inlined, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedTypeTree(tree: untpd.TypeTree, pt: Type)(using Ctx, CState): Tree =
    tree match {
  ...
  def typedSingletonTypeTree(tree: untpd.SingletonTypeTree)(using Ctx, CState): SingletonTypeTree = {
    val ref1 = typedExpr(tree.ref)
    checkStable(ref1.tpe, tree.sourcePos, "singleton type")
    assignType(cpy.SingletonTypeTree(tree)(ref1), ref1)
  }

  def typedRefinedTypeTree(tree: untpd.RefinedTypeTree)(using Ctx, CState): TypTree = {
  ...
  def typedAppliedTypeTree(tree: untpd.AppliedTypeTree)(using Ctx, CState): Tree = {
    tree.args match
  ...
  private def typeIndexedLambdaTypeTree(
      tree: untpd.LambdaTypeTree, tparams: List[untpd.TypeDef], body: untpd.Tree)(using Ctx, CState) =
  ...
  def typedLambdaTypeTree(tree: untpd.LambdaTypeTree)(using Ctx, CState): Tree =
    val LambdaTypeTree(tparams, body) = tree
    index(tparams)
    typeIndexedLambdaTypeTree(tree, tparams, body)

  def typedTermLambdaTypeTree(tree: untpd.TermLambdaTypeTree)(using Ctx, CState): Tree =
    if dependentEnabled then
      errorTree(tree, i"Not yet implemented: (...) =>> ...")
    else
      errorTree(tree, dependentStr)

  def typedMatchTypeTree(tree: untpd.MatchTypeTree, pt: Type)(using Ctx, CState): Tree = {
    val bound1 =
  ...
  def typedByNameTypeTree(tree: untpd.ByNameTypeTree)(using Ctx, CState): ByNameTypeTree = {
    val result1 = typed(tree.result)
    assignType(cpy.ByNameTypeTree(tree)(result1), result1)
  }

  def typedTypeBoundsTree(tree: untpd.TypeBoundsTree, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedBind(tree: untpd.Bind, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedAlternative(tree: untpd.Alternative, pt: Type)(using Ctx, CState): Alternative = {
   ...
   *  enclosing class.
   */
  def annotContext(mdef: untpd.Tree, sym: Symbol)(using Ctx, CState): Ctx = {
  ...
  def completeAnnotations(mdef: untpd.MemberDef, sym: Symbol)(using Ctx, CState): Unit = {
  ...
  def typedAnnotation(annot: untpd.Tree)(using Ctx, CState): Tree =
    typed(annot, defn.AnnotationClass.typeRef)

  def typedValDef(vdef: untpd.ValDef, sym: Symbol)(using Ctx, CState): Tree = {
  ...
  def typedDefDef(ddef: untpd.DefDef, sym: Symbol)(using Ctx, CState): Tree = {
  ...
  def typedTypeDef(tdef: untpd.TypeDef, sym: Symbol)(using Ctx, CState): Tree = {
  ...
  def typedClassDef(cdef: untpd.TypeDef, cls: ClassSymbol)(using Ctx, CState): Tree = {
  ...
  protected def addAccessorDefs(cls: Symbol, body: List[Tree])(using Ctx, CState): List[Tree] =
   ...
   */
  def ensureFirstIsClass(parents: List[Type], span: Span)(using Ctx, CState): List[Type] = {
  ...
  def ensureFirstTreeIsClass(parents: List[Tree], span: Span)(using Ctx, CState): List[Tree] = parents match {
   ...
   */
  def ensureConstrCall(cls: ClassSymbol, parents: List[Tree])(using Ctx, CState): List[Tree] = {
  ...
  def localDummy(cls: ClassSymbol, impl: untpd.Template)(using Ctx, CState): Symbol =
    newLocalDummy(cls, impl.span)

  def typedImport(imp: untpd.Import, sym: Symbol)(using Ctx, CState): Import = {
  ...
  def typedPackageDef(tree: untpd.PackageDef)(using Ctx, CState): Tree =
  ...
  def typedAnnotated(tree: untpd.Annotated, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedTypedSplice(tree: untpd.TypedSplice)(using Ctx, CState): Tree =
    tree.splice match {
  ...
  def typedAsFunction(tree: untpd.PostfixOp, pt: Type)(using Ctx, CState): Tree = {
   ...
   */
  def typedInfixOp(tree: untpd.InfixOp, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typedTuple(tree: untpd.Tuple, pt: Type)(using Ctx, CState): Tree = {
  ...
  protected def retrieveSym(tree: untpd.Tree)(using Ctx, CState): Symbol = tree.removeAttachment(SymOfTree) match {
    case Some(sym) =>
      assertSamePeriod()
      sym.ensureCompleted()
      sym
  ...
  def typedUnadapted(initTree: untpd.Tree, pt: Type = WildcardType)(using Ctx, CState): Tree =
   ...
   *                     at the present time
   */
  def typedUnadapted(initTree: untpd.Tree, pt: Type, locked: TypeVars)(using Ctx, CState): Tree = {
    ...
    xtree.removeAttachment(TypedAhead) match {
      ...
      case none =>
        ...
        def typedNamed(tree: untpd.NameTree, pt: Type)(using Ctx, CState): Tree = {
  ...
  protected def simplify(tree: Tree, pt: Type, locked: TypeVars)(using Ctx, CState): tree.type = {
  ...
  protected def makeContextualFunction(tree: untpd.Tree, pt: Type)(using Ctx, CState): Tree = {
  ...
  def typed(tree: untpd.Tree, pt: Type, locked: TypeVars)(using Ctx, CState): Tree =
  ...
  def typed(tree: untpd.Tree, pt: Type = WildcardType)(using Ctx, CState): Tree =
    typed(tree, pt, ctx.typerState.ownedVars)

  def typedTrees(trees: List[untpd.Tree])(using Ctx, CState): List[Tree] =
    trees mapconserve (typed(_))

  def typedStats(stats: List[untpd.Tree], exprOwner: Symbol)(using Ctx, CState): (List[Tree], Ctx) = {
    val buf = new mutable.ListBuffer[Tree]
    val enumContexts = new mutable.HashMap[Symbol, Ctx]
    val initialNotNullInfos = ctx.notNullInfos
      // A map from `enum` symbols to the contexts enclosing their definitions
    @tailrec def traverse(stats: List[untpd.Tree])(using Ctx, CState): (List[Tree], Ctx) = stats match {
    ...
    def finalize(stat: Tree)(using Ctx, CState): Tree = stat match {
   ...
   */
  def adaptCreationContext(mdef: untpd.DefTree)(using Ctx, CState): Boolean =
   ...
   */
  protected def inlineExpansion(mdef: DefDef)(using Ctx, CState): List[Tree] =
  ...
  def typedExpr(tree: untpd.Tree, pt: Type = WildcardType)(using Ctx, CState): Tree =
    withoutMode(Mode.PatternOrTypeBits)(typed(tree, pt))
  def typedType(tree: untpd.Tree, pt: Type = WildcardType)(using Ctx, CState): Tree = // todo: retract mode between Type and Pattern?
    withMode(Mode.Type)(typed(tree, pt))
  def typedPattern(tree: untpd.Tree, selType: Type = WildcardType)(using Ctx, CState): Tree =
    withMode(Mode.Pattern)(typed(tree, selType))

  def tryEither[T](op: (Ctx, CState) ?=> T)(fallBack: (T, TyperState) => T)(using Ctx, CState): T = {
    val nestedCtx = ctx.fresh.setNewTyperState()
   ...
   */
  def tryAlternatively[T](op1: (Ctx, CState) ?=> T)(op2: (Ctx, CState) ?=> T)(using Ctx, CState): T =
  ...
  def isApplyProto(pt: Type)(using Ctx, CState): Boolean = pt.revealIgnored match {
  ...
  def tryNew[T >: Untyped <: Type]
    (treesInst: Instance[T])(tree: Trees.Tree[T], pt: Type, fallBack: TyperState => Tree)(using Ctx, CState): Tree = {
   ...
   */
  def tryInsertApplyOrImplicit(tree: Tree, pt: ProtoType, locked: TypeVars)(fallBack: => Tree)(using Ctx, CState): Tree = {
    ...
    def tryApply(using Ctx, CState) = {
      val pt1 = pt.withContext(ctx, currentPeriod)
    ...
    def tryImplicit(fallBack: => Tree) =
      tryInsertImplicitOnQualifier(tree, pt.withContext(ctx, currentPeriod), locked)
   ...
   */
  def tryInsertImplicitOnQualifier(tree: Tree, pt: Type, locked: TypeVars)(using Ctx, CState): Option[Tree] = trace(i"try insert impl on qualifier $tree $pt") {
    tree match {
   ...
   */
  def adapt(tree: Tree, pt: Type, locked: TypeVars, tryGadtHealing: Boolean = true)(using Ctx, CState): Tree =
    try
  ...
  final def adapt(tree: Tree, pt: Type)(using Ctx, CState): Tree =
    adapt(tree, pt, ctx.typerState.ownedVars)

  private def adapt1(tree: Tree, pt: Type, locked: TypeVars, tryGadtHealing: Boolean)(using Ctx, CState): Tree = {
    assert(pt.exists && !pt.isInstanceOf[ExprType] || ctx.reporter.errorsReported)
    def methodStr = err.refStr(methPart(tree).tpe)

    def readapt(tree: Tree, shouldTryGadtHealing: Boolean = tryGadtHealing)(using Ctx, CState) = adapt(tree, pt, locked, shouldTryGadtHealing)
    def readaptSimplified(tree: Tree)(using Ctx, CState) = readapt(simplify(tree, pt, locked))
    ...
    def adaptNoArgsImplicitMethod(wtp: MethodType): Tree = {
      ...
      def addImplicitArgs(using Ctx, CState) = {
    ...
    def adaptNoArgsOther(wtp: Type): Tree = {
      ...
      if (isContextFunctionRef(wtp) &&
        ...
        if (tree.symbol eq defn.StringContext_f) {
          ...
          // a call to dottyBench.internal.StringContext.f which we can implement using the new macros.
          ...
          report.warning("Scala 2 macro cannot be used in Dotty, this call will crash at runtime. See https://dottyBench.epfl.ch/docs/reference/dropped-features/macros.html", tree.sourcePos.startPos)
          ...
          report.error(
            """Scala 2 macro cannot be used in Dotty. See https://dottyBench.epfl.ch/docs/reference/dropped-features/macros.html
     ...
     */
    def captureWildcards(tp: Type)(using Ctx, CState): Type = tp match {
    ...
    def adaptToSubType(wtp: Type): Tree = {
      ...
      pt match {
        case SelectionProto(name, mbrType, _, _) =>
          def tryExtension(using Ctx, CState): Tree =
            try
  ...
  def suppressInline(using Ctx, CState): Boolean = currentlyAfterTyper
   ...
   */
  protected def matchingApply(methType: MethodOrPoly, pt: FunProto)(using Ctx, CState): Boolean =
   ...
   */
  protected def checkEqualityEvidence(tree: tpd.Tree, pt: Type)(using Ctx, CState) : Unit =
    tree match {
  ...
  private def checkStatementPurity(tree: tpd.Tree)(original: untpd.Tree, exprOwner: Symbol)(using Ctx, CState): Unit =
  ...
  private def typedScala2MacroBody(call: untpd.Tree)(using Ctx, CState): Tree =
    // TODO check that call is to a method with valid signature
    def typedPrefix(tree: untpd.RefTree)(splice: (Ctx, CState) ?=> Tree => Tree)(using Ctx, CState): Tree = {
      tryAlternatively {
