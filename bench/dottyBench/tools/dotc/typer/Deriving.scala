...
trait Deriving {
   ...
   *                   `derives` instance.
   */
  class Deriver(cls: ClassSymbol, codePos: SourcePosition)(using Ctx, CState) {
    ...
    def syntheticDefs: List[tpd.Tree] = {
      ...
      def typeclassInstance(sym: Symbol)(using Ctx, CState): List[Type] => (List[List[tpd.Tree]] => tpd.Tree) = {
      ...
      def syntheticDef(sym: Symbol): Tree = inCtx(ctx.fresh.setOwner(sym).setNewScope) {
