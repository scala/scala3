...
object Implicits {
  ...
  class RenamedImplicitRef(val underlyingRef: TermRef, val alias: TermName) extends ImplicitRef {
    def implicitName(using Ctx, CState): TermName = alias
  }
   ...
   */
  def hasExtMethod(tp: Type, expected: Type)(using Ctx, CState) = expected match
    case SelectionProto(name, _, _, _) =>
      tp.memberBasedOnFlags(name.toExtensionName, required = ExtensionMethod).exists
    case _ => false

  def strictEquality(using Ctx, CState): Boolean =
  ...
  abstract class ImplicitRefs(initCtx: Context) {
    ...
    protected given Ctx = irefCtx
    protected given CState = initCtx.cstate
    ...
    private def widenSingleton(tp: Type)(using Ctx, CState): Type = {
    ...
    protected def isAccessible(ref: TermRef)(using Ctx, CState): Boolean

    /** Return those references in `refs` that are compatible with type `pt`. */
    protected def filterMatching(pt: Type)(using Ctx, CState): List[Candidate] = {
      ...
      def candidateKind(ref: TermRef)(using Ctx, CState): Candidate.Kind = { /*trace(i"candidateKind $ref $pt")*/
  ...
  class OfTypeImplicits(tp: Type, override val companionRefs: TermRefSet)(initctx: Context) extends ImplicitRefs(initctx) {
    ...
    override def isAccessible(ref: TermRef)(using Ctx, CState): Boolean =
   ...
   */
  class ContextualImplicits(val refs: List[ImplicitRef], val outerImplicits: ContextualImplicits)(
    ...
    def exclude(root: Symbol): ContextualImplicits =
      if (this == NoContext.implicits) this
        ...
        else new ContextualImplicits(refs, outerExcluded)(irefCtx.toContext)
      }
  }
  ...
  object SearchFailure {
    def apply(tpe: SearchFailureType)(using Ctx, CState): SearchFailure = {
      val id = tpe match
  ...
  abstract class SearchFailureType extends ErrorType {
    ...
    def clarify(tp: Type)(using Ctx, CState): Type = tp

    final protected def qualify(using Ctx, CState): String = expectedType match {
    ...
    def explanation(using Ctx, CState): String

    def msg(using Ctx, CState): Message = explanation
     ...
     */
    def whyNoConversion(using Ctx, CState): String = ""
  }
  ...
  extends SearchFailureType {
     ...
     *  what was expected
     */
    override def clarify(tp: Type)(using Ctx, CState): Type =
  ...
  @sharable val NoMatchingImplicitsFailure: SearchFailure =
    SearchFailure(NoMatchingImplicits)(using NoContext, Nowhere)

  /** An ambiguous implicits failure */
  class AmbiguousImplicits(val alt1: SearchSuccess, val alt2: SearchSuccess, val expectedType: Type, val argument: Tree) extends SearchFailureType {
    def explanation(using Ctx, CState): String =
      em"both ${err.refStr(alt1.ref)} and ${err.refStr(alt2.ref)} $qualify"
    override def whyNoConversion(using Ctx, CState): String = {
  ...
  class FailedExtension(extApp: Tree, val expectedType: Type) extends SearchFailureType:
    ...
    def explanation(using Ctx, CState) = em"$extApp does not $qualify"
}
...
trait ImplicitRunInfo:
   ...
   */
  def implicitScope(tp: Type)(using Ctx, CState): OfTypeImplicits =
...
trait Implicits { self: Typer =>
  ...
  override def viewExists(from: Type, to: Type)(using Ctx, CState): Boolean =
       !from.isError
    && !to.isError
    && !currentlyAfterTyper
   ...
   */
  def inferView(from: Tree, to: Type)(using Ctx, CState): SearchResult = {
    record("inferView")
    if    to.isAny
       || to.isAnyRef
   ...
   */
  def inferImplicitArg(formal: Type, span: Span)(using Ctx, CState): Tree =
  ...
  def implicitArgTree(formal: Type, span: Span)(using Ctx, CState): Tree = {
  ...
  def missingArgMsg(arg: Tree, pt: Type, where: String)(using Ctx, CState): String = {
    ...
    arg.tpe match {
      case ambi: AmbiguousImplicits =>
        ...
        def userDefinedAmbiguousImplicitMsg(alt: SearchSuccess, raw: String) = {
          ...
          def resolveTypes(targs: List[Tree])(using Ctx, CState) =
      ...
      case _ =>
        ...
        def hiddenImplicitsAddendum: String =
          ...
          def findHiddenImplicitsCtx(c: Ctx): Ctx =
            if c == NoContext then c
            else c.freshOver(findHiddenImplicitsCtx(c.outer)).toContext.addMode(Mode.FindHiddenImplicits)
  ...
  def implicitParamString(paramName: TermName, methodStr: String, tree: Tree)(using Ctx, CState): String =
    tree match {
   ...
   */
  def assumedCanEqual(ltp: Type, rtp: Type)(using Ctx, CState) = {
  ...
  def checkCanEqual(ltp: Type, rtp: Type, span: Span)(using Ctx, CState): Unit =
   ...
   */
  def inferImplicit(pt: Type, argument: Tree, span: Span)(using Ctx, CState): SearchResult =
  ...
  def typedImplicit(cand: Candidate, pt: Type, argument: Tree, span: Span)(using Ctx, CState): SearchResult =  trace(i"typed implicit ${cand.ref}, pt = $pt, implicitsEnabled == ${ctx.mode is ImplicitsEnabled}", implicits, show = true) {
    if ctx.run.isCancelled then NoMatchingImplicitsFailure
      ...
      val adapted =
        if (argument.isEmpty)
          ...
          def tryConversion(using Ctx, CState) = {
            val untpdConv =
          ...
          pt match
            case SelectionProto(name: TermName, mbrType, _, _) if cand.isExtension =>
              def tryExtension(using Ctx, CState) =
  ...
  class ImplicitSearch(protected val pt: Type, protected val argument: Tree, span: Span)(using Ctx, CState) {
...
abstract class SearchHistory { outer =>
   ...
   */
  def nest(cand: Candidate, pt: Type)(using Ctx, CState): SearchHistory =
    new SearchHistory {
      val root = outer.root
   ...
   */
  def checkDivergence(cand: Candidate, pt: Type)(using Ctx, CState): Boolean = {
   ...
   */
  def recursiveRef(pt: Type)(using Ctx, CState): Type = {
    val widePt = pt.widenExpr
  ...
  def linkBynameImplicit(tpe: Type)(using Ctx, CState): TermRef =
    root.linkBynameImplicit(tpe)
  def refBynameImplicit(tpe: Type)(using Ctx, CState): Type =
    root.refBynameImplicit(tpe)
  def defineBynameImplicit(tpe: Type, result: SearchSuccess)(using Ctx, CState): SearchResult =
    root.defineBynameImplicit(tpe, result)

  // This is NOOP unless at the root of this search history.
  def emitDictionary(span: Span, result: SearchResult)(using Ctx, CState): SearchResult = result
...
final class SearchRoot extends SearchHistory {
   ...
   */
  override def linkBynameImplicit(tpe: Type)(using Ctx, CState): TermRef =
   ...
   */
  override def refBynameImplicit(tpe: Type)(using Ctx, CState): Type =
   ...
   */
  override def defineBynameImplicit(tpe: Type, result: SearchSuccess)(using Ctx, CState): SearchResult =
    implicitDictionary.get(tpe) match {
      case Some((ref, _)) =>
   ...
   */
  override def emitDictionary(span: Span, result: SearchResult)(using Ctx, CState): SearchResult =
...
sealed class TermRefSet(using Ctx, CState):
...
object TermRefSet:
  ...
  @sharable val empty = new TermRefSet(using NoContext, Nowhere):
