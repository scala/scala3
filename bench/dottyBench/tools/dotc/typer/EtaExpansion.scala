...
abstract class Lifter {
  ...
  def noLift(expr: Tree)(using Ctx, CState): Boolean
  ...
  protected def liftedDef(sym: TermSymbol, rhs: Tree)(using Ctx, CState): MemberDef = ValDef(sym, rhs)

  /** Is lifting performed on erased terms? */
  protected def isErased = false

  private def lift(defs: mutable.ListBuffer[Tree], expr: Tree, prefix: TermName = EmptyTermName)(using Ctx, CState): Tree =
    if (noLift(expr)) expr
    else {
   ...
   *     lhs += expr
   */
  def liftAssigned(defs: mutable.ListBuffer[Tree], tree: Tree)(using Ctx, CState): Tree = tree match {
  ...
  private def liftArg(defs: mutable.ListBuffer[Tree], arg: Tree, prefix: TermName = EmptyTermName)(using Ctx, CState): Tree =
    arg match {
   ...
   */
  def liftArgs(defs: mutable.ListBuffer[Tree], methRef: Type, args: List[Tree])(using Ctx, CState): List[Tree] =
    methRef.widen match {
      case mt: MethodType =>
   ...
   *
   */
  def liftApp(defs: mutable.ListBuffer[Tree], tree: Tree)(using Ctx, CState): Tree = tree match {
    case Apply(fn, args) =>
   ...
   */
  def liftPrefix(defs: mutable.ListBuffer[Tree], tree: Tree)(using Ctx, CState): Tree =
    if (isIdempotentExpr(tree)) tree else lift(defs, tree)
}

/** No lifting at all */
object NoLift extends Lifter {
  def noLift(expr: tpd.Tree)(using Ctx, CState): Boolean = true
}

/** Lift all impure arguments */
class LiftImpure extends Lifter {
  def noLift(expr: tpd.Tree)(using Ctx, CState): Boolean = tpd.isPureExpr(expr)
}
object LiftImpure extends LiftImpure

/** Lift all impure or complex arguments */
class LiftComplex extends Lifter {
  def noLift(expr: tpd.Tree)(using Ctx, CState): Boolean = tpd.isPurePath(expr)
...
object LiftToDefs extends LiftComplex {
  ...
  override def liftedDef(sym: TermSymbol, rhs: tpd.Tree)(using Ctx, CState): tpd.DefDef = tpd.DefDef(sym, rhs)
}
...
object EtaExpansion extends LiftImpure {
   ...
   */
  def etaExpand(tree: Tree, mt: MethodType, xarity: Int)(using Ctx, CState): untpd.Tree = {
    import untpd._
