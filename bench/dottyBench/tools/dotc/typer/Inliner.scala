...
import dottyBench.tools.dotc.util.{SimpleIdentityMap, SimpleIdentitySet, SourceFile, SourcePosition}
...
import dottyBench.tools.dotc.transform.{Splicer, TreeMapWithStages}
...
object Inliner {
   ...
   */
  def hasBodyToInline(sym: SymDenotation)(using Ctx, CState): Boolean =
    sym.isInlineMethod && sym.hasAnnotation(defn.BodyAnnot)

  /** The body to inline for method `sym`, or `EmptyTree` if none exists.
   *  @pre  hasBodyToInline(sym)
   */
  def bodyToInline(sym: SymDenotation)(using Ctx, CState): Tree =
   ...
   */
  def inlineCall(tree: Tree)(using Ctx, CState): Tree = {
    ...
    def liftFromInlined(call: Tree) = new TreeMap:
      override def transform(t: Tree)(using Ctx, CState) =
   ...
   */
  def inlinedUnapply(unapp: tpd.UnApply)(using Ctx, CState): Tree = {
   ...
   */
  def bodyRetainer(mdef: DefDef)(using Ctx, CState): DefDef =
  ...
  def dropInlined(inlined: Inlined)(using Ctx, CState): Tree =
    val tree1 =
  ...
  def reposition(tree: Tree, callSpan: Span)(using Ctx, CState): Tree = {
    ...
    class Reposition extends TreeMap(cpyWithNewSource) {
      ...
      override def transform(tree: Tree)(using Ctx, CState): Tree = {
        ...
        inCtx(ctx.withSource(curSource)) {
          tree match
   ...
   */
  def inlineCallTrace(callSym: Symbol, pos: SourcePosition)(using Ctx, CState): Tree = {
  ...
  object Intrinsics {
    import dottyBench.tools.dotc.reporting.Diagnostic.Error
    private enum ErrorKind:
      case Parser, Typer

    private def compileForErrors(tree: Tree, stopAfterParser: Boolean)(using Ctx, CState): List[(ErrorKind, Error)] =
    ...
    private def packError(kind: ErrorKind, error: Error)(using Ctx, CState): Tree =
    ...
    private def packErrors(errors: List[(ErrorKind, Error)])(using Ctx, CState): Tree =
    ...
    def typeChecks(tree: Tree)(using Ctx, CState): Tree =
    ...
    def typeCheckErrors(tree: Tree)(using Ctx, CState): Tree =
 ...
 */
class Inliner(call: tpd.Tree, rhsToInline: tpd.Tree)(using Ctx, CState) {
  import tpd._
  import Inliner._
  ...
  private def newSym(name: Name, flags: FlagSet, info: Type)(using Ctx, CState): Symbol =
  ...
  private def paramBindingDef(name: Name, paramtp: Type, arg0: Tree,
                              bindingsBuf: mutable.ListBuffer[ValOrDefDef])(using Ctx, CState): ValOrDefDef = {
    val arg = arg0 match {
  ...
  object isElideableExpr {
    def isStatElideable(tree: Tree)(using Ctx, CState): Boolean = unsplice(tree) match {
      case EmptyTree
         | TypeDef(_, _)
         | Import(_, _)
         | DefDef(_, _, _, _, _) =>
   ...
   */
  def integrate(tree: Tree, originalOwner: Symbol)(using Ctx, CState): Tree =
    // assertAllPositioned(tree)   // debug
  ...
  object reducer {
    ...
    private object NewInstance {
      def unapply(tree: Tree)(using Ctx, CState): Option[(Symbol, List[Tree], List[Tree], Boolean)] = {
     ...
     */
    def reduceProjection(tree: Tree)(using Ctx, CState): Tree = {
     ...
     */
    def normalizeBinding(binding: ValOrDefDef)(using Ctx, CState) = {
    ...
    private object InlineableArg {
      ...
      def unapply(tree: Trees.Ident[?])(using Ctx, CState): Option[Tree] = {
    ...
    object ConstantValue {
      def unapply(tree: Tree)(using Ctx, CState): Option[Any] = tree.tpe.widenTermRefExpr.normalized match {
        case ConstantType(Constant(x)) => Some(x)
        case _ => None
      }
    }

    def tryInline(tree: Tree)(using Ctx, CState): Tree = tree match {
    ...
    /** Rewrite an application
      ...
      */
    def betaReduce(tree: Tree)(using Ctx, CState): Tree = tree match {
     ...
     */
    def reduceInlineMatch(scrutinee: Tree, scrutType: Type, cases: List[CaseDef], typer: Typer)(using Ctx, CState): MatchRedux = {
      ...
      def reducePattern(
        ...
        def getTypeBindsMap(pat: Tree, tpt: Tree): TypeBindsMap = {
          val getBinds = new TreeAccumulator[Set[TypeSymbol]] {
            def apply(syms: Set[TypeSymbol], t: Tree)(using Ctx, CState): Set[TypeSymbol] = {
              val syms1 = t match {
        ...
        def addTypeBindings(typeBinds: TypeBindsMap)(using Ctx, CState): Unit =
        ...
        def registerAsGadtSyms(typeBinds: TypeBindsMap)(using Ctx, CState): Unit =
  ...
  class InlineTyper(initialErrorCount: Int) extends ReTyper {
    ...
    override def ensureAccessible(tpe: Type, superAccess: Boolean, pos: SourcePosition)(using Ctx, CState): Type = {
      tpe match {
    ...
    override def typedIf(tree: untpd.If, pt: Type)(using Ctx, CState): Tree =
    ...
    override def typedValDef(vdef: untpd.ValDef, sym: Symbol)(using Ctx, CState): Tree =
      val vdef1 =
    ...
    override def typedApply(tree: untpd.Apply, pt: Type)(using Ctx, CState): Tree =
    ...
    override def typedMatchFinish(tree: untpd.Match, sel: Tree, wideSelType: Type, cases: List[untpd.CaseDef], pt: Type)(using Ctx, CState) =
    ...
    override def suppressInline(using Ctx, CState) =
   ...
   */
  def dropUnusedDefs(bindings: List[MemberDef], tree: Tree)(using Ctx, CState): (List[MemberDef], Tree) = {
    ...
    if (typeBindings.nonEmpty) {
      ...
      val countRefs = new TreeTraverser {
        override def traverse(t: Tree)(using Ctx, CState) = {
  ...
  private def expandMacro(body: Tree, span: Span)(using Ctx, CState) = {
    assert(level == 0)
    ...
    val evaluatedSplice = inContext(tastyreflect.MacroExpansion.context(inlinedFrom)(using combinedContext)) {
   ...
   */
  private def macroDependencies(tree: Tree)(using Ctx, CState) =
    new TreeAccumulator[List[Symbol]] {
      private var level = -1
      override def apply(syms: List[Symbol], tree: tpd.Tree)(using Ctx, CState): List[Symbol] =
