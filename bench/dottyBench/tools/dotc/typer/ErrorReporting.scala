...
object ErrorReporting {
  ...
  def errorTree(tree: untpd.Tree, msg: Message, pos: SourcePosition)(using Ctx, CState): tpd.Tree =
    tree.withType(errorType(msg, pos))

  def errorTree(tree: untpd.Tree, msg: Message)(using Ctx, CState): tpd.Tree =
    errorTree(tree, msg, tree.sourcePos)

  def errorTree(tree: untpd.Tree, msg: TypeError, pos: SourcePosition)(using Ctx, CState): tpd.Tree =
    tree.withType(errorType(msg, pos))

  def errorType(msg: Message, pos: SourcePosition)(using Ctx, CState): ErrorType = {
    report.error(msg, pos)
    ErrorType(msg)
  }

  def errorType(ex: TypeError, pos: SourcePosition)(using Ctx, CState): ErrorType = {
    report.error(ex, pos)
    ErrorType(ex.toMessage)
  }

  def wrongNumberOfTypeArgs(fntpe: Type, expectedArgs: List[ParamInfo], actual: List[untpd.Tree], pos: SourcePosition)(using Ctx, CState): ErrorType =
    errorType(WrongNumberOfTypeArgs(fntpe, expectedArgs, actual), pos)

  class Errors(using Ctx, CState) {
  ...
  def err(using Ctx, CState): Errors = new Errors
}
