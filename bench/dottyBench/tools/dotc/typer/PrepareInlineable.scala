...
object PrepareInlineable {
  ...
  class InlineAccessors extends AccessProxies {
     ...
     */
    override def passReceiverAsArg(name: Name)(using Ctx, CState): Boolean = name match {
    ...
    abstract class MakeInlineableMap(val inlineSym: Symbol) extends TreeMap with Insert {
      ...
      def preTransform(tree: Tree)(using Ctx, CState): Tree

      def postTransform(tree: Tree)(using Ctx, CState): Tree = tree match {
    ...
    class MakeInlineableDirect(inlineSym: Symbol) extends MakeInlineableMap(inlineSym) {
      def preTransform(tree: Tree)(using Ctx, CState): Tree = tree match {
      ...
      override def ifNoHost(reference: RefTree)(using Ctx, CState): Tree = reference
    }
     ...
     */
    def makeInlineable(tree: Tree)(using Ctx, CState): Tree = {
  ...
  def isLocalOrParam(sym: Symbol, inlineMethod: Symbol)(using Ctx, CState): Boolean =
    sym.isContainedIn(inlineMethod) && sym != inlineMethod

  def isLocal(sym: Symbol, inlineMethod: Symbol)(using Ctx, CState): Boolean =
  ...
  def wrapRHS(original: untpd.DefDef, tpt: Tree, rhs: Tree)(using Ctx, CState): Tree =
   ...
   */
  def dropInlineIfError(sym: Symbol, op: => Tree)(using Ctx, CState): Tree =
  ...
  def registerInlineInfo(
      inlined: Symbol, treeExpr: (Ctx, CState) ?=> Tree)(using Ctx, CState): Unit =
    inlined.unforcedAnnotation(defn.BodyAnnot) match {
      ...
      case _ =>
        if (!currentlyAfterTyper) {
          val inlineCtx = ctx
          val inlineState = currentPeriod
          inlined.updateAnnotation(LazyBodyAnnotation {
            given Ctx = inlineCtx
            given CState = inlineState
  ...
  def checkInlineMethod(inlined: Symbol, body: Tree)(using Ctx, CState): body.type = {
