...
object Docstrings {
   ...
   */
  def cookComment(sym: Symbol, owner: Symbol)(using Ctx, CState): Option[Comment] =
    ctx.docCtx.flatMap { docCtx =>
      expand(sym, owner)(using ctx)(using docCtx)
    }

  private def expand(sym: Symbol, owner: Symbol)(using Ctx, CState)(using docCtx: ContextDocstrings): Option[Comment] =
  ...
  private def expandComment(sym: Symbol, owner: Symbol, comment: Comment)(using Ctx, CState)(using docCtx: ContextDocstrings): Comment = {
  ...
  private def expandComment(sym: Symbol)(using Ctx, CState)(using docCtx: ContextDocstrings): Option[Comment] =
