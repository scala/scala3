...
import Symbols._
import Decorators._
import dottyBench.tools.dotc.parsing.JavaParsers.JavaParser
...
class FrontEnd extends Phase {
  ...
  def monitor(doing: String)(body: => Unit)(using Ctx, CState): Unit =
    try body
    catch {
      case NonFatal(ex) =>
  ...
  def parse(using Ctx, CState): Unit = monitor("parsing") {
  ...
  def enterSyms(using Ctx, CState): Unit = monitor("indexing") {
    val unit = ctx.compilationUnit
    ctx.typer.index(unit.untpdTree)
    typr.println("entered: " + unit.source)
  }

  def typeCheck(using Ctx, CState): Unit = monitor("typechecking") {
    try
      val unit = ctx.compilationUnit
  ...
  private def firstTopLevelDef(trees: List[tpd.Tree])(using Ctx, CState): Symbol = trees match {
  ...
  override def runOn(units: List[CompilationUnit])(using Context): List[CompilationUnit] = {
    ...
    val unitContexts =
      for unit <- units yield
        ...
        currentContext.fresh.setCompilationUnit(unit)
