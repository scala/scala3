...
object Inferencing {
   ...
   */
  def isFullyDefined(tp: Type, force: ForceDegree.Value)(using Ctx, CState): Boolean = {
   ...
   */
  def fullyDefinedType(tp: Type, what: String, span: Span)(using Ctx, CState): Type =
   ...
   */
  def instantiateSelected(tp: Type, tvars: List[Type])(using Ctx, CState): Unit =
    if (tvars.nonEmpty)
   ...
   */
  def instantiateDependent(tp: Type, tparams: List[Symbol], vparamss: List[List[Symbol]])(using Ctx, CState): Unit = {
  ...
  private class IsFullyDefinedAccumulator(force: ForceDegree.Value, minimizeSelected: Boolean = false)
    (using Ctx, CState) extends TypeAccumulator[Boolean] {
  ...
  def approximateGADT(tp: Type)(using Ctx, CState): Type = {
  ...
  private class ApproximateGadtAccumulator(using Ctx, CState) extends TypeMap {
   ...
   */
  def replaceSingletons(tp: Type)(using Ctx, CState): Unit = {
  ...
  def inferTypeParams(tree: Tree, pt: Type)(using Ctx, CState): Tree = tree.tpe match {
    case tl: TypeLambda =>
  ...
  def isSkolemFree(tp: Type)(using Ctx, CState): Boolean =
   ...
   */
  def tvarsInParams(tree: Tree, locked: TypeVars)(using Ctx, CState): List[TypeVar] = {
   ...
   */
  private def instDirection(param: TypeParamRef)(using Ctx, CState): Int = {
   ...
   */
  def companionRef(tp: Type)(using Ctx, CState): Type =
   ...
   */
  def maximizeType(tp: Type, span: Span, fromScala2x: Boolean)(using Ctx, CState): List[Symbol] = {
   ...
   */
  private def variances(tp: Type)(using Ctx, CState): VarianceMap = {
...
trait Inferencing { this: Typer =>
   ...
   */
  def interpolateTypeVars(tree: Tree, pt: Type, locked: TypeVars)(using Ctx, CState): tree.type = {
