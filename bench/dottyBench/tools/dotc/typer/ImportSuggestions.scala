...
trait ImportSuggestions:
   ...
   */
  private def suggestionRoots(using Ctx, CState) =
    val seen = mutable.Set[TermRef]()

    def lookInside(root: Symbol)(using Ctx, CState): Boolean =
    ...
    def nestedRoots(site: Type)(using Ctx, CState): List[Symbol] =
    ...
    def rootsStrictlyIn(ref: Type)(using Ctx, CState): List[TermRef] =
      val site = ref.widen
    ...
    def rootsIn(ref: TermRef)(using Ctx, CState): List[TermRef] =
    ...
    def rootsOnPath(tp: Type)(using Ctx, CState): List[TermRef] = tp match
      case ref: TermRef => rootsIn(ref) ::: rootsOnPath(ref.prefix)
      case _ => Nil

    def recur(using Ctx, CState): List[TermRef] =
   ...
   */
  private def importSuggestions(pt: Type)(using Ctx, CState): (List[TermRef], List[TermRef]) =
    val timer = new Timer()
   ...
   */
  private def reduceTimeBudget(used: Int)(using Ctx, CState) =
   ...
   */
  extension (refs: List[(TermRef, String)]) def distinctRefs(using Ctx, CState): List[TermRef] = refs match
    case (ref, str) :: refs1 =>
   ...
   */
  extension (refs: List[TermRef]) def best(n: Int)(using Ctx, CState): List[TermRef] =
    ...
    for ref <- refs do
      ...
      while i < filled && diff == 0 do
        diff = withoutMode(Mode.ImplicitsEnabled)(compare(ref, top(i)))
        if diff > 0 then
          rest += top(i)
          top(i) = ref
        i += 1
   ...
   */
  override def importSuggestionAddendum(pt: Type)(using Ctx, CState): String =
