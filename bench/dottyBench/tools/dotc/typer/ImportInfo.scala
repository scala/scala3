...
object ImportInfo {
  ...
  def rootImport(rootRef: RootRef)(using Ctx, CState): ImportInfo =
    ...
    if isPredef then                                   // do not import any2stringadd
      selectors = untpd.ImportSelector(untpd.Ident(nme.any2stringadd), untpd.Ident(nme.WILDCARD))
        :: selectors
    def expr(using Ctx, CState) = tpd.Ident(refFn())
    def imp(using Ctx, CState) = tpd.Import(expr, selectors)
 ...
 *                         scala.Predef or dottyBench.DottyPredef in the start context, false otherwise.
 */
class ImportInfo(symf: (Ctx, CState) ?=> Symbol,
                 val selectors: List[untpd.ImportSelector],
                 symNameOpt: Option[TermName],
                 val isRootImport: Boolean = false) extends Showable {

  def sym(using Ctx, CState): Symbol = {
    if (mySym == null) {
      mySym = symf
      assert(mySym != null)
    }
  ...
  def site(using Ctx, CState): Type = sym.info match {
  ...
  def givenBound(using Ctx, CState) =
   ...
   */
  def unimported(using Ctx, CState): Symbol =
  ...
  def featureImported(feature: TermName, owner: Symbol)(using Ctx, CState): Boolean =
