...
trait Dynamic {
   ...
   */
  def typedDynamicApply(tree: untpd.Apply, pt: Type)(using Ctx, CState): Tree = {
   ...
   */
  def typedDynamicSelect(tree: untpd.Select, targs: List[Tree], pt: Type)(using Ctx, CState): Tree =
    typedApply(coreDynamic(tree.qualifier, nme.selectDynamic, tree.name, tree.span, targs), pt)

  /** Translate selection that does not typecheck according to the normal rules into a updateDynamic.
   *    foo.bar = baz ~~> foo.updateDynamic(bar)(baz)
   */
  def typedDynamicAssign(tree: untpd.Assign, pt: Type)(using Ctx, CState): Tree = {
  ...
  private def coreDynamic(qual: untpd.Tree, dynName: Name, name: Name, selSpan: Span, targs: List[untpd.Tree])(using Ctx, CState): untpd.Apply = {
   ...
   *  type.
   */
  def handleStructural(tree: Tree)(using Ctx, CState): Tree = {
