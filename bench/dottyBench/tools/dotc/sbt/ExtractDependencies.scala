...
import dottyBench.tools.io
import dottyBench.tools.io.{AbstractFile, PlainFile, ZipArchive}
...
class ExtractDependencies extends Phase {
   ...
   */
  def recordDependency(dep: ClassDependency)(using Ctx, CState): Unit = {
...
object ExtractDependencies {
  def classNameAsString(sym: Symbol)(using Ctx, CState): String =
...
private class ExtractDependenciesCollector extends tpd.TreeTraverser { thisTreeTraverser =>
  ...
  private def responsibleForImports(using Ctx, CState) = {
    def firstClassOrModule(tree: Tree) = {
      val acc = new TreeAccumulator[Symbol] {
        def apply(x: Symbol, t: Tree)(using Ctx, CState) =
          t match {
   ...
   */
  private def resolveDependencySource(using Ctx, CState): Symbol = {
  ...
  private def addUsedName(name: Name, scope: UseScope)(using Ctx, CState): Unit = {
  ...
  private def mangledName(sym: Symbol)(using Ctx, CState): Name = {
  ...
  private def addMemberRefDependency(sym: Symbol)(using Ctx, CState): Unit =
  ...
  private def addInheritanceDependencies(tree: Template)(using Ctx, CState): Unit =
  ...
  private def ignoreDependency(sym: Symbol)(using Ctx, CState) =
    !sym.exists ||
   ...
   */
  override def traverse(tree: Tree)(using Ctx, CState): Unit = try {
    tree match {
   ...
   */
  private abstract class TypeDependencyTraverser(using Ctx, CState) extends TypeTraverser() {
  ...
  def addTypeDependency(tpe: Type)(using Ctx, CState): Unit = {
  ...
  def addPatMatDependency(tpe: Type)(using Ctx, CState): Unit = {
    val traverser = new TypeDependencyTraverser {
