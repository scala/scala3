...
class CompilationUnit protected (val source: SourceFile) {
   ...
   */
  def suspend()(using Ctx, CState): Nothing =
...
object CompilationUnit {
  ...
  def apply(clsd: ClassDenotation, unpickled: Tree, forceTrees: Boolean)(using Ctx, CState): CompilationUnit =
    apply(new SourceFile(clsd.symbol.associatedFile, Array.empty[Char]), unpickled, forceTrees)

  /** Make a compilation unit, given picked bytes and unpickled tree */
  def apply(source: SourceFile, unpickled: Tree, forceTrees: Boolean)(using Ctx, CState): CompilationUnit = {
   ...
   */
  def apply(source: SourceFile, mustExist: Boolean = true)(using Ctx, CState): CompilationUnit = {
    val src =
      if (!mustExist)
        source
