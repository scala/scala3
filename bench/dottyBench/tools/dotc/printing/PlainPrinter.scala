...
class PlainPrinter(_ctx: Context) extends Printer {
  ...
  protected given [DummyToEnforceDef] as CState = curCtx.cstate
  ...
  private def useColors(using Context) = ctx.settings.color.value == "always"

  protected def coloredStr(text: String, color: String): String =
    if useColors then color + text + SyntaxHighlighting.NoColor else text
