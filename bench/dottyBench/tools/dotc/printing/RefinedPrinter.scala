...
import dottyBench.tools.dotc.util.{NameTransformer, SourcePosition}
import dottyBench.tools.dotc.ast.untpd.{MemberDef, Modifiers, PackageDef, RefTree, Template, TypeDef, ValOrDefDef}
...
class RefinedPrinter(_ctx: Context) extends PlainPrinter(_ctx) {
  ...
  def withEnclosingDef(enclDef: Tree[? >: Untyped])(op: => Text): Text = {
    ...
    if (enclDef.hasType && enclDef.symbol.exists)
      myCtx = currentContext.withOwner(enclDef.symbol)
  ...
  def inPattern(op: => Text): Text = {
    val savedCtx = myCtx
    myCtx = withMode(Mode.Pattern)(currentContext)
