...
object Highlighting {
  ...
  abstract class Highlight(private val highlight: String) {
    ...
    def +(other: Highlight)(using Ctx, CState): HighlightBuffer =
      new HighlightBuffer(this) + other

    def +(other: String)(using Ctx, CState): HighlightBuffer =
      new HighlightBuffer(this) + other
  }

  abstract class Modifier(private val mod: String, text: String) extends Highlight(Console.RESET) {
    override def show(using Ctx, CState): String =
      if (ctx.settings.color.value == "never") ""
      else mod + super.show
  }

  case class HighlightBuffer(hl: Highlight)(using Ctx, CState) {
