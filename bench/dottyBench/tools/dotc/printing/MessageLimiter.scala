...
abstract class MessageLimiter:
  ...
  protected def recursionLimitExceeded()(using Ctx, CState): Unit = ()

  final def controlled(op: => Text)(using Ctx, CState): Text =
...
class DefaultMessageLimiter extends MessageLimiter:
  override def recursionLimitExceeded()(using Ctx, CState): Unit =
    if ctx.debug then
