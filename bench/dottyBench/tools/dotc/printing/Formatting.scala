...
object Formatting {
  ...
  class StringFormatter(protected val sc: StringContext) {
    protected def showArg(arg: Any)(using Ctx, CState): String = arg match {
      case arg: Showable =>
        try arg.show
        catch {
    ...
    private def treatArg(arg: Any, suffix: String)(using Ctx, CState): (Any, String) = arg match {
    ...
    def assemble(args: Seq[Any])(using Ctx, CState): String = {
  ...
  class ErrorMessageFormatter(sc: StringContext) extends StringFormatter(sc):
    override protected def showArg(arg: Any)(using Ctx, CState): String =
  ...
  private def wrapNonSensical(arg: Any, str: String)(using Ctx, CState): String = {
    import Message._
  ...
  private class Seen extends mutable.HashMap[SeenKey, List[Recorded]] {
    ...
    def record(str: String, isType: Boolean, entry: Recorded)(using Ctx, CState): String = {
  ...
  def explanation(entry: AnyRef)(using Ctx, CState): String = {
  ...
  /** Turns a `Seen` into a `String` to produce an explanation for types on the
    ...
    */
  private def explanations(seen: Seen)(using Ctx, CState): String = {
  ...
  private def errorMessageCtx(using Ctx, CState): Ctx =
  ...
  private def explainCtx(seen: Seen)(using Ctx, CState): Ctx =
  ...
  /** Entrypoint for explanation string interpolator:
    ...
    * ```
    */
  def explained(op: (Ctx, CState) ?=> String)(using Ctx, CState): String = {
    val seen = new Seen
  ...
  /** When getting a type mismatch it is useful to disambiguate placeholders like:
    ...
    */
  def disambiguateTypes(args: Type*)(using Ctx, CState): (String, Ctx) = {
    val seen = new Seen
  ...
  /** This method will produce a colored type diff from the given arguments.
    ...
    */
  def typeDiff(found: Type, expected: Type)(using Ctx, CState): (String, String) = {
  ...
  def hl(s: String)(using Ctx, CState): String =
