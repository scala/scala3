...
object Signatures {
   ...
   */
  def callInfo(path: List[tpd.Tree], span: Span)(using Ctx, CState): (Int, Int, List[SingleDenotation]) =
    path match {
  ...
  def toSignature(denot: SingleDenotation)(using Ctx, CState): Option[Signature] = {
    val symbol = denot.symbol
    val docComment = ParsedComment.docOf(symbol)
    val classTree = symbol.topLevelClass.asClass.rootTree

    def toParamss(tp: MethodType)(using Ctx, CState): List[List[Param]] = {
   ...
   */
  private def alternativesFromError(err: ErrorType, params: List[tpd.Tree])(using Ctx, CState): (Int, List[SingleDenotation]) = {
    val alternatives =
      err.msg match
