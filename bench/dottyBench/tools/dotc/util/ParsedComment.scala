...
import dottyBench.tools.dotc.core.Comments.{Comment, CommentsContext}
...
class ParsedComment(val comment: Comment) {
   ...
   */
  def renderAsMarkdown(using Ctx, CState): String = {
...
object ParsedComment {
   ...
   */
  def docOf(symbol: Symbol)(using Ctx, CState): Option[ParsedComment] = {
  ...
  private case class TagFormatter(title: String, fn: (Context, List[String]) => String) {
     ...
     */
    def apply(items: List[String])(using Ctx, CState): Option[String] = items match {
      case Nil =>
        None
      case items =>
        Some(s"""${bold(title)}
                |${fn(combinedContext, items)}
                |""".stripMargin)
    }
  }

  /** Is the color enabled in the context? */
  private def colorEnabled(using Ctx, CState): Boolean =
    ctx.settings.color.value != "never"

  /** Show `str` in bold */
  private def bold(str: String)(using Ctx, CState): String =
