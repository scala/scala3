...
import core._
import ast.Trees._
import Contexts._, Phases._, Symbols._, Decorators._, ContextOps._
...
object MegaPhase {
  ...
  abstract class MiniPhase extends Phase {
    ...
    def prepareForIdent(tree: Ident)(using Ctx, CState): Ctx = ctx
    def prepareForSelect(tree: Select)(using Ctx, CState): Ctx = ctx
    def prepareForThis(tree: This)(using Ctx, CState): Ctx = ctx
    def prepareForSuper(tree: Super)(using Ctx, CState): Ctx = ctx
    def prepareForApply(tree: Apply)(using Ctx, CState): Ctx = ctx
    def prepareForTypeApply(tree: TypeApply)(using Ctx, CState): Ctx = ctx
    def prepareForLiteral(tree: Literal)(using Ctx, CState): Ctx = ctx
    def prepareForNew(tree: New)(using Ctx, CState): Ctx = ctx
    def prepareForTyped(tree: Typed)(using Ctx, CState): Ctx = ctx
    def prepareForAssign(tree: Assign)(using Ctx, CState): Ctx = ctx
    def prepareForBlock(tree: Block)(using Ctx, CState): Ctx = ctx
    def prepareForIf(tree: If)(using Ctx, CState): Ctx = ctx
    def prepareForClosure(tree: Closure)(using Ctx, CState): Ctx = ctx
    def prepareForMatch(tree: Match)(using Ctx, CState): Ctx = ctx
    def prepareForCaseDef(tree: CaseDef)(using Ctx, CState): Ctx = ctx
    def prepareForLabeled(tree: Labeled)(using Ctx, CState): Ctx = ctx
    def prepareForReturn(tree: Return)(using Ctx, CState): Ctx = ctx
    def prepareForWhileDo(tree: WhileDo)(using Ctx, CState): Ctx = ctx
    def prepareForTry(tree: Try)(using Ctx, CState): Ctx = ctx
    def prepareForSeqLiteral(tree: SeqLiteral)(using Ctx, CState): Ctx = ctx
    def prepareForInlined(tree: Inlined)(using Ctx, CState): Ctx = ctx
    def prepareForTypeTree(tree: TypeTree)(using Ctx, CState): Ctx = ctx
    def prepareForBind(tree: Bind)(using Ctx, CState): Ctx = ctx
    def prepareForAlternative(tree: Alternative)(using Ctx, CState): Ctx = ctx
    def prepareForUnApply(tree: UnApply)(using Ctx, CState): Ctx = ctx
    def prepareForValDef(tree: ValDef)(using Ctx, CState): Ctx = ctx
    def prepareForDefDef(tree: DefDef)(using Ctx, CState): Ctx = ctx
    def prepareForTypeDef(tree: TypeDef)(using Ctx, CState): Ctx = ctx
    def prepareForTemplate(tree: Template)(using Ctx, CState): Ctx = ctx
    def prepareForPackageDef(tree: PackageDef)(using Ctx, CState): Ctx = ctx
    def prepareForStats(trees: List[Tree])(using Ctx, CState): Ctx = ctx
    def prepareForUnit(tree: Tree)(using Ctx, CState): Ctx = ctx
    def prepareForOther(tree: Tree)(using Ctx, CState): Ctx = ctx

    def transformIdent(tree: Ident)(using Ctx, CState): Tree = tree
    def transformSelect(tree: Select)(using Ctx, CState): Tree = tree
    def transformThis(tree: This)(using Ctx, CState): Tree = tree
    def transformSuper(tree: Super)(using Ctx, CState): Tree = tree
    def transformApply(tree: Apply)(using Ctx, CState): Tree = tree
    def transformTypeApply(tree: TypeApply)(using Ctx, CState): Tree = tree
    def transformLiteral(tree: Literal)(using Ctx, CState): Tree = tree
    def transformNew(tree: New)(using Ctx, CState): Tree = tree
    def transformTyped(tree: Typed)(using Ctx, CState): Tree = tree
    def transformAssign(tree: Assign)(using Ctx, CState): Tree = tree
    def transformBlock(tree: Block)(using Ctx, CState): Tree = tree
    def transformIf(tree: If)(using Ctx, CState): Tree = tree
    def transformClosure(tree: Closure)(using Ctx, CState): Tree = tree
    def transformMatch(tree: Match)(using Ctx, CState): Tree = tree
    def transformCaseDef(tree: CaseDef)(using Ctx, CState): Tree = tree
    def transformLabeled(tree: Labeled)(using Ctx, CState): Tree = tree
    def transformReturn(tree: Return)(using Ctx, CState): Tree = tree
    def transformWhileDo(tree: WhileDo)(using Ctx, CState): Tree = tree
    def transformTry(tree: Try)(using Ctx, CState): Tree = tree
    def transformSeqLiteral(tree: SeqLiteral)(using Ctx, CState): Tree = tree
    def transformInlined(tree: Inlined)(using Ctx, CState): Tree = tree
    def transformTypeTree(tree: TypeTree)(using Ctx, CState): Tree = tree
    def transformBind(tree: Bind)(using Ctx, CState): Tree = tree
    def transformAlternative(tree: Alternative)(using Ctx, CState): Tree = tree
    def transformUnApply(tree: UnApply)(using Ctx, CState): Tree = tree
    def transformValDef(tree: ValDef)(using Ctx, CState): Tree = tree
    def transformDefDef(tree: DefDef)(using Ctx, CState): Tree = tree
    def transformTypeDef(tree: TypeDef)(using Ctx, CState): Tree = tree
    def transformTemplate(tree: Template)(using Ctx, CState): Tree = tree
    def transformPackageDef(tree: PackageDef)(using Ctx, CState): Tree = tree
    def transformStats(trees: List[Tree])(using Ctx, CState): List[Tree] = trees
    def transformUnit(tree: Tree)(using Ctx, CState): Tree = tree
    def transformOther(tree: Tree)(using Ctx, CState): Tree = tree

    /** Transform tree using all transforms of current group (including this one) */
    def transformAllDeep(tree: Tree)(using Ctx, CState): Tree =
      superPhase.transformTree(tree, 0)

    /** Transform tree using all transforms following the current one in this group */
    def transformFollowingDeep(tree: Tree)(using Ctx, CState): Tree =
      superPhase.transformTree(tree, idxInGroup + 1)

    /** Transform single node using all transforms following the current one in this group */
    def transformFollowing(tree: Tree)(using Ctx, CState): Tree =
...
class MegaPhase(val miniPhases: Array[MiniPhase]) extends Phase {
  ...
  def transformNode(tree: Tree, start: Int)(using Ctx, CState): Tree = {
  ...
  def transformTree(tree: Tree, start: Int)(using Ctx, CState): Tree = {
    def localContext(using Ctx, CState) = {
      val sym = tree.symbol
      val owner = if (sym.is(PackageVal)) sym.moduleClass else sym
      ctx.fresh.setOwner(owner)
    }

    def transformNamed(tree: Tree, start: Int, outerCtx: Ctx): Tree = tree match {
      case tree: Ident =>
        given Ctx = prepIdent(tree, start)(using outerCtx)
        goIdent(tree, start)
      case tree: Select =>
        given Ctx = prepSelect(tree, start)(using outerCtx)
        val qual = transformTree(tree.qualifier, start)
        goSelect(cpy.Select(tree)(qual, tree.name), start)
      case tree: ValDef =>
        given Ctx = prepValDef(tree, start)(using outerCtx)
        def mapValDef(using Ctx, CState) = {
      ...
      case tree: DefDef =>
        given Ctx = prepDefDef(tree, start)(using outerCtx)
        def mapDefDef(using Ctx, CState) = {
      ...
      case tree: Bind =>
        given Ctx = prepBind(tree, start)(using outerCtx)
        val body = transformTree(tree.body, start)
        goBind(cpy.Bind(tree)(tree.name, body), start)
      case _ =>
        given Ctx = prepOther(tree, start)(using outerCtx)
        goOther(tree, start)
    }

    def transformUnnamed(tree: Tree, start: Int, outerCtx: Ctx): Tree = tree match {
      case tree: Apply =>
        given Ctx = prepApply(tree, start)(using outerCtx)
        val fun = transformTree(tree.fun, start)
        val args = transformTrees(tree.args, start)
        goApply(cpy.Apply(tree)(fun, args), start)
      case tree: TypeTree =>
        given Ctx = prepTypeTree(tree, start)(using outerCtx)
        goTypeTree(tree, start)
      case tree: Thicket =>
        cpy.Thicket(tree)(transformTrees(tree.trees, start))
      case tree: This =>
        given Ctx = prepThis(tree, start)(using outerCtx)
        goThis(tree, start)
      case tree: Literal =>
        given Ctx = prepLiteral(tree, start)(using outerCtx)
        goLiteral(tree, start)
      case tree: Block =>
        given Ctx = prepBlock(tree, start)(using outerCtx)
        val stats = transformStats(tree.stats, ctx.owner, start)
        val expr = transformTree(tree.expr, start)
        goBlock(cpy.Block(tree)(stats, expr), start)
      case tree: TypeApply =>
        given Ctx = prepTypeApply(tree, start)(using outerCtx)
        val fun = transformTree(tree.fun, start)
        val args = transformTrees(tree.args, start)
        goTypeApply(cpy.TypeApply(tree)(fun, args), start)
      case tree: If =>
        given Ctx = prepIf(tree, start)(using outerCtx)
      ...
      case tree: New =>
        given Ctx = prepNew(tree, start)(using outerCtx)
        val tpt = transformTree(tree.tpt, start)
        goNew(cpy.New(tree)(tpt), start)
      case tree: Typed =>
        given Ctx = prepTyped(tree, start)(using outerCtx)
        val expr = transformTree(tree.expr, start)
        val tpt = transformTree(tree.tpt, start)
        goTyped(cpy.Typed(tree)(expr, tpt), start)
      case tree: CaseDef =>
        given Ctx = prepCaseDef(tree, start)(using outerCtx)
      ...
      case tree: Super =>
        given Ctx = prepSuper(tree, start)(using outerCtx)
        goSuper(tree, start)
      case tree: Template =>
        given Ctx = prepTemplate(tree, start)(using outerCtx)
      ...
      case tree: Match =>
        given Ctx = prepMatch(tree, start)(using outerCtx)
      ...
      case tree: UnApply =>
        given Ctx = prepUnApply(tree, start)(using outerCtx)
        val fun = transformTree(tree.fun, start)
      ...
      case tree: PackageDef =>
        given Ctx = prepPackageDef(tree, start)(using outerCtx)
        def mapPackage(using Ctx, CState) = {
      ...
      case tree: Return =>
        given Ctx = prepReturn(tree, start)(using outerCtx)
        val expr = transformTree(tree.expr, start)
      ...
      case tree: WhileDo =>
        given Ctx = prepWhileDo(tree, start)(using outerCtx)
        val cond = transformTree(tree.cond, start)
        val body = transformTree(tree.body, start)
        goWhileDo(cpy.WhileDo(tree)(cond, body), start)
      case tree: Alternative =>
        given Ctx = prepAlternative(tree, start)(using outerCtx)
  ...
  def transformSpecificTree[T <: Tree](tree: T, start: Int)(using Ctx, CState): T =
    transformTree(tree, start).asInstanceOf[T]

  def transformStats(trees: List[Tree], exprOwner: Symbol, start: Int)(using Ctx, CState): List[Tree] = {
    def transformStat(stat: Tree)(using Ctx, CState): Tree = stat match {
  ...
  def transformUnit(tree: Tree)(using Ctx, CState): Tree = {
    val nestedCtx = prepUnit(tree, 0)
    val tree1 = transformTree(tree, 0)(using nestedCtx)
    goUnit(tree1, 0)(using nestedCtx)
  }

  def transformTrees(trees: List[Tree], start: Int)(using Ctx, CState): List[Tree] =
    flatten(trees.mapConserve(transformTree(_, start)))

  def transformSpecificTrees[T <: Tree](trees: List[T], start: Int)(using Ctx, CState): List[T] =
  ...
  def prepIdent(tree: Ident, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxIdentPrepPhase(start)
    if (phase == null) ctx
    else prepIdent(tree, phase.idxInGroup + 1)(using phase.prepareForIdent(tree))
  }

  def goIdent(tree: Ident, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepSelect(tree: Select, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxSelectPrepPhase(start)
    if (phase == null) ctx
    else prepSelect(tree, phase.idxInGroup + 1)(using phase.prepareForSelect(tree))
  }

  def goSelect(tree: Select, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepThis(tree: This, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxThisPrepPhase(start)
    if (phase == null) ctx
    else prepThis(tree, phase.idxInGroup + 1)(using phase.prepareForThis(tree))
  }

  def goThis(tree: This, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepSuper(tree: Super, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxSuperPrepPhase(start)
    if (phase == null) ctx
    else prepSuper(tree, phase.idxInGroup + 1)(using phase.prepareForSuper(tree))
  }

  def goSuper(tree: Super, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepApply(tree: Apply, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxApplyPrepPhase(start)
    if (phase == null) ctx
    else prepApply(tree, phase.idxInGroup + 1)(using phase.prepareForApply(tree))
  }

  def goApply(tree: Apply, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepTypeApply(tree: TypeApply, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxTypeApplyPrepPhase(start)
    if (phase == null) ctx
    else prepTypeApply(tree, phase.idxInGroup + 1)(using phase.prepareForTypeApply(tree))
  }

  def goTypeApply(tree: TypeApply, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepLiteral(tree: Literal, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxLiteralPrepPhase(start)
    if (phase == null) ctx
    else prepLiteral(tree, phase.idxInGroup + 1)(using phase.prepareForLiteral(tree))
  }

  def goLiteral(tree: Literal, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepNew(tree: New, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxNewPrepPhase(start)
    if (phase == null) ctx
    else prepNew(tree, phase.idxInGroup + 1)(using phase.prepareForNew(tree))
  }

  def goNew(tree: New, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepTyped(tree: Typed, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxTypedPrepPhase(start)
    if (phase == null) ctx
    else prepTyped(tree, phase.idxInGroup + 1)(using phase.prepareForTyped(tree))
  }

  def goTyped(tree: Typed, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepAssign(tree: Assign, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxAssignPrepPhase(start)
    if (phase == null) ctx
    else prepAssign(tree, phase.idxInGroup + 1)(using phase.prepareForAssign(tree))
  }

  def goAssign(tree: Assign, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepBlock(tree: Block, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxBlockPrepPhase(start)
    if (phase == null) ctx
    else prepBlock(tree, phase.idxInGroup + 1)(using phase.prepareForBlock(tree))
  }

  def goBlock(tree: Block, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepIf(tree: If, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxIfPrepPhase(start)
    if (phase == null) ctx
    else prepIf(tree, phase.idxInGroup + 1)(using phase.prepareForIf(tree))
  }

  def goIf(tree: If, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepClosure(tree: Closure, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxClosurePrepPhase(start)
    if (phase == null) ctx
    else prepClosure(tree, phase.idxInGroup + 1)(using phase.prepareForClosure(tree))
  }

  def goClosure(tree: Closure, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepMatch(tree: Match, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxMatchPrepPhase(start)
    if (phase == null) ctx
    else prepMatch(tree, phase.idxInGroup + 1)(using phase.prepareForMatch(tree))
  }

  def goMatch(tree: Match, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepCaseDef(tree: CaseDef, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxCaseDefPrepPhase(start)
    if (phase == null) ctx
    else prepCaseDef(tree, phase.idxInGroup + 1)(using phase.prepareForCaseDef(tree))
  }

  def goCaseDef(tree: CaseDef, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepLabeled(tree: Labeled, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxLabeledPrepPhase(start)
    if (phase == null) ctx
    else prepLabeled(tree, phase.idxInGroup + 1)(using phase.prepareForLabeled(tree))
  }

  def goLabeled(tree: Labeled, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepReturn(tree: Return, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxReturnPrepPhase(start)
    if (phase == null) ctx
    else prepReturn(tree, phase.idxInGroup + 1)(using phase.prepareForReturn(tree))
  }

  def goReturn(tree: Return, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepWhileDo(tree: WhileDo, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxWhileDoPrepPhase(start)
    if (phase == null) ctx
    else prepWhileDo(tree, phase.idxInGroup + 1)(using phase.prepareForWhileDo(tree))
  }

  def goWhileDo(tree: WhileDo, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepTry(tree: Try, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxTryPrepPhase(start)
    if (phase == null) ctx
    else prepTry(tree, phase.idxInGroup + 1)(using phase.prepareForTry(tree))
  }

  def goTry(tree: Try, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepSeqLiteral(tree: SeqLiteral, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxSeqLiteralPrepPhase(start)
    if (phase == null) ctx
    else prepSeqLiteral(tree, phase.idxInGroup + 1)(using phase.prepareForSeqLiteral(tree))
  }

  def goSeqLiteral(tree: SeqLiteral, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepInlined(tree: Inlined, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxInlinedPrepPhase(start)
    if (phase == null) ctx
    else prepInlined(tree, phase.idxInGroup + 1)(using phase.prepareForInlined(tree))
  }

  def goInlined(tree: Inlined, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepTypeTree(tree: TypeTree, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxTypeTreePrepPhase(start)
    if (phase == null) ctx
    else prepTypeTree(tree, phase.idxInGroup + 1)(using phase.prepareForTypeTree(tree))
  }

  def goTypeTree(tree: TypeTree, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepBind(tree: Bind, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxBindPrepPhase(start)
    if (phase == null) ctx
    else prepBind(tree, phase.idxInGroup + 1)(using phase.prepareForBind(tree))
  }

  def goBind(tree: Bind, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepAlternative(tree: Alternative, start: Int)(using Ctx, CState): Ctx = {
  ...
  def goAlternative(tree: Alternative, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepUnApply(tree: UnApply, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxUnApplyPrepPhase(start)
    if (phase == null) ctx
    else prepUnApply(tree, phase.idxInGroup + 1)(using phase.prepareForUnApply(tree))
  }

  def goUnApply(tree: UnApply, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepValDef(tree: ValDef, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxValDefPrepPhase(start)
    if (phase == null) ctx
    else prepValDef(tree, phase.idxInGroup + 1)(using phase.prepareForValDef(tree))
  }

  def goValDef(tree: ValDef, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepDefDef(tree: DefDef, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxDefDefPrepPhase(start)
    if (phase == null) ctx
    else prepDefDef(tree, phase.idxInGroup + 1)(using phase.prepareForDefDef(tree))
  }

  def goDefDef(tree: DefDef, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepTypeDef(tree: TypeDef, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxTypeDefPrepPhase(start)
    if (phase == null) ctx
    else prepTypeDef(tree, phase.idxInGroup + 1)(using phase.prepareForTypeDef(tree))
  }

  def goTypeDef(tree: TypeDef, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepTemplate(tree: Template, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxTemplatePrepPhase(start)
    if (phase == null) ctx
    else prepTemplate(tree, phase.idxInGroup + 1)(using phase.prepareForTemplate(tree))
  }

  def goTemplate(tree: Template, start: Int)(using Ctx, CState): Tree = {
  ...
  def prepPackageDef(tree: PackageDef, start: Int)(using Ctx, CState): Ctx = {
    val phase = nxPackageDefPrepPhase(start)
    if (phase == null) ctx
    else prepPackageDef(tree, phase.idxInGroup + 1)(using phase.prepareForPackageDef(tree))
  }

  def goPackageDef(tree: PackageDef, start: Int)(using Ctx, CState): Tree = {
