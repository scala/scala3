...
class ElimRepeated extends MiniPhase with InfoTransformer { thisPhase =>
  ...
  override def transform(ref: SingleDenotation)(using Ctx, CState): SingleDenotation =
  ...
  override def mayChange(sym: Symbol)(using Ctx, CState): Boolean = sym.is(Method)

  private def overridesJava(sym: Symbol)(using Ctx, CState) = sym.allOverriddenSymbols.exists(_.is(JavaDefined))

  private def hasVarargsAnnotation(sym: Symbol)(using Ctx, CState) = sym.hasAnnotation(defn.VarargsAnnot)

  private def parentHasAnnotation(sym: Symbol)(using Ctx, CState) = sym.allOverriddenSymbols.exists(hasVarargsAnnotation)

  /** Eliminate repeated parameters from method types. */
  private def elimRepeated(tp: Type)(using Ctx, CState): Type = tp.stripTypeVar match
  ...
  private def seqToArray(tree: Tree)(using Ctx, CState): Tree = tree match
  ...
  private def arrayToSeq(tree: Tree)(using Ctx, CState): Tree =
  ...
  private def hasRepeatedParams(sym: Symbol)(using Ctx, CState): Boolean =
    sym.info.paramInfoss.flatten.exists(_.isRepeatedParam)

  /** Is this the type of a method that has a repeated parameter type as
   *  its last parameter in the last parameter list?
   */
  private def isValidJavaVarArgs(tp: Type)(using Ctx, CState): Boolean = tp match
    case mt: MethodType =>
   ...
   *  forwards it to `ddef`.
   */
  private def addVarArgsForwarder(ddef: DefDef, isBridge: Boolean)(using Ctx, CState): Tree =
  ...
  private def toJavaVarArgs(tp: Type)(using Ctx, CState): Type = tp match
    case tp: PolyType =>
   ...
   */
  private def varargArrayType(tp: Type)(using Ctx, CState): Type =
