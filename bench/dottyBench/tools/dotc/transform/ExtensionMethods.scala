...
class ExtensionMethods extends MiniPhase with DenotTransformer with FullParameterization { thisPhase =>
  ...
  override def transform(ref: SingleDenotation)(using Ctx, CState): SingleDenotation = ref match {
  ...
  protected def rewiredTarget(target: Symbol, derived: Symbol)(using Ctx, CState): Symbol =
    if (isMethodWithExtension(target) &&
        target.owner.linkedClass == derived.owner) extensionMethod(target)
    else NoSymbol

  private def createExtensionMethod(imeth: Symbol, staticClass: Symbol)(using Ctx, CState): TermSymbol = {
  ...
  override def transformDefDef(tree: tpd.DefDef)(using Ctx, CState): tpd.Tree =
...
object ExtensionMethods {
  ...
  def extensionName(imeth: Symbol)(using Ctx, CState): TermName =
    ExtMethName(imeth.name.asTermName)

  /** Return the extension method that corresponds to given instance method `meth`. */
  def extensionMethod(imeth: Symbol)(using Ctx, CState): TermSymbol =
