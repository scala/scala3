...
import dottyBench.tools.dotc.core.Types.MethodType
...
class StringInterpolatorOpt extends MiniPhase {
  ...
  override def checkPostCondition(tree: tpd.Tree)(using Ctx, CState): Unit = {
    tree match {
      case tree: RefTree =>
        val sym = tree.symbol
  ...
  private object Literals {
    def unapply(tree: SeqLiteral)(using Ctx, CState): Option[List[Literal]] = {
      tree.elems match {
  ...
  private object StringContextApply {
    def unapply(tree: Select)(using Ctx, CState): Boolean = {
  ...
  private object SOrRawInterpolator {
    def unapply(tree: Tree)(using Ctx, CState): Option[(List[Literal], List[Tree])] = {
      tree match {
  ...
  private object StringContextIntrinsic {
    def unapply(tree: Apply)(using Ctx, CState): Option[(List[Literal], List[Tree])] = {
      tree match {
