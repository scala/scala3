...
class CheckReentrant extends MiniPhase {
  ...
  def isIgnored(sym: Symbol)(using Ctx, CState): Boolean =
  ...
  def scanning(sym: Symbol)(op: => Unit)(using Ctx, CState): Unit = {
  ...
  def addVars(cls: ClassSymbol)(using Ctx, CState): Unit =
