...
import dottyBench.tools.dotc.ast.{Trees, tpd}
...
class SuperAccessors(thisPhase: DenotTransformer) {
  ...
  private def withInvalidCurrentClass[A](trans: => A)(using Ctx, CState): A = {
  ...
  private def validCurrentClass(using Ctx, CState): Boolean =
  ...
  private def superAccessorCall(sel: Select)(using Ctx, CState) = {
  ...
  /** Check selection `super.f` for conforming to rules. If necessary,
    ...
    */
  private def transformSuperSelect(sel: Select)(using Ctx, CState): Tree = {
  ...
  /** Disallow some super.XX calls targeting Any methods which would
    ...
    */
  private def isDisallowed(sym: Symbol)(using Ctx, CState) =
  ...
  def transformSelect(tree: Tree, targs: List[Tree])(using Ctx, CState): Tree = {
  ...
  def wrapTemplate(tree: Template)(op: Template => Template)(using Ctx, CState): Template = {
  ...
  def wrapDefDef(ddef: DefDef)(op: => DefDef)(using Ctx, CState): DefDef =
