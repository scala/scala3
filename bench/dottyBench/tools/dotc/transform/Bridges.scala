...
class Bridges(root: ClassSymbol, thisPhase: DenotTransformer)(using Ctx, CState) {
  import ast.tpd._

  assert(currentPhase == erasurePhase.next)

  private class BridgesCursor(using Ctx, CState) extends OverridingPairs.Cursor(root) {
  ...
  private def addBridgeIfNeeded(member: Symbol, other: Symbol) = {
    ...
    def info(sym: Symbol)(using Ctx, CState) = sym.info
    def desc(sym: Symbol)= {
      val infoStr = atPhase(erasurePhase)(info(sym)) match {
    ...
    if member.signature == other.signature then
      if !member.info.matches(other.info) then
        atPhase(elimErasedValueTypePhase.next) {
          report.error(em"""bridge generated for member ${desc(member)}
                        |which overrides ${desc(other)}
                        |clashes with definition of the member itself; both have erased type ${info(member)}."""",
                       bridgePosFor(member))
        }
  ...
  def add(stats: List[untpd.Tree]): List[untpd.Tree] =
    val opc = atPhase(erasurePhase)(new BridgesCursor())
