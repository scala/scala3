...
object ValueClasses {
  ...
  def isDerivedValueClass(sym: Symbol)(using Ctx, CState): Boolean = {
    val d = sym.denot
  ...
  def isMethodWithExtension(sym: Symbol)(using Ctx, CState): Boolean =
  ...
  def valueClassUnbox(cls: ClassSymbol)(using Ctx, CState): Symbol =
   ...
   */
  def u2evt(cls: ClassSymbol)(using Ctx, CState): Symbol =
   ...
   *  and further described in [[ExtensionMethods]].
   */
  def evt2u(cls: ClassSymbol)(using Ctx, CState): Symbol =
    cls.linkedClass.info.decl(nme.EVT2U).symbol

  /** The unboxed type that underlies a derived value class */
  def underlyingOfValueClass(sym: ClassSymbol)(using Ctx, CState): Type =
    valueClassUnbox(sym).info.resultType

  /** Whether a value class wraps itself */
  def isCyclic(cls: ClassSymbol)(using Ctx, CState): Boolean = {
    def recur(seen: Set[Symbol], clazz: ClassSymbol)(using Ctx, CState): Boolean =
      (seen contains clazz) || {
