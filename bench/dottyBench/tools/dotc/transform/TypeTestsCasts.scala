...
object TypeTestsCasts {
   ...
   *  8. otherwise, TRUE
   */
  def checkable(X: Type, P: Type, span: Span)(using Ctx, CState): Boolean = {
    def isAbstract(P: Type) = !P.dealias.typeSymbol.isClass

    def replaceP(tp: Type)(using Ctx, CState) = new TypeMap {
    ...
    def replaceX(tp: Type)(using Ctx, CState) = new TypeMap {
      def apply(tp: Type) = tp match {
        case tref: TypeRef if tref.typeSymbol.isPatternBound =>
    ...
    def stripTypeParam(tp: Type)(using Ctx, CState) = new ApproximatingTypeMap {
    ...
    def typeArgsTrivial(X: Type, P: AppliedType)(using Ctx, CState) = inCtx(ctx.fresh.setExploreTyperState().setFreshGADTBounds) {
  ...
  def interceptTypeApply(tree: TypeApply)(using Ctx, CState): Tree = trace(s"transforming ${tree.show}", show = true) {
    ...
    def interceptWith(expr: Tree): Tree =
      if (expr.isEmpty) tree
        ...
        def transformIsInstanceOf(expr: Tree, testType: Type, flagUnrelated: Boolean): Tree = {
          ...
          def unreachable(why: => String)(using Ctx, CState): Boolean = {
            if (flagUnrelated)
           ...
           */
          def checkSensical(foundClasses: List[Symbol])(using Ctx, CState): Boolean =
