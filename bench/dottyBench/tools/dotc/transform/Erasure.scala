...
import dottyBench.tools.dotc.core.{Constants, Flags}
...
class Erasure extends Phase with DenotTransformer {
  ...
  def run(using Ctx, CState): Unit =
    ...
    unit.tpdTree = atPhase(this.next) {
      eraser.typedExpr(unit.tpdTree)(using ctx.fresh.setTyper(eraser))
    }
   ...
   */
  def assertErased(tree: tpd.Tree)(using Ctx, CState): Unit = {
  ...
  def assertErased(tp: Type, tree: tpd.Tree = tpd.EmptyTree)(using Ctx, CState): Unit = {
...
object Erasure {
  ...
  def partialApply(fn: Tree, args: List[Tree])(using Ctx, CState): Tree =
  ...
  private def applyResultType(mt: MethodType, args: List[Tree])(using Ctx, CState): Type =
   ...
   */
  def expandedMethodType(mt: MethodType, origFun: Tree)(using Ctx, CState): MethodType =
    mt.paramInfos match
      case JavaArrayType(elemType) :: Nil if elemType.isRef(defn.ObjectClass) =>
        val origArity = beforeErasure(totalParamCount(origFun.symbol))
  ...
  object Boxing:
    ...
    def isUnbox(sym: Symbol)(using Ctx, CState): Boolean =
      sym.name == nme.unbox && sym.owner.linkedClass.isPrimitiveValueClass

    def isBox(sym: Symbol)(using Ctx, CState): Boolean =
      sym.name == nme.box && sym.owner.linkedClass.isPrimitiveValueClass

    def boxMethod(cls: ClassSymbol)(using Ctx, CState): Symbol =
      cls.linkedClass.info.member(nme.box).symbol
    def unboxMethod(cls: ClassSymbol)(using Ctx, CState): Symbol =
     ...
     */
    private def safelyRemovableUnboxArg(tree: Tree)(using Ctx, CState): Tree = tree match {
    ...
    def constant(tree: Tree, const: Tree)(using Ctx, CState): Tree =
      (if (isPureExpr(tree)) const else Block(tree :: Nil, const)).withSpan(tree.span)

    final def box(tree: Tree, target: => String = "")(using Ctx, CState): Tree = trace(i"boxing ${tree.showSummary}: ${tree.tpe} into $target") {
    ...
    def unbox(tree: Tree, pt: Type)(using Ctx, CState): Tree = trace(i"unboxing ${tree.showSummary}: ${tree.tpe} as a $pt") {
      pt match {
     ...
     */
    def cast(tree: Tree, pt: Type)(using Ctx, CState): Tree = trace(i"cast ${tree.tpe.widen} --> $pt", show = true) {
     ...
     */
    def adaptToType(tree: Tree, pt: Type)(using Ctx, CState): Tree = pt match
     ...
     */
    def adaptClosure(tree: tpd.Closure)(using Ctx, CState): Tree = {
     ...
     */
    def etaExpand(tree: Tree, mt: MethodType, pt: Type)(using Ctx, CState): Tree =
      ...
      def abstracted(args: List[Tree], tp: Type, pt: Type)(using Ctx, CState): Tree =
  ...
  class Typer(erasurePhase: DenotTransformer) extends typer.ReTyper with NoChecking {
    ...
    def isErased(tree: Tree)(using Ctx, CState): Boolean = tree match {
    ...
    /** Check that Java statics and packages can only be used in selections.
      */
    private def checkValue(tree: Tree, proto: Type)(using Ctx, CState): tree.type =
      if (!proto.isInstanceOf[SelectionProto] && !proto.isInstanceOf[ApplyingProto]) then
        checkValue(tree)
      tree

    private def checkValue(tree: Tree)(using Ctx, CState): Unit =
    ...
    private def checkNotErased(tree: Tree)(using Ctx, CState): tree.type = {
    ...
    def erasedDef(sym: Symbol)(using Ctx, CState): Thicket = {
      if (sym.owner.isClass) sym.dropAfter(erasurePhase)
      tpd.EmptyTree
    }

    def erasedType(tree: untpd.Tree)(using Ctx, CState): Type = {
      val tp = tree.typeOpt
      if (tree.isTerm) erasedRef(tp) else valueErasure(tp)
    }

    override def promote(tree: untpd.Tree)(using Ctx, CState): tree.ThisTree[Type] = {
      assert(tree.hasType)
     ...
     */
    override def typedTypeTree(tree: untpd.TypeTree, pt: Type)(using Ctx, CState): TypeTree =
      tree.withType(erasure(tree.tpe))

    /** This override is only needed to semi-erase type ascriptions */
    override def typedTyped(tree: untpd.Typed, pt: Type)(using Ctx, CState): Tree = {
    ...
    override def typedLiteral(tree: untpd.Literal)(using Ctx, CState): Tree =
    ...
    override def typedSelect(tree: untpd.Select, pt: Type)(using Ctx, CState): Tree = {
      ...
      def isJvmAccessible(cls: Symbol): Boolean =
        ...
        !cls.is(Flags.JavaDefined) || {
          ...
          val boundary = beforeErasure(cls.accessBoundary(cls.owner))
    ...
    override def typedTypeApply(tree: untpd.TypeApply, pt: Type)(using Ctx, CState): Tree = {
    ...
    override def typedApply(tree: untpd.Apply, pt: Type)(using Ctx, CState): Tree =
      ...
      if fun.symbol == defn.cbnArg then
        ...
        val origFunType = beforeErasure(origFun.tpe.widen)
    ...
    override def typedSeqLiteral(tree: untpd.SeqLiteral, pt: Type)(using Ctx, CState): SeqLiteral =
    ...
    override def typedMatch(tree: untpd.Match, pt: Type)(using Ctx, CState): Tree =
      super.typedMatch(tree, adaptProto(tree, pt))

    override def typedTry(tree: untpd.Try, pt: Type)(using Ctx, CState): Try =
      super.typedTry(tree, adaptProto(tree, pt))

    private def adaptProto(tree: untpd.Tree, pt: Type)(using Ctx, CState) =
    ...
    override def typedDefDef(ddef: untpd.DefDef, sym: Symbol)(using Ctx, CState): Tree =
      if sym.isEffectivelyErased || sym.name.is(BodyRetainerName) then
        ...
        def skipContextClosures(rhs: Tree, crCount: Int)(using Ctx, CState): Tree =
    ...
    private def outerParamDefs(constr: Symbol)(using Ctx, CState): List[ValDef] =
     ...
     */
    private def addRetainedInlineBodies(stats: List[untpd.Tree])(using Ctx, CState): List[untpd.Tree] =
    ...
    override def typedTypeDef(tdef: untpd.TypeDef, sym: Symbol)(using Ctx, CState): Tree =
      EmptyTree

    override def typedAnnotated(tree: untpd.Annotated, pt: Type)(using Ctx, CState): Tree =
      typed(tree.arg, pt)

    override def typedStats(stats: List[untpd.Tree], exprOwner: Symbol)(using Ctx, CState): (List[Tree], Ctx) = {
      val stats0 = beforeErasure(addRetainedInlineBodies(stats))
      val stats1 =
  ...
  private def takesBridges(sym: Symbol)(using Ctx, CState): Boolean =
