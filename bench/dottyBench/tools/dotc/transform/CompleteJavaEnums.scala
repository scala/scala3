...
class CompleteJavaEnums extends MiniPhase with InfoTransformer { thisPhase =>
  ...
  def transformInfo(tp: Type, sym: Symbol)(using Ctx, CState): Type =
   ...
   */
  private def addConstrParams(tp: Type)(using Ctx, CState): Type = tp match {
    case tp: PolyType =>
   ...
   */
  private def addedParams(owner: Symbol, flag: FlagSet)(using Ctx, CState): List[ValDef] = {
   ...
   */
  private def addEnumConstrArgs(targetCls: Symbol, parents: List[Tree], args: List[Tree])(using Ctx, CState): List[Tree] =
    parents.map {
   ...
   *  for java interop.
   */
  private def addedEnumForwarders(clazz: Symbol)(using Ctx, CState): List[ValDef] = {
   ...
   *       }
   */
  override def transformTemplate(templ: Template)(using Ctx, CState): Template = {
