...
class TailRec extends MiniPhase {
  ...
  override def transformDefDef(tree: DefDef)(using Ctx, CState): Tree = {
  ...
  class TailRecElimination(method: Symbol, enclosingClass: ClassSymbol, paramSyms: List[Symbol], isMandatory: Boolean) extends TreeMap {
    ...
    def continueLabel(using Ctx, CState): Symbol = {
    ...
    private def getVarForRewrittenThis()(using Ctx, CState): Symbol =
      varForRewrittenThis match {
        case Some(sym) => sym
        case none =>
          val tpe =
    ...
    private def getVarForRewrittenParam(param: Symbol)(using Ctx, CState): Symbol =
    ...
    def transform(tree: Tree, tailPosition: Boolean)(using Ctx, CState): Tree =
    ...
    def yesTailTransform(tree: Tree)(using Ctx, CState): Tree =
    ...
    def noTailTransform(tree: Tree)(using Ctx, CState): Tree =
      if (isTraversalNeeded) transform(tree, tailPosition = false)
      else tree

    def noTailTransforms[Tr <: Tree](trees: List[Tr])(using Ctx, CState): List[Tr] =
