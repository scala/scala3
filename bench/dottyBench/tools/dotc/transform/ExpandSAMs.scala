...
class ExpandSAMs extends MiniPhase {
  ...
  def isPlatformSam(cls: ClassSymbol)(using Ctx, CState): Boolean =
    ctx.platform.isSam(cls)

  override def transformBlock(tree: Block)(using Ctx, CState): Tree = tree match {
   ...
   *  ```
   */
  private def toPartialFunction(tree: Block, tpe: Type)(using Ctx, CState): Tree = {
    ...
    anon.rhs match {
      case PartialFunctionRHS(pf) =>
        ...
        def translateMatch(tree: Match, pfParam: Symbol, cases: List[CaseDef], defaultValue: Tree)(using Ctx, CState) = {
        ...
        def isDefinedAtRhs(paramRefss: List[List[Tree]])(using Ctx, CState) = {
        ...
        def applyOrElseRhs(paramRefss: List[List[Tree]])(using Ctx, CState) = {
  ...
  private def checkRefinements(tpe: Type, tree: Tree)(using Ctx, CState): Type = tpe.dealias match {
