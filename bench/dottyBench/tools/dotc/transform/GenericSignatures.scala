...
object GenericSignatures {
   ...
   */
  def javaSig(sym0: Symbol, info: Type)(using Ctx, CState): Option[String] =
    // Avoid generating a signature for local symbols.
    if (sym0.isLocal) None
    else atPhase(erasurePhase)(javaSig0(sym0, info))

  @noinline
  private final def javaSig0(sym0: Symbol, info: Type)(using Ctx, CState): Option[String] = {
    ...
    def jsig(tp0: Type, toplevel: Boolean = false, primitiveOK: Boolean = true): Unit = {
      ...
      tp match {
        ...
        case mtpe: MethodType =>
          ...
          def effectiveParamInfoss(t: Type)(using Ctx, CState): List[List[Type]] = t match {
  ...
  /** The intersection dominator (SLS 3.7) of a list of types is computed as follows.
    ...
    */
  private def intersectionDominator(parents: List[Type])(using Ctx, CState): Type =
   ...
   */
  private def minimizeParents(cls: Symbol, parents: List[Type])(using Ctx, CState): List[Type] = if (parents.isEmpty) parents else {
  ...
  private def hiBounds(bounds: TypeBounds)(using Ctx, CState): List[Type] = bounds.hi.widenDealias match {
  ...
  private def isTypeParameterInSig(sym: Symbol, initialSymbol: Symbol)(using Ctx, CState) =
  ...
  private def rebindInnerClass(pre: Type, cls: Symbol)(using Ctx, CState): Type = {
    val owner = cls.owner
    if (owner.is(PackageClass) || owner.isTerm) pre else cls.owner.info /* .tpe_* */
  }

  private object RefOrAppliedType {
    def unapply(tp: Type)(using Ctx, CState): Option[(Symbol, Type, List[Type])] = tp match {
  ...
  private def needsJavaSig(tp: Type, throwsArgs: List[Type])(using Ctx, CState): Boolean = !ctx.settings.YnoGenericSig.value && {
      def needs(tp: Type) = (new NeedsSigCollector).apply(false, tp)
      needs(tp) || throwsArgs.exists(needs)
  }

  private class NeedsSigCollector(using Ctx, CState) extends TypeAccumulator[Boolean] {
