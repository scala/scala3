...
abstract class AccessProxies {
   ...
   *  as a parameterer?
   */
  protected def passReceiverAsArg(accessorName: Name)(using Ctx, CState): Boolean = false
   ...
   */
  private def accessorDefs(cls: Symbol)(using Ctx, CState): Iterator[DefDef] =
  ...
  def addAccessorDefs(cls: Symbol, body: List[Tree])(using Ctx, CState): List[Tree] = {
  ...
  trait Insert {
    ...
    def ifNoHost(reference: RefTree)(using Ctx, CState): Tree = {
      assert(false, "no host found for $reference with ${reference.symbol.showLocated} from ${ctx.owner}")
      reference
    }

    /** A fresh accessor symbol */
    private def newAccessorSymbol(owner: Symbol, name: TermName, info: Type, span: Span)(using Ctx, CState): TermSymbol = {
    ...
    protected def accessorSymbol(owner: Symbol, accessorName: TermName, accessorInfo: Type, accessed: Symbol)(using Ctx, CState): Symbol = {
    ...
    private def rewire(reference: RefTree, accessor: Symbol)(using Ctx, CState): Tree = {
      reference match {
    ...
    def useSetter(getterRef: Tree)(using Ctx, CState): Tree = getterRef match {
    ...
    /** Create an accessor unless one exists already, and replace the original
      ...
      */
    def useAccessor(reference: RefTree)(using Ctx, CState): Tree = {
    ...
    def accessorIfNeeded(tree: Tree)(using Ctx, CState): Tree = tree match {
...
object AccessProxies {
   ...
   */
  def hostForAccessorOf(accessed: Symbol)(using Ctx, CState): Symbol = {
