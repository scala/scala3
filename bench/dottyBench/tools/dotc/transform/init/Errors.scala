...
object Errors {
  ...
  def show(errs: Errors)(using Ctx, CState): String =
  ...
  sealed trait Error {
    ...
    def issue(using Ctx, CState): Unit =
      report.warning(show + stacktrace, source.sourcePos)

    def toErrors: Errors = Set(this)

    def stacktrace(using Ctx, CState): String = if (trace.isEmpty) "" else " Calling trace:\n" + {
      var indentCount = 0
      var last: String = ""
  ...
  case class AccessNonInit(field: Symbol, trace: Vector[Tree]) extends Error {
    ...
    override def issue(using Ctx, CState): Unit =
  ...
  case class PromoteThis(pot: ThisRef, source: Tree, trace: Vector[Tree]) extends Error {
    def show(using Ctx, CState): String = "Promote the value under initialization to fully-initialized."
  }
