...
object Potentials {
  ...
  def show(pots: Potentials)(using Ctx, CState): String =
  ...
  sealed trait Potential {
    def size: Int
    def show(using Ctx, CState): String
    def source: Tree
  }
  ...
  case class ThisRef(classSymbol: ClassSymbol)(val source: Tree) extends Potential {
    val size: Int = 1
    def show(using Ctx, CState): String = classSymbol.name.show + ".this"
  ...
  case class SuperRef(pot: Potential, supercls: ClassSymbol)(val source: Tree) extends Potential {
    val size: Int = 1
    def show(using Ctx, CState): String = pot.show + ".super[" + supercls.name.show + "]"
  }
  ...
  case class Warm(classSymbol: ClassSymbol, outer: Potential)(val source: Tree) extends Potential {
    def size: Int = 1
    def show(using Ctx, CState): String = "Warm[" + classSymbol.show + ", outer = " + outer.show + "]"
  ...
  case class Outer(pot: Potential, classSymbol: ClassSymbol)(val source: Tree) extends Potential {
    def size: Int = 1
    def show(using Ctx, CState): String = "Outer[" + pot.show + ", " + classSymbol.show + "]"
  }
  ...
  case class FieldReturn(potential: Potential, field: Symbol)(val source: Tree) extends Potential {
    ...
    def show(using Ctx, CState): String = potential.show + "." + field.name.show
  }
  ...
  case class MethodReturn(potential: Potential, method: Symbol)(val source: Tree) extends Potential {
    ...
    def size: Int = potential.size + 1
    def show(using Ctx, CState): String = potential.show + "." + method.name.show
  }

  /** The object whose initialization status is unknown */
  case class Cold()(val source: Tree) extends Potential {
    def size: Int = 1
    def show(using Ctx, CState): String = "Cold"
  }
  ...
  extension (ps: Potentials) def select (symbol: Symbol, source: Tree)(using Ctx, CState): Summary =
