...
object Checking {
  ...
  def checkClassBody(cdef: TypeDef)(implicit state: State): Unit = traceOp("checking " + cdef.symbol.show, init) {
    ...
    def checkClassBodyStat(tree: Tree)(using Ctx, CState): Unit = traceOp("checking " + tree.show, init) {
      tree match {
        case vdef : ValDef =>
    ...
    def checkCtor(ctor: Symbol, tp: Type, source: Tree)(using Ctx, CState): Unit = {
      val cls = ctor.owner
