...
object TreeExtractors {
  ...
  object BinaryOp {
    def unapply(t: Tree)(using Ctx, CState): Option[(Tree, Symbol, Tree)] = t match {
 ...
 /** Match new C(args) and extract (C, args) */
  object NewWithArgs {
    def unapply(t: Tree)(using Ctx, CState): Option[(Type, List[Tree])] = t match {
  ...
  object ValueClassUnbox {
    def unapply(t: Tree)(using Ctx, CState): Option[Tree] = t match {
