...
class ExplicitOuter extends MiniPhase with InfoTransformer { thisPhase =>
  ...
  override def transformInfo(tp: Type, sym: Symbol)(using Ctx, CState): Type = tp match {
  ...
  override def mayChange(sym: Symbol)(using Ctx, CState): Boolean = sym.isClass && !sym.is(JavaDefined)
   ...
   */
  override def transformTemplate(impl: Template)(using Ctx, CState): Tree = {
  ...
  override def transformClosure(tree: Closure)(using Ctx, CState): tpd.Tree = {
...
object ExplicitOuter {
  ...
  def ensureOuterAccessors(cls: ClassSymbol)(using Ctx, CState): Unit =
  ...
  private def newOuterAccessors(cls: ClassSymbol)(using Ctx, CState) =
   ...
   */
  private def outerClass(cls: ClassSymbol)(using Ctx, CState): Symbol = {
   ...
   *  - otherwise O[?, ..., ?]
   */
  private def newOuterSym(owner: ClassSymbol, cls: ClassSymbol, name: TermName, flags: FlagSet)(using Ctx, CState) = {
  ...
  private def newOuterParamAccessor(cls: ClassSymbol)(using Ctx, CState) =
    newOuterSym(cls, cls, nme.OUTER, Private | Local | ParamAccessor)

  /** A new outer accessor for class `cls` which is a member of `owner` */
  private def newOuterAccessor(owner: ClassSymbol, cls: ClassSymbol)(using Ctx, CState) = {
  ...
  private def outerAccName(cls: ClassSymbol)(using Ctx, CState): TermName =
    nme.OUTER.expandedName(cls)

  /** Class needs an outer pointer, provided there is a reference to an outer this in it. */
  def needsOuterIfReferenced(cls: ClassSymbol)(using Ctx, CState): Boolean =
    !(cls.isStatic ||
   ...
   */
  private def needsOuterAlways(cls: ClassSymbol)(using Ctx, CState): Boolean =
  ...
  private def hasLocalInstantiation(cls: ClassSymbol)(using Ctx, CState): Boolean =
  ...
  private def outerParamAccessor(cls: ClassSymbol)(using Ctx, CState): TermSymbol =
   ...
   */
  def outerAccessor(cls: ClassSymbol)(using Ctx, CState): Symbol =
  ...
  private def hasOuter(cls: ClassSymbol)(using Ctx, CState): Boolean =
  ...
  def hasOuterParam(cls: ClassSymbol)(using Ctx, CState): Boolean =
   ...
   */
  def referencesOuter(cls: Symbol, tree: Tree)(using Ctx, CState): Boolean = {
  ...
  private def outerPrefix(tpe: Type)(using Ctx, CState): Type = tpe match {
    case tpe: TypeRef =>
      tpe.symbol match {
   ...
   */
  private def fixThis(tpe: Type)(using Ctx, CState): Type = tpe match {
  ...
  extension (sym: Symbol) def isOuterParamAccessor(using Ctx, CState): Boolean =
  ...
  /** If `cls` has an outer parameter add one to the method type `tp`. */
    def addParam(cls: ClassSymbol, tp: Type)(using Ctx, CState): Type =
     ...
     */
    def args(fun: Tree)(using Ctx, CState): List[Tree] =
          ...
          if (count == 0 || count < 0 && treeCls == toCls) tree
            ...
            val outerAcc = tree match
              case tree: This if tree.symbol == enclClass && !enclClass.is(Trait) =>
                atPhaseNoLater(lambdaLiftPhase)(outerParamAccessor(enclClass))
                // lambdalift mangles local class names, which means we cannot reliably find outer acessors anymore
              case _ =>
                atPhaseNoLater(lambdaLiftPhase)(outerAccessor(treeCls.asClass))
