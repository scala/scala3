...
class FirstTransform extends MiniPhase with InfoTransformer { thisPhase =>
  ...
  override protected def mayChange(sym: Symbol)(using Ctx, CState): Boolean = sym.isClass
  ...
  private def reorderAndComplete(stats: List[Tree])(using Ctx, CState): List[Tree] = {
  ...
  override def transformDefDef(ddef: DefDef)(using Ctx, CState): Tree = {
  ...
  override def transformStats(trees: List[Tree])(using Ctx, CState): List[Tree] =
  ...
  private object collectBinders extends TreeAccumulator[List[Ident]] {
    def apply(annots: List[Ident], t: Tree)(using Ctx, CState): List[Ident] = t match {
      case t @ Bind(_, body) =>
   ...
   */
  private def toTypeTree(tree: Tree)(using Ctx, CState) = {
  ...
  override def transformOther(tree: Tree)(using Ctx, CState): Tree = tree match {
  ...
  override def transformTyped(tree: Typed)(using Ctx, CState): Tree =
  ...
  override def transformBlock(tree: Block)(using Ctx, CState): Tree =
    constToLiteral(tree)

  override def transformIf(tree: If)(using Ctx, CState): Tree =
    tree.cond.tpe match {
   ...
   *      false || y   ==>  y
   */
  private def foldCondition(tree: Apply)(using Ctx, CState) = tree.fun match {
