...
class TupleOptimizations extends MiniPhase with IdentityDenotTransformer {
  ...
  private def transformTupleCons(tree: tpd.Apply)(using Ctx, CState): Tree = {
  ...
  private def transformTupleTail(tree: tpd.Apply)(using Ctx, CState): Tree = {
  ...
  private def transformTupleSize(tree: tpd.Apply)(using Ctx, CState): Tree =
    tree.tpe.tryNormalize match {
      case tp: ConstantType => Literal(tp.value)
      case _ => tree
    }

  private def transformTupleConcat(tree: tpd.Apply)(using Ctx, CState): Tree = {
  ...
  private def transformTupleApply(tree: tpd.Apply)(using Ctx, CState): Tree = {
  ...
  private def transformTupleToArray(tree: tpd.Apply)(using Ctx, CState): Tree = {
    val Apply(_, tup :: Nil) = tree
  ...
  private def knownTupleFromElements(tpes: List[Type], elements: List[Tree])(using Ctx, CState) = {
  ...
  private def knownTupleFromIterator(size: Int, it: Tree)(using Ctx, CState): Tree =
    if (size == 0)
  ...
  private def tupleSelectors(tup: Tree, size: Int)(using Ctx, CState): List[Tree] =
