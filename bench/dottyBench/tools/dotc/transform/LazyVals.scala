...
class LazyVals extends MiniPhase with IdentityDenotTransformer {
  ...
  private def nullableFor(sym: Symbol)(using Ctx, CState) = {
  ...
  def transformLazyVal(tree: ValOrDefDef)(using Ctx, CState): Tree = {
    val sym = tree.symbol
    if (!sym.is(Lazy) ||
  ...
  /** Append offset fields to companion objects
    */
  override def transformTemplate(template: Template)(using Ctx, CState): Tree = {
  ...
  /** Make an eager val that would implement synthetic module.
    ...
    *
    */
  def transformSyntheticModule(tree: ValOrDefDef)(using Ctx, CState): Thicket = {
    val sym = tree.symbol
   ...
   *  ```
   */
  def transformLocalDef(x: ValOrDefDef)(using Ctx, CState): Thicket = {
  ...
  private def nullOut(nullables: List[Symbol])(using Ctx, CState): List[Tree] =
    nullables.map { field =>
  ...
  /** Create thread-unsafe lazy accessor equivalent to such code
    ...
    * ```
    */
  def mkThreadUnsafeDef(sym: Symbol, flag: Symbol, target: Symbol, rhs: Tree)(using Ctx, CState): DefDef = {
  ...
  /** Create thread-unsafe lazy accessor for not-nullable types  equivalent to such code
    ...
    * ```
    */
  def mkDefThreadUnsafeNonNullable(sym: Symbol, target: Symbol, rhs: Tree)(using Ctx, CState): DefDef = {
    val targetRef = ref(target)
  ...
  def transformMemberDefThreadUnsafe(x: ValOrDefDef)(using Ctx, CState): Thicket = {
  ...
  def mkThreadSafeDef(methodSymbol: TermSymbol,
                      ...
                      casFlag: Tree,
                      setFlagState: Tree,
                      waitOnLock: Tree)(using Ctx, CState): DefDef = {
  ...
  def transformMemberDefThreadSafe(x: ValOrDefDef)(using Ctx, CState): Thicket = {
    assert(!(x.symbol is Mutable))
