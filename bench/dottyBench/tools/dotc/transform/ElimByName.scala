...
class ElimByName extends TransformByNameApply with InfoTransformer {
  ...
  private def applyIfFunction(tree: Tree, ftree: Tree)(using Ctx, CState) =
  ...
  override def transformTypeApply(tree: TypeApply)(using Ctx, CState): Tree = tree match {
  ...
  def transformInfo(tp: Type, sym: Symbol)(using Ctx, CState): Type = tp match {
    case ExprType(rt) => defn.FunctionOf(Nil, rt)
    case _ => tp
  }

  override def mayChange(sym: Symbol)(using Ctx, CState): Boolean = sym.isTerm && exprBecomesFunction(sym)
}
