...
class Constructors extends MiniPhase with IdentityDenotTransformer { thisPhase =>
  ...
  private def markUsedPrivateSymbols(tree: RefTree)(using Ctx, CState): Unit = {
  ...
  override def transformValDef(tree: tpd.ValDef)(using Ctx, CState): tpd.Tree = {
   ...
   */
  private def noDirectRefsFrom(tree: Tree)(using Ctx, CState) =
    tree.isDef && tree.symbol.isClass

  /** Class members that can be eliminated if referenced only from their own
   *  constructor.
   */
  private def mightBeDropped(sym: Symbol)(using Ctx, CState) =
  ...
  override def transformTemplate(tree: Template)(using Ctx, CState): Tree = {
    ...
    object intoConstr extends TreeMap {
      ...
      def apply(tree: Tree, prevOwner: Symbol)(using Ctx, CState): Tree =
    ...
    val copyParams = accessors flatMap { acc =>
      if (!isRetained(acc)) {
        ...
        if (param.hasAnnotation(defn.ConstructorOnlyAnnot))
          report.error(em"${acc.name} is marked `@constructorOnly` but it is retained as a field in ${acc.owner}", acc.sourcePos)
