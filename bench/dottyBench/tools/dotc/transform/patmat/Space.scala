...
trait SpaceLogic {
   ...
   */
  def simplify(space: Space, aggressive: Boolean = false)(using Ctx, CState): Space = trace(s"simplify ${show(space)}, aggressive = $aggressive --> ", debug, x => show(x.asInstanceOf[Space]))(space match {
  ...
  def flatten(space: Space)(using Ctx, CState): List[Space] = space match {
    case Prod(tp, fun, spaces, full) =>
  ...
  def isSubspace(a: Space, b: Space)(using Ctx, CState): Boolean = trace(s"${show(a)} < ${show(b)}", debug) {
  ...
  def intersect(a: Space, b: Space)(using Ctx, CState): Space = trace(s"${show(a)} & ${show(b)}", debug, x => show(x.asInstanceOf[Space])) {
  ...
  def minus(a: Space, b: Space)(using Ctx, CState): Space = trace(s"${show(a)} - ${show(b)}", debug, x => show(x.asInstanceOf[Space])) {
...
object SpaceEngine {
   ...
   */
  def isIrrefutableUnapply(unapp: tpd.Tree, patSize: Int)(using Ctx, CState): Boolean = {
   ...
   */
  def isIrrefutableUnapplySeq(unapp: tpd.Tree, patSize: Int)(using Ctx, CState): Boolean = {
    val unappResult = unapp.tpe.widen.finalResultType
    unappResult.isRef(defn.SomeClass) ||
    (unapp.symbol.is(Synthetic) && unapp.symbol.owner.linkedClass.is(Case)) ||  // scala2 compatibility
...
class SpaceEngine(using Ctx, CState) extends SpaceLogic {
  import tpd._
  ...
  private def unapplySeqInfo(resTp: Type, pos: SourcePosition)(using Ctx, CState): (Int, Type, Type) = {
    var resultTp = resTp
  ...
  def satisfiable(sp: Space): Boolean = {
    ...
    def checkConstraint(constrs: List[(Type, Type)])(using Ctx, CState): Boolean = {
