...
object ProtectedAccessors {
  ...
  def insideBoundaryOf(sym: Symbol)(using Ctx, CState): Boolean =
   ...
   */
  def needsAccessorIfNotInSubclass(sym: Symbol)(using Ctx, CState): Boolean =
  ...
  def needsAccessor(sym: Symbol)(using Ctx, CState): Boolean =
...
class ProtectedAccessors extends MiniPhase {
  ...
  object Accessors extends AccessProxies {
    val insert: Insert = new Insert {
      ...
      def needsAccessor(sym: Symbol)(using Ctx, CState) = ProtectedAccessors.needsAccessor(sym)

      override def ifNoHost(reference: RefTree)(using Ctx, CState): Tree = {
  ...
  override def transformTemplate(tree: Template)(using Ctx, CState): Tree =
