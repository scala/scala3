...
class SyntheticMembers(thisPhase: DenotTransformer) {
  ...
  private def initSymbols(using Ctx, CState) =
  ...
  def valueSymbols(using Ctx, CState): List[Symbol] = { initSymbols; myValueSymbols }
  def caseSymbols(using Ctx, CState): List[Symbol] = { initSymbols; myCaseSymbols }
  def caseModuleSymbols(using Ctx, CState): List[Symbol] = { initSymbols; myCaseModuleSymbols }
  def enumCaseSymbols(using Ctx, CState): List[Symbol] = { initSymbols; myEnumCaseSymbols }

  private def existingDef(sym: Symbol, clazz: ClassSymbol)(using Ctx, CState): Symbol = {
    val existing = sym.matchingMember(clazz.thisType)
    if (existing != sym && !existing.is(Deferred)) existing
    else NoSymbol
  }

  private def synthesizeDef(sym: TermSymbol, rhsFn: List[List[Tree]] => (Ctx, CState) ?=> Tree)(using Ctx, CState): Tree =
   ...
   */
  def caseAndValueMethods(clazz: ClassSymbol)(using Ctx, CState): List[Tree] = {
    ...
    def syntheticDef(sym: Symbol): Tree = {
      ...
      def syntheticRHS(vrefss: List[List[Tree]])(using Ctx, CState): Tree = synthetic.name match {
     ...
     *  ```
     */
    def productElementBody(arity: Int, index: Tree)(using Ctx, CState): Tree = {
      // case N => _${N + 1}
     ...
     *  ```
     */
    def productElementNameBody(arity: Int, index: Tree)(using Ctx, CState): Tree = {
     ...
     *
     */
    def equalsBody(that: Tree)(using Ctx, CState): Tree = {
     ...
     *  ```
     */
    def valueHashCodeBody(using Ctx, CState): Tree = {
     ...
     */
    def chooseHashcode(using Ctx, CState) =
     ...
     *  ```
     */
    def caseHashCodeBody(using Ctx, CState): Tree = {
    ...
    def hashImpl(sym: Symbol)(using Ctx, CState): Tree =
   ...
   */
  def serializableObjectMethod(clazz: ClassSymbol)(using Ctx, CState): List[Tree] = {
   ...
   *  ```
   */
  def fromProductBody(caseClass: Symbol, param: Tree)(using Ctx, CState): Tree = {
   ...
   *  O is O.type.
   */
  def ordinalBody(cls: Symbol, param: Tree)(using Ctx, CState): Tree =
   ...
   */
  def addMirrorSupport(impl: Template)(using Ctx, CState): Template = {
    ...
    def addMethod(name: TermName, info: Type, cls: Symbol, body: (Symbol, Tree) => (Ctx, CState) ?=> Tree): Unit = {
  ...
  def addSyntheticMembers(impl: Template)(using Ctx, CState): Template = {
    val clazz = ctx.owner.asClass
    addMirrorSupport(
