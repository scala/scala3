...
class ReifyQuotes extends MacroTransform {
  ...
  protected def newTransformer(using Context): Transformer = new Transformer {
    override def transform(tree: tpd.Tree)(using Ctx, CState): tpd.Tree =
      new QuoteReifier(null, new mutable.HashMap[Symbol, Tree => Tree], new Embedded, ctx.owner)(combinedContext).transform(tree)
  }
  ...
  private class QuoteReifier(outer: QuoteReifier, capturers: mutable.HashMap[Symbol, Tree => Tree],
    ...
    def nested(isQuote: Boolean)(using Ctx, CState): QuoteReifier = {
      val nestedEmbedded = if (level > 1 || (level == 1 && isQuote)) embedded else new Embedded
      new QuoteReifier(this, capturers, nestedEmbedded, ctx.owner)(combinedContext)
    }
    ...
    private def pickledQuote(body: Tree, splices: List[Tree], originalTp: Type, isType: Boolean)(using Ctx, CState) = {
     ...
     *      }
     */
    private def makeLambda(tree: Tree)(using Ctx, CState): Tree = {
      def body(arg: Tree)(using Ctx, CState): Tree = {
        var i = 0
    ...
    private def transformWithCapturer(tree: Tree)(capturer: mutable.Map[Symbol, Tree] => Tree => Tree)(using Ctx, CState): Tree = {
    ...
    private def isCaptured(sym: Symbol, level: Int)(using Ctx, CState): Boolean =
      level == 1 && levelOf(sym) == 1 && capturers.contains(sym)

    /** Transform `tree` and return the resulting tree and all `embedded` quotes
     *  or splices as a pair.
     */
    private def splitQuote(tree: Tree)(using Ctx, CState): (Tree, List[Tree]) = {
      val tree1 = stipTypeAnnotations(transform(tree))
      (tree1, embedded.getTrees)
    }

    private def splitSplice(tree: Tree)(using Ctx, CState): (Tree, List[Tree]) = {
      val tree1 = makeLambda(tree)
      (tree1, embedded.getTrees)
    }

    private def stipTypeAnnotations(tree: Tree)(using Ctx, CState): Tree =
     ...
     */
    private def makeHole(isTermHole: Boolean, body: Tree, splices: List[Tree], tpe: Type)(using Ctx, CState): Hole = {
      val idx = embedded.addTree(body, NoSymbol)

      /** Remove references to local types that will not be defined in this quote */
      def getTypeHoleType(using Ctx, CState) = new TypeMap() {
      ...
      /** Remove references to local types that will not be defined in this quote */
      def getTermHoleType(using Ctx, CState) = new TypeMap() {
        override def apply(tp: Type): Type = tp match
    ...
    private def liftList(list: List[Tree], tpe: Type)(using Ctx, CState): Tree =
...
object ReifyQuotes {
  ...
  class Embedded(trees: mutable.ListBuffer[tpd.Tree] = mutable.ListBuffer.empty, map: mutable.Map[Symbol, tpd.Tree] = mutable.Map.empty) {
    ...
    def getHoleType(body: tpd.Tree, splice: tpd.Tree)(using Ctx, CState): Type =
    ...
    def isLiftedSymbol(sym: Symbol)(using Ctx, CState): Boolean = map.contains(sym)
