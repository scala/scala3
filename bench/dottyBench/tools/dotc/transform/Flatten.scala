...
class Flatten extends MiniPhase with SymTransformer {
  ...
  private def liftedDefs(using Ctx, CState) = ctx.store(LiftedDefs)

  override def initContext(ctx: FreshCtx): Unit =
    LiftedDefs = ctx.addLocation[mutable.ListBuffer[Tree]](null)

  def transformSym(ref: SymDenotation)(using Ctx, CState): SymDenotation =
  ...
  override def prepareForPackageDef(tree: PackageDef)(using Ctx, CState): FreshCtx =
    ctx.fresh.updateStore(LiftedDefs, new mutable.ListBuffer[Tree])

  private def liftIfNested(tree: Tree)(using Ctx, CState) =
  ...
  override def transformStats(stats: List[Tree])(using Ctx, CState): List[Tree] =
  ...
  override def transformTypeDef(tree: TypeDef)(using Ctx, CState): Tree =
    liftIfNested(tree)
}
