...
import dottyBench.tools.dotc.core.NameKinds.FlatName
import dottyBench.tools.dotc.core.Names.{Name, TermName}
...
import dottyBench.tools.dotc.core.Denotations.staticRef
import dottyBench.tools.dotc.core.{NameKinds, TypeErasure}
...
object Splicer {
   ...
   *  See: `Staging`
   */
  def splice(tree: Tree, pos: SourcePosition, classLoader: ClassLoader)(using Ctx, CState): Tree = tree match {
    case Quoted(quotedTree) => quotedTree
    case _ =>
      val macroOwner = newSymbol(ctx.owner, nme.MACROkw, Macro | Synthetic, defn.AnyType, coord = tree.span)
      try
        inCtx(ctx.withOwner(macroOwner)) {
        ...
        case NonFatal(ex) =>
          val msg =
            s"""Failed to evaluate macro.
               ...
               |    ${ex.getStackTrace.takeWhile(_.getClassName != "dottyBench.tools.dotc.transform.Splicer$").drop(1).mkString("\n    ")}
             """.stripMargin
  ...
  def checkEscapedVariables(tree: Tree, expansionOwner: Symbol)(using Ctx, CState): tree.type =
    new TreeTraverser {
      ...
      private def isEscapedVariable(sym: Symbol)(using Ctx, CState): Boolean =
  ...
  /** Check that the Tree can be spliced. `${'{xyz}}` becomes `xyz`
    ...
    *  See: `Staging`
    */
  def checkValidMacroBody(tree: Tree)(using Ctx, CState): Unit = tree match {
    case Quoted(_) => // ok
    case _ =>
      type Env = Set[Symbol]
  ...
  private class Interpreter(pos: SourcePosition, classLoader: ClassLoader)(using Ctx, CState) {
    ...
    private object MissingClassDefinedInCurrentRun {
      def unapply(targetException: NoClassDefFoundError)(using Ctx, CState): Option[Symbol] = {
  ...
  object Call {
     ...
     */
    def unapply(arg: Tree)(using Ctx, CState): Option[(RefTree, List[List[Tree]])] =
      Call0.unapply(arg).map((fn, args) => (fn, args.reverse))

    private object Call0 {
      def unapply(arg: Tree)(using Ctx, CState): Option[(RefTree, List[List[Tree]])] = arg match {
