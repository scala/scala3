...
abstract class MacroTransform extends Phase {
  ...
  override def run(using Ctx, CState): Unit =
    ...
    unit.tpdTree = atPhase(transformPhase) {
      newTransformer(using combinedContext).transform(unit.tpdTree)
    }
   ...
   */
  protected def transformPhase: Phase = this

  class Transformer extends TreeMap(cpy = cpyBetweenPhases) {

    protected def localCtx(tree: Tree)(using Ctx, CState): FreshCtx = {
      val sym = tree.symbol
      val owner = if (sym.is(PackageVal)) sym.moduleClass else sym
      ctx.fresh.setTree(tree).setOwner(owner)
    }

    def transformStats(trees: List[Tree], exprOwner: Symbol)(using Ctx, CState): List[Tree] = {
