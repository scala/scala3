...
object NonLocalReturns {
  import ast.tpd._
  def isNonLocalReturn(ret: Return)(using Ctx, CState): Boolean =
...
class NonLocalReturns extends MiniPhase {
  ...
  private def ensureConforms(tree: Tree, pt: Type)(using Ctx, CState) =
    if (tree.tpe <:< pt) tree
    else Erasure.Boxing.adaptToType(tree, pt)

  private def nonLocalReturnControl(using Ctx, CState) = defn.NonLocalReturnControlClass.typeRef

  /** The type of a non-local return expression with given argument type */
  private def nonLocalReturnExceptionType(argtype: Type)(using Ctx, CState) =
  ...
  private def nonLocalReturnKey(meth: Symbol)(using Ctx, CState) =
   ...
   */
  private def nonLocalReturnThrow(expr: Tree, meth: Symbol)(using Ctx, CState) =
    Throw(
      New(
   ...
   *  }
   */
  private def nonLocalReturnTry(body: Tree, key: TermSymbol, meth: Symbol)(using Ctx, CState) = {
  ...
  override def transformReturn(tree: Return)(using Ctx, CState): Tree =
