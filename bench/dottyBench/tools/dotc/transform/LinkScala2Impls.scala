...
class LinkScala2Impls extends MiniPhase with IdentityDenotTransformer { thisPhase =>
  ...
  private def addStaticForwarders(mixin: ClassSymbol)(using Ctx, CState): Unit = {
  ...
  override def prepareForTemplate(impl: Template)(using Ctx, CState): Ctx = {
  ...
  private def implMethod(meth: Symbol)(using Ctx, CState): Symbol = {
