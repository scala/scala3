...
object LambdaLift {
  ...
  class Lifter(thisPhase: MiniPhase with DenotTransformer)(using Ctx, CState) {
     ...
     */
    def isLocal(sym: Symbol)(using Ctx, CState): Boolean = {
     ...
     */
    def narrowLiftedOwner(sym: Symbol, owner: Symbol)(using Ctx, CState): Unit =
     ...
     *  }
     */
    private def markFree(sym: Symbol, enclosure: Symbol)(using Ctx, CState): Symbol = try {
    ...
    private def markCalled(callee: Symbol, caller: Symbol)(using Ctx, CState): Unit = {
    ...
    private class CollectDependencies extends TreeTraverser {
      def traverse(tree: Tree)(using Ctx, CState) = try { //debug
        val sym = tree.symbol
    ...
    private def computeFreeVars()(using Ctx, CState): Unit =
      while ({
    ...
    private def computeLiftedOwners()(using Ctx, CState): Unit =
      while ({
    ...
    private def newName(sym: Symbol)(using Ctx, CState): Name =
    ...
    private def generateProxies()(using Ctx, CState): Unit =
    ...
    private def liftedInfo(local: Symbol)(using Ctx, CState): Type = local.info match {
    ...
    private def liftLocals()(using Ctx, CState): Unit = {
    ...
    def currentEnclosure(using Ctx, CState): Symbol =
      ctx.owner.enclosingMethodOrClass

    private def inCurrentOwner(sym: Symbol)(using Ctx, CState) =
      sym.enclosure == currentEnclosure

    private def proxy(sym: Symbol)(using Ctx, CState): Symbol = {
    ...
    def memberRef(sym: Symbol)(using Ctx, CState): Tree = {
    ...
    def proxyRef(sym: Symbol)(using Ctx, CState): Tree = {
      val psym = atPhase(thisPhase)(proxy(sym))
      thisPhase.transformFollowingDeep(if (psym.owner.isTerm) ref(psym) else memberRef(psym))
    }

    def addFreeArgs(sym: Symbol, args: List[Tree])(using Ctx, CState): List[Tree] =
      free get sym match {
        case Some(fvs) => fvs.toList.map(proxyRef(_)) ++ args
        case _ => args
      }

    def addFreeParams(tree: Tree, proxies: List[Symbol])(using Ctx, CState): Tree = proxies match {
      case Nil => tree
      case proxies =>
        val sym = tree.symbol
    ...
    def liftDef(tree: MemberDef)(using Ctx, CState): Tree = {
...
class LambdaLift extends MiniPhase with IdentityDenotTransformer { thisPhase =>
  ...
  private def lifter(using Ctx, CState) = ctx.store(Lifter)
  ...
  override def transformClosure(tree: Closure)(using Ctx, CState): Closure =
  ...
  override def transformReturn(tree: Return)(using Ctx, CState): Tree = tree.expr match {
  ...
  override def transformTemplate(tree: Template)(using Ctx, CState): Template = {
    val cls = ctx.owner
    val lft = lifter
