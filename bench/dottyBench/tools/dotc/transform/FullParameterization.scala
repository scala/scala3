...
trait FullParameterization {
   ...
   *  otherwise NoSymbol.
   */
  protected def rewiredTarget(referenced: Symbol, derived: Symbol)(using Ctx, CState): Symbol
   ...
   */
  protected def rewiredTarget(tree: Tree, derived: Symbol)(using Ctx, CState): Symbol =
    rewiredTarget(tree.symbol, derived)
   ...
   */
  def fullyParameterizedType(info: Type, clazz: ClassSymbol, abstractOverClass: Boolean = true, liftThisType: Boolean = false)(using Ctx, CState): Type = {
   ...
   */
  private def allInstanceTypeParams(originalDef: DefDef, abstractOverClass: Boolean)(using Ctx, CState): List[Symbol] =
   ...
   */
  def fullyParameterizedDef(derived: TermSymbol, originalDef: DefDef, abstractOverClass: Boolean = true)(using Ctx, CState): Tree =
    polyDefDef(derived, trefs => vrefss => {
       ...
       */
      def rewireTree(tree: Tree, targs: List[Tree])(using Ctx, CState): Tree = {
   ...
   */
  def forwarder(derived: TermSymbol, originalDef: DefDef, abstractOverClass: Boolean = true, liftThisType: Boolean = false)(using Ctx, CState): Tree = {
    val fun =
      ref(derived.termRef)
...
object FullParameterization {
   ...
   */
  def memberSignature(info: Type)(using Ctx, CState): Signature = info match {
    case info: PolyType =>
