...
import dottyBench.tools.dotc.core.Annotations.BodyAnnotation
...
class PCPCheckAndHeal(@constructorOnly ictx: Context) extends TreeMapWithStages(ictx) with Checking {
  ...
  override protected def transformQuotation(body: Tree, quote: Tree)(using Ctx, CState): Tree = {
  ...
  private def checkAnnotations(tree: Tree)(using Ctx, CState): Unit =
    tree match
      case tree: DefTree =>
  ...
  private def healInfo(tree: Tree, pos: SourcePosition)(using Ctx, CState): Unit =
   ...
   */
  private def healType(pos: SourcePosition)(using Ctx, CState) = new TypeMap {
  ...
  private def healTypeOfTerm(pos: SourcePosition)(using Ctx, CState) = new TypeMap {
    def apply(tp: Type): Type =
      tp match
   ...
   */
  protected def tryHeal(sym: Symbol, tp: TypeRef, pos: SourcePosition)(using Ctx, CState): TypeRef = {
  ...
  private def levelError(sym: Symbol, tp: Type, pos: SourcePosition)(using Ctx, CState): tp.type = {
    def symStr =
...
object PCPCheckAndHeal {
  ...
  class QuoteTypeTags(span: Span)(using Ctx, CState) {
