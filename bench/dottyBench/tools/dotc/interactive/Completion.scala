package dottyBench.tools.dotc.interactive

import java.nio.charset.Charset

import dottyBench.tools.dotc.ast.Trees._
import dottyBench.tools.dotc.ast.untpd
import dottyBench.tools.dotc.config.Printers.interactiv
import dottyBench.tools.dotc.core.Contexts._
import dottyBench.tools.dotc.core.CheckRealizable
import dottyBench.tools.dotc.core.Decorators.StringInterpolators
import dottyBench.tools.dotc.core.Denotations.SingleDenotation
...
import dottyBench.tools.dotc.core.NameOps.NameDecorator
import dottyBench.tools.dotc.core.Symbols.{NoSymbol, Symbol, defn}
import dottyBench.tools.dotc.core.Scopes
import dottyBench.tools.dotc.core.StdNames.{nme, tpnme}
import dottyBench.tools.dotc.core.TypeError
import dottyBench.tools.dotc.core.Types.{NameFilter, NamedType, NoType, Type}
import dottyBench.tools.dotc.printing.Texts._
import dottyBench.tools.dotc.util.{NameTransformer, NoSourcePosition, SourcePosition}
...
object Completion {
   ...
   */
  def completions(pos: SourcePosition)(using Ctx, CState): (Int, List[Completion]) = {
  ...
  private def computeCompletions(pos: SourcePosition, path: List[Tree])(using Ctx, CState): (Int, List[Completion]) = {
  ...
  private class CompletionBuffer(val mode: Mode, val prefix: String, pos: SourcePosition) {
     ...
     * the same `Completion`.
     */
    def getCompletions(using Ctx, CState): List[Completion] = {
     ...
     */
    private def description(symbols: List[Symbol])(using Ctx, CState): String =
      symbols match {
        case sym :: Nil =>
     ...
     */
    def addScopeCompletions(using Ctx, CState): Unit = {
     ...
     * considered.
     */
    def addMemberCompletions(qual: Tree)(using Ctx, CState): Unit =
     ...
     */
    private def add(sym: Symbol, nameInScope: Name)(using Ctx, CState) =
      if (sym.exists &&
    ...
    private def addMember(site: Type, name: Name, nameInScope: Name)(using Ctx, CState) =
     ...
     */
    private def include(sym: Symbol, nameInScope: Name)(using Ctx, CState): Boolean =
     ...
     */
    private def accessibleMembers(site: Type)(using Ctx, CState): Seq[Symbol] = site match {
    ...
    private def addAccessibleMembers(site: Type)(using Ctx, CState): Unit =
     ...
     */
    private def addImportCompletions(using Ctx, CState): Unit = {
     ...
     */
    private def implicitConversionTargets(qual: Tree)(using Ctx, CState): Set[Type] = {
      val typer = ctx.typer
  ...
  private class RenameAwareScope extends Scopes.MutableScope {
    ...
    def enter[T <: Symbol](sym: T, name: Name)(using Ctx, CState): T = {
