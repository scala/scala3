...
object Interactive {
  ...
  def enclosingType(trees: List[SourceTree], pos: SourcePosition)(using Ctx, CState): Type = {
   ...
   */
  def enclosingTree(trees: List[SourceTree], pos: SourcePosition)(using Ctx, CState): Tree =
    enclosingTree(pathTo(trees, pos))

  /** The closes enclosing tree with a symbol, or the `EmptyTree`.
   */
  def enclosingTree(path: List[Tree])(using Ctx, CState): Tree =
   ...
   * @see sourceSymbol
   */
  def enclosingSourceSymbols(path: List[Tree], pos: SourcePosition)(using Ctx, CState): List[Symbol] = {
    val syms = path match {
   ...
   */
  def matchSymbol(tree: Tree, sym: Symbol, include: Include.Set)(using Ctx, CState): Boolean = {
  ...
  def namedTrees(trees: List[SourceTree], include: Include.Set, sym: Symbol)
   (using Ctx, CState): List[SourceTree] =
    if (!sym.exists)
      Nil
    else
  ...
  def namedTrees(trees: List[SourceTree],
                ...
                )(using Ctx, CState): List[SourceTree] = safely {
    ...
    def traverser(source: SourceFile) =
      new untpd.TreeTraverser {
        ...
        override def traverse(tree: untpd.Tree)(using Ctx, CState) =
          tree match {
   ...
   */
  def pathTo(trees: List[SourceTree], pos: SourcePosition)(using Ctx, CState): List[Tree] =
    trees.find(_.pos.contains(pos)) match {
      case Some(tree) => pathTo(tree.tree, pos.span)
      case None => Nil
    }

  def pathTo(tree: Tree, span: Span)(using Ctx, CState): List[Tree] =
  ...
  def contextOfStat(stats: List[Tree], stat: Tree, exprOwner: Symbol, ctx: Context): Context =
    ...
    stats match {
      ...
      case first :: _ if first eq stat =>
        ctx.exprContext(stat, exprOwner).toContext
      case (imp: Import) :: rest =>
        contextOfStat(rest, stat, exprOwner, ctx.importContext(imp, inContext(ctx){imp.symbol}).toContext)
      case _ :: rest =>
        contextOfStat(rest, stat, exprOwner, ctx)
    }

  def contextOfPath(path: List[Tree])(using Ctx, CState): Context = path match {
    case Nil | _ :: Nil =>
    ...
    case nested :: encl :: rest =>
      ...
      try encl match {
        case tree @ PackageDef(pkg, stats) =>
          ...
          else contextOfStat(stats, nested, pkg.symbol.moduleClass, outer.packageContext(tree, tree.symbol).toContext)
        case tree: DefDef =>
          assert(tree.symbol.exists)
        ...
        case tree: MemberDef =>
          assert(tree.symbol.exists)
          outer.localContext(tree, tree.symbol).toContext
        ...
        case tree @ Template(constr, parents, self, _) =>
          if ((constr :: self :: parents).contains(nested)) combinedContext
          else contextOfStat(tree.body, nested, tree.symbol, outer.inClassContext(self.symbol).toContext)
        case _ =>
          outer
      }
      catch {
  ...
  def enclosingDefinitionInPath(path: List[Tree])(using Ctx, CState): Tree =
  ...
  def findDefinitions(path: List[Tree], pos: SourcePosition, driver: InteractiveDriver): List[SourceTree] = {
    given c as Context = driver.currentCtx
  ...
  def localize(symbol: Symbol, sourceDriver: InteractiveDriver, targetDriver: InteractiveDriver): Symbol = {
    ...
    def in[T](driver: InteractiveDriver)(fn: (Ctx, CState) ?=> T): T =
      fn(using driver.currentCtx, driver.currentCtx.cstate)
   ...
   */
  def implementationFilter(sym: Symbol)(using Ctx, CState): NameTree => Boolean =
    if (sym.isClass) {
      case td: TypeDef =>
   ...
   */
  def isRenamed(tree: NameTree)(using Ctx, CState): Boolean = {
