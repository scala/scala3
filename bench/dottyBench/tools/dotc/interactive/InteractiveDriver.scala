...
import dottyBench.tools.io.{ AbstractFile, ClassPath, ClassRepresentation, PlainFile, VirtualFile }
...
class InteractiveDriver(val settings: List[String]) extends Driver {
  ...
  def currentCState: CState = currentCtx.cstate
  ...
  private val (zipClassPaths, dirClassPaths) = currentCtx.platform.classPath(using currentCtx, currentCState) match {
   ...
   */
  def sourceTrees(using Ctx, CState): List[SourceTree] = sourceTreesContaining("")
   ...
   * from the target directory.
   */
  def sourceTreesContaining(id: String)(using Ctx, CState): List[SourceTree] = {
   ...
   */
  def allTrees(using Ctx, CState): List[SourceTree] = allTreesContaining("")
   ...
   * on this project's classpath.
   */
  def allTreesContaining(id: String)(using Ctx, CState): List[SourceTree] = {
   ...
   */
  private def treesFromClassName(className: TypeName, id: String)(using Ctx, CState): List[SourceTree] = {
   ...
   */
  private def cleanup(tree: tpd.Tree)(using Ctx, CState): Unit = {
