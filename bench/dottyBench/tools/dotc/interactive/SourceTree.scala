...
case class SourceTree(tree: tpd.Import | tpd.NameTree, source: SourceFile) {
  ...
  final def pos(using Ctx, CState): SourcePosition = source.atSpan(tree.span)

  /** The position of the name in `tree` */
  def namePos(using Ctx, CState): SourcePosition = tree match {
...
object SourceTree {
  def fromSymbol(sym: ClassSymbol, id: String = "")(using Ctx, CState): List[SourceTree] =
