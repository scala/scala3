...
object DenotTransformers {
  ...
  trait DenotTransformer extends Phase {
    ...
    def lastPhaseId(using Ctx): Int = ctx.base.nextDenotTransformerId(id + 1)

    /** The validity period of the transformed denotations in the given context */
    def validFor(using Ctx, CState): Period =
  ...
  trait InfoTransformer extends DenotTransformer {
     ...
     */
    protected def mayChange(sym: Symbol)(using Ctx, CState): Boolean = true
  }
  ...
  trait SymTransformer extends DenotTransformer {
    ...
    def transformSym(sym: SymDenotation)(using Ctx, CState): SymDenotation

    def transform(ref: SingleDenotation)(using Ctx, CState): SingleDenotation = ref match {
  ...
  trait IdentityDenotTransformer extends DenotTransformer {
    def transform(ref: SingleDenotation)(using Ctx, CState): SingleDenotation = ref
  }
}
