...
sealed abstract class GadtConstraint extends Showable {
  ...
  def bounds(sym: Symbol)(using Ctx, CState): TypeBounds
   ...
   */
  def fullBounds(sym: Symbol)(using Ctx, CState): TypeBounds

  /** Is `sym1` ordered to be less than `sym2`? */
  def isLess(sym1: Symbol, sym2: Symbol)(using Ctx, CState): Boolean

  /** Add symbols to constraint, correctly handling inter-dependencies.
   *
   * @see [[ConstraintHandling.addToConstraint]]
   */
  def addToConstraint(syms: List[Symbol])(using Ctx, CState): Boolean
  def addToConstraint(sym: Symbol)(using Ctx, CState): Boolean = addToConstraint(sym :: Nil)

  /** Further constrain a symbol already present in the constraint. */
  def addBound(sym: Symbol, bound: Type, isUpper: Boolean)(using Ctx, CState): Boolean
   ...
   */
  def contains(sym: Symbol)(using Ctx, CState): Boolean

  def isEmpty: Boolean
  final def nonEmpty: Boolean = !isEmpty

  /** See [[ConstraintHandling.approximation]] */
  def approximation(sym: Symbol, fromBelow: Boolean)(using Ctx, CState): Type
  ...
  def debugBoundsDescription(using Ctx, CState): String
}
...
final class ProperGadtConstraint private(
  ...
  import dottyBench.tools.dotc.config.Printers.{gadts, gadtsConstr}

  given (using Ctx, CState) as Context = combinedContext
  ...
  def subsumes(left: GadtConstraint, right: GadtConstraint, pre: GadtConstraint)(using Ctx, CState): Boolean = {
  ...
  override def addToConstraint(params: List[Symbol])(using Ctx, CState): Boolean = {
    ...
    val poly1 = PolyType(params.map { sym => DepParamName.fresh(sym.name.toTypeName) })(
      pt => params.map { param =>
        ...
        def substDependentSyms(tp: Type, isUpper: Boolean)(using Ctx, CState): Type = {
  ...
  override def addBound(sym: Symbol, bound: Type, isUpper: Boolean)(using Ctx, CState): Boolean = {
  ...
  override def isLess(sym1: Symbol, sym2: Symbol)(using Ctx, CState): Boolean =
    constraint.isLess(tvarOrError(sym1).origin, tvarOrError(sym2).origin)

  override def fullBounds(sym: Symbol)(using Ctx, CState): TypeBounds =
    mapping(sym) match {
      case null => null
      case tv =>
  ...
  override def bounds(sym: Symbol)(using Ctx, CState): TypeBounds =
    mapping(sym) match {
      case null => null
      case tv =>
  ...
  override def contains(sym: Symbol)(using Ctx, CState): Boolean = mapping(sym) ne null

  override def approximation(sym: Symbol, fromBelow: Boolean)(using Ctx, CState): Type = {
  ...
  private def externalize(param: TypeParamRef)(using Ctx, CState): Type =
    reverseMapping(param) match {
      case sym: Symbol => sym.typeRef
      case null => param
    }

  private def tvarOrError(sym: Symbol)(using Ctx, CState): TypeVar =
  ...
  )(using Ctx, CState): Boolean = tp match {
  ...
  private class ContainsNoInternalTypesAccumulator(using Ctx, CState) extends TypeAccumulator[Boolean] {
  ...
  override def debugBoundsDescription(using Ctx, CState): String = {
...
@sharable object EmptyGadtConstraint extends GadtConstraint {
  override def bounds(sym: Symbol)(using Ctx, CState): TypeBounds = null
  override def fullBounds(sym: Symbol)(using Ctx, CState): TypeBounds = null

  override def isLess(sym1: Symbol, sym2: Symbol)(using Ctx, CState): Boolean = unsupported("EmptyGadtConstraint.isLess")

  override def isEmpty: Boolean = true

  override def contains(sym: Symbol)(using Ctx, CState) = false

  override def addToConstraint(params: List[Symbol])(using Ctx, CState): Boolean = unsupported("EmptyGadtConstraint.addToConstraint")
  override def addBound(sym: Symbol, bound: Type, isUpper: Boolean)(using Ctx, CState): Boolean = unsupported("EmptyGadtConstraint.addBound")

  override def approximation(sym: Symbol, fromBelow: Boolean)(using Ctx, CState): Type = unsupported("EmptyGadtConstraint.approximation")

  override def fresh = new ProperGadtConstraint
  override def restore(other: GadtConstraint): Unit =
    if (!other.isEmpty) sys.error("cannot restore a non-empty GADTMap")

  override def debugBoundsDescription(using Ctx, CState): String = "EmptyGadtConstraint"
