...
abstract class Constraint extends Showable {
   ...
   */
  def addLess(p1: TypeParamRef, p2: TypeParamRef)(using Ctx, CState): This
   ...
   */
  def replace(param: TypeParamRef, tp: Type)(using Ctx, CState): This
  ...
  def remove(tl: TypeLambda)(using Ctx, CState): This

  /** A new constraint with entry `tl` renamed to a fresh type lambda */
  def rename(tl: TypeLambda)(using Ctx, CState): This
   ...
   */
  def & (other: Constraint, otherHasErrors: Boolean)(using Ctx, CState): Constraint
   ...
   */
  def occursAtToplevel(param: TypeParamRef, tp: Type)(using Ctx, CState): Boolean

  /** Check that constraint only refers to TypeParamRefs bound by itself */
  def checkClosed()(using Ctx, CState): Unit
}
