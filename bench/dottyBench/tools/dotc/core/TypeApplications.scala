...
import dottyBench.tools.dotc.config.Config
...
object TypeApplications {
   ...
   */
  object EtaExpansion {
    def apply(tycon: Type)(using Ctx, CState): Type = {
      assert(tycon.typeParams.nonEmpty, tycon)
      tycon.EtaExpand(tycon.typeParamSymbols)
    }

    def unapply(tp: Type)(using Ctx, CState): Option[TypeRef] = tp match {
   ...
   */
  def EtaExpandIfHK(tparams: List[TypeParamInfo], args: List[Type])(using Ctx, CState): List[Type] =
   ...
   */
  class Reducer(tycon: TypeLambda, args: List[Type])(using Ctx, CState) extends TypeMap {
...
class TypeApplications(val self: Type) extends AnyVal {
   ...
   */
  final def typeParams(using Ctx, CState): List[TypeParamInfo] = {
    record("typeParams")
  ...
  final def hkTypeParams(using Ctx, CState): List[TypeParamInfo] =
    if (isLambdaSub) typeParams else Nil

  /** If `self` is a generic class, its type parameter symbols, otherwise Nil */
  final def typeParamSymbols(using Ctx, CState): List[TypeSymbol] = typeParams match {
    case (_: Symbol) :: _ =>
  ...
  def isLambdaSub(using Ctx, CState): Boolean = hkResult.exists

  /** Is self type of kind "*"? */
  def hasSimpleKind(using Ctx, CState): Boolean =
   ...
   */
  def hkResult(using Ctx, CState): Type = self.dealias match {
    case self: TypeRef =>
   ...
   */
  def hasSameKindAs(other: Type)(using Ctx, CState): Boolean = {
  ...
  def safeDealias(using Ctx, CState): Type = self match {
   ...
   *  to `[X1, ..., Xn] -> TC[X1, ..., Xn]`.
   */
  def EtaExpand(tparams: List[TypeParamInfo])(using Ctx, CState): Type =
  ...
  def ensureLambdaSub(using Ctx, CState): Type =
    if (isLambdaSub) self else EtaExpansion(self)

  /** Eta expand if `self` is a (non-lambda) class reference and `bound` is a higher-kinded type */
  def EtaExpandIfHK(bound: Type)(using Ctx, CState): Type = {
   ...
   *  @param  args   = `U1,...,Un`
   */
  final def appliedTo(args: List[Type])(using Ctx, CState): Type = {
    record("appliedTo")
  ...
  final def appliedTo(arg: Type)(using Ctx, CState): Type = appliedTo(arg :: Nil)
  final def appliedTo(arg1: Type, arg2: Type)(using Ctx, CState): Type = appliedTo(arg1 :: arg2 :: Nil)

  final def applyIfParameterized(args: List[Type])(using Ctx, CState): Type =
   ...
   */
  final def safeAppliedTo(args: List[Type])(using Ctx, CState): Type = self match {
   ...
   */
  final def toBounds(using Ctx, CState): TypeBounds = self match {
   ...
   */
  def translateParameterized(from: ClassSymbol, to: ClassSymbol, wildcardArg: Boolean = false)(using Ctx, CState): Type = self match {
   ...
   */
  def translateFromRepeated(toArray: Boolean, translateWildcard: Boolean = false)(using Ctx, CState): Type =
  ...
  def translateToRepeated(from: ClassSymbol)(using Ctx, CState): Type =
   ...
   */
  def translateJavaArrayElementType(using Ctx, CState): Type =
   ...
   */
  final def argInfos(using Ctx, CState): List[Type] = self.stripTypeVar.stripAnnots match {
    case AppliedType(tycon, args) => args
    case _ => Nil
  }

  /** Argument types where existential types in arguments are disallowed */
  def argTypes(using Ctx, CState): List[Type] = argInfos mapConserve noBounds

  /** Argument types where existential types in arguments are approximated by their lower bound */
  def argTypesLo(using Ctx, CState): List[Type] = argInfos.mapConserve(_.loBound)

  /** Argument types where existential types in arguments are approximated by their upper bound  */
  def argTypesHi(using Ctx, CState): List[Type] = argInfos.mapConserve(_.hiBound)

  /** If this is the image of a type argument; recover the type argument,
   *  otherwise NoType.
   */
  final def argInfo(using Ctx, CState): Type = self match {
  ...
  def dropAlias(using Ctx, CState): Type = self match {
    case TypeAlias(alias) => alias
    case _ => self
  }

  /** The element type of a sequence or array */
  def elemType(using Ctx, CState): Type = self.widenDealias match {
