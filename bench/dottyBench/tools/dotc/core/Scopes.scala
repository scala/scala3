...
object Scopes {
   ...
   */
  type SymbolSynthesizer = Name => (Ctx, CState) ?=> Symbol
  ...
  abstract class Scope extends printing.Showable {
     ...
     */
    def toList(using Ctx, CState): List[Symbol]

    /** Return all symbols as an iterator in the order they were entered in this scope.
     */
    def iterator(using Ctx, CState): Iterator[Symbol] = toList.iterator

    /** Is the scope empty? */
    def isEmpty: Boolean = lastEntry eq null

    /** Applies a function f to all Symbols of this Scope. */
    def foreach[U](f: Symbol => U)(using Ctx, CState): Unit = toList.foreach(f)

    /** Selects all Symbols of this Scope which satisfy a predicate. */
    def filter(p: Symbol => Boolean)(using Ctx, CState): List[Symbol] = {
      ensureComplete()
    ...
    def exists(p: Symbol => Boolean)(using Ctx, CState): Boolean = filter(p).nonEmpty

    /** Finds the first Symbol of this Scope satisfying a predicate, if any. */
    def find(p: Symbol => Boolean)(using Ctx, CState): Symbol = filter(p) match {
      case sym :: _ => sym
      case _ => NoSymbol
    }

    /** Returns a new mutable scope with the same content as this one. */
    def cloneScope(using Ctx, CState): MutableScope

    /** Lookup a symbol entry matching given name. */
    def lookupEntry(name: Name)(using Ctx, CState): ScopeEntry

    /** Lookup next entry with same name as this one */
    def lookupNextEntry(entry: ScopeEntry)(using Ctx, CState): ScopeEntry

    /** Lookup a symbol */
    final def lookup(name: Name)(using Ctx, CState): Symbol = {
     ...
     */
    final def lookupAll(name: Name)(using Ctx, CState): Iterator[Symbol] = new Iterator[Symbol] {
     ...
     *  in `this.toList`.
     */
    final def denotsNamed(name: Name, select: SymDenotation => Boolean = selectAll)(using Ctx, CState): PreDenotation = {
     ...
     */
    final def filteredScope(p: Symbol => Boolean)(using Ctx, CState): Scope = {
     ...
     */
    protected def ensureComplete()(using Ctx, CState): Unit = ()
  }
  ...
  class MutableScope protected[Scopes](initElems: ScopeEntry, initSize: Int, val nestingLevel: Int = 0)
    ...
    protected[Scopes] def this(base: Scope)(using Ctx, CState) =
    ...
    protected def newScopeEntry(name: Name, sym: Symbol)(using Ctx, CState): ScopeEntry = {
    ...
    protected def newScopeEntry(sym: Symbol)(using Ctx, CState): ScopeEntry =
      newScopeEntry(sym.name, sym)

    private def enterInHash(e: ScopeEntry)(using Ctx, CState): Unit = {
    ...
    final def enter[T <: Symbol](sym: T)(using Ctx, CState): T = {
    ...
    final def enterUnique(sym: Symbol)(using Ctx, CState): Unit = {
      assert(lookup(sym.name) == NoSymbol, (sym.showLocated, lookup(sym.name).showLocated))
      enter(sym)
    }

    private def ensureCapacity(tableSize: Int)(using Ctx, CState): Unit =
      if (size >= tableSize * FillFactor) createHash(tableSize * 2)

    private def createHash(tableSize: Int)(using Ctx, CState): Unit =
      if (size > tableSize * FillFactor) createHash(tableSize * 2)
      else {
    ...
    private def enterAllInHash(e: ScopeEntry, n: Int = 0)(using Ctx, CState): Unit =
      if (e ne null)
        if (n < MaxRecursions) {
    ...
    final def unlink(e: ScopeEntry)(using Ctx, CState): Unit = {
      if (lastEntry == e)
        lastEntry = e.prev
      else {
        var e1 = lastEntry
    ...
    final def unlink(sym: Symbol)(using Ctx, CState): Unit =
      unlink(sym, sym.name)

    /** remove symbol from this scope if it is present under the given name */
    final def unlink(sym: Symbol, name: Name)(using Ctx, CState): Unit = {
      var e = lookupEntry(name)
      while (e ne null) {
     ...
     */
    final def replace(prev: Symbol, replacement: Symbol)(using Ctx, CState): Unit = {
    ...
    /** Lookup a symbol entry matching given name.
     */
    override def lookupEntry(name: Name)(using Ctx, CState): ScopeEntry = {
    ...
    /** lookup next entry with same name as this one */
    override final def lookupNextEntry(entry: ScopeEntry)(using Ctx, CState): ScopeEntry = {
      var e = entry
      if (hashTable ne null)
     ...
     */
    override final def toList(using Ctx, CState): List[Symbol] = {
    ...
    override def implicitDecls(using Ctx, CState): List[TermRef] = {
      ensureComplete()
     ...
     */
    final def sorted(using Ctx, CState): List[Symbol] = toList
    ...
    override def checkConsistent()(using Ctx, CState): Unit = {
      ensureComplete()
      var e = lastEntry
      while (e != null) {
  ...
  def newNestedScope(outer: Scope)(using Ctx, CState): MutableScope = new MutableScope(outer)

  /** Create a new scope with given initial elements */
  def newScopeWith(elems: Symbol*)(using Ctx, CState): MutableScope = {
    val scope = newScope
  ...
  object EmptyScope extends Scope {
    ...
    override def toList(using Ctx, CState): List[Symbol] = Nil
    override def cloneScope(using Ctx, CState): MutableScope = unsupported("cloneScope")
    override def lookupEntry(name: Name)(using Ctx, CState): ScopeEntry = null
    override def lookupNextEntry(entry: ScopeEntry)(using Ctx, CState): ScopeEntry = null
  }
