...
class TreeUnpickler(reader: TastyReader,
   ...
   */
  def enter(roots: Set[SymDenotation])(using Ctx, CState): Unit = {
    this.roots = roots
  ...
  /** The unpickled trees */
  def unpickle(mode: UnpickleMode)(using Ctx, CState): List[Tree] = {
...
// ------ Reading types -----------------------------------------------------
    ...
    def readParamTypes[T <: Type](n: Int)(using Ctx, CState): List[T] =
      if n == 0 then Nil
      else
    ...
    def readSymRef()(using Ctx, CState): Symbol = symbolAt(readAddr())

    /** The symbol at given address; createa new one if none exists yet */
    def symbolAt(addr: Addr)(using Ctx, CState): Symbol = symAtAddr.get(addr) match {
      case Some(sym) =>
        sym
      case None =>
    ...
    def symbolAtCurrent()(using Ctx, CState): Symbol = symAtAddr.get(currentAddr) match {
      case Some(sym) =>
    ...
    def readConstant(tag: Int)(using Ctx, CState): Constant = (tag: @switch) match {
      case UNITconst =>
        Constant(())
      case TRUEconst =>
        Constant(true)
    ...
    /** Read a type */
    def readType()(using Ctx, CState): Type = {
      val start = currentAddr
      val tag = readByte()
    ...
    private def readSymNameRef()(using Ctx, CState): Type = {
      val sym = readSymRef()
      val prefix = readType()
    ...
    private def readPackageRef()(using Ctx, CState): TermSymbol = {
      val name = readName()
    ...
    def readTermRef()(using Ctx, CState): TermRef =
...
// ------ Reading definitions -----------------------------------------------------
    ...
    private def localContext(owner: Symbol)(using Ctx, CState) =
      ctx.fresh.setOwner(owner)

    private def normalizeFlags(tag: Int, givenFlags: FlagSet, name: Name, isAbsType: Boolean, rhsIsEmpty: Boolean)(using Ctx, CState): FlagSet = {
      val lacksDefinition =
        rhsIsEmpty &&
    ...
    def isAbstractType(ttag: Int)(using Ctx, CState): Boolean = nextUnsharedTag match {
      case LAMBDAtpt =>
        val rdr = fork
     ...
     */
    def createSymbol()(using Ctx, CState): Symbol = nextByte match {
    ...
    private def createBindSymbol()(using Ctx, CState): Symbol = {
      val start = currentAddr
      val tag = readByte()
      val end = readEnd()
      var name: Name = readName()
      ...
      val completer = new LazyType {
        def complete(denot: SymDenotation)(using Ctx, CState) =
     ...
     *  @return  the created symbol
     */
    def createMemberSymbol()(using Ctx, CState): Symbol = {
      val start = currentAddr
      val tag = readByte()
      val end = readEnd()
      var name: Name = readName()
        ...
        roots.find(root => (root.owner eq ctx.owner) && root.name == name) match {
          case Some(rootd) =>
            ...
            rootd.info = adjustIfModule(
                new Completer(subReader(start, end))(using combinedContext) with SymbolLoaders.SecondCompleter)
            rootd.flags = flags &~ Touched // allow one more completion
            rootd.setPrivateWithin(privateWithin)
            seenRoots += rootd.symbol
            rootd.symbol
          case _ =>
            val completer = adjustIfModule(new Completer(subReader(start, end))(using combinedContext))
            if (isClass)
      ...
      if (isClass) {
        ...
        sym.addAnnotation(LazyBodyAnnotation {
          val ctx1 = localContext(sym).addMode(Mode.ReadPositions)
          inCtx(sourceChangeContext(Addr(0))(using ctx1)) {
    ...
    def readModifiers[WithinType, AnnotType]
        (end: Addr, readAnnot: (Ctx, CState) ?=> Symbol => AnnotType, readWithin: (Ctx, CState) ?=> WithinType, defaultWithin: WithinType)
        (using Ctx, CState): (FlagSet, List[Symbol => AnnotType], WithinType) = {
    ...
    private val readTypedWithin: (Ctx, CState) ?=> Symbol = readType().typeSymbol

    private val readTypedAnnot: (Ctx, CState) ?=> Symbol => Annotation =
      readByte()
      val end = readEnd()
      val tp = readType()
     ...
     */
    def indexStats(end: Addr)(using Ctx, CState): FlagSet = {
     ...
     */
    def processPackage[T](op: (RefTree, Addr) => (Ctx, CState) ?=> T)(using Ctx, CState): T = {
     ...
     */
    def indexParams(tag: Int)(using Ctx, CState): Unit =
      while (nextByte == tag) {
        symbolAtCurrent()
        skipTree()
      }
     ...
     *  at current address.
     */
    def indexTemplateParams()(using Ctx, CState): Unit = {
     ...
     */
    def readIndexedDef()(using Ctx, CState): Tree = treeAtAddr.remove(currentAddr) match {
      case Some(tree) =>
    ...
    private def readNewDef()(using Ctx, CState): Tree = {
      val sctx = sourceChangeContext()
      ...
      def readParamss(using Ctx, CState): List[List[ValDef]] = nextByte match
        case PARAM | PARAMEND =>
      ...
      def readRhs(using Ctx, CState): LazyTree =
        if (nothingButMods(end))
          ...
          new Trees.Lazy[Tree] {
            def complete(using Ctx, CState) = typer.Inliner.bodyToInline(sym)
          }
        else
        ...
        case TYPEDEF | TYPEPARAM =>
          if (sym.isClass) {
            ...
            sym.info = new NoCompleter {
              override def completerTypeParams(sym: Symbol)(using Ctx, CState) =
                rhs.tpe.typeParams
            }
    ...
    private def readTemplate(using Ctx, CState): Template = {
      val start = currentAddr
    ...
    def skipToplevel()(using Ctx, CState): Unit= {
    ...
    def isTopLevel(using Ctx, CState): Boolean =
      nextByte == IMPORT || nextByte == PACKAGE

    def readTopLevel()(using Ctx, CState): List[Tree] = {
    ...
    def readIndexedStat(exprOwner: Symbol)(using Ctx, CState): Tree = nextByte match {
    ...
    def readImport()(using Ctx, CState): Tree = {
      val start = currentAddr
      assert(sourcePathAt(start).isEmpty)
      readByte()
      readEnd()
    ...
    def readSelectors(givenPrefix: Boolean)(using Ctx, CState): List[untpd.ImportSelector] =
    ...
    def readIndexedStats(exprOwner: Symbol, end: Addr)(using Ctx, CState): List[Tree] =
      until(end)(readIndexedStat(exprOwner))

    def readStats(exprOwner: Symbol, end: Addr)(using Ctx, CState): List[Tree] = {
      fork.indexStats(end)
      readIndexedStats(exprOwner, end)
    }

    def readIndexedParams[T <: MemberDef](tag: Int)(using Ctx, CState): List[T] =
      collectWhile(nextByte == tag) { readIndexedDef().asInstanceOf[T] }

    def readParams[T <: MemberDef](tag: Int)(using Ctx, CState): List[T] =
...
// ------ Reading trees -----------------------------------------------------
    ...
    def readTerm()(using Ctx, CState): Tree = {  // TODO: rename to readTree
      val sctx = sourceChangeContext()
    ...
    def readTpt()(using Ctx, CState): Tree = {
      val sctx = sourceChangeContext()
    ...
    def readCases(end: Addr)(using Ctx, CState): List[CaseDef] =
    ...
    def readCase()(using Ctx, CState): CaseDef = {
      val sctx = sourceChangeContext()
    ...
    def readLater[T <: AnyRef](end: Addr, op: TreeReader => (Ctx, CState) ?=> T)(using Ctx, CState): Trees.Lazy[T] =
      readLaterWithOwner(end, op)(ctx.owner)

    def readLaterWithOwner[T <: AnyRef](end: Addr, op: TreeReader => (Ctx, CState) ?=> T)(using Ctx, CState): Symbol => Trees.Lazy[T] = {
...
// ------ Setting positions ------------------------------------------------
    ...
    def spanAt(addr: Addr)(using Ctx, CState): Span =
    ...
    def coordAt(addr: Addr)(using Ctx, CState): Coord = {
    ...
    def sourcePathAt(addr: Addr)(using Ctx, CState): String =
      if (ctx.mode.is(Mode.ReadPositions))
        posUnpicklerOpt match {
          case Some(posUnpickler) =>
     ...
     */
    def sourceChangeContext(addr: Addr = currentAddr)(using Ctx, CState): Ctx = {
    ...
    def setSpan[T <: untpd.Tree](addr: Addr, tree: T)(using Ctx, CState): tree.type = {
      val span = spanAt(addr)
  ...
  class LazyReader[T <: AnyRef](
      ...
      op: TreeReader => (Ctx, CState) ?=> T) extends Trees.Lazy[T] {
    def complete(using Ctx, CState): T = {
      pickling.println(i"starting to read at ${reader.reader.currentAddr} with owner $owner")
      atPhaseNoLater(picklerPhase) {
        inMode(mode) {
          op(reader)(using ctx.withOwner(owner).withSource(source))
        }
      }
    }
  }
  ...
  class OwnerTree(val addr: Addr, tag: Int, reader: TreeReader, val end: Addr) {
    ...
    def findOwner(addr: Addr)(using Ctx, CState): Symbol = {
