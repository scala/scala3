...
class TreePickler(pickler: TastyPickler) {
  ...
  def preRegister(tree: Tree)(using Ctx, CState): Unit = tree match {
    case tree: MemberDef =>
  ...
  private def pickleSymRef(sym: Symbol)(using Ctx, CState) = symRefs.get(sym) match {
    case Some(label) =>
  ...
  private def pickleForwardSymRef(sym: Symbol)(using Ctx, CState) = {
    val ref = reserveRef(relative = false)
    assert(!sym.is(Flags.Package), sym)
    forwardSymRefs(sym) = ref :: forwardSymRefs.getOrElse(sym, Nil)
  }

  private def isLocallyDefined(sym: Symbol)(using Ctx, CState) =
    sym.topLevelClass.isLinkedWith(pickler.rootCls)

  def pickleConstant(c: Constant)(using Ctx, CState): Unit = c.tag match {
    case UnitTag =>
      writeByte(UNITconst)
    case BooleanTag =>
  ...
  def pickleVariances(tp: Type)(using Ctx, CState): Unit = tp match
  ...
  def pickleType(tpe0: Type, richTypes: Boolean = false)(using Ctx, CState): Unit = {
  ...
  private def pickleNewType(tpe: Type, richTypes: Boolean)(using Ctx, CState): Unit = tpe match {
  ...
  def pickleMethodic(tag: Int, tpe: LambdaType, mods: FlagSet)(using Ctx, CState): Unit = {
    writeByte(tag)
    withLength {
  ...
  def pickleParamRef(tpe: ParamRef)(using Ctx, CState): Boolean = {
  ...
  def pickleTpt(tpt: Tree)(using Ctx, CState): Unit =
    pickleTree(tpt)

  def pickleTreeUnlessEmpty(tree: Tree)(using Ctx, CState): Unit = {
    if (!tree.isEmpty) pickleTree(tree)
  }

  def pickleDef(tag: Int, sym: Symbol, tpt: Tree, rhs: Tree = EmptyTree, pickleParams: => Unit = ())(using Ctx, CState): Unit = {
  ...
  def pickleParam(tree: Tree)(using Ctx, CState): Unit = {
  ...
  def pickleParams(trees: List[Tree])(using Ctx, CState): Unit = {
    trees.foreach(preRegister)
    trees.foreach(pickleParam)
  }

  def pickleStats(stats: List[Tree])(using Ctx, CState): Unit = {
    stats.foreach(preRegister)
    stats.foreach(stat => if (!stat.isEmpty) pickleTree(stat))
  }

  def pickleTree(tree: Tree)(using Ctx, CState): Unit = {
  ...
  def pickleSelectors(selectors: List[untpd.ImportSelector])(using Ctx, CState): Unit =
  ...
  def pickleSelector(tag: Int, id: untpd.Ident)(using Ctx, CState): Unit = {
    registerTreeAddr(id)
    writeByte(tag)
    pickleName(id.name)
  }

  def pickleModifiers(sym: Symbol)(using Ctx, CState): Unit = {
    import Flags._
    var flags = sym.flags
  ...
  def pickleFlags(flags: FlagSet, isTerm: Boolean)(using Ctx, CState): Unit = {
    import Flags._
  ...
  private def isUnpicklable(owner: Symbol, ann: Annotation)(using Ctx, CState) = ann match {
  ...
  def pickleAnnotation(owner: Symbol, ann: Annotation)(using Ctx, CState): Unit = {
...
// ---- main entry points ---------------------------------------
  ...
  def pickle(trees: List[Tree])(using Ctx, CState): Unit = {
