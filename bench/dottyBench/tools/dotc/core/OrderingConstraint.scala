...
object OrderingConstraint {
  ...
  private def newConstraint(boundsMap: ParamBounds, lowerMap: ParamOrdering, upperMap: ParamOrdering)(using Ctx, CState) : OrderingConstraint = {
  ...
  abstract class ConstraintLens[T <: AnyRef: ClassTag] {
    ...
    def updateEntries(c: OrderingConstraint, poly: TypeLambda, entries: Array[T])(using Ctx, CState): OrderingConstraint
    def initial: T
    ...
    def update(prev: OrderingConstraint, current: OrderingConstraint,
        poly: TypeLambda, idx: Int, entry: T)(using Ctx, CState): OrderingConstraint = {
    ...
    def update(prev: OrderingConstraint, current: OrderingConstraint,
        param: TypeParamRef, entry: T)(using Ctx, CState): OrderingConstraint =
      update(prev, current, param.binder, param.paramNum, entry)

    def map(prev: OrderingConstraint, current: OrderingConstraint,
        poly: TypeLambda, idx: Int, f: T => T)(using Ctx, CState): OrderingConstraint =
     update(prev, current, poly, idx, f(apply(current, poly, idx)))

    def map(prev: OrderingConstraint, current: OrderingConstraint,
        param: TypeParamRef, f: T => T)(using Ctx, CState): OrderingConstraint =
  ...
  val boundsLens: ConstraintLens[Type] = new ConstraintLens[Type] {
    def entries(c: OrderingConstraint, poly: TypeLambda): Array[Type] =
      c.boundsMap(poly)
    def updateEntries(c: OrderingConstraint, poly: TypeLambda, entries: Array[Type])(using Ctx, CState): OrderingConstraint =
  ...
  val lowerLens: ConstraintLens[List[TypeParamRef]] = new ConstraintLens[List[TypeParamRef]] {
    def entries(c: OrderingConstraint, poly: TypeLambda): Array[List[TypeParamRef]] =
      c.lowerMap(poly)
    def updateEntries(c: OrderingConstraint, poly: TypeLambda, entries: Array[List[TypeParamRef]])(using Ctx, CState): OrderingConstraint =
...
// ---------- Info related to TypeParamRefs -------------------------------------------
  ...
  def nonParamBounds(param: TypeParamRef)(using Ctx, CState): TypeBounds =
    entry(param).bounds
...
// ---------- Adding TypeLambdas --------------------------------------------------
  ...
  private def stripParams(tp: Type, paramBuf: mutable.ListBuffer[TypeParamRef],
      isUpper: Boolean)(using Ctx, CState): Type = tp match {
  ...
  private def normalizedType(tp: Type, paramBuf: mutable.ListBuffer[TypeParamRef],
      isUpper: Boolean)(using Ctx, CState): Type =
    stripParams(tp, paramBuf, isUpper)
      .orElse(if (isUpper) defn.AnyKindType else defn.NothingType)

  def add(poly: TypeLambda, tvars: List[TypeVar])(using Ctx, CState): This = {
   ...
   */
  private def init(poly: TypeLambda)(using Ctx, CState): This = {
    var current = this
...
// ---------- Updates ------------------------------------------------------------
   ...
   */
  private def ensureNonCyclic(param: TypeParamRef, inst: Type)(using Ctx, CState): Type =
   ...
   */
  private def order(current: This, param1: TypeParamRef, param2: TypeParamRef)(using Ctx, CState): This =
  ...
  private def updateEntry(current: This, param: TypeParamRef, tp: Type)(using Ctx, CState): This = {
  ...
  def updateEntry(param: TypeParamRef, tp: Type)(using Ctx, CState): This =
    updateEntry(this, param, ensureNonCyclic(param, tp)).checkNonCyclic()

  def addLess(param1: TypeParamRef, param2: TypeParamRef)(using Ctx, CState): This =
    order(this, param1, param2).checkNonCyclic()

  def unify(p1: TypeParamRef, p2: TypeParamRef)(using Ctx, CState): This =
...
// ---------- Replacements and Removals -------------------------------------
   ...
   */
  def replace(param: TypeParamRef, tp: Type)(using Ctx, CState): OrderingConstraint =
  ...
  def remove(pt: TypeLambda)(using Ctx, CState): This = {
...
// ----------- Joins -----------------------------------------------------
  ...
  def & (other: Constraint, otherHasErrors: Boolean)(using Ctx, CState): OrderingConstraint = {
  ...
  def rename(tl: TypeLambda)(using Ctx, CState): OrderingConstraint = {
    assert(contains(tl))
  ...
  def ensureFresh(tl: TypeLambda)(using Ctx, CState): TypeLambda =
    if (contains(tl)) {
...
// ---------- Checking -----------------------------------------------
  ...
  def checkNonCyclic()(using Ctx, CState): this.type =
  ...
  def occursAtToplevel(param: TypeParamRef, inst: Type)(using Ctx, CState): Boolean =

    def occurs(tp: Type)(using Ctx, CState): Boolean = tp match
      case tp: AndOrType =>
  ...
  override def checkClosed()(using Ctx, CState): Unit =
