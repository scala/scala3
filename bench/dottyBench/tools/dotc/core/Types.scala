...
//    }
     ...
     */
    def isProvisional(using Ctx, CState): Boolean = mightBeProvisional && testProvisional

    private def testProvisional(using Ctx, CState) = {
    ...
    final def isStable(using Ctx, CState): Boolean = stripTypeVar match {
     ...
     */
    def isRef(sym: Symbol, skipRefined: Boolean = true)(using Ctx, CState): Boolean = stripAnnots.stripTypeVar match {
      case this1: TypeRef =>
    ...
    def isDirectRef(sym: Symbol)(using Ctx, CState): Boolean = stripTypeVar match {
      case this1: TypeRef =>
    ...
    def isAny(using Ctx, CState): Boolean     = isRef(defn.AnyClass, skipRefined = false)
    def isAnyRef(using Ctx, CState): Boolean  = isRef(defn.ObjectClass, skipRefined = false)
    def isAnyKind(using Ctx, CState): Boolean = isRef(defn.AnyKindClass, skipRefined = false)

    /** Does this type refer exactly to class symbol `sym`, instead of to a subclass of `sym`?
     *  Implemented like `isRef`, but follows more types: all type proxies as well as and- and or-types
     */
    private[Types] def isTightPrefix(sym: Symbol)(using Ctx, CState): Boolean = stripTypeVar match {
    ...
    final def derivesFrom(cls: Symbol)(using Ctx, CState): Boolean = {
     ...
     */
    def isArgPrefixOf(symd: SymDenotation)(using Ctx, CState): Boolean =
    ...
    def isBottomType(using Ctx, CState): Boolean = this match {
      case tp: TypeRef => tp.symbol eq defn.NothingClass
      case _ => false
    }

      /** Is this type exactly Any (no vars, aliases, refinements etc allowed)? */
    def isTopType(using Ctx, CState): Boolean = this match {
      case tp: TypeRef => tp.symbol eq defn.AnyClass
      case _ => false
    }

    /** Is this type a (possibly aliased) singleton type? */
    def isSingleton(using Ctx, CState): Boolean = dealias.isInstanceOf[SingletonType]

    /** Is this type of kind `AnyKind`? */
    def hasAnyKind(using Ctx, CState): Boolean = {
    ...
    final def isNotNull(using Ctx, CState): Boolean = this match {
    ...
    final def isError(using Ctx, CState): Boolean = stripTypeVar.isInstanceOf[ErrorType]

    /** Is some part of the widened version of this type produced as a repair for an error? */
    def isErroneous(using Ctx, CState): Boolean =
      widen.existsPart(_.isError, forceLazy = false)

    /** Does the type carry an annotation that is an instance of `cls`? */
    @tailrec final def hasAnnotation(cls: ClassSymbol)(using Ctx, CState): Boolean = stripTypeVar match {
    ...
    def derivesAnnotWith(p: Annotation => Boolean)(using Ctx, CState): Boolean = this match {
    ...
    def occursIn(that: Type)(using Ctx, CState): Boolean =
      that.existsPart(this == _)

    /** Does this type not refer to TypeParamRefs or uninstantiated TypeVars? */
    final def isGround(using Ctx, CState): Boolean =
      (new isGroundAccumulator).apply(true, this)

    /** Is this a type of a repeated parameter? */
    def isRepeatedParam(using Ctx, CState): Boolean =
      typeSymbol eq defn.RepeatedParamClass

    /** Is this the type of a method that has a repeated parameter type as
     *  last parameter type?
     */
    def isVarArgsMethod(using Ctx, CState): Boolean = stripPoly match {
     ...
     */
    def isNullaryMethod(using Ctx, CState): Boolean = stripPoly match {
     ...
     */
    def isMatch(using Ctx, CState): Boolean = stripTypeVar.stripAnnots match {
...
// ----- Higher-order combinators -----------------------------------
     ...
     */
    final def existsPart(p: Type => Boolean, forceLazy: Boolean = true)(using Ctx, CState): Boolean =
      new ExistsAccumulator(p, forceLazy).apply(false, this)

    /** Returns true if all parts of this type satisfy predicate `p`.
     */
    final def forallParts(p: Type => Boolean)(using Ctx, CState): Boolean =
      !existsPart(!p(_))

    /** Performs operation on all parts of this type */
    final def foreachPart(p: Type => Unit, stopAtStatic: Boolean = false)(using Ctx, CState): Unit =
      new ForeachAccumulator(p, stopAtStatic).apply((), this)

    /** The parts of this type which are type or term refs */
    final def namedParts(using Ctx, CState): collection.Set[NamedType] =
    ...
    def namedPartsWith(p: NamedType => Boolean, excludeLowerBounds: Boolean = false)
      (using Ctx, CState): collection.Set[NamedType] =
      new NamedPartsAccumulator(p, excludeLowerBounds).apply(mutable.LinkedHashSet(), this)

    /** Map function `f` over elements of an AndType, rebuilding with function `g` */
    def mapReduceAnd[T](f: Type => T)(g: (T, T) => T)(using Ctx, CState): T = stripTypeVar match {
    ...
    final def mapReduceOr[T](f: Type => T)(g: (T, T) => T)(using Ctx, CState): T = stripTypeVar match {
...
// ----- Associated symbols ----------------------------------------------
    ...
    @tailrec final def typeSymbol(using Ctx, CState): Symbol = this match {
     ...
     */
    final def classSymbol(using Ctx, CState): Symbol = this match {
     ...
     */
    final def parentSymbols(include: Symbol => Boolean)(using Ctx, CState): List[Symbol] = this match {
      case tp: ClassInfo =>
        tp.cls :: Nil
      case tp: TypeRef =>
        val sym = tp.symbol
     ...
     */
    final def classSymbols(using Ctx, CState): List[ClassSymbol] =
      parentSymbols(_.isClass).asInstanceOf

    /** The term symbol associated with the type */
    @tailrec final def termSymbol(using Ctx, CState): Symbol = this match {
     ...
     */
    def baseClasses(using Ctx, CState): List[ClassSymbol] = {
      record("baseClasses")
      this match {
        case tp: TypeProxy =>
...
// ----- Member access -------------------------------------------------
     ...
     */
    @tailrec final def decls(using Ctx, CState): Scope = this match {
      case tp: ClassInfo =>
        tp.decls
      case tp: TypeProxy =>
        tp.underlying.decls
     ...
     */
    final def decl(name: Name)(using Ctx, CState): Denotation = {
      record("decl")
      findDecl(name, EmptyFlags)
    }

    /** A denotation containing the non-private declaration(s) in this type with the given name */
    final def nonPrivateDecl(name: Name)(using Ctx, CState): Denotation =
     ...
     */
    @tailrec final def findDecl(name: Name, excluded: FlagSet)(using Ctx, CState): Denotation = this match {
      case tp: ClassInfo =>
    ...
    final def member(name: Name)(using Ctx, CState): Denotation = {
      record("member")
      memberBasedOnFlags(name, required = EmptyFlags, excluded = EmptyFlags)
    }

    /** The non-private member of this type with the given name. */
    final def nonPrivateMember(name: Name)(using Ctx, CState): Denotation = {
    ...
    final def memberBasedOnFlags(name: Name, required: FlagSet = EmptyFlags, excluded: FlagSet = EmptyFlags)(using Ctx, CState): Denotation = {
     ...
     */
    final def findMember(name: Name, pre: Type, required: FlagSet = EmptyFlags, excluded: FlagSet = EmptyFlags)(using Ctx, CState): Denotation = {
      ...
      try go(this)
        ...
        case ex: Throwable =>
          ...
          def showPrefixSafely(pre: Type)(using Ctx, CState): String = pre.stripTypeVar match {
     ...
     */
    final def memberNames(keepOnly: NameFilter, pre: Type = this)(using Ctx, CState): Set[Name] = this match {
      case tp: ClassInfo =>
    ...
    def memberDenots(keepOnly: NameFilter, f: (Name, mutable.Buffer[SingleDenotation]) => Unit)(using Ctx, CState): Seq[SingleDenotation] = {
    ...
    final def abstractTermMembers(using Ctx, CState): Seq[SingleDenotation] = {
     ...
     *
     */
    final def possibleSamMethods(using Ctx, CState): Seq[SingleDenotation] = {
    ...
    final def abstractTypeMembers(using Ctx, CState): Seq[SingleDenotation] = {
    ...
    /** The set of abstract type members of this type. */
    final def nonClassTypeMembers(using Ctx, CState): Seq[SingleDenotation] = {
    ...
    final def typeAliasMembers(using Ctx, CState): Seq[SingleDenotation] = {
    ...
    final def typeMembers(using Ctx, CState): Seq[SingleDenotation] = {
      record("typeMembers")
     ...
     *                be returned
     */
    final def implicitMembers(using Ctx, CState): List[TermRef] = {
    ...
    final def memberClasses(using Ctx, CState): Seq[SingleDenotation] = {
      record("memberClasses")
      memberDenots(typeNameFilter,
        (name, buf) => buf ++= member(name).altsWith(x => x.isClass))
    }

    final def fields(using Ctx, CState): Seq[SingleDenotation] = {
      record("fields")
    ...
    final def membersBasedOnFlags(required: FlagSet, excluded: FlagSet)(using Ctx, CState): Seq[SingleDenotation] = {
    ...
    final def allMembers(using Ctx, CState): Seq[SingleDenotation] = {
      record("allMembers")
      memberDenots(takeAllFilter, (name, buf) => buf ++= member(name).alternatives)
    }

    /** The info of `sym`, seen as a member of this type. */
    final def memberInfo(sym: Symbol)(using Ctx, CState): Type =
      sym.info.asSeenFrom(this, sym.owner)

    /** This type seen as if it were the type of a member of prefix type `pre`
     *  declared in class `cls`.
     */
    final def asSeenFrom(pre: Type, cls: Symbol)(using Ctx, CState): Type = {
      record("asSeenFrom")
...
// ----- Subtype-related --------------------------------------------
    ...
    final def <:<(that: Type)(using Ctx, CState): Boolean = {
      record("<:<")
      ctx.typeComparer.topLevelSubType(this, that)
    }

    /** Is this type a subtype of that type? */
    final def frozen_<:<(that: Type)(using Ctx, CState): Boolean = {
      record("frozen_<:<")
    ...
    def isValueSubType(that: Type)(using Ctx, CState): Boolean = widen match {
    ...
    def relaxed_<:<(that: Type)(using Ctx, CState): Boolean =
     ...
     */
    final def overrides(that: Type, matchLoosely: => Boolean, checkClassInfo: Boolean = true)(using Ctx, CState): Boolean = {
     ...
     *      parameter types.
     */
    def matches(that: Type)(using Ctx, CState): Boolean = {
      record("matches")
     ...
     *  vice versa.
     */
    def matchesLoosely(that: Type)(using Ctx, CState): Boolean =
      (this matches that) || {
    ...
    final def baseType(base: Symbol)(using Ctx, CState): Type = {
      record("baseType")
      base.denot match {
    ...
    def & (that: Type)(using Ctx, CState): Type = {
      record("&")
     ...
     */
    def safe_& (that: Type)(using Ctx, CState): Type = (this, that) match {
     ...
     */
    def recoverable_&(that: Type)(using Ctx, CState): Type =
      try this & that
      catch {
...
// ----- Unwrapping types -----------------------------------------------
     ...
     */
    def stripTypeVar(using Ctx, CState): Type = this

    /** Remove all AnnotatedTypes wrapping this type.
      */
    def stripAnnots(using Ctx, CState): Type = this

    def rewrapAnnots(tp: Type)(using Ctx, CState): Type = tp.stripTypeVar match {
      case AnnotatedType(tp1, annot) => AnnotatedType(rewrapAnnots(tp1), annot)
      case _ => this
    }

    /** Strip PolyType prefixes */
    def stripPoly(using Ctx, CState): Type = this match {
      case tp: PolyType => tp.resType.stripPoly
      case _ => this
    }

    /** Strip LazyRef wrappers */
    def stripLazyRef(using Ctx, CState): Type = this match
     ...
     *  <o.x.type>.widen = o.C
     */
    final def widen(using Ctx, CState): Type = widenSingleton match {
     ...
     *  base type by applying one or more `underlying` dereferences.
     */
    final def widenSingleton(using Ctx, CState): Type = stripTypeVar.stripAnnots match {
     ...
     */
    final def widenTermRefExpr(using Ctx, CState): Type = stripTypeVar match {
    ...
    final def widenIfUnstable(using Ctx, CState): Type = stripTypeVar match {
    ...
    final def widenSkolem(using Ctx, CState): Type = this match {
     ...
     */
    def widenUnion(using Ctx, CState): Type = widen match {
    ...
    def widenUnionWithoutNull(using Ctx, CState): Type = widen match {
     ...
     */
    def widenSingletons(using Ctx, CState): Type = dealias match {
      case tp: SingletonType =>
        tp.widen
      case tp: OrType =>
     ...
     *  Overridden and cached in OrType.
     */
    def atoms(using Ctx, CState): Atoms = dealias match
      case tp: SingletonType =>
    ...
    private def dealias1(keep: AnnotatedType => (Ctx, CState) ?=> Boolean)(using Ctx, CState): Type = this match {
      case tp: TypeRef =>
     ...
     */
    final def dealias(using Ctx, CState): Type = dealias1(keepNever)

    /** Follow aliases and dereferences LazyRefs and instantiated TypeVars until type
     *  is no longer alias type, LazyRef, or instantiated type variable.
     *  Goes through annotated types and rewraps annotations on the result.
     */
    final def dealiasKeepAnnots(using Ctx, CState): Type = dealias1(keepAlways)

    /** Like `dealiasKeepAnnots`, but keeps only refining annotations */
    final def dealiasKeepRefiningAnnots(using Ctx, CState): Type = dealias1(keepIfRefining)

    /** The result of normalization using `tryNormalize`, or the type itself if
     *  tryNormlize yields NoType
     */
    final def normalized(using Ctx, CState): Type = {
     ...
     *  otherwise NoType
     */
    def tryNormalize(using Ctx, CState): Type = NoType

    private def widenDealias1(keep: AnnotatedType => (Ctx, CState) ?=> Boolean)(using Ctx, CState): Type = {
      val res = this.widen.dealias1(keep)
      if (res eq this) res else res.widenDealias1(keep)
    }

    /** Perform successive widenings and dealiasings until none can be applied anymore */
    final def widenDealias(using Ctx, CState): Type = widenDealias1(keepNever)

    /** Perform successive widenings and dealiasings while rewrapping annotations, until none can be applied anymore */
    final def widenDealiasKeepAnnots(using Ctx, CState): Type = widenDealias1(keepAlways)

    /** Perform successive widenings and dealiasings while rewrapping refining annotations, until none can be applied anymore */
    final def widenDealiasKeepRefiningAnnots(using Ctx, CState): Type = widenDealias1(keepIfRefining)

    /** Widen from constant type to its underlying non-constant
     *  base type.
     */
    final def deconst(using Ctx, CState): Type = stripTypeVar match {
      case tp: ConstantType => tp.value.tpe
      case _ => this
    }

    /** Dealias, and if result is a dependent function type, drop the `apply` refinement. */
    final def dropDependentRefinement(using Ctx, CState): Type = dealias match {
      case RefinedType(parent, nme.apply, _) => parent
      case tp => tp
    }

    /** The type constructor of an applied type, otherwise the type itself */
    final def typeConstructor(using Ctx, CState): Type = this match {
     ...
     */
    def underlyingClassRef(refinementOK: Boolean)(using Ctx, CState): Type = dealias match {
      case tp: TypeRef =>
        if (tp.symbol.isClass) tp
     ...
     */
    def underlyingIterator(using Ctx, CState): Iterator[Type] = new Iterator[Type] {
     ...
     */
    def lookupRefined(name: Name)(using Ctx, CState): Type = {
    ...
    def select(name: Name)(using Ctx, CState): Type =
      NamedType(this, name, member(name)).reduceProjection

    /** The type <this . name> with given denotation, reduced if possible. */
    def select(name: Name, denot: Denotation)(using Ctx, CState): Type =
      NamedType(this, name, denot).reduceProjection

    /** The type <this . sym>, reduced if possible */
    def select(sym: Symbol)(using Ctx, CState): Type =
      NamedType(this, sym).reduceProjection

    def select(name: TermName)(using Ctx, CState): TermRef =
      TermRef(this, name, member(name))

    def select(name: TermName, sig: Signature)(using Ctx, CState): TermRef =
...
// ----- Access to parts --------------------------------------------
     ...
     */
    @tailrec final def normalizedPrefix(using Ctx, CState): Type = this match {
      case tp: NamedType =>
    ...
    def parents(using Ctx, CState): List[Type] = this match {
    ...
    def firstParent(using Ctx, CState): Type = parents match {
      case p :: _ => p
      case _ => defn.AnyType
    }

    /** The parameter types of a PolyType or MethodType, Empty list for others */
    final def paramInfoss(using Ctx, CState): List[List[Type]] = stripPoly match {
    ...
    final def paramNamess(using Ctx, CState): List[List[TermName]] = stripPoly match {
    ...
    final def firstParamTypes(using Ctx, CState): List[Type] = stripPoly match {
      case mt: MethodType => mt.paramInfos
      case _ => Nil
    }

    /** Is this either not a method at all, or a parameterless method? */
    final def isParameterless(using Ctx, CState): Boolean = stripPoly match {
      case mt: MethodType => false
      case _ => true
    }

    /** Is this (an alias of) the `scala.Null` type? */
    final def isNullType(using Ctx, CState) = isRef(defn.NullClass)

    /** The resultType of a LambdaType, or ExprType, the type itself for others */
    def resultType(using Ctx, CState): Type = this
     ...
     */
    def finalResultType(using Ctx, CState): Type = resultType.stripPoly match {
      case mt: MethodType => mt.resultType.finalResultType
      case _ => resultType
    }

    /** This type seen as a TypeBounds */
    final def bounds(using Ctx, CState): TypeBounds = this match {
     ...
     */
    def typeParamNamed(name: TypeName)(using Ctx, CState): Symbol =
     ...
     */
    def deepenProto(using Ctx, CState): Type = this
...
// ----- Substitutions -----------------------------------------------------
     ...
     */
    final def subst(from: List[Symbol], to: List[Type])(using Ctx, CState): Type =
     ...
     */
    final def substSym(from: List[Symbol], to: List[Symbol])(using Ctx, CState): Type =
     ...
     */
    final def substApprox(from: List[Symbol], to: List[Type])(using Ctx, CState): Type =
...
// ----- misc -----------------------------------------------------------
     ...
     */
    def toFunctionType(dropLast: Int = 0)(using Ctx, CState): Type = this match {
     ...
     */
    def signature(using Ctx, CState): Signature = Signature.NotAMethod

    /** Drop annotation of given `cls` from this type */
    def dropAnnot(cls: Symbol)(using Ctx, CState): Type = stripTypeVar match {
    ...
    def dropRepeatedAnnot(using Ctx, CState): Type = dropAnnot(defn.RepeatedAnnot)

    def annotatedToRepeated(using Ctx, CState): Type = this match {
    ...
    def coveringSet(using Ctx, CState): Set[Symbol] =
    ...
    def typeSize(using Ctx, CState): Int =
     ...
     */
    def showWithUnderlying(n: Int = 1)(using Ctx, CState): String = this match {
     ...
     */
    def simplified(using Ctx, CState): Type = TypeOps.simplify(this, null)
...
// ----- Type categories ----------------------------------------------
  ...
  abstract class TypeProxy extends Type {
    ...
    def underlying(using Ctx, CState): Type
     ...
     */
    def superType(using Ctx, CState): Type = underlying match {
      case TypeBounds(_, hi) => hi
      case st => st
    }

    /** Same as superType, except that opaque types are treated as transparent aliases */
    def translucentSuperType(using Ctx, CState): Type = superType
  }
  ...
  trait SingletonType extends TypeProxy with ValueType {
    def isOverloaded(using Ctx, CState): Boolean = false
  }
  ...
  trait ProtoType extends Type {
    def isMatchedBy(tp: Type, keepConstraint: Boolean = false)(using Ctx, CState): Boolean
    def fold[T](x: T, ta: TypeAccumulator[T])(using Ctx, CState): T
    def map(tm: TypeMap)(using Ctx, CState): ProtoType

    /** If this prototype captures a context, the same prototype except that the result
     *  captures the given context `ctx`.
     */
    def withContext(newCtx: Ctx, newCS: CState): ProtoType = this
  ...
  trait NarrowCached extends Type {
    ...
    override def narrow(using Ctx, CState): TermRef = {
...
// --- NamedTypes ------------------------------------------------------------------
  ...
  abstract class NamedType extends CachedProxyType with ValueType with SignatureCachingType { self =>
     ...
     */
    final def name(using Ctx, CState): ThisName = {
     ...
     */
    protected[dotc] def computeSignature(using Ctx, CState): Signature =
     ...
     */
    private def currentSignature(using Ctx, CState): Signature =
    ...
    private def sigFromDenot(lastd: Denotation)(using Ctx, CState) =
    ...
    final def symbol(using Ctx, CState): Symbol =
    ...
    private def computeSymbol(using Ctx, CState): Symbol =
      designator match {
        case sym: Symbol =>
     ...
     *  current run.
     */
    def denotationIsCurrent(using Ctx, CState): Boolean =
     ...
     */
    final def currentSymbol(using Ctx, CState): Symbol = designator match {
      case sym: Symbol => sym
     ...
     */
    private[Types] def stableInRunSymbol(using Ctx, CState): Symbol =
      if (checkedPeriod.runId == currentRunId) lastSymbol
      else symbol

    def info(using Ctx, CState): Type = denot.info

    /** The denotation currently denoted by this type */
    final def denot(using Ctx, CState): Denotation = {
    ...
    private def computeDenot(using Ctx, CState): Denotation = {
    ...
    private def disambiguate(d: Denotation)(using Ctx, CState): Denotation =
      disambiguate(d, currentSignature)

    private def disambiguate(d: Denotation, sig: Signature)(using Ctx, CState): Denotation =
      if (sig != null)
    ...
    private def memberDenot(name: Name, allowPrivate: Boolean)(using Ctx, CState): Denotation = {
    ...
    private def memberDenot(prefix: Type, name: Name, allowPrivate: Boolean)(using Ctx, CState): Denotation =
      if (allowPrivate) prefix.member(name) else prefix.nonPrivateMember(name)

    private def argDenot(param: TypeSymbol)(using Ctx, CState): Denotation = {
      val cls = param.owner
     ...
     */
    def recomputeDenot()(using Ctx, CState): Unit =
      setDenot(memberDenot(name, allowPrivate = !symbol.exists || symbol.is(Private)))

    private def setDenot(denot: Denotation)(using Ctx, CState): Unit = {
    ...
    private def checkDenot()(using Ctx, CState) = {}

    private def checkSymAssign(sym: Symbol)(using Ctx, CState) = {
     ...
     */
    private def infoDependsOnPrefix(symd: SymDenotation, prefix: Type)(using Ctx, CState): Boolean =
      symd.maybeOwner.membersNeedAsSeenFrom(prefix) && !symd.is(NonMember)

    /** Is this a reference to a class or object member? */
    def isMemberRef(using Ctx, CState): Boolean = designator match {
     ...
     */
    def reduceProjection(using Ctx, CState): Type =
      if (isType) {
     ...
     */
    final def controlled[T](op: => T)(using Ctx, CState): T = try {
     ...
     */
    def argForParam(pre: Type)(using Ctx, CState): Type = {
      val tparam = symbol
     ...
     *     (S | T)#A --> S#A | T#A
     */
    def derivedSelect(prefix: Type)(using Ctx, CState): Type =
    ...
    final def withSym(sym: Symbol)(using Ctx, CState): ThisType =
     ...
     */
    final def withDenot(denot: Denotation)(using Ctx, CState): ThisType =
      if (denot.exists) {
    ...
    final def withPrefix(prefix: Type)(using Ctx, CState): NamedType = {
   ...
   */
  trait ImplicitRef {
    def implicitName(using Ctx, CState): TermName
  ...
  abstract case class TermRef(override val prefix: Type,
    ...
    override def underlying(using Ctx, CState): Type = {
      val d = denot
      if (d.isOverloaded) NoType else d.info
    }

    override def isOverloaded(using Ctx, CState): Boolean = denot.isOverloaded

    def alternatives(using Ctx, CState): List[TermRef] =
      denot.alternatives.map(withDenot(_))

    def altsWith(p: Symbol => Boolean)(using Ctx, CState): List[TermRef] =
      denot.altsWith(p).map(withDenot(_))

    def implicitName(using Ctx, CState): TermName = name
  ...
  abstract case class TypeRef(override val prefix: Type,
     ...
     *  based tests.
     */
    def canDropAlias(using Ctx, CState) =
    ...
    override def underlying(using Ctx, CState): Type = info

    override def translucentSuperType(using Ctx, CState) = info match {
    ...
    def validated(using Ctx, CState): this.type =
      this
  }
  ...
  private def assertUnerased()(using Ctx, CState) =
   ...
   */
  private def designatorFor(prefix: Type, name: Name, denot: Denotation)(using Ctx, CState): Designator = {
  ...
  object NamedType {
    def isType(desig: Designator)(using Ctx, CState): Boolean = desig match {
      case sym: Symbol => sym.isType
      case name: Name => name.isTypeName
    }
    def apply(prefix: Type, designator: Designator)(using Ctx, CState): NamedType =
      if (isType(designator)) TypeRef.apply(prefix, designator)
      else TermRef.apply(prefix, designator)
    def apply(prefix: Type, designator: Name, denot: Denotation)(using Ctx, CState): NamedType =
  ...
  object TermRef {
    ...
    def apply(prefix: Type, desig: Designator)(using Ctx, CState): TermRef =
     ...
     */
    def apply(prefix: Type, name: TermName, denot: Denotation)(using Ctx, CState): TermRef =
  ...
  object TypeRef {
    ...
    def apply(prefix: Type, desig: Designator)(using Ctx, CState): TypeRef =
     ...
     *  from the denotation's symbol if the latter exists, or else it is the given name.
     */
    def apply(prefix: Type, name: TypeName, denot: Denotation)(using Ctx, CState): TypeRef =
      apply(prefix, designatorFor(prefix, name, denot)).withDenot(denot)
  }
  ...
  abstract case class ThisType(tref: TypeRef) extends CachedProxyType with SingletonType {
    def cls(using Ctx, CState): ClassSymbol = tref.stableInRunSymbol match {
  ...
  object ThisType {
    ...
    def raw(tref: TypeRef)(using Ctx, CState): CachedThisType =
  ...
  abstract case class SuperType(thistpe: Type, supertpe: Type) extends CachedProxyType with SingletonType {
    override def underlying(using Ctx, CState): Type = supertpe
    override def superType(using Ctx, CState): Type =
      thistpe.baseType(supertpe.typeSymbol)
    def derivedSuperType(thistpe: Type, supertpe: Type)(using Ctx, CState): Type =
  ...
  object SuperType {
    def apply(thistpe: Type, supertpe: Type)(using Ctx, CState): SuperType = {
  ...
  abstract case class ConstantType(value: Constant) extends CachedProxyType with SingletonType {
    override def underlying(using Ctx, CState): Type = value.tpe
  ...
  object ConstantType {
    def apply(value: Constant)(using Ctx, CState): ConstantType = {
      assertUnerased()
      unique(new CachedConstantType(value))
    }
  }

  case class LazyRef(private var refFn: (Ctx, CState) ?=> Type, reportCycles: Boolean = false) extends UncachedProxyType with ValueType {
    private var myRef: Type = null
    private var computed = false

    def ref(using Ctx, CState): Type =
      if computed then
        if myRef == null then
     ...
     */
    def update(tp: Type)(using Ctx, CState) =
    ...
    override def underlying(using Ctx, CState): Type = ref
  ...
  abstract case class RefinedType(parent: Type, refinedName: Name, refinedInfo: Type) extends RefinedOrRecType {
    ...
    override def underlying(using Ctx, CState): Type = parent

    private def badInst =
      throw new AssertionError(s"bad instantiation: $this")

    def checkInst(using Ctx, CState): this.type = this // debug hook

    def derivedRefinedType(parent: Type, refinedName: Name, refinedInfo: Type)(using Ctx, CState): Type =
    ...
    def wrapIfMember(parent: Type)(using Ctx, CState): Type =
  ...
  object RefinedType {
    @tailrec def make(parent: Type, names: List[Name], infos: List[Type])(using Ctx, CState): Type =
      if (names.isEmpty) parent
      else make(RefinedType(parent, names.head, infos.head), names.tail, infos.tail)

    def apply(parent: Type, name: Name, info: Type)(using Ctx, CState): RefinedType = {
  ...
  class RecType(parentExp: RecType => Type) extends RefinedOrRecType with BindingType {
    ...
    def derivedRecType(parent: Type)(using Ctx, CState): RecType =
      if (parent eq this.parent) this
      else RecType(rt => parent.substRecThis(this, rt.recThis))

    def rebind(parent: Type)(using Ctx, CState): Type =
      if (parent eq this.parent) this
      else RecType.closeOver(rt => parent.substRecThis(this, rt.recThis))

    def isReferredToBy(tp: Type)(using Ctx, CState): Boolean = {
    ...
    private def checkInst(using Ctx, CState): this.type = this // debug hook
  }
  ...
  object RecType {
     ...
     *         of length > 1
     */
    def apply(parentExp: RecType => Type)(using Ctx, CState): RecType = {
    ...
    def closeOver(parentExp: RecType => Type)(using Ctx, CState): Type = {
  ...
  abstract class AndOrType extends CachedGroundType with ValueType {
    ...
    def derivedAndOrType(tp1: Type, tp2: Type)(using Ctx, CState) =
  ...
  abstract case class AndType(tp1: Type, tp2: Type) extends AndOrType {
    ...
    override final def baseClasses(using Ctx, CState): List[ClassSymbol] = {
    ...
    def derivedAndType(tp1: Type, tp2: Type)(using Ctx, CState): Type =
      if ((tp1 eq this.tp1) && (tp2 eq this.tp2)) this
      else AndType.make(tp1, tp2, checkValid = true)

    def derived_& (tp1: Type, tp2: Type)(using Ctx, CState): Type =
      if ((tp1 eq this.tp1) && (tp2 eq this.tp2)) this
      else tp1 & tp2
  ...
  object AndType {
    def apply(tp1: Type, tp2: Type)(using Ctx, CState): AndType = {
      assert(tp1.isValueTypeOrWildcard &&
             tp2.isValueTypeOrWildcard, i"$tp1 & $tp2 / " + s"$tp1 & $tp2")
      unchecked(tp1, tp2)
    }

    def unchecked(tp1: Type, tp2: Type)(using Ctx, CState): AndType = {
      assertUnerased()
     ...
     *  going through `&`).
     */
    def make(tp1: Type, tp2: Type, checkValid: Boolean = false)(using Ctx, CState): Type =
  ...
  abstract case class OrType(tp1: Type, tp2: Type) extends AndOrType {
    ...
    def join(using Ctx, CState): Type = {
    ...
    private def ensureAtomsComputed()(using Ctx, CState): Unit =
    ...
    override def atoms(using Ctx, CState): Atoms =
      ensureAtomsComputed()
      myAtoms

    override def widenSingletons(using Ctx, CState): Type = {
      ensureAtomsComputed()
      myWidened
    }

    def derivedOrType(tp1: Type, tp2: Type)(using Ctx, CState): Type =
      if ((tp1 eq this.tp1) && (tp2 eq this.tp2)) this
      else OrType.make(tp1, tp2)
  ...
  object OrType {
    def apply(tp1: Type, tp2: Type)(using Ctx, CState): OrType = {
      assertUnerased()
      unique(new CachedOrType(tp1, tp2))
    }
    def make(tp1: Type, tp2: Type)(using Ctx, CState): Type =
      if (tp1 eq tp2) tp1
      else apply(tp1, tp2)
  }
   ...
   */
  object OrNull {
    def apply(tp: Type)(using Ctx, CState) =
      OrType(tp, defn.NullType)
    def unapply(tp: Type)(using Ctx, CState): Option[Type] =
  ...
  trait SignatureCachingType extends TermType {
    ...
    final override def signature(using Ctx, CState): Signature = {
  ...
  trait MethodicType extends TermType {
    protected def resultSignature(using Ctx, CState): Signature = try resultType match {
  ...
  extends CachedProxyType with TermType with MethodicType {
    ...
    override def underlying(using Ctx, CState): Type = resType

    override def signature(using Ctx, CState): Signature = Signature.NotAMethod

    def derivedExprType(resType: Type)(using Ctx, CState): ExprType =
  ...
  object ExprType {
    def apply(resultType: Type)(using Ctx, CState): ExprType = {
      assertUnerased()
  ...
  trait LambdaType extends BindingType with TermType { self =>
    ...
    def isResultDependent(using Ctx, CState): Boolean
    def isParamDependent(using Ctx, CState): Boolean
    ...
    final def instantiateParamInfos(argTypes: => List[Type])(using Ctx, CState): List[Type] =
    ...
    final def instantiate(argTypes: => List[Type])(using Ctx, CState): Type =
     ...
     */
    def integrate(tparams: List[ParamInfo], tp: Type)(using Ctx, CState): Type =
    ...
    final def derivedLambdaType(paramNames: List[ThisName] = this.paramNames,
                          ...
                          resType: Type = this.resType)(using Ctx, CState): LambdaType =
      if ((paramNames eq this.paramNames) && (paramInfos eq this.paramInfos) && (resType eq this.resType)) this
      else newLikeThis(paramNames, paramInfos, resType)

    def newLikeThis(paramNames: List[ThisName], paramInfos: List[PInfo], resType: Type)(using Ctx, CState): This =
  ...
  abstract class HKLambda extends CachedProxyType with LambdaType {
    final override def underlying(using Ctx, CState): Type = resType
  ...
  trait TermLambda extends LambdaType { thisLambdaType =>
    ...
    override def resultType(using Ctx, CState): Type =
    ...
    private def depStatus(initial: DependencyStatus, tp: Type)(using Ctx, CState): DependencyStatus = {
     ...
     */
    private def dependencyStatus(using Ctx, CState): DependencyStatus =
     ...
     */
    private def paramDependencyStatus(using Ctx, CState): DependencyStatus =
     ...
     */
    def isResultDependent(using Ctx, CState): Boolean = dependencyStatus == TrueDeps
     ...
     */
    def isParamDependent(using Ctx, CState): Boolean = paramDependencyStatus == TrueDeps
    ...
    def nonDependentResultApprox(using Ctx, CState): Type =
  ...
  abstract class LambdaTypeCompanion[N <: Name, PInfo <: Type, LT <: LambdaType] {
    ...
    def apply(paramNames: List[N])(paramInfosExp: LT => List[PInfo], resultTypeExp: LT => Type)(using Ctx, CState): LT
    def apply(paramNames: List[N], paramInfos: List[PInfo], resultType: Type)(using Ctx, CState): LT =
      apply(paramNames)(_ => paramInfos, _ => resultType)
    def apply(paramInfos: List[PInfo])(resultTypeExp: LT => Type)(using Ctx, CState): LT =
      apply(syntheticParamNames(paramInfos.length))(_ => paramInfos, resultTypeExp)
    def apply(paramInfos: List[PInfo], resultType: Type)(using Ctx, CState): LT =
      apply(syntheticParamNames(paramInfos.length), paramInfos, resultType)

    protected def toPInfo(tp: Type)(using Ctx, CState): PInfo

    def fromParams[PI <: ParamInfo.Of[N]](params: List[PI], resultType: Type)(using Ctx, CState): Type =
  ...
  extends LambdaTypeCompanion[TermName, Type, LT] {
    def toPInfo(tp: Type)(using Ctx, CState): Type = tp
  ...
  extends LambdaTypeCompanion[TypeName, TypeBounds, LT] {
    def toPInfo(tp: Type)(using Ctx, CState): TypeBounds = (tp: @unchecked) match {
      case tp: TypeBounds => tp
  ...
  abstract class MethodTypeCompanion(val prefixString: String) extends TermLambdaCompanion[MethodType] { self =>
     ...
     */
    def fromSymbols(params: List[Symbol], resultType: Type)(using Ctx, CState): MethodType = {
    ...
    final def apply(paramNames: List[TermName])(paramInfosExp: MethodType => List[Type], resultTypeExp: MethodType => Type)(using Ctx, CState): MethodType =
      checkValid(unique(new CachedMethodType(paramNames)(paramInfosExp, resultTypeExp, self)))

    def checkValid(mt: MethodType)(using Ctx, CState): mt.type = {
  ...
  object MethodTpe {
    def unapply(mt: MethodType)(using Ctx, CState): Some[(List[TermName], List[Type], Type)] =
  ...
  trait TypeLambda extends LambdaType {
    ...
    def isResultDependent(using Ctx, CState): Boolean = true
    def isParamDependent(using Ctx, CState): Boolean = true
    ...
    def derivedLambdaAbstraction(paramNames: List[TypeName], paramInfos: List[TypeBounds], resType: Type)(using Ctx, CState): Type =
      resType match {
  ...
  extends HKLambda with TypeLambda {
    ...
    override def newLikeThis(paramNames: List[ThisName], paramInfos: List[PInfo], resType: Type)(using Ctx, CState): This =
      newLikeThis(paramNames, declaredVariances, paramInfos, resType)

    def newLikeThis(paramNames: List[ThisName], variances: List[Variance], paramInfos: List[PInfo], resType: Type)(using Ctx, CState): This =
      HKTypeLambda(paramNames, variances)(
          x => paramInfos.mapConserve(_.subst(this, x).asInstanceOf[PInfo]),
          x => resType.subst(this, x))

    def withVariances(variances: List[Variance])(using Ctx, CState): This =
  ...
  extends MethodOrPoly with TypeLambda {
     ...
     */
    def flatten(using Ctx, CState): PolyType = resType match {
      case that: PolyType =>
  ...
  object HKTypeLambda extends TypeLambdaCompanion[HKTypeLambda] {
    def apply(paramNames: List[TypeName])(
        ...
        resultTypeExp: HKTypeLambda => Type)(using Ctx, CState): HKTypeLambda =
    ...
    def any(n: Int)(using Ctx, CState): HKTypeLambda =
      apply(syntheticParamNames(n))(
        pt => List.fill(n)(TypeBounds.empty), pt => defn.AnyType)

    override def fromParams[PI <: ParamInfo.Of[TypeName]](params: List[PI], resultType: Type)(using Ctx, CState): Type =
      resultType match
     ...
     */
    def boundsFromParams[PI <: ParamInfo.Of[TypeName]](params: List[PI], bounds: TypeBounds)(using Ctx, CState): TypeBounds = {
  ...
  object PolyType extends TypeLambdaCompanion[PolyType] {
    def apply(paramNames: List[TypeName])(
        ...
        resultTypeExp: PolyType => Type)(using Ctx, CState): PolyType =
      unique(new PolyType(paramNames)(paramInfosExp, resultTypeExp))

    def unapply(tl: PolyType): Some[(List[LambdaParam], Type)] =
      Some((tl.typeParams, tl.resType))

    def any(n: Int)(using Ctx, CState): PolyType =
      apply(syntheticParamNames(n))(
        pt => List.fill(n)(TypeBounds.empty), pt => defn.AnyType)
  }
  ...
  case class LambdaParam(tl: TypeLambda, n: Int) extends ParamInfo {
    ...
    def isTypeParam(using Ctx, CState): Boolean = tl.paramNames.head.isTypeName
    def paramName(using Ctx, CState): tl.ThisName = tl.paramNames(n)
    def paramInfo(using Ctx, CState): tl.PInfo = tl.paramInfos(n)
    def paramInfoAsSeenFrom(pre: Type)(using Ctx, CState): tl.PInfo = paramInfo
    def paramInfoOrCompleter(using Ctx, CState): Type = paramInfo
    def paramRef(using Ctx, CState): Type = tl.paramRefs(n)
  ...
  extends CachedProxyType with ValueType {
    ...
    def isGround(acc: TypeAccumulator[Boolean])(using Ctx, CState): Boolean = {
      if (!isGroundKnown) {
    ...
    override def underlying(using Ctx, CState): Type = tycon
    ...
    override def translucentSuperType(using Ctx, CState): Type = tycon match {
    ...
    override def tryNormalize(using Ctx, CState): Type = tycon match {
      case tycon: TypeRef =>
    ...
    def tryCompiletimeConstantFold(using Ctx, CState): Type = tycon match {
    ...
    def lowerBound(using Ctx, CState): Type = tycon.stripTypeVar match {
      case tycon: TypeRef =>
        tycon.info match {
          case TypeBounds(lo, hi) =>
    ...
    def tyconTypeParams(using Ctx, CState): List[ParamInfo] = {
      val tparams = tycon.typeParams
      if (tparams.isEmpty) HKTypeLambda.any(args.length).typeParams else tparams
    }

    def hasWildcardArg(using Ctx, CState): Boolean = args.exists(isBounds)

    def derivedAppliedType(tycon: Type, args: List[Type])(using Ctx, CState): Type =
  ...
  object AppliedType {
    def apply(tycon: Type, args: List[Type])(using Ctx, CState): AppliedType = {
      assertUnerased()
  ...
  abstract case class TypeParamRef(binder: TypeLambda, paramNum: Int) extends ParamRef {
     ...
     */
    override def occursIn(that: Type)(using Ctx, CState): Boolean = !that.isGround && super.occursIn(that)

    /** Looking only at the structure of `bound`, is one of the following true?
     *     - fromBelow and param <:< bound
     *     - !fromBelow and param >:> bound
     */
    def occursIn(bound: Type, fromBelow: Boolean)(using Ctx, CState): Boolean = bound.stripTypeVar match {
  ...
  abstract case class RecThis(binder: RecType) extends BoundType with SingletonType {
    type BT = RecType
    override def underlying(using Ctx, CState): RecType = binder
  ...
  case class SkolemType(info: Type) extends UncachedProxyType with ValueType with SingletonType {
    override def underlying(using Ctx, CState): Type = info
    def derivedSkolemType(info: Type)(using Ctx, CState): SkolemType =
    ...
    def repr(using Ctx, CState): Name = {
  ...
  class QualSkolemType(info: Type) extends SkolemType(info) {
    override def derivedSkolemType(info: Type)(using Ctx, CState): SkolemType =
  ...
  final class TypeVar private(initOrigin: TypeParamRef, creatorState: TyperState, nestingLevel: Int) extends CachedProxyType with ValueType {
    ...
    def instantiateWith(tp: Type)(using Ctx, CState): Type = {
  ...
  object TypeVar:
    def apply(initOrigin: TypeParamRef, creatorState: TyperState)(using Ctx, CState) =
  ...
  abstract case class MatchType(bound: Type, scrutinee: Type, cases: List[Type]) extends CachedProxyType with ValueType {
    def derivedMatchType(bound: Type, scrutinee: Type, cases: List[Type])(using Ctx, CState): MatchType =
      if (bound.eq(this.bound) && scrutinee.eq(this.scrutinee) && cases.eqElements(this.cases)) this
      else MatchType(bound, scrutinee, cases)

    def caseType(tp: Type)(using Ctx, CState): Type = tp match {
      case tp: HKTypeLambda => caseType(tp.resType)
      case defn.MatchCase(_, body) => body
    }

    def alternatives(using Ctx, CState): List[Type] = cases.map(caseType)
    def underlying(using Ctx, CState): Type = bound

    private var myReduced: Type = null
    private var reductionContext: mutable.Map[Type, Type] = null

    override def tryNormalize(using Ctx, CState): Type = reduced.normalized

    def reduced(using Ctx, CState): Type = {
  ...
  object MatchType {
    def apply(bound: Type, scrutinee: Type, cases: List[Type])(using Ctx, CState): MatchType =
  ...
  abstract case class ClassInfo(
     ...
     */
    def selfType(using Ctx, CState): Type = {
    ...
    override def parents(using Ctx, CState): List[Type] = {
      if (parentsCache == null)
        parentsCache = classParents.mapConserve(_.asSeenFrom(prefix, cls.owner))
      parentsCache
    }

    protected def newLikeThis(prefix: Type, classParents: List[Type], decls: Scope, selfInfo: TypeOrSymbol)(using Ctx, CState): ClassInfo =
      ClassInfo(prefix, cls, classParents, decls, selfInfo)

    def derivedClassInfo(prefix: Type)(using Ctx, CState): ClassInfo =
      if (prefix eq this.prefix) this
      else newLikeThis(prefix, classParents, decls, selfInfo)

    def derivedClassInfo(prefix: Type = this.prefix, classParents: List[Type] = this.classParents, decls: Scope = this.decls, selfInfo: TypeOrSymbol = this.selfInfo)(using Ctx, CState): ClassInfo =
     ...
     */
    def integrateOpaqueMembers(using Ctx, CState): ClassInfo =
      decls.toList.foldLeft(this) { (cinfo, sym) =>
        if sym.isOpaqueAlias then
          cls.setFlag(Opaque)
          def force(using Ctx, CState) =
  ...
  extends CachedClassInfo(prefix, cls, Nil, decls, selfInfo) {
    ...
    def finalized(parents: List[Type])(using Ctx, CState): ClassInfo =
      ClassInfo(prefix, cls, parents, decls, selfInfo)

    override def newLikeThis(prefix: Type, classParents: List[Type], decls: Scope, selfInfo: TypeOrSymbol)(using Ctx, CState): ClassInfo =
  ...
  object ClassInfo {
    def apply(prefix: Type, cls: ClassSymbol, classParents: List[Type], decls: Scope, selfInfo: TypeOrSymbol = NoType)(using Ctx, CState): ClassInfo =
  ...
  abstract case class TypeBounds(lo: Type, hi: Type) extends CachedProxyType with TypeType {
    ...
    override def underlying(using Ctx, CState): Type = hi

    /** The non-alias type bounds type with given bounds */
    def derivedTypeBounds(lo: Type, hi: Type)(using Ctx, CState): TypeBounds =
      if ((lo eq this.lo) && (hi eq this.hi)) this
      else TypeBounds(lo, hi)

    def contains(tp: Type)(using Ctx, CState): Boolean = tp match {
    ...
    def & (that: TypeBounds)(using Ctx, CState): TypeBounds =
    ...
    override def & (that: Type)(using Ctx, CState): Type = that match {
  ...
  abstract class AliasingBounds(val alias: Type) extends TypeBounds(alias, alias) {
    ...
    def derivedAlias(alias: Type)(using Ctx, CState): AliasingBounds
  ...
  object TypeBounds {
    def apply(lo: Type, hi: Type)(using Ctx, CState): TypeBounds =
      unique(new RealTypeBounds(lo, hi))
    def empty(using Ctx, CState): TypeBounds = apply(defn.NothingType, defn.AnyType)
    def emptyPolyKind(using Ctx, CState): TypeBounds = apply(defn.NothingType, defn.AnyKindType)
    def upper(hi: Type)(using Ctx, CState): TypeBounds = apply(defn.NothingType, hi)
    def lower(lo: Type)(using Ctx, CState): TypeBounds = apply(lo, defn.AnyType)
  }

  object TypeAlias {
    def apply(alias: Type)(using Ctx, CState): TypeAlias = unique(new TypeAlias(alias))
    def unapply(tp: TypeAlias): Option[Type] = Some(tp.alias)
  }

  object MatchAlias {
    def apply(alias: Type)(using Ctx, CState): MatchAlias = unique(new MatchAlias(alias))
  ...
  case class AnnotatedType(parent: Type, annot: Annotation) extends UncachedProxyType with ValueType {
    ...
    override def stripAnnots(using Ctx, CState): Type = parent.stripAnnots

    private var isRefiningKnown = false
    private var isRefiningCache: Boolean = _

    def isRefining(using Ctx, CState): Boolean = {
      if (!isRefiningKnown) {
  ...
  abstract case class JavaArrayType(elemType: Type) extends CachedGroundType with ValueType {
    def derivedJavaArrayType(elemtp: Type)(using Ctx, CState): JavaArrayType =
  ...
  object JavaArrayType {
    def apply(elemType: Type)(using Ctx, CState): JavaArrayType = unique(new CachedJavaArrayType(elemType))
  }
  ...
  abstract class ErrorType extends FlexType {
    def msg(using Ctx, CState): Message
  }

  object ErrorType:
    def apply(m: Message)(using Ctx, CState): ErrorType =
  ...
  object UnspecifiedErrorType extends ErrorType {
    override def msg(using Ctx, CState): Message = "unspecified error"
  }
  ...
  abstract case class WildcardType(optBounds: Type) extends CachedGroundType with TermType {
    def derivedWildcardType(optBounds: Type)(using Ctx, CState): WildcardType =
  ...
  @sharable object WildcardType extends WildcardType(NoType) {
    def apply(bounds: TypeBounds)(using Ctx, CState): WildcardType = unique(new CachedWildcardType(bounds))
  }
   ...
   */
  object SAMType {
    def zeroParamClass(tp: Type)(using Ctx, CState): Type = tp match {
      case tp: ClassInfo =>
    ...
    def isInstantiatable(tp: Type)(using Ctx, CState): Boolean = zeroParamClass(tp) match {
    ...
    def unapply(tp: Type)(using Ctx, CState): Option[MethodType] =
  ...
  abstract class VariantTraversal:
     ...
     */
    protected def isStaticPrefix(pre: Type)(using Ctx, CState): Boolean = true

    protected def stopBecauseStaticOrLocal(tp: NamedType)(using Ctx, CState): Boolean =
      (tp.prefix eq NoPrefix)
      || stopAtStatic && tp.currentSymbol.isStatic && isStaticPrefix(tp.prefix)
  end VariantTraversal

  abstract class TypeMap(using protected var mapCtx: Ctx, protected var mapState: CState)
    ...
    def mapOver(tp: Type): Type = {
      ...
      tp match {
        ...
        case tp: LazyRef =>
          LazyRef {
            val ref1 = tp.ref
            if currentRunId == currentRunId(using mapCtx, mapState) then
              this(ref1)
            else // splice in run that comes from LazyRef into map context
              val savedCtx = mapCtx
              val savedState = mapState
              val newPeriod = Period(currentRunId, currentPhaseId(using mapCtx, mapState))
              mapCtx = mapCtx.fresh.setPeriod(newPeriod).setRun(ctx.run)
              mapState = newPeriod
              try this(ref1)
              finally
                mapCtx = savedCtx
  ...
  abstract class DeepTypeMap(using Ctx, CState) extends TypeMap {
  ...
  @sharable object IdentityTypeMap extends TypeMap(using NoContext, Nowhere) {
   ...
   */
  abstract class ApproximatingTypeMap(using Ctx, CState) extends TypeMap { thisMap =>
  ...
  abstract class TypeAccumulator[T](using protected val accCtx: Ctx, protected val accState: CState)
  ...
  abstract class TypeTraverser(using Ctx, CState) extends TypeAccumulator[Unit] {
    def traverse(tp: Type): Unit
    def apply(x: Unit, tp: Type): Unit = traverse(tp)
    protected def traverseChildren(tp: Type): Unit = foldOver((), tp)
  }

  class ExistsAccumulator(p: Type => Boolean, forceLazy: Boolean = true)(using Ctx, CState) extends TypeAccumulator[Boolean] {
    override def stopAtStatic: Boolean = false
    def apply(x: Boolean, tp: Type): Boolean =
      x || p(tp) || (forceLazy || !tp.isInstanceOf[LazyRef]) && foldOver(x, tp)
  }

  class ForeachAccumulator(p: Type => Unit, override val stopAtStatic: Boolean)(using Ctx, CState) extends TypeAccumulator[Unit] {
  ...
  class NamedPartsAccumulator(p: NamedType => Boolean, excludeLowerBounds: Boolean = false)
    (using Ctx, CState) extends TypeAccumulator[mutable.Set[NamedType]] {
    override def stopAtStatic: Boolean = false
  ...
  class isGroundAccumulator(using Ctx, CState) extends TypeAccumulator[Boolean] {
  ...
  class TypeSizeAccumulator(using Ctx, CState) extends TypeAccumulator[Int] {
  ...
  class CoveringSetAccumulator(using Ctx, CState) extends TypeAccumulator[Set[Symbol]] {
    val seen = new java.util.IdentityHashMap[Type, Type]
  ...
  abstract class NameFilter {
    def apply(pre: Type, name: Name)(using Ctx, CState): Boolean
  }
  ...
  object typeNameFilter extends NameFilter {
    def apply(pre: Type, name: Name)(using Ctx, CState): Boolean = name.isTypeName
  }
  ...
  object takeAllFilter extends NameFilter {
    def apply(pre: Type, name: Name)(using Ctx, CState): Boolean = true
  }
  ...
  def isWatched(tp: Type)(using Ctx, CState): Boolean = tp match {
  ...
  private val keepAlways: AnnotatedType => (Ctx, CState) ?=> Boolean = _ => true
  private val keepNever: AnnotatedType => (Ctx, CState) ?=> Boolean = _ => false
  private val keepIfRefining: AnnotatedType => (Ctx, CState) ?=> Boolean = _.isRefining
