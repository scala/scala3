...
object Substituters:
  ...
  final def subst(tp: Type, from: BindingType, to: BindingType, theMap: SubstBindingMap)(using Ctx, CState): Type =
    tp match {
      case tp: BoundType =>
  ...
  final def subst1(tp: Type, from: Symbol, to: Type, theMap: Subst1Map)(using Ctx, CState): Type =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
  ...
  final def subst2(tp: Type, from1: Symbol, to1: Type, from2: Symbol, to2: Type, theMap: Subst2Map)(using Ctx, CState): Type =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
  ...
  final def subst(tp: Type, from: List[Symbol], to: List[Type], theMap: SubstMap)(using Ctx, CState): Type =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
        var fs = from
  ...
  final def substSym(tp: Type, from: List[Symbol], to: List[Symbol], theMap: SubstSymMap)(using Ctx, CState): Type =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
        var fs = from
  ...
  final def substThis(tp: Type, from: ClassSymbol, to: Type, theMap: SubstThisMap)(using Ctx, CState): Type =
    tp match {
      case tp: ThisType =>
  ...
  final def substRecThis(tp: Type, from: Type, to: Type, theMap: SubstRecThisMap)(using Ctx, CState): Type =
    tp match {
  ...
  final def substParam(tp: Type, from: ParamRef, to: Type, theMap: SubstParamMap)(using Ctx, CState): Type =
    tp match {
      case tp: BoundType =>
  ...
  final def substParams(tp: Type, from: BindingType, to: List[Type], theMap: SubstParamsMap)(using Ctx, CState): Type =
    tp match {
      case tp: ParamRef =>
  ...
  final class SubstBindingMap(from: BindingType, to: BindingType)(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = subst(tp, from, to, this)
  }

  final class Subst1Map(from: Symbol, to: Type)(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = subst1(tp, from, to, this)
  }

  final class Subst2Map(from1: Symbol, to1: Type, from2: Symbol, to2: Type)(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = subst2(tp, from1, to1, from2, to2, this)
  }

  final class SubstMap(from: List[Symbol], to: List[Type])(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = subst(tp, from, to, this)
  }

  final class SubstSymMap(from: List[Symbol], to: List[Symbol])(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = substSym(tp, from, to, this)
  }

  final class SubstThisMap(from: ClassSymbol, to: Type)(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = substThis(tp, from, to, this)
  }

  final class SubstRecThisMap(from: Type, to: Type)(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = substRecThis(tp, from, to, this)
  }

  final class SubstParamMap(from: ParamRef, to: Type)(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = substParam(tp, from, to, this)
  }

  final class SubstParamsMap(from: BindingType, to: List[Type])(using Ctx, CState) extends DeepTypeMap {
    def apply(tp: Type): Type = substParams(tp, from, to, this)
  }

  /** An approximating substitution that can handle wildcards in the `to` list */
  final class SubstApproxMap(from: List[Symbol], to: List[Type])(using Ctx, CState) extends ApproximatingTypeMap {
