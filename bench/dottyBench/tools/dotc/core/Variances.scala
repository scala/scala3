...
object Variances {
  ...
  def varianceInSyms(syms: List[Symbol])(tparam: Symbol)(using Ctx, CState): Variance =
    syms.foldLeft(Bivariant) ((v, sym) => v & varianceInSym(sym)(tparam))

  /** Compute variance of type parameter `tparam` in type of symbol `sym`. */
  def varianceInSym(sym: Symbol)(tparam: Symbol)(using Ctx, CState): Variance =
    if (sym.isAliasType) cut(varianceInType(sym.info)(tparam))
    else varianceInType(sym.info)(tparam)

  /** Compute variance of type parameter `tparam` in all types `tps`. */
  def varianceInTypes(tps: List[Type])(tparam: Symbol)(using Ctx, CState): Variance =
   ...
   */
  def varianceInArgs(tps: List[Type], tparams1: List[Symbol])(tparam: Symbol)(using Ctx, CState): Variance = {
  ...
  def varianceInAnnots(annots: List[Annotation])(tparam: Symbol)(using Ctx, CState): Variance =
  ...
  def varianceInAnnot(annot: Annotation)(tparam: Symbol)(using Ctx, CState): Variance =
    varianceInType(annot.tree.tpe)(tparam)

  /** Compute variance of type parameter <code>tparam</code> in type <code>tp</code>. */
  def varianceInType(tp: Type)(tparam: Symbol)(using Ctx, CState): Variance = tp match {
    case TermRef(pre, _) =>
  ...
  def setStructuralVariances(lam: HKTypeLambda)(using Ctx, CState): Unit =
   ...
   */
   def varianceConforms(tparam1: TypeParamInfo, tparam2: TypeParamInfo)(using Ctx, CState): Boolean =
   ...
   */
  def variancesConform(tparams1: List[TypeParamInfo], tparams2: List[TypeParamInfo])(using Ctx, CState): Boolean =
  ...
  def varianceSign(sym: Symbol)(using Ctx, CState): String =
