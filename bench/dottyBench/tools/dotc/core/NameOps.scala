...
object NameOps {
  ...
  implicit class NameDecorator[N <: Name](private val name: N) extends AnyVal {
     ...
     */
    def expandedName(base: Symbol, kind: QualifiedNameKind = ExpandedName)(using Ctx, CState): N =
    ...
    def freshened(using Ctx, CState): N = likeSpacedN {
      name.toTermName match {
    ...
    def specializedFor(classTargs: List[Type], classTargsNames: List[Name], methodTargs: List[Type], methodTarsNames: List[Name])(using Ctx, CState): N = {
    ...
    def compactified(using Ctx, CState): TermName = termName(compactify(name.toString))
