...
import dottyBench.tools.dotc.core.Flags.JavaDefined
import dottyBench.tools.dotc.core.StdNames.{jnme, nme}
...
object JavaNullInterop {
   ...
   */
  def nullifyMember(sym: Symbol, tp: Type, isEnumValueDef: Boolean)(using Ctx, CState): Type = {
  ...
  private def hasNotNullAnnot(sym: Symbol)(using Ctx, CState): Boolean =
   ...
   */
  private def nullifyExceptReturnType(tp: Type)(using Ctx, CState): Type =
    new JavaNullMap(true)(tp)

  /** Nullifies a Java type by adding `| UncheckedNull` in the relevant places. */
  private def nullifyType(tp: Type)(using Ctx, CState): Type =
   ...
   */
  private class JavaNullMap(var outermostLevelAlreadyNullable: Boolean)(using Ctx, CState) extends TypeMap {
