...
object SymDenotations {
  ...
  class SymDenotation private[SymDenotations] (
    ...
    */
    ...
    /** The flag set */
    final def flags(using Ctx, CState): FlagSet = { ensureCompleted(); myFlags }
    ...
    final def flagsString(using Ctx, CState): String = flags.flagsString
     ...
     *  in `butNot` are set?
     */
    final def isOneOf(fs: FlagSet, butNot: FlagSet)(using Ctx, CState): Boolean =
      (if (isCurrent(fs) && isCurrent(butNot)) myFlags else flags).isOneOf(fs, butNot)

    /** Has this denotation all of the flags in `fs` set? */
    final def isAllOf(fs: FlagSet)(using Ctx, CState): Boolean =
      (if (isCurrent(fs)) myFlags else flags).isAllOf(fs)

    /** Has this denotation all of the flags in `fs` set, whereas none of the flags
     *  in `butNot` are set?
     */
    final def isAllOf(fs: FlagSet, butNot: FlagSet)(using Ctx, CState): Boolean =
    ...
    final def completeFrom(completer: LazyType)(using Ctx, CState): Unit =
     ...
     *     "avoid clash" suffix
     */
    def effectiveName(using Ctx, CState): Name =
     ...
     */
    @tailrec
    final def privateWithin(using Ctx, CState): Symbol = myInfo match {
     ...
     */
    protected[dotc] final def setPrivateWithin(pw: Symbol)(using Ctx, CState): Unit = {
      if (isCompleting)
    ...
    final def annotations(using Ctx, CState): List[Annotation] = {
    ...
    final def hasAnnotation(cls: Symbol)(using Ctx, CState): Boolean =
      dropOtherAnnotations(annotations, cls).nonEmpty

    /** Apply transform `f` to all annotations of this denotation */
    final def transformAnnotations(f: Annotation => Annotation)(using Ctx, CState): Unit =
      annotations = annotations.mapConserve(f)

    /** Keep only those annotations that satisfy `p` */
    final def filterAnnotations(p: Annotation => Boolean)(using Ctx, CState): Unit =
      annotations = annotations.filterConserve(p)

    /** Optionally, the annotation matching the given class symbol */
    final def getAnnotation(cls: Symbol)(using Ctx, CState): Option[Annotation] =
     ...
     */
    final def unforcedAnnotation(cls: Symbol)(using Ctx, CState): Option[Annotation] =
    ...
    final def removeAnnotation(cls: Symbol)(using Ctx, CState): Unit =
      annotations = myAnnotations.filterNot(_ matches cls)

    /** Remove any annotations with same class as `annot`, and add `annot` */
    final def updateAnnotation(annot: Annotation)(using Ctx, CState): Unit = {
      removeAnnotation(annot.symbol)
      addAnnotation(annot)
    }

    /** Add all given annotations to this symbol */
    final def addAnnotations(annots: TraversableOnce[Annotation])(using Ctx, CState): Unit =
      annots.iterator.foreach(addAnnotation)

    @tailrec
    private def dropOtherAnnotations(anns: List[Annotation], cls: Symbol)(using Ctx, CState): List[Annotation] = anns match {
    ...
    final def setParamss(tparams: List[Symbol], vparamss: List[List[Symbol]])(using Ctx, CState): Unit =
      rawParamss = (tparams :: vparamss).filterConserve(!_.isEmpty)

    final def setParamssFromDefs(tparams: List[TypeDef[?]], vparamss: List[List[ValDef[?]]])(using Ctx, CState): Unit =
     ...
     */
    final def paramSymss(using Ctx, CState): List[List[Symbol]] =
     ...
     *  @see ensureCompleted
     */
    final def completeOnce()(using Ctx, CState): Unit = myInfo match {
     ...
     *  @see completeOnce
     */
    final def ensureCompleted()(using Ctx, CState): Unit = info
     ...
     */
    final def unforcedDecls(using Ctx, CState): Scope = myInfo match {
      case cinfo: LazyType =>
     ...
     */
    final private[core] def currentPackageDecls(using Ctx, CState): MutableScope = myInfo match {
     ...
     *
     */
    def opaqueToBounds(info: Type, rhs: tpd.Tree, tparams: List[TypeParamInfo])(using Ctx, CState): Type =
    ...
    final def expandedName(using Ctx, CState): Name =
    ...
    final def originalName(using Ctx, CState): Name = initial.effectiveName

    /** The owner with which the denoting symbol was created. */
    final def originalOwner(using Ctx, CState): Symbol = initial.maybeOwner
     ...
     */
    def fullNameSeparated(kind: QualifiedNameKind)(using Ctx, CState): Name =
     ...
     *  Drops package objects. Represents each term in the owner chain by a simple `_$`.
     */
    def fullNameSeparated(prefixKind: QualifiedNameKind, kind: QualifiedNameKind, name: Name)(using Ctx, CState): Name =
    ...
    def flatName(using Ctx, CState): Name = fullNameSeparated(FlatName)

    /** `fullName` where `.' is the separator character */
    def fullName(using Ctx, CState): Name = fullNameSeparated(QualifiedName)

    /** The name given in an `@alpha` annotation if one is present, `name` otherwise */
    final def erasedName(using Ctx, CState): Name =
    ...
    final def isRealClass(using Ctx, CState): Boolean = isClass && !is(Trait)
     ...
     *  @pre `isCompleting` is false, or this is a ModuleCompleter or SymbolLoader
     */
    final def markAbsent()(using Ctx, CState): Unit = {
      if (isCompleting)
        assert(myInfo.isInstanceOf[ModuleCompleter | SymbolLoader],
     ...
     */
    @tailrec
    final def isAbsent(canForce: Boolean = true)(using Ctx, CState): Boolean = myInfo match {
      case myInfo: ModuleCompleter =>
    ...
    final def isEmptyPackage(using Ctx, CState): Boolean =
      name.toTermName == nme.EMPTY_PACKAGE && owner.isRoot

    /** Is this symbol the empty package class or its companion object? */
    final def isEffectiveRoot(using Ctx, CState): Boolean = isRoot || isEmptyPackage

    /** Is this symbol an anonymous class? */
    final def isAnonymousClass(using Ctx, CState): Boolean =
      isClass && initial.name.isAnonymousClassName

    final def isAnonymousFunction(using Ctx, CState): Boolean =
      this.symbol.is(Method) && initial.name.isAnonymousFunctionName

    final def isAnonymousModuleVal(using Ctx, CState): Boolean =
    ...
    def isNotRuntimeClass(using Ctx, CState): Boolean = defn.NotRuntimeClasses contains symbol
     ...
     */
    final def isRefinementClass(using Ctx, CState): Boolean =
      name == tpnme.REFINE_CLASS

    /** Is this symbol a package object or its module class? */
    def isPackageObject(using Ctx, CState): Boolean =
      name.isPackageObjectName && owner.is(Package) && this.is(Module)

    /** Is this symbol a toplevel definition in a package object? */
    def isWrappedToplevelDef(using Ctx, CState): Boolean =
      !isConstructor && owner.isPackageObject

    /** Is this symbol an abstract type? */
    final def isAbstractType(using Ctx, CState): Boolean = this.is(DeferredType)

    /** Is this symbol an alias type? */
    final def isAliasType(using Ctx, CState): Boolean = isAbstractOrAliasType && !this.is(Deferred)

    /** Is this symbol an abstract or alias type? */
    final def isAbstractOrAliasType: Boolean = isType & !isClass

    /** Is this symbol an abstract type or type parameter? */
    final def isAbstractOrParamType(using Ctx, CState): Boolean = this.isOneOf(DeferredOrTypeParam)

    /** Is this symbol a user-defined opaque alias type? */
    def isOpaqueAlias(using Ctx, CState): Boolean = is(Opaque) && !isClass

    /** Is this symbol a module that contains opaque aliases? */
    def containsOpaques(using Ctx, CState): Boolean = is(Opaque) && isClass

    def seesOpaques(using Ctx, CState): Boolean =
      containsOpaques ||
     ...
     */
    final def isSelfSym(using Ctx, CState): Boolean = owner.infoOrCompleter match {
     ...
     */
    final def isContainedIn(boundary: Symbol)(using Ctx, CState): Boolean = {
    ...
    final def isProperlyContainedIn(boundary: Symbol)(using Ctx, CState): Boolean =
    ...
    final def isStaticOwner(using Ctx, CState): Boolean =
      myFlags.is(ModuleClass) && (myFlags.is(PackageClass) || isStatic)

    /** Is this denotation defined in the same scope and compilation unit as that symbol? */
    final def isCoDefinedWith(other: Symbol)(using Ctx, CState): Boolean =
    ...
    /** Is this a denotation of a stable term (or an arbitrary type)?
      ...
      */
    final def isStableMember(using Ctx, CState): Boolean = {
     ...
     *  initialization code?
     */
    def isNoInitsClass(using Ctx, CState): Boolean =
      isClass &&
     ...
     */
    final def isRealMethod(using Ctx, CState): Boolean =
      this.is(Method, butNot = Accessor) && !isAnonymousFunction

    /** Is this a getter? */
    final def isGetter(using Ctx, CState): Boolean =
      this.is(Accessor) && !originalName.isSetterName && !originalName.isScala2LocalSuffix

    /** Is this a setter? */
    final def isSetter(using Ctx, CState): Boolean =
      this.is(Accessor) &&
    ...
    final def isPrimaryConstructor(using Ctx, CState): Boolean =
      isConstructor && owner.primaryConstructor == symbol

    /** Does this symbol denote the static constructor of its enclosing class? */
    final def isStaticConstructor(using Ctx, CState): Boolean =
      name.isStaticConstructorName

    /** Is this a subclass of the given class `base`? */
    def isSubClass(base: Symbol)(using Ctx, CState): Boolean = false
     ...
     */
    def derivesFrom(base: Symbol)(using Ctx, CState): Boolean = false

    /** Is this a Scala or Java annotation ? */
    def isAnnotation(using Ctx, CState): Boolean =
      isClass && derivesFrom(defn.AnnotationClass)

    /** Is this symbol a class that extends `java.io.Serializable` ? */
    def isSerializable(using Ctx, CState): Boolean =
      isClass && derivesFrom(defn.JavaSerializableClass)

    /** Is this symbol a class that extends `AnyVal`? */
    final def isValueClass(using Ctx, CState): Boolean =
      val di = initial
      di.isClass
    ...
    final def isNullableClass(using Ctx, CState): Boolean =
     ...
     */
    final def isNullableClassAfterErasure(using Ctx, CState): Boolean =
     ...
     *  of their owner.
     */
    final def isAccessibleFrom(pre: Type, superAccess: Boolean = false, whyNot: StringBuffer = null)(using Ctx, CState): Boolean = {
     ...
     */
    def membersNeedAsSeenFrom(pre: Type)(using Ctx, CState): Boolean =
      !(  this.isTerm
    ...
    def isAsConcrete(that: Symbol)(using Ctx, CState): Boolean =
     ...
     */
    def hasDefaultParams(using Ctx, CState): Boolean =
     ...
     */
    def matchNullaryLoosely(using Ctx, CState): Boolean = {
      def test(sym: Symbol) =
        sym.is(JavaDefined) ||
     ...
     *  otherwise NoSymbol
     */
    final def moduleClass(using Ctx, CState): Symbol = {
      def notFound = {
     ...
     *  otherwise NoSymbol
     */
    final def sourceModule(using Ctx, CState): Symbol =
    ...
    def accessedFieldOrGetter(using Ctx, CState): Symbol = {
     ...
     */
    def underlyingSymbol(using Ctx, CState): Symbol =
      if (is(Accessor)) accessedFieldOrGetter orElse symbol else symbol

    /** The chain of owners of this denotation, starting with the denoting symbol itself */
    final def ownersIterator(using Ctx, CState): Iterator[Symbol] = new Iterator[Symbol] {
    ...
    final def skipWeakOwner(using Ctx, CState): Symbol =
      if (isWeakOwner) owner.skipWeakOwner else symbol

    /** The owner, skipping package objects and non-lazy valdefs. */
    final def effectiveOwner(using Ctx, CState): Symbol = owner.skipWeakOwner
     ...
     */
    final def enclosingClass(using Ctx, CState): Symbol = {
    ...
    final def lexicallyEnclosingClass(using Ctx, CState): Symbol =
     ...
     */
    final def isExtensibleClass(using Ctx, CState): Boolean =
      isClass && !isOneOf(FinalOrModuleClass) && !isAnonymousClass

    /** A symbol is effectively final if it cannot be overridden in a subclass */
    final def isEffectivelyFinal(using Ctx, CState): Boolean =
     ...
     */
    final def isEffectivelySealed(using Ctx, CState): Boolean =
      isOneOf(FinalOrSealed) || isClass && !isOneOf(EffectivelyOpenFlags)

    final def isSuperTrait(using Ctx, CState): Boolean =
      isClass
      && (is(SuperTrait)
    ...
    final def enclosingClassNamed(name: Name)(using Ctx, CState): Symbol = {
     ...
     */
    final def enclosingMethod(using Ctx, CState): Symbol =
     ...
     */
    final def enclosingExtensionMethod(using Ctx, CState): Symbol =
     ...
     */
    final def topLevelClass(using Ctx, CState): Symbol = {
    ...
    final def isTopLevelClass(using Ctx, CState): Boolean =
    ...
    final def enclosingPackageClass(using Ctx, CState): Symbol =
    ...
    def registerCompanion(target: Symbol)(using Ctx, CState) = ()

    /** The registered companion; overridden in ClassDenotation */
    def registeredCompanion(using Ctx, CState): Symbol = NoSymbol
     ...
     */
    final def companionModule(using Ctx, CState): Symbol =
      if (is(Module)) sourceModule
      else registeredCompanion.sourceModule

    private def companionType(using Ctx, CState): Symbol =
     ...
     */
    final def companionClass(using Ctx, CState): Symbol =
      companionType.suchThat(_.isClass).symbol

    final def scalacLinkedClass(using Ctx, CState): Symbol =
     ...
     */
    private def companionNamed(name: TypeName)(using Ctx, CState): Symbol =
      if (owner.isClass)
     ...
     *  NoSymbol otherwise.
     */
    final def linkedClass(using Ctx, CState): Symbol =
     ...
     */
    final def enclosingSubClass(using Ctx, CState): Symbol =
     ...
     *  containing object.
     */
    def opaqueAlias(using Ctx, CState): Type = {
     ...
     */
    final def matchingDecl(inClass: Symbol, site: Type)(using Ctx, CState): Symbol = {
     ...
     */
    final def matchingMember(site: Type)(using Ctx, CState): Symbol = {
     ...
     */
    final def canMatchInheritedSymbols(using Ctx, CState): Boolean =
      maybeOwner.isClass && memberCanMatchInheritedSymbols

    /** If false, this class member cannot possibly participate in an override,
     *  either as overrider or overridee.
     */
    final def memberCanMatchInheritedSymbols(using Ctx, CState): Boolean =
      !isConstructor && !is(Private)

    /** The symbol, in class `inClass`, that is overridden by this denotation in class `siteClass`.*/
    final def overriddenSymbol(inClass: ClassSymbol, siteClass: ClassSymbol = owner.asClass)(using Ctx, CState): Symbol =
    ...
    final def allOverriddenSymbols(using Ctx, CState): Iterator[Symbol] =
    ...
    final def nextOverriddenSymbol(using Ctx, CState): Symbol = {
    ...
    final def extendedOverriddenSymbols(using Ctx, CState): Iterator[Symbol] =
      if (!canMatchInheritedSymbols) Iterator.empty
      else overriddenFromType(owner.asClass.classInfo.selfType)

    private def overriddenFromType(tp: Type)(using Ctx, CState): Iterator[Symbol] =
     ...
     */
    final def overridingSymbol(inClass: ClassSymbol)(using Ctx, CState): Symbol =
     ...
     */
    final def superSymbolIn(base: Symbol)(using Ctx, CState): Symbol = {
     ...
     */
    @tailrec final def isIncompleteIn(base: Symbol)(using Ctx, CState): Boolean =
      this.is(Deferred) ||
      this.is(AbsOverride) && {
     ...
     */
    final def accessBoundary(base: Symbol)(using Ctx, CState): Symbol =
    ...
    final def isPublic(using Ctx, CState): Boolean =
      accessBoundary(owner) == defn.RootClass

    /** The primary constructor of a class or trait, NoSymbol if not applicable. */
    def primaryConstructor(using Ctx, CState): Symbol = NoSymbol
     ...
     */
    def currentSymbol(using Ctx, CState): Symbol = {
    ...
    def typeParams(using Ctx, CState): List[TypeSymbol] = Nil

    /** The type This(cls), where cls is this class, NoPrefix for all other symbols */
    def thisType(using Ctx, CState): Type = NoPrefix

    def typeRef(using Ctx, CState): TypeRef =
      TypeRef(owner.thisType, symbol)

    def termRef(using Ctx, CState): TermRef =
      TermRef(owner.thisType, symbol)

    /** The typeRef applied to its own type parameters */
    def appliedRef(using Ctx, CState): Type =
    ...
    def nestingLevel(using Ctx, CState): Int =
    ...
    final def copySymDenotation(
      ...
      rawParamss: List[List[Symbol]] = null)(
        using Ctx, CState): SymDenotation = {
     ...
     */
    def copyCaches(from: SymDenotation, phase: Phase)(using Ctx, CState): this.type = this
    ...
    override def installAfter(phase: DenotTransformer)(using Ctx, CState): Unit =
     ...
     */
    override def transformAfter(phase: DenotTransformer, f: SymDenotation => SymDenotation)(using Ctx, CState): Unit =
      super.transformAfter(phase, f)

    /** Set flag `flags` in current phase and in all phases that follow */
    def setFlagFrom(phase: DenotTransformer, flags: FlagSet)(using Ctx, CState): Unit =
      setFlag(flags)
    ...
    def ensureNotPrivate(using Ctx, CState): SymDenotation =
      if (is(Private))
     ...
     */
    def children(using Ctx, CState): List[Symbol] =

      def completeChildrenIn(owner: Symbol)(using Ctx, CState) =
  ...
  class ClassDenotation private[SymDenotations] (
    ...
    private def memberCache(using Ctx, CState): LRUCache[Name, PreDenotation] = {
    ...
    private def baseTypeCache(using Ctx, CState): BaseTypeMap = {
    ...
    def invalidateMemberCaches(sym: Symbol)(using Ctx, CState): Unit =
    ...
    override def copyCaches(from: SymDenotation, phase: Phase)(using Ctx, CState): this.type = {
      from match {
    ...
    def classInfo(using Ctx, CState): ClassInfo = info.asInstanceOf[ClassInfo]
     ...
     */
    private def typeParamsFromDecls(using Ctx, CState) =
      unforcedDecls.filter(sym =>
        sym.is(TypeParam) && sym.owner == symbol).asInstanceOf[List[TypeSymbol]]

    /** The type parameters of this class */
    override final def typeParams(using Ctx, CState): List[TypeSymbol] = {
    ...
    def classParents(using Ctx, CState): List[Type] = info match {
      case classInfo: ClassInfo => classInfo.parents
      case _ => Nil
    }

    /** The symbol of the superclass, NoSymbol if no superclass exists */
    def superClass(using Ctx, CState): Symbol = classParents match {
      case parent :: _ =>
     ...
     */
    def givenSelfType(using Ctx, CState): Type = classInfo.selfInfo match {
      case tp: Type => tp
   ...
   // ------ class-specific operations -----------------------------------
     ...
     */
    override def thisType(using Ctx, CState): Type = {
      if (myThisType == null) myThisType = computeThisType
      myThisType
    }

    private def computeThisType(using Ctx, CState): Type = {
    ...
    override def typeRef(using Ctx, CState): TypeRef = {
      if (myTypeRef == null) myTypeRef = super.typeRef
      myTypeRef
    }

    override def appliedRef(using Ctx, CState): Type = classInfo.appliedRef

    private def baseData(using BaseData, Ctx, CState): (List[ClassSymbol], BaseClassSet) = {
     ...
     */
    def baseClasses(using BaseData, Ctx, CState): List[ClassSymbol] =
      baseData._1

    /** A bitset that contains the superId's of all base classes */
    private def baseClassSet(using BaseData, Ctx, CState): BaseClassSet =
      baseData._2

    def computeBaseData(using BaseData, Ctx, CState): (List[ClassSymbol], BaseClassSet) = {
      def emptyParentsExpected =
        is(Package) || (symbol == defn.AnyClass) || currentlyAfterErasure && (symbol == defn.ObjectClass)
      if classParents.isEmpty && !emptyParentsExpected then
        val onBehalf = summon[BaseData]
    ...
    final override def derivesFrom(base: Symbol)(using Ctx, CState): Boolean =
      !isAbsent() &&
      base.isClass &&
      (  (symbol eq base)
    ...
    final override def isSubClass(base: Symbol)(using Ctx, CState): Boolean =
      derivesFrom(base) ||
        base.isClass && (
     ...
     */
    final def mayHaveCommonChild(that: ClassSymbol)(using Ctx, CState): Boolean =
    ...
    protected def proceedWithEnter(sym: Symbol, mscope: MutableScope)(using Ctx, CState): Boolean = true
     ...
     */
    def enter(sym: Symbol, scope: Scope = EmptyScope)(using Ctx, CState): Unit = {
    ...
    def enterNoReplace(sym: Symbol, scope: MutableScope)(using Ctx, CState): Unit =
      scope.enter(sym)
     ...
     */
    def replace(prev: Symbol, replacement: Symbol)(using Ctx, CState): Unit = {
     ...
     *  someone does a findMember on a subclass.
     */
    def delete(sym: Symbol)(using Ctx, CState): Unit = {
     ...
     */
    def ensureTypeParamsInCorrectOrder()(using Ctx, CState): Unit = {
     ...
     */
    final def membersNamed(name: Name)(using Ctx, CState): PreDenotation = {
     ...
     */
    final def nonPrivateMembersNamed(name: Name)(using Ctx, CState): PreDenotation = {
    ...
    private[core] def computeNPMembersNamed(name: Name)(using Ctx, CState): PreDenotation = {
    ...
    override final def findMember(name: Name, pre: Type, required: FlagSet, excluded: FlagSet)(using Ctx, CState): Denotation = {
    ...
    final def baseTypeOf(tp: Type)(using Ctx, CState): Type = {
    ...
    def memberNames(keepOnly: NameFilter)(using MemberNames, Ctx, CState): Set[Name] =
    ...
    def computeMemberNames(keepOnly: NameFilter)(using MemberNames, Ctx, CState): Set[Name] = {
      var names = Set[Name]()
    ...
    override final def fullNameSeparated(kind: QualifiedNameKind)(using Ctx, CState): Name = {
    ...
    override def fullName(using Ctx, CState): Name = super.fullName

    override def primaryConstructor(using Ctx, CState): Symbol = {
     ...
     */
    def paramAccessors(using Ctx, CState): List[Symbol] =
      unforcedDecls.filter(_.is(ParamAccessor))

    /** If this class has the same `decls` scope reference in `phase` and
     *  `phase.next`, install a new denotation with a cloned scope in `phase.next`.
     */
    def ensureFreshScopeAfter(phase: DenotTransformer)(using Ctx, CState): Unit =
    ...
    override def registerCompanion(companion: Symbol)(using Ctx, CState) =
      if (companion.isClass && !isAbsent(canForce = false) && !companion.isAbsent(canForce = false))
        myCompanion = companion

    override def registeredCompanion(using Ctx, CState) =
    ...
    override def nestingLevel(using Ctx, CState) =
  ...
  final class PackageClassDenotation private[SymDenotations] (
    ...
    def packageObjs(using Ctx, CState): List[ClassDenotation] = {
     ...
     *  `sym` as a member.
     */
    def packageObjFor(sym: Symbol)(using Ctx, CState): Symbol = {
     ...
     */
    override def computeNPMembersNamed(name: Name)(using Ctx, CState): PreDenotation = {
    ...
    override def memberNames(keepOnly: NameFilter)(using MemberNames, Ctx, CState): Set[Name] = {
     ...
     */
    override def proceedWithEnter(sym: Symbol, mscope: MutableScope)(using Ctx, CState): Boolean = {
    ...
    def unlinkFromFile(file: AbstractFile)(using Ctx, CState): Unit = {
  ...
  extends SymDenotation(NoSymbol, NoSymbol, "<none>".toTermName, Permanent, NoType) {
    ...
    override def computeAsSeenFrom(pre: Type)(using Ctx, CState): SingleDenotation = this
    override def mapInfo(f: Type => Type)(using Ctx, CState): SingleDenotation = this
    NoSymbol.denot = this
   ...
   */
  def canBeLocal(name: Name, flags: FlagSet)(using Ctx, CState) =
  ...
  def SymDenotation(
    ...
    initFlags: FlagSet,
    initInfo: Type,
    initPrivateWithin: Symbol = NoSymbol)(using Ctx, CState): SymDenotation = {
    val result =
      if (symbol.isClass)
  ...
  def stillValid(denot: SymDenotation)(using Ctx, CState): Boolean =
  ...
  private[SymDenotations] def stillValidInOwner(denot: SymDenotation)(using Ctx, CState): Boolean = try {
  ...
  def traceInvalid(denot: Denotation)(using Ctx, CState): Boolean = {
   ...
   */
  def staleOK(using Ctx, CState): Boolean =
  ...
  def acceptStale(denot: SingleDenotation)(using Ctx, CState): Boolean =
    staleOK && {
...
// ---- Completion --------------------------------------------------------
  ...
  abstract class LazyType extends UncachedGroundType
    ...
    private var mySourceModuleFn: (Ctx, CState) ?=> Symbol = LazyType.NoSymbolFn
    private var myModuleClassFn: (Ctx, CState) ?=> Symbol = LazyType.NoSymbolFn

    /** The type parameters computed by the completer before completion has finished */
    def completerTypeParams(sym: Symbol)(using Ctx, CState): List[TypeParamInfo] =
      if (sym.is(Touched)) Nil // return `Nil` instead of throwing a cyclic reference
      else sym.info.typeParams

    def decls: Scope = myDecls
    def sourceModule(using Ctx, CState): Symbol = mySourceModuleFn
    def moduleClass(using Ctx, CState): Symbol = myModuleClassFn

    def withDecls(decls: Scope): this.type = { myDecls = decls; this }
    def withSourceModule(sourceModuleFn: (Ctx, CState) ?=> Symbol): this.type = { mySourceModuleFn = sourceModuleFn; this }
    def withModuleClass(moduleClassFn: (Ctx, CState) ?=> Symbol): this.type = { myModuleClassFn = moduleClassFn; this }

    override def toString: String = getClass.toString
  }

  object LazyType:
    private val NoSymbolFn = (using _: Ctx, _: CState) => NoSymbol
  ...
  trait NoCompleter extends LazyType {
    def complete(denot: SymDenotation)(using Ctx, CState): Unit = unsupported("complete")
  }
  ...
  class ModuleCompleter(_moduleClass: ClassSymbol) extends LazyType {
    override def moduleClass(using Ctx, CState): ClassSymbol = _moduleClass
    def complete(denot: SymDenotation)(using Ctx, CState): Unit = {
  ...
  class StubInfo() extends LazyType {
    ...
    def initializeToDefaults(denot: SymDenotation, errMsg: Message)(using Ctx, CState): Unit = {
  ...
  trait InheritedCache {
    ...
    def isValid(using Ctx, CState): Boolean

    /** is the cache valid in current run at given phase? */
    def isValidAt(phase: Phase)(using Ctx, CState): Boolean
  ...
  trait MemberNames extends InheritedCache {
    def apply(keepOnly: NameFilter, clsd: ClassDenotation)(using MemberNames, Ctx, CState): Set[Name]
  }

  object MemberNames {
    implicit val None: MemberNames = new InvalidCache with MemberNames {
      def apply(keepOnly: NameFilter, clsd: ClassDenotation)(using MemberNames, Ctx, CState) = ???
    }
    def newCache()(using Ctx, CState): MemberNames = new MemberNamesImpl(currentPeriod)
  }
  ...
  trait BaseData extends InheritedCache {
    def apply(clsd: ClassDenotation)(using BaseData, Ctx, CState): (List[ClassSymbol], BaseClassSet)
    def signalProvisional(): Unit
  }

  object BaseData {
    implicit val None: BaseData = new InvalidCache with BaseData {
      def apply(clsd: ClassDenotation)(using BaseData, Ctx, CState) = ???
      def signalProvisional() = ()
    }
    def newCache()(using Ctx, CState): BaseData = new BaseDataImpl(currentPeriod)
  }
  ...
  private abstract class InheritedCacheImpl(val createdAt: Period) extends InheritedCache {
    ...
    def isValidAt(phase: Phase)(using Ctx, CState) =
  ...
  private class InvalidCache extends InheritedCache {
    def isValid(using Ctx, CState) = false
    def isValidAt(phase: Phase)(using Ctx, CState) = false
    def invalidate(): Unit = ()
  }
  ...
  private class MemberNamesImpl(createdAt: Period) extends InheritedCacheImpl(createdAt) with MemberNames {
    ...
    final def isValid(using Ctx, CState): Boolean =
      cache != null && isValidAt(currentPhase)

    private var locked = false

    private def onBehalf(using x: MemberNames): MemberNames = x
    ...
    def apply(keepOnly: NameFilter, clsd: ClassDenotation)(using MemberNames, Ctx, CState) = {
      ...
      try
        if (cached != null) cached
          ...
          locked = true
          val computed =
            try clsd.computeMemberNames(keepOnly)(using this)
  ...
  private class BaseDataImpl(createdAt: Period) extends InheritedCacheImpl(createdAt) with BaseData {
    ...
    final def isValid(using Ctx, CState): Boolean = valid && isValidAt(currentPhase)
    ...
    private def onBehalf(using x: BaseData): BaseData = x
    ...
    def apply(clsd: ClassDenotation)(using BaseData, Ctx, CState)
      ...
      try
        if (cache != null) cache
          ...
          provisional = false
          val computed =
            try clsd.computeBaseData(using this)
            finally locked = false
