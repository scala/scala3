...
import Periods._
import Contexts._
import dottyBench.tools.backend.jvm.GenBCode
...
object Phases {
  ...
  inline def phaseOf(id: PhaseId)(using Ctx): Phase =
    ctx.base.phases(id)
  ...
  trait PhasesBase {
    ...
    object NoPhase extends Phase {
      ...
      def transform(ref: SingleDenotation)(using Ctx, CState): SingleDenotation = unsupported("transform")
    }
    ...
    class TerminalPhase extends DenotTransformer {
      ...
      override def lastPhaseId(using Ctx): Int = id
    }
  ...
  abstract class Phase {
    ...
    def isRunnable(using Ctx): Boolean =
     ...
     */
    def checkPostCondition(tree: tpd.Tree)(using Ctx, CState): Unit = ()
    ...
    def isEnabled(using Ctx): Boolean = true
  ...
  def typerPhase(using Ctx): Phase                  = ctx.base.typerPhase
  def postTyperPhase(using Ctx): Phase              = ctx.base.postTyperPhase
  def sbtExtractDependenciesPhase(using Ctx): Phase = ctx.base.sbtExtractDependenciesPhase
  def picklerPhase(using Ctx): Phase                = ctx.base.picklerPhase
  def reifyQuotesPhase(using Ctx): Phase            = ctx.base.reifyQuotesPhase
  def refchecksPhase(using Ctx): Phase              = ctx.base.refchecksPhase
  def elimRepeatedPhase(using Ctx): Phase           = ctx.base.elimRepeatedPhase
  def extensionMethodsPhase(using Ctx): Phase       = ctx.base.extensionMethodsPhase
  def explicitOuterPhase(using Ctx): Phase          = ctx.base.explicitOuterPhase
  def gettersPhase(using Ctx): Phase                = ctx.base.gettersPhase
  def erasurePhase(using Ctx): Phase                = ctx.base.erasurePhase
  def elimErasedValueTypePhase(using Ctx): Phase    = ctx.base.elimErasedValueTypePhase
  def lambdaLiftPhase(using Ctx): Phase             = ctx.base.lambdaLiftPhase
  def flattenPhase(using Ctx): Phase                = ctx.base.flattenPhase
  def genBCodePhase(using Ctx): Phase               = ctx.base.genBCodePhase

  def unfusedPhases(using Ctx): Array[Phase] = ctx.base.phases
