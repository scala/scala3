...
class TypeError(msg: String) extends Exception(msg) {
  def this() = this("")
  final def toMessage(using Ctx, CState): Message =
    withMode(Mode.Printing)(produceMessage)
  def produceMessage(using Ctx, CState): Message = super.getMessage
...
class MalformedType(pre: Type, denot: Denotation, absMembers: Set[Name]) extends TypeError {
  override def produceMessage(using Ctx, CState): Message =
...
class MissingType(pre: Type, name: Name) extends TypeError {
  private def otherReason(pre: Type)(using Ctx, CState): String = pre match {
...
class RecursionOverflow(val op: String, details: => String, val previous: Throwable, val weight: Int) extends TypeError {
  ...
  def opsString(rs: List[RecursionOverflow])(using Ctx, CState): String = {
    val maxShown = 20
  ...
  override def produceMessage(using Ctx, CState): Message = NoExplanation {
...
object handleRecursive {
  def apply(op: String, details: => String, exc: Throwable, weight: Int = 1)(using Ctx, CState): Nothing =
...
object CyclicReference {
  def apply(denot: SymDenotation)(using Ctx, CState): CyclicReference = {
