...
class TypeComparer(initctx: Context) extends ConstraintHandling[AbsentContext] with PatternTypeConstrainer {
  ...
  protected def gadtBounds(sym: Symbol)(using Ctx, CState) = ctx.gadt.bounds(sym)
  ...
  protected def typeVarInstance(tvar: TypeVar)(using Ctx, CState): Type = tvar.underlying
  ...
  protected def recur(tp1: Type, tp2: Type): Boolean = trace(s"isSubType ${traceInfo(tp1, tp2)} $approx", subtyping) {
    ...
    def monitoredIsSubType = {
      if (pendingSubTypes == null) {
        ...
        if (Config.traceDeepSubTypeRecursions && !this.isInstanceOf[ExplainingTypeComparer])
          report.log(TypeComparer.explained(combinedContext.typeComparer.isSubType(tp1, tp2, approx)))
      }
    ...
    def firstTry: Boolean = tp2 match {
      case tp2: NamedType =>
        def compareNamed(tp1: Type, tp2: NamedType): Boolean =
          val ctx = comparerCtx
          given Ctx = ctx // optimization for performance
          val cs = ctx.cstate
          given CState = cs
          val info2 = tp2.info
          info2 match
     ...
     *  its type             dottyBench.tools.dotc.util.Property.Key[Namer.this.Deriver]
    |*  does not conform to  dottyBench.tools.dotc.util.Property.Key[Typer.this.Deriver & Namer.this.Deriver]
     */
  ...
  private def nonExprBaseType(tp: Type, cls: Symbol)(using Ctx, CState): Type =
  ...
  def widenInUnions(using Ctx, CState): Boolean =
  ...
  private def showType(tp: Type)(using Ctx, CState) = tp match {
  ...
  def showGoal(tp1: Type, tp2: Type)(using Ctx, CState): Unit = {
   ...
   */
  def provablyDisjoint(tp1: Type, tp2: Type)(using Ctx, CState): Boolean = {
...
object TypeComparer {
  ...
  private[core] def show(res: Any)(using Ctx, CState): String = res match {
  ...
  def explaining[T](say: String => Unit)(op: (Ctx, CState) ?=> T)(using Ctx, CState): T = {
  ...
  def explained[T](op: (Ctx, CState) ?=> T)(using Ctx, CState): String = {
...
class TrackingTypeComparer(initctx: Context) extends TypeComparer(initctx) {
  ...
  override def gadtBounds(sym: Symbol)(using Ctx, CState): TypeBounds = {
  ...
  override def typeVarInstance(tvar: TypeVar)(using Ctx, CState): Type = {
    footprint += tvar
    super.typeVarInstance(tvar)
  }

  def matchCases(scrut: Type, cases: List[Type])(using Ctx, CState): Type = {
