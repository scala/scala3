...
object ClassfileParser {
  ...
  def cook(using Ctx, CState): TypeMap = new TypeMap {
...
class ClassfileParser(
  ...
  private def currentIsTopLevel(using Ctx, CState) = classRoot.owner.is(Flags.PackageClass)
  ...
  def run()(using Ctx, CState): Option[Embedded] = try {
  ...
  def classNameToSymbol(name: Name)(using Ctx, CState): Symbol = innerClasses.get(name) match {
  ...
  def parseClass()(using Ctx, CState): Option[Embedded] = {
  ...
  def addEnclosingTParams()(using Ctx, CState): Unit = {
  ...
  def parseMember(method: Boolean)(using Ctx, CState): Unit = {
  ...
  final def objToAny(tp: Type)(using Ctx, CState): Type =
    if (tp.isDirectRef(defn.ObjectClass) && !currentPhase.erasedTypes) defn.AnyType else tp

  def constantTagToType(tag: Int)(using Ctx, CState): Type =
    (tag: @switch) match {
  ...
  private def sigToType(sig: SimpleName, owner: Symbol = null)(using Ctx, CState): Type = {
    var index = 0
    val end = sig.length
    ...
    def sig2type(tparams: immutable.Map[Name, Symbol], skiptvs: Boolean)(using Ctx, CState): Type = {
    ...
    def sig2typeBounds(tparams: immutable.Map[Name, Symbol], skiptvs: Boolean)(using Ctx, CState): Type = {
  ...
  def parseAnnotArg(skip: Boolean = false)(using Ctx, CState): Option[untpd.Tree] = {
   ...
   *  return None.
   */
  def parseAnnotation(attrNameIndex: Char, skip: Boolean = false)(using Ctx, CState): Option[Annotation] = try {
  ...
  def parseAttributes(sym: Symbol, symtype: Type)(using Ctx, CState): Type = {
    var newType = symtype
    ...
    def parseAttribute(): Unit = {
      ...
      attrName match {
        case tpnme.SignatureATTR =>
          ...
          if (ctx.debug && ctx.settings.verbose.value)
   ...
   */
  def addAnnotationConstructor(classInfo: TempClassInfoType)(using Ctx, CState): Unit =
    newSymbol(
   ...
   */
  private def enterOwnInnerClasses()(using Ctx, CState): Unit = {
   ...
   */
  def unpickleOrParseInnerClasses()(using Ctx, CState): Option[Embedded] = {
    val oldbp = in.bp
    try {
      skipSuperclasses()
      skipMembers() // fields
      ...
      def unpickleScala(bytes: Array[Byte]): Some[Embedded] = {
        ...
        val unpickler = new unpickleScala2.Scala2Unpickler(bytes, classRoot, moduleRoot)
  ...
  object innerClasses extends scala.collection.mutable.HashMap[Name, InnerClassEntry] {
     ...
     */
    def topLevelClass(name: Name)(using Ctx, CState): Symbol = {
     ...
     */
    def classSymbol(entry: InnerClassEntry)(using Ctx, CState): Symbol = {
      def getMember(sym: Symbol, name: Name)(using Ctx, CState): Symbol =
  ...
  private def getPrivateWithin(jflags: Int)(using Ctx, CState): Symbol =
  ...
  class ConstantPool {
    ...
    def getClassSymbol(index: Int)(using Ctx, CState): Symbol = {
     ...
     */
    def getClassOrArrayType(index: Int)(using Ctx, CState): Type = {
      if (index <= 0 || len <= index) errorBadIndex(index)
    ...
    def getType(index: Int)(using Ctx, CState): Type =
      sigToType(getExternalName(index))

    def getSuperClass(index: Int)(using Ctx, CState): Symbol = {
      assert(index != 0, "attempt to parse java.lang.Object from classfile")
      getClassSymbol(index)
    }

    def getConstant(index: Int, pt: Type = WildcardType)(using Ctx, CState): Constant = {
      if (index <= 0 || len <= index) errorBadIndex(index)
