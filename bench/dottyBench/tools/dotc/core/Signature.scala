...
case class Signature(paramsSig: List[ParamSig], resSig: TypeName) {
   ...
   */
  final def consistentParams(that: Signature)(using Ctx, CState): Boolean = {
   ...
   */
  final def matchDegree(that: Signature)(using Ctx, CState): MatchDegree =
  ...
  def clashes(that: Signature)(using Ctx, CState): Boolean =
   ...
   */
  def prependTermParams(params: List[Type], isJava: Boolean)(using Ctx, CState): Signature =
   ...
   *  Like Signature#apply, the result is only cacheable if `isUnderDefined == false`.
   */
  def prependTypeParams(typeParamSigsSectionLength: Int)(using Ctx, CState): Signature =
   ...
   */
  def isUnderDefined(using Ctx, CState): Boolean =
...
object Signature {
   ...
   *  been instantiated.
   */
  def apply(resultType: Type, isJava: Boolean)(using Ctx, CState): Signature = {
