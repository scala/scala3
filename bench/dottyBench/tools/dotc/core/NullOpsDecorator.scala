...
import dottyBench.tools.dotc.core.Symbols.defn
...
object NullOpsDecorator {
  ...
  implicit class NullOps(val self: Type) {
    ...
    def isUncheckedNullType(using Ctx, CState): Boolean = {
     ...
     */
    def stripNull(onlyUncheckedNull: Boolean = false)(using Ctx, CState): Type = {
      assert(ctx.explicitNulls)
    ...
    def stripUncheckedNull(using Ctx, CState): Type = self.stripNull(true)
     ...
     */
    def stripAllUncheckedNull(using Ctx, CState): Type = {
    ...
    def isNullableUnion(using Ctx, CState): Boolean = {
      val stripped = self.stripNull()
      stripped ne self
    }

    /** Is self (after widening and dealiasing) a type of the form `T | UncheckedNull`? */
    def isUncheckedNullableUnion(using Ctx, CState): Boolean = {
