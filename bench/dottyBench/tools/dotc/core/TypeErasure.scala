...
object TypeErasure {
  ...
  private def erasureDependsOnArgs(sym: Symbol)(using Ctx, CState) =
    sym == defn.ArrayClass || sym == defn.PairClass

  def normalizeClass(cls: ClassSymbol)(using Ctx, CState): ClassSymbol = {
   ...
   */
  def isErasedType(tp: Type)(using Ctx, CState): Boolean = tp match {
  ...
  object ErasedValueType {
    def apply(tycon: TypeRef, erasedUnderlying: Type)(using Ctx, CState): ErasedValueType = {
  ...
  */
  def erasure(tp: Type)(using Ctx, CState): Type = beforeErasure(
    erasureFn(isJava = false, semiEraseVCs = false, isConstructor = false, wildcardOK = false)(tp))
   ...
   *  @param tp            The type to erase.
   */
  def valueErasure(tp: Type)(using Ctx, CState): Type = beforeErasure(
    erasureFn(isJava = false, semiEraseVCs = true, isConstructor = false, wildcardOK = false)(tp))

  /** Like value class erasure, but value classes erase to their underlying type erasure */
  def fullErasure(tp: Type)(using Ctx, CState): Type =
    valueErasure(tp) match
      case ErasedValueType(_, underlying) => erasure(underlying)
      case etp => etp

  def sigName(tp: Type, isJava: Boolean)(using Ctx, CState): TypeName = {
   ...
   */
  def transformInfo(sym: Symbol, tp: Type)(using Ctx, CState): Type = {
    ...
    else if (sym.isConstructor) outer.addParam(sym.owner.asClass, beforeErasure(erase(tp)))
    else if (sym.is(Label)) beforeErasure(erase.eraseResult(sym.info))
    else beforeErasure(erase.eraseInfo(tp, sym)) match {
   ...
   */
  def classify(tp: Type)(using Ctx, CState): Type =
   ...
   */
  def isUnboundedGeneric(tp: Type)(using Ctx, CState): Boolean = tp.dealias match {
  ...
  def isGeneric(tp: Type)(using Ctx, CState): Boolean = tp.dealias match {
   ...
   */
  def erasedLub(tp1: Type, tp2: Type)(using Ctx, CState): Type = {
    ...
    tp1 match {
      case JavaArrayType(elem1) =>
        import dottyBench.tools.dotc.transform.TypeUtils._
        tp2 match {
   ...
   */
  def erasedGlb(tp1: Type, tp2: Type, isJava: Boolean)(using Ctx, CState): Type = tp1 match {
    case JavaArrayType(elem1) =>
      tp2 match {
   ...
   */
  def hasStableErasure(tp: Type)(using Ctx, CState): Boolean = tp match {
    case tp: TypeRef if !tp.symbol.isOpaqueAlias =>
      tp.info match {
...
class TypeErasure(isJava: Boolean, semiEraseVCs: Boolean, isConstructor: Boolean, wildcardOK: Boolean) {
   ...
   */
  private def apply(tp: Type)(using Ctx, CState): Type = tp match {
    case _: ErasedValueType =>
      tp
    case tp: TypeRef =>
      val sym = tp.symbol
  ...
  private def eraseArray(tp: Type)(using Ctx, CState) = {
  ...
  private def erasePair(tp: Type)(using Ctx, CState): Type = {
   ...
   */
  def eraseInfo(tp: Type, sym: Symbol)(using Ctx, CState): Type =
    val tp1 = tp match
  ...
  private def eraseDerivedValueClassRef(tref: TypeRef)(using Ctx, CState): Type = {
  ...
  private def eraseNormalClassRef(tref: TypeRef)(using Ctx, CState): Type = {
    val cls = tref.symbol.asClass
    (if (cls.owner.is(Package)) normalizeClass(cls) else cls).typeRef
  }

  /** The erasure of a function result type. */
  private def eraseResult(tp: Type)(using Ctx, CState): Type = tp match {
    case tp: TypeRef =>
   ...
   */
  private def sigName(tp: Type)(using Ctx, CState): TypeName = try
    tp match {
      case tp: TypeRef =>
        if (!tp.denot.exists)
