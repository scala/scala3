...
class Definitions {
  ...
  private var initState: CState = _

  private given currentContext[Dummy_so_its_a_def] as Context = initCtx
  private given currentCState[Dummy_so_its_a_def] as CState = initState
   ...
   *  objects in scala so that they can be used from dottyBench. They will
  ...
  def SubTypeClass(using Ctx, CState): ClassSymbol = requiredClass("scala.<:<")
  @tu lazy val SubType_refl: Symbol = SubTypeClass.companionModule.requiredMethod(nme.refl)

  def DummyImplicitClass(using Ctx, CState): ClassSymbol = requiredClass("scala.DummyImplicit")
  ...
  def DottyPredefModule: Symbol = {
    if (myDottyPredefModule == null) {
      myDottyPredefModule = requiredModule("dottyBench.DottyPredef")
  ...
  @tu lazy val DottyArraysModule: Symbol = requiredModule("dottyBench.runtime.Arrays")
    def newGenericArrayMethod(using Ctx, CState): TermSymbol = DottyArraysModule.requiredMethod("newGenericArray")
    def newArrayMethod(using Ctx, CState): TermSymbol = DottyArraysModule.requiredMethod("newArray")
  ...
  def SeqClass(using Ctx, CState): ClassSymbol = SeqType.symbol.asClass
  ...
  def ArrayClass(using Ctx, CState): ClassSymbol = ArrayType.symbol.asClass
  ...
  def UnitClass(using Ctx, CState): ClassSymbol = UnitType.symbol.asClass
  def UnitModuleClass(using Ctx, CState): Symbol = UnitType.symbol.asClass.linkedClass
  ...
  def BooleanClass(using Ctx, CState): ClassSymbol = BooleanType.symbol.asClass
  ...
  def LongClass(using Ctx, CState): ClassSymbol = LongType.symbol.asClass
  ...
  def FloatClass(using Ctx, CState): ClassSymbol = FloatType.symbol.asClass
  @tu lazy val DoubleType: TypeRef = valueTypeRef("scala.Double", java.lang.Double.TYPE, DoubleEnc, nme.specializedTypeNames.Double)
  def DoubleClass(using Ctx, CState): ClassSymbol = DoubleType.symbol.asClass

  @tu lazy val BoxedUnitClass: ClassSymbol = requiredClass("scala.runtime.BoxedUnit")
    def BoxedUnit_UNIT(using Ctx, CState): TermSymbol = BoxedUnitClass.linkedClass.requiredValue("UNIT")
    def BoxedUnit_TYPE(using Ctx, CState): TermSymbol = BoxedUnitClass.linkedClass.requiredValue("TYPE")
  ...
  def ThrowableClass(using Ctx, CState): ClassSymbol = ThrowableType.symbol.asClass
  @tu lazy val SerializableType: TypeRef       = JavaSerializableClass.typeRef
  def SerializableClass(using Ctx, CState): ClassSymbol = SerializableType.symbol.asClass
    ...
    cls.infoOrCompleter match {
      case completer: ClassfileLoader =>
        cls.info = new ClassfileLoader(completer.classfile) {
          override def complete(root: SymDenotation)(using Ctx, CState): Unit = {
            super.complete(root)
  ...
  @tu lazy val InternalStringContextMacroModule: Symbol = requiredModule("dottyBench.internal.StringContextMacro")
  ...
  def IteratorModule(using Ctx, CState): Symbol = IteratorClass.companionModule
  ...
  @tu lazy val EqlClass: ClassSymbol = requiredClass("scala.Eql")
    def Eql_eqlAny(using Ctx, CState): TermSymbol = EqlClass.companionModule.requiredMethod(nme.eqlAny)
  ...
  @tu lazy val StatsModule: Symbol = requiredModule("dottyBench.tools.dotc.util.Stats")
  ...
  def TupleClass(using Ctx, CState): ClassSymbol = TupleTypeRef.symbol.asClass
  ...
  def NonEmptyTupleClass(using Ctx, CState): ClassSymbol = NonEmptyTupleTypeRef.symbol.asClass
  ...
  def TupleXXLModule(using Ctx, CState): Symbol = TupleXXLClass.companionModule

    def TupleXXL_fromIterator(using Ctx, CState): Symbol = TupleXXLModule.requiredMethod("fromIterator")
  ...
  def TupledFunctionClass(using Ctx, CState): ClassSymbol = TupledFunctionTypeRef.symbol.asClass

  @tu lazy val InternalTupledFunctionTypeRef: TypeRef = requiredClassRef("scala.internal.TupledFunction")
  def InternalTupleFunctionClass(using Ctx, CState): ClassSymbol = InternalTupledFunctionTypeRef.symbol.asClass
  def InternalTupleFunctionModule(using Ctx, CState): Symbol = requiredModule("scala.internal.TupledFunction")
  ...
  object FunctionOf {
    def apply(args: List[Type], resultType: Type, isContextual: Boolean = false, isErased: Boolean = false)(using Ctx, CState): Type =
      FunctionType(args.length, isContextual, isErased).appliedTo(args ::: resultType :: Nil)
    def unapply(ft: Type)(using Ctx, CState): Option[(List[Type], Type, Boolean, Boolean)] = {
  ...
  object PartialFunctionOf {
    def apply(arg: Type, result: Type)(using Ctx, CState): Type =
      PartialFunctionClass.typeRef.appliedTo(arg :: result :: Nil)
    def unapply(pft: Type)(using Ctx, CState): Option[(Type, List[Type])] =
  ...
  object ArrayOf {
    def apply(elem: Type)(using Ctx, CState): Type =
      if (currentlyAfterErasure) JavaArrayType(elem)
      else ArrayType.appliedTo(elem :: Nil)
    def unapply(tp: Type)(using Ctx, CState): Option[Type] = tp.dealias match {
  ...
  object MatchCase {
    def apply(pat: Type, body: Type)(using Ctx, CState): Type =
      MatchCaseClass.typeRef.appliedTo(pat, body)
    def unapply(tp: Type)(using Ctx, CState): Option[(Type, Type)] = tp match {
      case AppliedType(tycon, pat :: body :: Nil) if tycon.isRef(MatchCaseClass) =>
        Some((pat, body))
      case _ =>
        None
    }
    def isInstance(tp: Type)(using Ctx, CState): Boolean = tp match {
   ...
   */
  object MultiArrayOf {
    def apply(elem: Type, ndims: Int)(using Ctx, CState): Type =
      if (ndims == 0) elem else ArrayOf(apply(elem, ndims - 1))
    def unapply(tp: Type)(using Ctx, CState): Option[(Type, Int)] = tp match {
      case ArrayOf(elemtp) =>
  ...
  final def isCompiletime_S(sym: Symbol)(using Ctx, CState): Boolean =
  ...
  final def isCompiletimeAppliedType(sym: Symbol)(using Ctx, CState): Boolean = {
  ...
  def AbstractFunctionClass(n: Int)(using Ctx, CState): Symbol = AbstractFunctionClassPerRun()(using ctx)(n)
  ...
  def FunctionClass(n: Int, isContextual: Boolean = false, isErased: Boolean = false)(using Ctx, CState): Symbol =
  ...
  def FunctionType(n: Int, isContextual: Boolean = false, isErased: Boolean = false)(using Ctx, CState): TypeRef =
  ...
  def scalaClassName(cls: Symbol)(using Ctx, CState): TypeName =
    if (cls.isClass && cls.owner == ScalaPackageClass) cls.asClass.name else EmptyTypeName

  /** If type `ref` refers to a class in the scala package, its name, otherwise EmptyTypeName */
  def scalaClassName(ref: Type)(using Ctx, CState): TypeName = scalaClassName(ref.classSymbol)
  ...
  def isTupleType(tp: Type)(using Ctx, CState): Boolean = {
  ...
  def tupleTypes(tp: Type, bound: Int = Int.MaxValue)(using Ctx, CState): Option[List[Type]] = {
  ...
  def isProductSubType(tp: Type)(using Ctx, CState): Boolean = tp.derivesFrom(ProductClass)

  /** Is `tp` (an alias) of either a scala.FunctionN or a scala.ContextFunctionN
   *  instance?
   */
  def isNonRefinedFunction(tp: Type)(using Ctx, CState): Boolean = {
  ...
  def isFunctionType(tp: Type)(using Ctx, CState): Boolean =
  ...
  def isSpecializableFunction(cls: ClassSymbol, paramTypes: List[Type], retType: Type)(using Ctx, CState): Boolean =
  ...
  def functionArity(tp: Type)(using Ctx, CState): Int = tp.dropDependentRefinement.dealias.argInfos.length - 1
   ...
   */
  def asContextFunctionType(tp: Type)(using Ctx, CState): Type =
  ...
  def isContextFunctionType(tp: Type)(using Ctx, CState): Boolean =
  ...
  object ContextFunctionType:
    def unapply(tp: Type)(using Ctx, CState): Option[(List[Type], Type, Boolean)] =
  ...
  def isErasedFunctionType(tp: Type)(using Ctx, CState): Boolean =
  ...
  def isInfix(sym: Symbol)(using Ctx, CState): Boolean =
  ...
  class PerRun[T](generate: (Ctx, CState) ?=> T) {
...
//    valueTypeNamesToJavaType(vcls.name) = jtype
  ...
  def boxedType(tp: Type)(using Ctx, CState): TypeRef = {
  ...
  def typeTag(tp: Type)(using Ctx, CState): Name = typeTags(scalaClassName(tp))

//  /** The `Class[?]` of a primitive value type name */
//  def valueTypeNameToJavaType(name: TypeName)(using Ctx, CState): Option[Class[?]] =
//    valueTypeNamesToJavaType.get(if (name.firstPart eq nme.scala) name.lastPart.toTypeName else name)
  ...
  def isValueSubType(tref1: TypeRef, tref2: TypeRef)(using Ctx, CState): Boolean =
  ...
  def init()(using c: Context): Unit = {
    this.initCtx = c
    this.initState = c.cstate
    if (!isInitialized) {
