...
import util.Spans._
import dottyBench.tools.dotc.ast.{tpd, untpd}, ast.tpd._
...
import scala.annotation.{switch, constructorOnly}
import reporting._
...
object Scala2Unpickler {
  ...
  def translateTempPoly(tp: Type)(using Ctx, CState): Type = tp match {
  ...
  def addConstructorTypeParams(denot: SymDenotation)(using Ctx, CState): Unit = {
   ...
   */
  def arrayToRepeated(tp: Type)(using Ctx, CState): Type = tp match {
    case tp: MethodType =>
  ...
  def ensureConstructor(cls: ClassSymbol, scope: Scope)(using Ctx, CState): Unit = {
  ...
  def setClassInfo(denot: ClassDenotation, info: Type, fromScala2: Boolean, selfInfo: Type = NoType)(using Ctx, CState): Unit = {
 ...
 */
class Scala2Unpickler(bytes: Array[Byte], classRoot: ClassDenotation, moduleClassRoot: ClassDenotation)(
  using @constructorOnly ictx: Ctx, @constructorOnly ics: CState)
  ...
  val moduleRoot: SymDenotation = moduleClassRoot.sourceModule.denot
  ...
  protected def errorBadSignature(msg: String, original: Option[RuntimeException] = None)(using Ctx, CState): Nothing = {
  ...
  protected def handleRuntimeException(ex: RuntimeException)(using Ctx, CState): Nothing = ex match {
  ...
  protected def isSymbolEntry(i: Int)(using Ctx, CState): Boolean = {
   ...
   */
  protected def isRefinementSymbolEntry(i: Int)(using Ctx, CState): Boolean = {
    val savedIndex = readIndex
    readIndex = index(i)
  ...
  protected def isRefinementClass(sym: Symbol)(using Ctx, CState): Boolean =
    sym.name == tpnme.REFINE_CLASS

  protected def isLocal(sym: Symbol)(using Ctx, CState): Boolean = isUnpickleRoot(sym.topLevelClass)

  protected def isUnpickleRoot(sym: Symbol)(using Ctx, CState): Boolean = {
    val d = sym.denot
  ...
  /** Read a name */
  protected def readName()(using Ctx, CState): Name = {
    val tag = readByte()
    val len = readNat()
    tag match {
  ...
  protected def readTermName()(using Ctx, CState): TermName = readName().toTermName
  protected def readTypeName()(using Ctx, CState): TypeName = readName().toTypeName

  /** Read a symbol */
  protected def readSymbol()(using Ctx, CState): Symbol = readDisambiguatedSymbol(alwaysTrue)()

  /** Read a symbol, with possible disambiguation */
  protected def readDisambiguatedSymbol(p: Symbol => Boolean)()(using Ctx, CState): Symbol = {
  ...
  class LocalUnpickler extends LazyType {
    ...
    def complete(denot: SymDenotation)(using Ctx, CState): Unit = try {
      def parseToCompletion(denot: SymDenotation)(using Ctx, CState) = {
        val tag = readByte()
        val end = readNat() + readIndex
        def atEnd = readIndex == end
  ...
  class ClassUnpickler(infoRef: Int) extends LocalUnpickler with TypeParamsCompleter {
    private def readTypeParams()(using Ctx, CState): List[TypeSymbol] = {
      val tag = readByte()
      val end = readNat() + readIndex
      if (tag == POLYtpe) {
    ...
    private def loadTypeParams(using Ctx, CState) =
    ...
    def init()(using Ctx, CState): List[TypeSymbol] = loadTypeParams
   ...
   *    tp { name: T }
   */
  def elimExistentials(boundSyms: List[Symbol], tp: Type)(using Ctx, CState): Type = {
   ...
   */
  private def readPrefix()(using Ctx, CState): Type = readTypeRef() match {
   ...
   */
  protected def readType()(using Ctx, CState): Type = {
    val tag = readByte()
    val end = readNat() + readIndex
    (tag: @switch) match {
      case NOtpe =>
  ...
  def readTypeParams()(using Ctx, CState): List[Symbol] = {
    val tag = readByte()
    val end = readNat() + readIndex
    if (tag == POLYtpe) {
      val unusedRestperef = readNat()
  ...
  def noSuchTypeTag(tag: Int, end: Int)(using Ctx, CState): Type =
    errorBadSignature("bad type tag: " + tag)

  /** Read a constant */
  protected def readConstant()(using Ctx, CState): Constant = {
    val tag = readByte().toInt
    val len = readNat()
    (tag: @switch) match {
  ...
  def noSuchConstantTag(tag: Int, len: Int)(using Ctx, CState): Constant =
    errorBadSignature("bad constant tag: " + tag)

  /** Read children and store them into the corresponding symbol.
   */
  protected def readChildren()(using Ctx, CState): Unit = {
    val tag = readByte()
    assert(tag == CHILDREN)
    val end = readNat() + readIndex
  ...
  protected def readSymbolRef()(using Ctx, CState): Symbol = { //OPT inlined from: at(readNat(), readSymbol) to save on closure creation
    val i = readNat()
    var r = entries(i)
    if (r eq null) {
      val savedIndex = readIndex
  ...
  protected def readDisambiguatedSymbolRef(p: Symbol => Boolean)(using Ctx, CState): Symbol =
    at(readNat(), () => readDisambiguatedSymbol(p)())

  protected def readNameRef()(using Ctx, CState): Name = at(readNat(), () => readName())
  protected def readTypeRef()(using Ctx, CState): Type = at(readNat(), () => readType()) // after the NMT_TRANSITION period, we can leave off the () => ... ()
  protected def readConstantRef()(using Ctx, CState): Constant = at(readNat(), () => readConstant())

  protected def readTypeNameRef()(using Ctx, CState): TypeName = readNameRef().toTypeName
  protected def readTermNameRef()(using Ctx, CState): TermName = readNameRef().toTermName

  protected def readAnnotationRef()(using Ctx, CState): Annotation = at(readNat(), () => readAnnotation())

  protected def readModifiersRef(isType: Boolean)(using Ctx, CState): Modifiers = at(readNat(), () => readModifiers(isType))
  protected def readTreeRef()(using Ctx, CState): Tree = at(readNat(), () => readTree())

  /** Read an annotation argument, which is pickled either
   *  as a Constant or a Tree.
   */
  protected def readAnnotArg(i: Int)(using Ctx, CState): Tree = bytes(index(i)) match {
   ...
   */
  private def readArrayAnnotArg()(using Ctx, CState): Tree = {
  ...
  private def readAnnotInfoArg()(using Ctx, CState): Tree = {
    readByte() // skip the `annotinfo` tag
    val end = readNat() + readIndex
    readAnnotationContents(end)
  }

  protected def readClassfileAnnotArg(i: Int)(using Ctx, CState): Tree = bytes(index(i)) match {
   ...
   */
  protected def readAnnotationContents(end: Int)(using Ctx, CState): Tree = {
    val atp = readTypeRef()
    val args = {
   ...
   */
  protected def readSymbolAnnotation()(using Ctx, CState): Unit = {
    val tag = readByte()
    if (tag != SYMANNOT)
   ...
   */
  protected def readAnnotation()(using Ctx, CState): Annotation = {
    val tag = readByte()
    if (tag != ANNOTINFO)
   ...
   */
  protected def deferredAnnot(end: Int)(using Ctx, CState): Annotation = {
    val start = readIndex
    val atp = readTypeRef()
  ...
  protected def readTree()(using Ctx, CState): Tree = {
...
*/
  ...
  def noSuchTreeTag(tag: Int, end: Int)(using Ctx, CState): Nothing =
    errorBadSignature("unknown tree type (" + tag + ")")

  def unimplementedTree(what: String)(using Ctx, CState): Nothing =
    errorBadSignature(s"cannot read $what trees from Scala 2.x signatures")

  def readModifiers(isType: Boolean)(using Ctx, CState): Modifiers = {
    val tag = readNat()
    if (tag != MODIFIERS)
  ...
  protected def readTemplateRef()(using Ctx, CState): Template =
    readTreeRef() match {
      case templ: Template => templ
      case other =>
        errorBadSignature("expected a template (" + other + ")")
    }
  protected def readCaseDefRef()(using Ctx, CState): CaseDef =
    readTreeRef() match {
      case tree: CaseDef => tree
      case other =>
        errorBadSignature("expected a case def (" + other + ")")
    }
  protected def readValDefRef()(using Ctx, CState): ValDef =
    readTreeRef() match {
      case tree: ValDef => tree
      case other =>
        errorBadSignature("expected a ValDef (" + other + ")")
    }
  protected def readIdentRef()(using Ctx, CState): Ident =
    readTreeRef() match {
      case tree: Ident => tree
      case other =>
        errorBadSignature("expected an Ident (" + other + ")")
    }
  protected def readTypeDefRef()(using Ctx, CState): TypeDef =
    readTreeRef() match {
