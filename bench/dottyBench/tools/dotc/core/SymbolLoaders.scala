...
import dottyBench.tools.io.{ ClassPath, ClassRepresentation, AbstractFile }
...
object SymbolLoaders {
  ...
  private def enterNew(
      ...
      completer: SymbolLoader, scope: Scope = EmptyScope)(using Ctx, CState): Symbol = {
    val comesFromScan =
  ...
  def enterClass(
      ...
      flags: FlagSet = EmptyFlags, scope: Scope = EmptyScope)(using Ctx, CState): Symbol = {
  ...
  def enterModule(
      owner: Symbol, name: PreName, completer: SymbolLoader,
      modFlags: FlagSet = EmptyFlags, clsFlags: FlagSet = EmptyFlags, scope: Scope = EmptyScope)(using Ctx, CState): Symbol = {
   ...
   */
  def enterPackage(owner: Symbol, pname: TermName, completer: (TermSymbol, ClassSymbol) => PackageLoader)(using Ctx, CState): Symbol = {
  ...
  def enterClassAndModule(
      owner: Symbol, name: PreName, completer: SymbolLoader,
      flags: FlagSet = EmptyFlags, scope: Scope = EmptyScope)(using Ctx, CState): Unit = {
  ...
  def enterToplevelsFromSource(
      ...
      scope: Scope = EmptyScope)(using Ctx, CState): Unit =
    if src.exists && !src.isDirectory
      ...
      def enterScanned(unit: CompilationUnit)(using Ctx, CState) = {
   ...
   */
  def binaryOnly(owner: Symbol, name: String)(using Ctx, CState): Boolean =
    name == "package" &&
      (owner.fullName.toString == "scala" || owner.fullName.toString == "scala.reflect")

  /** Initialize toplevel class and module symbols in `owner` from class path representation `classRep`
   */
  def initializeFromClassPath(owner: Symbol, classRep: ClassRepresentation)(using Ctx, CState): Unit =
  ...
  class PackageLoader(_sourceModule: TermSymbol, classPath: ClassPath)
    ...
    override def sourceModule(using Ctx, CState): TermSymbol = _sourceModule
    def description(using Ctx, CState): String = "package loader " + sourceModule.fullName

    private var enterFlatClasses: Option[() => (Ctx, CState) ?=> Unit] = None
    ...
    final class PackageScope extends MutableScope {
      override def newScopeEntry(name: Name, sym: Symbol)(using Ctx, CState): ScopeEntry =
      ...
      override def ensureComplete()(using Ctx, CState): Unit =
    ...
    private def enterClasses(root: SymDenotation, packageName: String, flat: Boolean)(using Ctx, CState) = {
    ...
    def doComplete(root: SymDenotation)(using Ctx, CState): Unit = {
      ...
      enterFlatClasses = Some { () =>
        ...
        inCtxAndState(ctx, currentPeriod){enterClasses(root, packageName, flat = true)}
      }
...
abstract class SymbolLoader extends LazyType { self =>
   ...
   */
  def description(using Ctx, CState): String
  ...
  def proxy: SymbolLoader = new SymbolLoader {
    ...
    def description(using Ctx, CState): String = "proxy to ${self.description}"
  }
  ...
  protected def rootDenots(rootDenot: ClassDenotation)(using Ctx, CState): (ClassDenotation, ClassDenotation) = {
...
class ClassfileLoader(val classfile: AbstractFile) extends SymbolLoader {
  ...
  def description(using Ctx, CState): String = "class file " + classfile.toString

  override def doComplete(root: SymDenotation)(using Ctx, CState): Unit =
    load(root)

  def load(root: SymDenotation)(using Ctx, CState): Unit = {
  ...
  private def mayLoadTreesFromTasty(using Ctx, CState): Boolean =
    ctx.settings.YretainTrees.value || ctx.settings.fromTasty.value
}

class SourcefileLoader(val srcfile: AbstractFile) extends SymbolLoader {
  def description(using Ctx, CState): String = "source file " + srcfile.toString
...
class NoLoader extends SymbolLoader with NoCompleter {
  def description(using Ctx, CState): String = "NoLoader"
  override def sourceFileOrNull: AbstractFile = null
  override def complete(root: SymDenotation)(using Ctx, CState): Unit =
