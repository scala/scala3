...
import Contexts._, Symbols._, Types._, Flags._, Scopes._, Decorators._, NameOps._, ContextOps._
...
object NamerOps:
   ...
   */
  def effectiveResultType(sym: Symbol, typeParams: List[Symbol], givenTp: Type)(using Ctx, CState): Type =
    if (sym.name == nme.CONSTRUCTOR) sym.owner.typeRef.appliedTo(typeParams.map(_.typeRef))
    else givenTp

  /** if isConstructor, make sure it has one non-implicit parameter list */
  def normalizeIfConstructor(termParamss: List[List[Symbol]], isConstructor: Boolean)(using Ctx, CState): List[List[Symbol]] =
    if (isConstructor &&
  ...
  def methodType(typeParams: List[Symbol], valueParamss: List[List[Symbol]], resultType: Type, isJava: Boolean = false)(using Ctx, CState): Type =
    val monotpe =
   ...
   */
  def adjustModuleCompleter(completer: LazyType, name: Name)(using Ctx, CState): LazyType =
  ...
  private def findModuleBuddy(name: Name, scope: Scope)(using Ctx, CState) = {
