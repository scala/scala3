...
object Comments {
  ...
  given CommentsContext as AnyRef:
    extension (c: Ctx) def docCtx: Option[ContextDocstrings] = c.property(ContextDoc)
  ...
  final case class Comment(span: Span, raw: String, expanded: Option[String], usecases: List[UseCase]) {
     ...
     */
    def expand(f: String => String)(using Ctx, CState): Comment = {
  ...
  object Comment {
    ...
    private def parseUsecases(expandedComment: String, span: Span)(using Ctx, CState): List[UseCase] =
     ...
     *  }}}
     */
    private def decomposeUseCase(body: String, span: Span, start: Int, end: Int)(using Ctx, CState): UseCase = {
  ...
  object UseCase {
    def apply(code: String, codePos: Span)(using Ctx, CState): UseCase = {
      val tree = {
  ...
  class CommentExpander {
    ...
    def expand(sym: Symbol, site: Symbol)(using Ctx, CState): String = {
     ...
     */
    def expandedDocComment(sym: Symbol, site: Symbol, docStr: String = "")(using Ctx, CState): String = {
    ...
    protected def superComment(sym: Symbol)(using Ctx, CState): Option[String] =
     ...
     */
    def cookedDocComment(sym: Symbol, docStr: String = "")(using Ctx, CState): String = cookedDocComments.getOrElseUpdate(sym, {
      var ownComment =
    ...
    protected def expandVariables(initialStr: String, sym: Symbol, site: Symbol)(using Ctx, CState): String = {
      val expandLimit = 10
    ...
    def defineVariables(sym: Symbol)(using Ctx, CState): Unit = {
      val Trim = "(?s)^[\\s&&[^\n\r]]*(.*?)\\s*$".r
     ...
     *  @param site  The class for which doc comments are generated
     */
    def lookupVariable(vble: String, site: Symbol)(using Ctx, CState): Option[String] = site match {
      case NoSymbol => None
      case _        =>
        val searchList =
     ...
     */
    def docCommentPos(sym: Symbol)(using Ctx, CState): Span =
     ...
     *  since r23926.
     */
    private def allInheritedOverriddenSymbols(sym: Symbol)(using Ctx, CState): List[Symbol] =
