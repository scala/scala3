...
object TypeOps:
   ...
   *  for what this means.
   */
  final def asSeenFrom(tp: Type, pre: Type, cls: Symbol)(using Ctx, CState): Type = {
    pre match {
  ...
  class AsSeenFromMap(pre: Type, cls: Symbol)(using Ctx, CState) extends ApproximatingTypeMap {
  ...
  def isLegalPrefix(pre: Type)(using Ctx, CState): Boolean =
    pre.isStable || !currentPhase.isTyper

  /** Implementation of Types#simplified */
  def simplify(tp: Type, theMap: SimplifyMap)(using Ctx, CState): Type = {
  ...
  class SimplifyMap(using Ctx, CState) extends TypeMap {
   ...
   */
  def orDominator(tp: Type)(using Ctx, CState): Type = {
   ...
   */
  def classBound(info: ClassInfo)(using Ctx, CState): Type = {
    val cls = info.cls
   ...
   */
  def avoid(tp: Type, symsToAvoid: => List[Symbol])(using Ctx, CState): Type = {
    val widenMap = new ApproximatingTypeMap {
      ...
      override def isStaticPrefix(pre: Type)(using Ctx, CState): Boolean = pre match
        case pre: NamedType =>
   ...
   *      p.package.x
   */
  def makePackageObjPrefixExplicit(tpe: NamedType)(using Ctx, CState): Type = {
  ...
  def boundsViolations(
      ...
      app: Type)(
      using Ctx, CState): List[BoundsViolation] = {
   ...
   */
  def refineUsingParent(parent: Type, child: Symbol)(using Ctx, CState): Type = {
   ...
   */
  private def instantiateToSubType(tp1: NamedType, tp2: Type)(using Ctx, CState): Type = {
    /** expose abstract type references to their bounds or tvars according to variance */
    class AbstractTypeMap(maximize: Boolean)(using Ctx, CState) extends TypeMap {
    ...
    def minTypeMap(using Ctx, CState) = new AbstractTypeMap(maximize = false)
    def maxTypeMap(using Ctx, CState) = new AbstractTypeMap(maximize = true)
  ...
  def nestedPairs(ts: List[Type])(using Ctx, CState): Type =
