...
class ExtractSemanticDB extends Phase:
  ...
  class Extractor extends TreeTraverser:
    ...
    private def excludeDef(sym: Symbol)(using Ctx, CState): Boolean =
      !sym.exists
      || sym.isLocalDummy
      || sym.is(Synthetic)
      || sym.isSetter
    ...
    private def excludeDefOrUse(sym: Symbol)(using Ctx, CState): Boolean =
      !sym.exists
      || sym.name.is(NameKinds.DefaultGetterName)
      || sym.isConstructor && (sym.owner.is(ModuleClass) || !sym.isGlobal)
      || excludeSymbol(sym)

    private def excludeSymbol(sym: Symbol)(using Ctx, CState): Boolean =
      !sym.exists
      || sym.name.isWildcard
      || excludeQual(sym)

    private def excludeQual(sym: Symbol)(using Ctx, CState): Boolean =
      !sym.exists
      || sym.isAnonymousFunction
      || sym.isAnonymousModuleVal
      || sym.name.isEmptyNumbered

    private def excludeChildren(sym: Symbol)(using Ctx, CState): Boolean =
      !sym.exists
      || sym.is(Param) && sym.info.bounds.hi.isInstanceOf[Types.HKTypeLambda]

    /** Uses of this symbol where the reference has given span should be excluded from semanticdb */
    private def excludeUse(qualifier: Option[Symbol], sym: Symbol)(using Ctx, CState): Boolean =
      !sym.exists
      || excludeDefOrUse(sym)
    ...
    private def traverseAnnotsOfDefinition(sym: Symbol)(using Ctx, CState): Unit =
    ...
    override def traverse(tree: Tree)(using Ctx, CState): Unit =
    ...
    private def funParamSymbol(funSym: Symbol)(using Ctx, CState): Name => String =
    ...
    private object PatternValDef:
      ...
      def unapply(tree: ValDef)(using Ctx, CState): Option[(Tree, Tree)] = tree.rhs match
      ...
      extension (tpe: Types.Type):
        private inline def isAnnotatedByUnchecked(using Ctx, CState) = tpe match
    ...
    extension (tree: NamedDefTree):
      private def adjustedNameSpan(using Ctx, CState): Span =
    ...
    private def addSymName(b: StringBuilder, sym: Symbol)(using Ctx, CState): Unit =
       ...
       */
      def localIdx(sym: Symbol)(using Ctx, CState): Int =
        val startPos =
    ...
    private def symbolName(sym: Symbol)(using Ctx, CState): String =
      val b = StringBuilder(20)
      addSymName(b, sym)
      b.toString

    private def range(span: Span, treeSource: SourceFile)(using Ctx, CState): Option[Range] =
    ...
    private def symbolKind(sym: Symbol, symkinds: Set[SymbolKind])(using Ctx, CState): SymbolInformation.Kind =
      if sym.isTypeParam
    ...
    private def symbolProps(sym: Symbol, symkinds: Set[SymbolKind])(using Ctx, CState): Int =
    ...
    private def symbolInfo(sym: Symbol, symbolName: String, symkinds: Set[SymbolKind])(using Ctx, CState): SymbolInformation =
      SymbolInformation(
        symbol = symbolName,
    ...
    private def registerSymbol(sym: Symbol, symbolName: String, symkinds: Set[SymbolKind])(using Ctx, CState): Unit =
    ...
    private def registerSymbolSimple(sym: Symbol)(using Ctx, CState): Unit =
      registerSymbol(sym, symbolName(sym), Set.empty)

    private def registerOccurrence(symbol: String, span: Span, role: SymbolOccurrence.Role, treeSource: SourceFile)(using Ctx, CState): Unit =
      val occ = SymbolOccurrence(symbol, range(span, treeSource), role)
      if !generated.contains(occ) && occ.symbol.nonEmpty then
        occurrences += occ
        generated += occ

    private def registerUseGuarded(qualSym: Option[Symbol], sym: Symbol, span: Span, treeSource: SourceFile)(using Ctx, CState) =
      if !excludeUse(qualSym, sym) then
        registerUse(sym, span, treeSource)

    private def registerUse(sym: Symbol, span: Span, treeSource: SourceFile)(using Ctx, CState): Unit =
      registerUse(symbolName(sym), span, treeSource)

    private def registerUse(symbol: String, span: Span, treeSource: SourceFile)(using Ctx, CState): Unit =
      registerOccurrence(symbol, span, SymbolOccurrence.Role.REFERENCE, treeSource)

    private def registerDefinition(sym: Symbol, span: Span, symkinds: Set[SymbolKind], treeSource: SourceFile)(using Ctx, CState) =
    ...
    private def spanOfSymbol(sym: Symbol, span: Span, treeSource: SourceFile)(using Ctx, CState): Span =
    ...
    extension (tree: DefDef):
      private def isSetterDef(using Ctx, CState): Boolean =
        tree.name.isSetterName && tree.mods.is(Accessor) && tree.vparamss.isSingleArg

    private def findGetters(ctorParams: Set[Names.TermName], body: List[Tree])(using Ctx, CState): Map[Names.TermName, ValDef] =
    ...
    extension (sym: Symbol):
      private def adjustIfCtorTyparam(using Ctx, CState) =
        if sym.isType && sym.owner.exists && sym.owner.isConstructor
          matchingMemberType(sym, sym.owner.owner)
        else
          sym

    private inline def matchingMemberType(ctorTypeParam: Symbol, classSym: Symbol)(using Ctx, CState) =
     ...
     */
    private def symbolKinds(tree: NamedDefTree)(using Ctx, CState): Set[SymbolKind] =
      if tree.symbol.isSelfSym
        Set.empty
      else
    ...
    private def ctorParams(
      vparamss: List[List[ValDef]], body: List[Tree])(using Ctx, CState): Unit =
...
object ExtractSemanticDB:
  ...
  def write(source: SourceFile, occurrences: List[SymbolOccurrence], symbolInfos: List[SymbolInformation])(using Ctx, CState): Unit =
