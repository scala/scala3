...
import dottyBench.tools.dotc.core
...
object Scala3:
  ...
  object Symbols:
    ...
    def displaySymbol(symbol: Symbol)(using Ctx, CState): String =
  ...
  given SymbolOps as AnyRef:
    extension (sym: Symbol):
      ...
      def ifExists(using Ctx, CState): Option[Symbol] = if sym.exists then Some(sym) else None

      def isScala2PackageObject(using Ctx, CState): Boolean =
        sym.name.isScala2PackageObjectName && sym.owner.is(Package) && sym.is(Module)

      def isAnonymous(using Ctx, CState): Boolean =
        sym.isAnonymousClass
        || sym.isAnonymousModuleVal
        || sym.isAnonymousFunction

      def matchingSetter(using Ctx, CState): Symbol =
