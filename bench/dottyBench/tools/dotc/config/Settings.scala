...
import dottyBench.tools.io.{ AbstractFile, Directory, JarArchive, PlainDirectory }
...
object Settings {
  ...
  object Setting {
    implicit class SettingDecorator[T](val setting: Setting[T]) extends AnyVal {
      def value(using Ctx): T = setting.valueIn(summon[Ctx].settingsState)
      def update(x: T)(using Ctx): SettingsState = setting.updateIn(summon[Ctx].settingsState, x)
      def isDefault(using Ctx): Boolean = setting.isDefaultIn(summon[Ctx].settingsState)
    }
  }
