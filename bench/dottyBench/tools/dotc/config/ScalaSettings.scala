...
import dottyBench.tools.io.{ Directory, PlainDirectory, AbstractFile }
...
class ScalaSettings extends Settings.SettingGroup {
  ...
  val XmaxInlinedTrees: Setting[Int] = IntSetting("-Xmax-inlined-trees", "Maximal number of inlined trees.", 40_000_000)
  ...
  object mixinForwarderChoices {
    def isTruthy(using Ctx, CState) = XmixinForceForwarders.value == "true"
    def isAtLeastJunit(using Ctx, CState) = isTruthy || XmixinForceForwarders.value == "junit"
  }
