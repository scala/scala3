...
import core.Mode
import dottyBench.tools.dotc.core.Symbols.{Symbol, NoSymbol}
import Diagnostic._
...
object Reporter {
  ...
  @sharable object NoReporter extends Reporter {
    ...
    override def report(dia: Diagnostic)(using Ctx, CState): Unit = ()
  }

  type ErrorHandler = (Diagnostic, Context) => Unit

  private val defaultIncompleteHandler: ErrorHandler =
    (mc, ctx) => ctx.reporter.report(mc)(using ctx, ctx.cstate)
...
abstract class Reporter extends interfaces.ReporterResult {
   ...
   */
  def reportsErrorsFor(op: (Ctx, CState) ?=> Unit)(using Ctx, CState): Boolean = {
  ...
  def report(dia: Diagnostic)(using Ctx, CState): Unit =
    if (!isHidden(dia)) {
      withMode(Mode.Printing)(doReport(dia)(using combinedContext))
      dia match {
        case dia: ConditionalWarning if !dia.enablingOption.value =>
  ...
  def incomplete(dia: Diagnostic)(using Ctx, CState): Unit =
    incompleteHandler(dia, combinedContext)
  ...
  def isHidden(dia: Diagnostic)(using Ctx, CState): Boolean =
