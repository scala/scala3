...
trait MessageRendering {
  ...
  /** When inlining a method call, if there's an error we'd like to get the
    ...
    */
  def outer(pos: SourcePosition, prefix: String)(using Ctx, CState): List[String] =
    if (pos.outer.exists)
  ...
  /** Get the sourcelines before and after the position, as well as the offset
    ...
    */
  def sourceLines(pos: SourcePosition, diagnosticLevel: String)(using Ctx, CState): (List[String], List[String], Int) = {
  ...
  def columnMarker(pos: SourcePosition, offset: Int, diagnosticLevel: String)(using Ctx, CState): String = {
    val prefix = " " * (offset - 1)
  ...
  /** The separator between errors containing the source file and error type
    ...
    */
  def posStr(pos: SourcePosition, diagnosticLevel: String, message: Message)(using Ctx, CState): String =
  ...
  def hl(diagnosticLevel: String)(str: String)(using Ctx, CState): String = diagnosticLevel match {
