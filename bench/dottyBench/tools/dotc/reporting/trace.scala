...
/** Exposes the {{{ trace("question") { op } }}} syntax.
  ...
  * Tracing depends on [[Config.tracingEnabled]] and [[dottyBench.tools.dotc.config.ScalaSettings.Ylog]].
...
abstract class TraceSyntax {
  ...
  inline def onDebug[TD](inline question: String)(inline op: TD)(using Ctx, CState): TD =
    conditionally(ctx.settings.YdebugTrace.value, question, false)(op)

  inline def conditionally[TC](inline cond: Boolean, inline question: String, inline show: Boolean)(op: => TC)(using Ctx, CState): TC =
  ...
  inline def apply[T](inline question: String, inline printer: Printers.Printer, inline showOp: Any => String)(op: => T)(using Ctx, CState): T =
    inline if (isForced || Config.tracingEnabled) {
  ...
  inline def apply[T](inline question: String, inline printer: Printers.Printer, inline show: Boolean)(op: => T)(using Ctx, CState): T =
    inline if (isForced || Config.tracingEnabled) {
      if (!isForced && printer.eq(config.Printers.noPrinter)) op
  ...
  inline def apply[T](inline question: String, inline printer: Printers.Printer)(inline op: T)(using Ctx, CState): T =
    apply[T](question, printer, false)(op)

  inline def apply[T](inline question: String, inline show: Boolean)(inline op: T)(using Ctx, CState): T =
    apply[T](question, Printers.default, show)(op)

  inline def apply[T](inline question: String)(inline op: T)(using Ctx, CState): T =
    apply[T](question, Printers.default, false)(op)

  private def showShowable(x: Any)(using Ctx, CState) = x match {
  ...
  def apply[T](leading: => String, trailing: Any => String)(op: => T)(using Ctx, CState): T = {
  ...
  def doApply[T](leading: => String, trailing: Any => String, log: String => Unit)(op: => T)(using Ctx, CState): T =
