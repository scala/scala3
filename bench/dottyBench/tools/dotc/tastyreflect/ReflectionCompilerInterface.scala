...
import dottyBench.tools.dotc.ast.{TreeTypeMap, Trees, tpd, untpd}
import dottyBench.tools.dotc.typer.{Implicits, Typer}
...
import dottyBench.tools.dotc.core.quoted.PickledQuotes
import dottyBench.tools.dotc.core.Symbols._
import dottyBench.tools.dotc.core.Decorators._
import dottyBench.tools.dotc.core.Types.SingletonType
import dottyBench.tools.dotc.tastyreflect.FromSymbol.{definitionFromSym, packageDefFromSym}
import dottyBench.tools.dotc.typer.Implicits.{AmbiguousImplicits, DivergingImplicit, NoMatchingImplicits, SearchFailure, SearchFailureType}
import dottyBench.tools.dotc.util.{SourceFile, SourcePosition, Spans}
...
class ReflectionCompilerInterface(val rootContext: core.Contexts.Context) extends CompilerInterface {
  ...
  private given core.Contexts.CState = rootContext.cstate
  ...
  given cstateFromContext(using ctx: Context) as CState = ctx.cstate
  ...
  def Symbol_localContext(self: Symbol)(using Context): Context =
    if (self.exists) currentContext.withOwner(self)
    else currentContext
  ...
  private def withDefaultPos[T <: Tree](fn: (Context) ?=> T)(using c: Context): T =
    fn(using currentContext.withSource(rootPosition.source).toContext(using c.cstate)).withSpan(rootPosition.span)
