...
trait TreeInfo[T >: Untyped <: Type] { self: Trees.Instance[T] =>
  ...
  class MatchingArgs(params: List[Symbol], args: List[Tree])(using Ctx, CState) {
  ...
  def isRepeatedParamType(tpt: Tree)(using Ctx, CState): Boolean = tpt match {
   ...
   */
  def isWildcardStarArg(tree: Tree)(using Ctx, CState): Boolean = unbind(tree) match {
  ...
  def allParamSyms(ddef: DefDef)(using Ctx, CState): List[Symbol] =
    (ddef.tparams ::: ddef.vparamss.flatten).map(_.symbol)

  /** Does this argument list end with an argument of the form <expr> : _* ? */
  def isWildcardStarArgList(trees: List[Tree])(using Ctx, CState): Boolean =
  ...
  def catchesThrowable(cdef: CaseDef)(using Ctx, CState): Boolean =
    catchesAllOf(cdef, defn.ThrowableType)

  /** Does this CaseDef catch everything of a certain Type? */
  def catchesAllOf(cdef: CaseDef, threshold: Type)(using Ctx, CState): Boolean =
    isDefaultCase(cdef) ||
    cdef.guard.isEmpty && {
   ...
   */
  def parentsKind(parents: List[Tree])(using Ctx, CState): FlagSet = parents match {
...
trait UntypedTreeInfo extends TreeInfo[Untyped] { self: Trees.Instance[Untyped] =>
  ...
  def isContextualClosure(tree: Tree)(using Ctx, CState): Boolean = unsplice(tree) match {
   ...
   */
  def defKind(tree: Tree)(using Ctx, CState): FlagSet = unsplice(tree) match {
   ...
   */
  def bodyKind(body: List[Tree])(using Ctx, CState): FlagSet =
  ...
  object IdPattern {
    def unapply(tree: Tree)(using Ctx, CState): Option[VarInfo] = tree match {
...
trait TypedTreeInfo extends TreeInfo[Type] { self: Trees.Instance[Type] =>
   ...
   *            Impure      otherwise
   */
  def statPurity(tree: Tree)(using Ctx, CState): PurityLevel = unsplice(tree) match {
    case EmptyTree
       | TypeDef(_, _)
       | Import(_, _)
       | DefDef(_, _, _, _, _) =>
   ...
   */
  def exprPurity(tree: Tree)(using Ctx, CState): PurityLevel = unsplice(tree) match {
    case EmptyTree
       | This(_)
       | Super(_, _)
       | Literal(_) =>
  ...
  def isPurePath(tree: Tree)(using Ctx, CState): Boolean = tree.tpe match {
    case tpe: ConstantType => exprPurity(tree) >= Pure
    case _ => exprPurity(tree) == PurePath
  }

  def isPureExpr(tree: Tree)(using Ctx, CState): Boolean =
    exprPurity(tree) >= Pure

  def isIdempotentPath(tree: Tree)(using Ctx, CState): Boolean = tree.tpe match {
    case tpe: ConstantType => exprPurity(tree) >= Idempotent
    case _ => exprPurity(tree) >= IdempotentPath
  }

  def isIdempotentExpr(tree: Tree)(using Ctx, CState): Boolean =
    exprPurity(tree) >= Idempotent

  def isPureBinding(tree: Tree)(using Ctx, CState): Boolean = statPurity(tree) >= Pure
   ...
   *               flags set.
   */
  def refPurity(tree: Tree)(using Ctx, CState): PurityLevel = {
    val sym = tree.symbol
  ...
  def isPureRef(tree: Tree)(using Ctx, CState): Boolean =
    refPurity(tree) == PurePath
  def isIdempotentRef(tree: Tree)(using Ctx, CState): Boolean =
   ...
   */
  def constToLiteral(tree: Tree)(using Ctx, CState): Tree = {
   ...
   */
  def mayBeVarGetter(sym: Symbol)(using Ctx, CState): Boolean = {
   ...
   */
  def isVariableOrGetter(tree: Tree)(using Ctx, CState): Boolean = {
    def sym = tree.symbol
  ...
  def isSelf(tree: Tree, enclClass: Symbol)(using Ctx, CState): Boolean = unsplice(tree) match {
  ...
  def stripCast(tree: Tree)(using Ctx, CState): Tree = {
  ...
  def decomposeTemplateBody(body: List[Tree])(using Ctx, CState): (List[Tree], List[Tree]) =
    body.partition {
  ...
  object closureDef {
    def unapply(tree: Tree)(using Ctx, CState): Option[DefDef] = tree match {
  ...
  def closureBody(tree: Tree)(using Ctx, CState): Tree = tree match {
    case closureDef(meth) => meth.rhs
    case _ => tree
  }

  /** The variables defined by a pattern, in reverse order of their appearance. */
  def patVars(tree: Tree)(using Ctx, CState): List[Symbol] = {
    val acc = new TreeAccumulator[List[Symbol]] {
      def apply(syms: List[Symbol], tree: Tree)(using Ctx, CState) = tree match {
  ...
  def isCatchCase(cdef: CaseDef)(using Ctx, CState): Boolean = cdef match {
  ...
  private def isSimpleThrowable(tp: Type)(using Ctx, CState): Boolean = tp match {
  ...
  def localSyms(stats: List[Tree])(using Ctx, CState): List[Symbol] =
  ...
  def definedSym(tree: Tree)(using Ctx, CState): Symbol =
   ...
   */
  def defPath(sym: Symbol, root: Tree)(using Ctx, CState): List[Tree] = trace.onDebug(s"defpath($sym with position ${sym.span}, ${root.show})") {
    require(sym.span.exists, sym)
    object accum extends TreeAccumulator[List[Tree]] {
      def apply(x: List[Tree], tree: Tree)(using Ctx, CState): List[Tree] =
   ...
   */
  def topLevelClasses(tree: Tree)(using Ctx, CState): List[ClassSymbol] = tree match {
  ...
  def sliceTopLevel(tree: Tree, cls: ClassSymbol)(using Ctx, CState): List[Tree] = tree match {
   ...
   */
  def definingStats(sym: Symbol)(using Ctx, CState): List[Tree] =
   ...
   */
  def tupleArgs(tree: Tree)(using Ctx, CState): List[Tree] = tree match {
   ...
   */
  def qualifier(tree: Tree)(using Ctx, CState): Tree = tree match {
   ...
   */
  def isStructuralTermSelectOrApply(tree: Tree)(using Ctx, CState): Boolean = {
  ...
  implicit class StructuralEqDeco(t1: Tree) {
    def === (t2: Tree)(using Ctx, CState): Boolean = (t1, t2) match {
  ...
  def assertAllPositioned(tree: Tree)(using Ctx, CState): Unit =
    tree.foreachSubTree {
  ...
  object Quoted {
     ...
     */
    def unapply(tree: tpd.Tree)(using Ctx, CState): Option[tpd.Tree] = tree match {
  ...
  object AssertNotNull :
    def apply(tree: tpd.Tree, tpnn: Type)(using Ctx, CState): tpd.Tree =
      tree.select(defn.Any_typeCast).appliedToType(AndType(tree.tpe, tpnn))

    def unapply(tree: tpd.TypeApply)(using Ctx, CState): Option[tpd.Tree] = tree match
