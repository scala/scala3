...
object NavigateAST {
   ...
   */
  def toUntyped(tree: tpd.Tree)(using Ctx, CState): untpd.Tree =
   ...
   */
  def untypedPath(tree: tpd.Tree, exactMatch: Boolean = false)(using Ctx, CState): List[Positioned] =
    tree match {
      case tree: MemberDef[?] =>
   ...
   *  the given `span`.
   */
  def untypedPath(span: Span)(using Ctx, CState): List[Positioned] =
   ...
   */
  def pathTo(span: Span, from: Positioned, skipZeroExtent: Boolean = false)(using Ctx, CState): List[Positioned] = {
