...
object Trees {
  ...
  extends Positioned
     ...
     */
    def withType(tpe: Type)(using Ctx, CState): ThisTree[Type] = {
     ...
     */
    private def checkChildrenTyped(it: Iterator[Any])(using Ctx, CState): Unit =
     ...
     *  kinds of trees
     */
    def denot(using Ctx, CState): Denotation = NoDenotation

    /** Shorthand for `denot.symbol`. */
    final def symbol(using Ctx, CState): Symbol = denot.symbol
  ...
  abstract class DenotingTree[-T >: Untyped](implicit @constructorOnly src: SourceFile) extends Tree[T] {
    ...
    override def denot(using Ctx, CState): Denotation = typeOpt match {
  ...
  abstract class ProxyTree[-T >: Untyped](implicit @constructorOnly src: SourceFile)  extends Tree[T] {
    ...
    override def denot(using Ctx, CState): Denotation = forwardTo.denot
  ...
  abstract class ValOrDefDef[-T >: Untyped](implicit @constructorOnly src: SourceFile) extends MemberDef[T] with WithLazyField[Tree[T]] {
    ...
    def rhs(using Ctx, CState): Tree[T] = forceIfLazy
  }
  ...
  case class This[-T >: Untyped] private[ast] (qual: untpd.Ident)(implicit @constructorOnly src: SourceFile)
    ...
    override def denot(using Ctx, CState): Denotation =
      typeOpt match {
  ...
  case class Template[-T >: Untyped] private[ast] (constr: DefDef[T], parentsOrDerived: List[Tree[T]], self: ValDef[T], private var preBody: LazyTreeList[T @uncheckedVariance])(implicit @constructorOnly src: SourceFile)
    ...
    def body(using Ctx, CState): List[Tree[T]] = forceIfLazy
  ...
  trait WithLazyField[+T <: AnyRef] {
    ...
    def forceIfLazy(using Ctx, CState): T = unforced match {
  ...
  abstract class Instance[T >: Untyped <: Type] { inst =>
    ...
    abstract class TreeCopier {
      ...
      def Ident(tree: Tree)(name: Name)(using Ctx, CState): Ident = tree match {
        case tree: Ident if name == tree.name => tree
        case _ => finalize(tree, untpd.Ident(name)(sourceFile(tree)))
      }
      def Select(tree: Tree)(qualifier: Tree, name: Name)(using Ctx, CState): Select = tree match {
      ...
      def Ref(tree: RefTree)(name: Name)(using Ctx, CState): RefTree = tree match {
        case Ident(_) => Ident(tree)(name)
        case Select(qual, _) => Select(tree)(qual, name)
      }
      def This(tree: Tree)(qual: untpd.Ident)(using Ctx, CState): This = tree match {
        case tree: This if (qual eq tree.qual) => tree
        case _ => finalize(tree, untpd.This(qual)(sourceFile(tree)))
      }
      def Super(tree: Tree)(qual: Tree, mix: untpd.Ident)(using Ctx, CState): Super = tree match {
        case tree: Super if (qual eq tree.qual) && (mix eq tree.mix) => tree
        case _ => finalize(tree, untpd.Super(qual, mix)(sourceFile(tree)))
      }
      def Apply(tree: Tree)(fun: Tree, args: List[Tree])(using Ctx, CState): Apply = tree match {
      ...
      def TypeApply(tree: Tree)(fun: Tree, args: List[Tree])(using Ctx, CState): TypeApply = tree match {
      ...
      def Closure(tree: Tree)(env: List[Tree], meth: Tree, tpt: Tree)(using Ctx, CState): Closure = tree match {
      ...
      def Match(tree: Tree)(selector: Tree, cases: List[CaseDef])(using Ctx, CState): Match = tree match {
      ...
      def CaseDef(tree: Tree)(pat: Tree, guard: Tree, body: Tree)(using Ctx, CState): CaseDef = tree match {
      ...
      def Labeled(tree: Tree)(bind: Bind, expr: Tree)(using Ctx, CState): Labeled = tree match {
      ...
      def Return(tree: Tree)(expr: Tree, from: Tree)(using Ctx, CState): Return = tree match {
        case tree: Return if (expr eq tree.expr) && (from eq tree.from) => tree
        case _ => finalize(tree, untpd.Return(expr, from)(sourceFile(tree)))
      }
      def WhileDo(tree: Tree)(cond: Tree, body: Tree)(using Ctx, CState): WhileDo = tree match {
      ...
      def Try(tree: Tree)(expr: Tree, cases: List[CaseDef], finalizer: Tree)(using Ctx, CState): Try = tree match {
      ...
      def SeqLiteral(tree: Tree)(elems: List[Tree], elemtpt: Tree)(using Ctx, CState): SeqLiteral = tree match {
      ...
      def Inlined(tree: Tree)(call: tpd.Tree, bindings: List[MemberDef], expansion: Tree)(using Ctx, CState): Inlined = tree match {
        case tree: Inlined if (call eq tree.call) && (bindings eq tree.bindings) && (expansion eq tree.expansion) => tree
        case _ => finalize(tree, untpd.Inlined(call, bindings, expansion)(sourceFile(tree)))
      }
      def SingletonTypeTree(tree: Tree)(ref: Tree)(using Ctx, CState): SingletonTypeTree = tree match {
        case tree: SingletonTypeTree if (ref eq tree.ref) => tree
        case _ => finalize(tree, untpd.SingletonTypeTree(ref)(sourceFile(tree)))
      }
      def RefinedTypeTree(tree: Tree)(tpt: Tree, refinements: List[Tree])(using Ctx, CState): RefinedTypeTree = tree match {
      ...
      def AppliedTypeTree(tree: Tree)(tpt: Tree, args: List[Tree])(using Ctx, CState): AppliedTypeTree = tree match {
        case tree: AppliedTypeTree if (tpt eq tree.tpt) && (args eq tree.args) => tree
        case _ => finalize(tree, untpd.AppliedTypeTree(tpt, args)(sourceFile(tree)))
      }
      def LambdaTypeTree(tree: Tree)(tparams: List[TypeDef], body: Tree)(using Ctx, CState): LambdaTypeTree = tree match {
      ...
      def TermLambdaTypeTree(tree: Tree)(params: List[ValDef], body: Tree)(using Ctx, CState): TermLambdaTypeTree = tree match {
      ...
      def MatchTypeTree(tree: Tree)(bound: Tree, selector: Tree, cases: List[CaseDef])(using Ctx, CState): MatchTypeTree = tree match {
      ...
      def ByNameTypeTree(tree: Tree)(result: Tree)(using Ctx, CState): ByNameTypeTree = tree match {
        case tree: ByNameTypeTree if (result eq tree.result) => tree
        case _ => finalize(tree, untpd.ByNameTypeTree(result)(sourceFile(tree)))
      }
      def TypeBoundsTree(tree: Tree)(lo: Tree, hi: Tree, alias: Tree)(using Ctx, CState): TypeBoundsTree = tree match {
      ...
      def Bind(tree: Tree)(name: Name, body: Tree)(using Ctx, CState): Bind = tree match {
        case tree: Bind if (name eq tree.name) && (body eq tree.body) => tree
        case _ => finalize(tree, untpd.Bind(name, body)(sourceFile(tree)))
      }
      def Alternative(tree: Tree)(trees: List[Tree])(using Ctx, CState): Alternative = tree match {
        case tree: Alternative if (trees eq tree.trees) => tree
        case _ => finalize(tree, untpd.Alternative(trees)(sourceFile(tree)))
      }
      def UnApply(tree: Tree)(fun: Tree, implicits: List[Tree], patterns: List[Tree])(using Ctx, CState): UnApply = tree match {
      ...
      def ValDef(tree: Tree)(name: TermName, tpt: Tree, rhs: LazyTree)(using Ctx, CState): ValDef = tree match {
      ...
      def DefDef(tree: Tree)(name: TermName, tparams: List[TypeDef], vparamss: List[List[ValDef]], tpt: Tree, rhs: LazyTree)(using Ctx, CState): DefDef = tree match {
      ...
      def TypeDef(tree: Tree)(name: TypeName, rhs: Tree)(using Ctx, CState): TypeDef = tree match {
        case tree: TypeDef if (name == tree.name) && (rhs eq tree.rhs) => tree
        case _ => finalize(tree, untpd.TypeDef(name, rhs)(sourceFile(tree)))
      }
      def Template(tree: Tree)(constr: DefDef, parents: List[Tree], derived: List[untpd.Tree], self: ValDef, body: LazyTreeList)(using Ctx, CState): Template = tree match {
      ...
      def Import(tree: Tree)(expr: Tree, selectors: List[untpd.ImportSelector])(using Ctx, CState): Import = tree match {
      ...
      def PackageDef(tree: Tree)(pid: RefTree, stats: List[Tree])(using Ctx, CState): PackageDef = tree match {
        case tree: PackageDef if (pid eq tree.pid) && (stats eq tree.stats) => tree
        case _ => finalize(tree, untpd.PackageDef(pid, stats)(sourceFile(tree)))
      }
      def Annotated(tree: Tree)(arg: Tree, annot: Tree)(using Ctx, CState): Annotated = tree match {
      ...
      def Thicket(tree: Tree)(trees: List[Tree])(using Ctx, CState): Thicket = tree match {
      ...
      def If(tree: If)(cond: Tree = tree.cond, thenp: Tree = tree.thenp, elsep: Tree = tree.elsep)(using Ctx, CState): If =
        If(tree: Tree)(cond, thenp, elsep)
      def Closure(tree: Closure)(env: List[Tree] = tree.env, meth: Tree = tree.meth, tpt: Tree = tree.tpt)(using Ctx, CState): Closure =
        Closure(tree: Tree)(env, meth, tpt)
      def CaseDef(tree: CaseDef)(pat: Tree = tree.pat, guard: Tree = tree.guard, body: Tree = tree.body)(using Ctx, CState): CaseDef =
        CaseDef(tree: Tree)(pat, guard, body)
      def Try(tree: Try)(expr: Tree = tree.expr, cases: List[CaseDef] = tree.cases, finalizer: Tree = tree.finalizer)(using Ctx, CState): Try =
        Try(tree: Tree)(expr, cases, finalizer)
      def UnApply(tree: UnApply)(fun: Tree = tree.fun, implicits: List[Tree] = tree.implicits, patterns: List[Tree] = tree.patterns)(using Ctx, CState): UnApply =
        UnApply(tree: Tree)(fun, implicits, patterns)
      def ValDef(tree: ValDef)(name: TermName = tree.name, tpt: Tree = tree.tpt, rhs: LazyTree = tree.unforcedRhs)(using Ctx, CState): ValDef =
        ValDef(tree: Tree)(name, tpt, rhs)
      def DefDef(tree: DefDef)(name: TermName = tree.name, tparams: List[TypeDef] = tree.tparams, vparamss: List[List[ValDef]] = tree.vparamss, tpt: Tree = tree.tpt, rhs: LazyTree = tree.unforcedRhs)(using Ctx, CState): DefDef =
        DefDef(tree: Tree)(name, tparams, vparamss, tpt, rhs)
      def TypeDef(tree: TypeDef)(name: TypeName = tree.name, rhs: Tree = tree.rhs)(using Ctx, CState): TypeDef =
        TypeDef(tree: Tree)(name, rhs)
      def Template(tree: Template)(constr: DefDef = tree.constr, parents: List[Tree] = tree.parents, derived: List[untpd.Tree] = tree.derived, self: ValDef = tree.self, body: LazyTreeList = tree.unforcedBody)(using Ctx, CState): Template =
        Template(tree: Tree)(constr, parents, derived, self, body)
    }

    /** Hook to indicate that a transform of some subtree should be skipped */
    protected def skipTransform(tree: Tree)(using Ctx, CState): Boolean = false
    ...
    abstract class TreeMap(val cpy: TreeCopier = inst.cpy) { self =>
      ...
      def transformStats(trees: List[Tree])(using Ctx, CState): List[Tree] =
        transform(trees)
      def transform(trees: List[Tree])(using Ctx, CState): List[Tree] =
        flatten(trees mapConserve (transform(_)))
      def transformSub[Tr <: Tree](tree: Tr)(using Ctx, CState): Tr =
        transform(tree).asInstanceOf[Tr]
      def transformSub[Tr <: Tree](trees: List[Tr])(using Ctx, CState): List[Tr] =
        transform(trees).asInstanceOf[List[Tr]]

      protected def transformMoreCases(tree: Tree)(using Ctx, CState): Tree = {
    ...
    abstract class TreeAccumulator[X] { self =>
      ...
      def apply(x: X, tree: Tree)(using Ctx, CState): X

      def apply(x: X, trees: Traversable[Tree])(using Ctx, CState): X = trees.foldLeft(x)(apply)
      def foldOver(x: X, tree: Tree)(using Ctx, CState): X =
        if (tree.source != ctx.source && tree.source.exists)
      ...
      def foldMoreCases(x: X, tree: Tree)(using Ctx, CState): X = {
    ...
    def rename(tree: NameTree, newName: Name)(using Ctx, CState): tree.ThisTree[T] = {
      tree match {
