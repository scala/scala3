...
class TreeTypeMap(
  ...
  val substTo: List[Symbol] = Nil)(using Ctx, CState) extends tpd.TreeMap {
  import tpd._
  ...
  override def transform(tree: tpd.Tree)(using Ctx, CState): tpd.Tree = treeMap(tree) match {
  ...
  override def transformStats(trees: List[tpd.Tree])(using Ctx, CState): List[Tree] =
    transformDefs(trees)._2

  def transformDefs[TT <: tpd.Tree](trees: List[TT])(using Ctx, CState): (TreeTypeMap, List[TT]) = {
