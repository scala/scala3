...
import dottyBench.tools.dotc.transform.{ExplicitOuter, Erasure}
...
object tpd extends Trees.Instance[Type] with TypedTreeInfo {
  ...
  private def ta(using Ctx, CState) = ctx.typeAssigner

  def Ident(tp: NamedType)(using Ctx, CState): Ident =
    ta.assignType(untpd.Ident(tp.name), tp)

  def Select(qualifier: Tree, name: Name)(using Ctx, CState): Select =
    ta.assignType(untpd.Select(qualifier, name), qualifier)

  def Select(qualifier: Tree, tp: NamedType)(using Ctx, CState): Select =
    untpd.Select(qualifier, tp.name).withType(tp)

  def This(cls: ClassSymbol)(using Ctx, CState): This =
    untpd.This(untpd.Ident(cls.name)).withType(cls.thisType)

  def Super(qual: Tree, mix: untpd.Ident, mixinClass: Symbol)(using Ctx, CState): Super =
    ta.assignType(untpd.Super(qual, mix), qual, mixinClass)

  def Super(qual: Tree, mixName: TypeName, mixinClass: Symbol = NoSymbol)(using Ctx, CState): Super =
    Super(qual, if (mixName.isEmpty) untpd.EmptyTypeIdent else untpd.Ident(mixName), mixinClass)

  def Apply(fn: Tree, args: List[Tree])(using Ctx, CState): Apply = {
  ...
  def TypeApply(fn: Tree, args: List[Tree])(using Ctx, CState): TypeApply = {
    assert(fn.isInstanceOf[RefTree] || fn.isInstanceOf[GenericApply[_]])
    ta.assignType(untpd.TypeApply(fn, args), fn, args)
  }

  def Literal(const: Constant)(using Ctx, CState): Literal =
    ta.assignType(untpd.Literal(const))

  def unitLiteral(using Ctx, CState): Literal =
    Literal(Constant(()))

  def nullLiteral(using Ctx, CState): Literal =
    Literal(Constant(null))

  def New(tpt: Tree)(using Ctx, CState): New =
    ta.assignType(untpd.New(tpt), tpt)

  def New(tp: Type)(using Ctx, CState): New = New(TypeTree(tp))

  def Typed(expr: Tree, tpt: Tree)(using Ctx, CState): Typed =
    ta.assignType(untpd.Typed(expr, tpt), tpt)

  def NamedArg(name: Name, arg: Tree)(using Ctx, CState): NamedArg =
    ta.assignType(untpd.NamedArg(name, arg), arg)

  def Assign(lhs: Tree, rhs: Tree)(using Ctx, CState): Assign =
    ta.assignType(untpd.Assign(lhs, rhs))

  def Block(stats: List[Tree], expr: Tree)(using Ctx, CState): Block =
    ta.assignType(untpd.Block(stats, expr), stats, expr)

  /** Join `stats` in front of `expr` creating a new block if necessary */
  def seq(stats: List[Tree], expr: Tree)(using Ctx, CState): Tree =
  ...
  def If(cond: Tree, thenp: Tree, elsep: Tree)(using Ctx, CState): If =
    ta.assignType(untpd.If(cond, thenp, elsep), thenp, elsep)

  def InlineIf(cond: Tree, thenp: Tree, elsep: Tree)(using Ctx, CState): If =
    ta.assignType(untpd.InlineIf(cond, thenp, elsep), thenp, elsep)

  def Closure(env: List[Tree], meth: Tree, tpt: Tree)(using Ctx, CState): Closure =
   ...
   *  otherwise specified).
   */
  def Closure(meth: TermSymbol, rhsFn: List[List[Tree]] => Tree, targs: List[Tree] = Nil, targetType: Type = NoType)(using Ctx, CState): Block = {
  ...
  def Lambda(tpe: MethodType, rhsFn: List[Tree] => Tree)(using Ctx, CState): Block = {
    val meth = newSymbol(ctx.owner, nme.ANON_FUN, Synthetic | Method, tpe)
    Closure(meth, tss => rhsFn(tss.head).changeOwner(ctx.owner, meth))
  }

  def CaseDef(pat: Tree, guard: Tree, body: Tree)(using Ctx, CState): CaseDef =
    ta.assignType(untpd.CaseDef(pat, guard, body), pat, body)

  def Match(selector: Tree, cases: List[CaseDef])(using Ctx, CState): Match =
    ta.assignType(untpd.Match(selector, cases), selector, cases)

  def InlineMatch(selector: Tree, cases: List[CaseDef])(using Ctx, CState): Match =
    ta.assignType(untpd.InlineMatch(selector, cases), selector, cases)

  def Labeled(bind: Bind, expr: Tree)(using Ctx, CState): Labeled =
    ta.assignType(untpd.Labeled(bind, expr))

  def Labeled(sym: TermSymbol, expr: Tree)(using Ctx, CState): Labeled =
    Labeled(Bind(sym, EmptyTree), expr)

  def Return(expr: Tree, from: Tree)(using Ctx, CState): Return =
    ta.assignType(untpd.Return(expr, from))

  def Return(expr: Tree, from: Symbol)(using Ctx, CState): Return =
    Return(expr, Ident(from.termRef))

  def WhileDo(cond: Tree, body: Tree)(using Ctx, CState): WhileDo =
    ta.assignType(untpd.WhileDo(cond, body))

  def Try(block: Tree, cases: List[CaseDef], finalizer: Tree)(using Ctx, CState): Try =
    ta.assignType(untpd.Try(block, cases, finalizer), block, cases)

  def SeqLiteral(elems: List[Tree], elemtpt: Tree)(using Ctx, CState): SeqLiteral =
    ta.assignType(untpd.SeqLiteral(elems, elemtpt), elems, elemtpt)

  def JavaSeqLiteral(elems: List[Tree], elemtpt: Tree)(using Ctx, CState): JavaSeqLiteral =
    ta.assignType(untpd.JavaSeqLiteral(elems, elemtpt), elems, elemtpt).asInstanceOf[JavaSeqLiteral]

  def Inlined(call: Tree, bindings: List[MemberDef], expansion: Tree)(using Ctx, CState): Inlined =
    ta.assignType(untpd.Inlined(call, bindings, expansion), bindings, expansion)

  def TypeTree(tp: Type)(using Ctx, CState): TypeTree =
    untpd.TypeTree().withType(tp)

  def SingletonTypeTree(ref: Tree)(using Ctx, CState): SingletonTypeTree =
    ta.assignType(untpd.SingletonTypeTree(ref), ref)

  def RefinedTypeTree(parent: Tree, refinements: List[Tree], refineCls: ClassSymbol)(using Ctx, CState): Tree =
    ta.assignType(untpd.RefinedTypeTree(parent, refinements), parent, refinements, refineCls)

  def AppliedTypeTree(tycon: Tree, args: List[Tree])(using Ctx, CState): AppliedTypeTree =
    ta.assignType(untpd.AppliedTypeTree(tycon, args), tycon, args)

  def ByNameTypeTree(result: Tree)(using Ctx, CState): ByNameTypeTree =
    ta.assignType(untpd.ByNameTypeTree(result), result)

  def LambdaTypeTree(tparams: List[TypeDef], body: Tree)(using Ctx, CState): LambdaTypeTree =
    ta.assignType(untpd.LambdaTypeTree(tparams, body), tparams, body)

  def MatchTypeTree(bound: Tree, selector: Tree, cases: List[CaseDef])(using Ctx, CState): MatchTypeTree =
    ta.assignType(untpd.MatchTypeTree(bound, selector, cases), bound, selector, cases)

  def TypeBoundsTree(lo: Tree, hi: Tree, alias: Tree = EmptyTree)(using Ctx, CState): TypeBoundsTree =
    ta.assignType(untpd.TypeBoundsTree(lo, hi, alias), lo, hi, alias)

  def Bind(sym: Symbol, body: Tree)(using Ctx, CState): Bind =
    ta.assignType(untpd.Bind(sym.name, body), sym)

  /** A pattern corresponding to `sym: tpe` */
  def BindTyped(sym: TermSymbol, tpe: Type)(using Ctx, CState): Bind =
    Bind(sym, Typed(Underscore(tpe), TypeTree(tpe)))

  def Alternative(trees: List[Tree])(using Ctx, CState): Alternative =
    ta.assignType(untpd.Alternative(trees), trees)

  def UnApply(fun: Tree, implicits: List[Tree], patterns: List[Tree], proto: Type)(using Ctx, CState): UnApply = {
    assert(fun.isInstanceOf[RefTree] || fun.isInstanceOf[GenericApply[_]])
    ta.assignType(untpd.UnApply(fun, implicits, patterns), proto)
  }

  def ValDef(sym: TermSymbol, rhs: LazyTree = EmptyTree)(using Ctx, CState): ValDef =
    ta.assignType(untpd.ValDef(sym.name, TypeTree(sym.info), rhs), sym)

  def SyntheticValDef(name: TermName, rhs: Tree)(using Ctx, CState): ValDef =
  ...
  def DefDef(sym: TermSymbol, tparams: List[TypeSymbol], vparamss: List[List[TermSymbol]],
             resultType: Type, rhs: Tree)(using Ctx, CState): DefDef =
  ...
  def DefDef(sym: TermSymbol, rhs: Tree = EmptyTree)(using Ctx, CState): DefDef =
    ta.assignType(DefDef(sym, Function.const(rhs) _), sym)

  def DefDef(sym: TermSymbol, rhsFn: List[List[Tree]] => Tree)(using Ctx, CState): DefDef =
   ...
   */
  def polyDefDef(sym: TermSymbol, rhsFn: List[Type] => List[List[Tree]] => Tree)(using Ctx, CState): DefDef = {
  ...
  def TypeDef(sym: TypeSymbol)(using Ctx, CState): TypeDef =
    ta.assignType(untpd.TypeDef(sym.name, TypeTree(sym.info)), sym)

  def ClassDef(cls: ClassSymbol, constr: DefDef, body: List[Tree], superArgs: List[Tree] = Nil)(using Ctx, CState): TypeDef = {
  ...
  def ClassDefWithParents(cls: ClassSymbol, constr: DefDef, parents: List[Tree], body: List[Tree])(using Ctx, CState): TypeDef = {
    val selfType =
   ...
   */
  def AnonClass(parents: List[Type], fns: List[TermSymbol], methNames: List[TermName])(using Ctx, CState): Block = {
  ...
  def Import(expr: Tree, selectors: List[untpd.ImportSelector])(using Ctx, CState): Import =
    ta.assignType(untpd.Import(expr, selectors), newImportSymbol(ctx.owner, expr))

  def PackageDef(pid: RefTree, stats: List[Tree])(using Ctx, CState): PackageDef =
    ta.assignType(untpd.PackageDef(pid, stats), pid)

  def Annotated(arg: Tree, annot: Tree)(using Ctx, CState): Annotated =
    ta.assignType(untpd.Annotated(arg, annot), arg, annot)

  def Throw(expr: Tree)(using Ctx, CState): Tree =
    ref(defn.throwMethod).appliedTo(expr)

  // ------ Making references ------------------------------------------------------

  def prefixIsElidable(tp: NamedType)(using Ctx, CState): Boolean = {
  ...
  def needsSelect(tp: Type)(using Ctx, CState): Boolean = tp match {
  ...
  def ref(sym: Symbol)(using Ctx, CState): Tree =
    ref(NamedType(sym.owner.thisType, sym.name, sym.denot))

  private def followOuterLinks(t: Tree)(using Ctx, CState) = t match {
    case t: This if currentlyAfterErasure && !(t.symbol == ctx.owner.enclosingClass || t.symbol.isStaticOwner) =>
      // after erasure outer paths should be respected
      ExplicitOuter.outer.path(toCls = t.tpe.widen.classSymbol)
    case t =>
      t
  }

  def singleton(tp: Type)(using Ctx, CState): Tree = tp.dealias match {
   ...
   */
  def pathFor(tp: Type)(using Ctx, CState): Tree = {
   ...
   */
  def newArray(elemTpe: Type, returnTpe: Type, span: Span, dims: JavaSeqLiteral)(using Ctx, CState): Tree = {
  ...
  def wrapArrayMethodName(elemtp: Type)(using Ctx, CState): TermName = {
   ...
   */
  def wrapArray(tree: Tree, elemtp: Type)(using Ctx, CState): Tree =
  ...
  def New(tp: Type, args: List[Tree])(using Ctx, CState): Apply =
    New(tp, tp.dealias.typeSymbol.primaryConstructor.asTerm, args)

  /** new C(args), calling given constructor `constr` of C */
  def New(tp: Type, constr: TermSymbol, args: List[Tree])(using Ctx, CState): Apply = {
   ...
   *  an error.)
   */
  def ModuleDef(sym: TermSymbol, body: List[Tree])(using Ctx, CState): tpd.Thicket = {
  ...
  /** A `_` with given type */
  def Underscore(tp: Type)(using Ctx, CState): Ident = untpd.Ident(nme.WILDCARD).withType(tp)

  def defaultValue(tpe: Type)(using Ctx, CState): Tree = {
    val tpw = tpe.widen
  ...
  private class FindLocalDummyAccumulator(cls: ClassSymbol)(using Ctx, CState) extends TreeAccumulator[Symbol] {
    def apply(sym: Symbol, tree: Tree)(using Ctx, CState) =
      if (sym.exists) sym
      else if (tree.isDef) {
  ...
  class TypedTreeCopier extends TreeCopier {
    ...
    override def Select(tree: Tree)(qualifier: Tree, name: Name)(using Ctx, CState): Select = {
    ...
    override def Apply(tree: Tree)(fun: Tree, args: List[Tree])(using Ctx, CState): Apply = {
    ...
    override def TypeApply(tree: Tree)(fun: Tree, args: List[Tree])(using Ctx, CState): TypeApply = {
    ...
    override def Literal(tree: Tree)(const: Constant)(using Ctx, CState): Literal =
      ta.assignType(untpdCpy.Literal(tree)(const))

    override def New(tree: Tree)(tpt: Tree)(using Ctx, CState): New =
      ta.assignType(untpdCpy.New(tree)(tpt), tpt)

    override def Typed(tree: Tree)(expr: Tree, tpt: Tree)(using Ctx, CState): Typed =
      ta.assignType(untpdCpy.Typed(tree)(expr, tpt), tpt)

    override def NamedArg(tree: Tree)(name: Name, arg: Tree)(using Ctx, CState): NamedArg =
      ta.assignType(untpdCpy.NamedArg(tree)(name, arg), arg)

    override def Assign(tree: Tree)(lhs: Tree, rhs: Tree)(using Ctx, CState): Assign =
      ta.assignType(untpdCpy.Assign(tree)(lhs, rhs))

    override def Block(tree: Tree)(stats: List[Tree], expr: Tree)(using Ctx, CState): Block = {
    ...
    override def If(tree: Tree)(cond: Tree, thenp: Tree, elsep: Tree)(using Ctx, CState): If = {
    ...
    override def Closure(tree: Tree)(env: List[Tree], meth: Tree, tpt: Tree)(using Ctx, CState): Closure = {
    ...
    override def Match(tree: Tree)(selector: Tree, cases: List[CaseDef])(using Ctx, CState): Match = {
    ...
    override def CaseDef(tree: Tree)(pat: Tree, guard: Tree, body: Tree)(using Ctx, CState): CaseDef = {
    ...
    override def Labeled(tree: Tree)(bind: Bind, expr: Tree)(using Ctx, CState): Labeled =
      ta.assignType(untpdCpy.Labeled(tree)(bind, expr))

    override def Return(tree: Tree)(expr: Tree, from: Tree)(using Ctx, CState): Return =
      ta.assignType(untpdCpy.Return(tree)(expr, from))

    override def WhileDo(tree: Tree)(cond: Tree, body: Tree)(using Ctx, CState): WhileDo =
      ta.assignType(untpdCpy.WhileDo(tree)(cond, body))

    override def Try(tree: Tree)(expr: Tree, cases: List[CaseDef], finalizer: Tree)(using Ctx, CState): Try = {
    ...
    override def Inlined(tree: Tree)(call: Tree, bindings: List[MemberDef], expansion: Tree)(using Ctx, CState): Inlined = {
    ...
    override def SeqLiteral(tree: Tree)(elems: List[Tree], elemtpt: Tree)(using Ctx, CState): SeqLiteral = {
    ...
    override def Annotated(tree: Tree)(arg: Tree, annot: Tree)(using Ctx, CState): Annotated = {
    ...
    override def If(tree: If)(cond: Tree = tree.cond, thenp: Tree = tree.thenp, elsep: Tree = tree.elsep)(using Ctx, CState): If =
      If(tree: Tree)(cond, thenp, elsep)
    override def Closure(tree: Closure)(env: List[Tree] = tree.env, meth: Tree = tree.meth, tpt: Tree = tree.tpt)(using Ctx, CState): Closure =
      Closure(tree: Tree)(env, meth, tpt)
    override def CaseDef(tree: CaseDef)(pat: Tree = tree.pat, guard: Tree = tree.guard, body: Tree = tree.body)(using Ctx, CState): CaseDef =
      CaseDef(tree: Tree)(pat, guard, body)
    override def Try(tree: Try)(expr: Tree = tree.expr, cases: List[CaseDef] = tree.cases, finalizer: Tree = tree.finalizer)(using Ctx, CState): Try =
  ...
  override def skipTransform(tree: Tree)(using Ctx, CState): Boolean = tree.tpe.isError

  implicit class TreeOps[ThisTree <: tpd.Tree](private val tree: ThisTree) extends AnyVal {

    def isValue(using Ctx, CState): Boolean =
      tree.isTerm && tree.tpe.widen.isValueType

    def isValueOrPattern(using Ctx, CState): Boolean =
    ...
    def shallowFold[T](z: T)(op: (T, tpd.Tree) => T)(using Ctx, CState): T =
      ShallowFolder(op).apply(z, tree)

    def deepFold[T](z: T)(op: (T, tpd.Tree) => T)(using Ctx, CState): T =
      DeepFolder(op).apply(z, tree)

    def find[T](pred: (tpd.Tree) => Boolean)(using Ctx, CState): Option[tpd.Tree] =
      shallowFold[Option[tpd.Tree]](None)((accum, tree) => if (pred(tree)) Some(tree) else accum)

    def subst(from: List[Symbol], to: List[Symbol])(using Ctx, CState): ThisTree =
     ...
     */
    def changeOwner(from: Symbol, to: Symbol)(using Ctx, CState): ThisTree = {
     ...
     * tree to be `newowner`
     */
    def changeNonLocalOwners(newOwner: Symbol)(using Ctx, CState): Tree = {
      val ownerAcc = new TreeAccumulator[immutable.Set[Symbol]] {
        def apply(ss: immutable.Set[Symbol], tree: Tree)(using Ctx, CState) = tree match {
          case tree: DefTree =>
     ...
     *  owner by `from` to `to`.
     */
    def changeOwnerAfter(from: Symbol, to: Symbol, trans: DenotTransformer)(using Ctx, CState): ThisTree =
    ...
    def select(name: Name)(using Ctx, CState): Select =
      Select(tree, name)
     ...
     */
    def select(name: Name, p: Symbol => Boolean)(using Ctx, CState): Select =
      select(tree.tpe.member(name).suchThat(p).symbol)

    /** A select node with the given type */
    def select(tp: NamedType)(using Ctx, CState): Select =
     ...
     */
    def select(sym: Symbol)(using Ctx, CState): Select = {
      val tp =
        if (sym.isType) {
    ...
    def selectWithSig(name: Name, sig: Signature)(using Ctx, CState): Tree =
     ...
     */
    def selectWithSig(sym: Symbol)(using Ctx, CState): Tree =
      selectWithSig(sym.name, sym.signature)

    /** A unary apply node with given argument: `tree(arg)` */
    def appliedTo(arg: Tree)(using Ctx, CState): Tree =
      appliedToArgs(arg :: Nil)

    /** An apply node with given arguments: `tree(arg, args0, ..., argsN)` */
    def appliedTo(arg: Tree, args: Tree*)(using Ctx, CState): Tree =
      appliedToArgs(arg :: args.toList)

    /** An apply node with given argument list `tree(args(0), ..., args(args.length - 1))` */
    def appliedToArgs(args: List[Tree])(using Ctx, CState): Apply =
      Apply(tree, args)

    /** An applied node that accepts only varargs as arguments */
    def appliedToVarargs(args: List[Tree], tpt: Tree)(using Ctx, CState): Tree =
      appliedTo(repeated(args, tpt))

    /** The current tree applied to given argument lists:
     *  `tree (argss(0)) ... (argss(argss.length -1))`
     */
    def appliedToArgss(argss: List[List[Tree]])(using Ctx, CState): Tree =
      argss.foldLeft(tree: Tree)(Apply(_, _))

    /** The current tree applied to (): `tree()` */
    def appliedToNone(using Ctx, CState): Apply = appliedToArgs(Nil)

    /** The current tree applied to given type argument: `tree[targ]` */
    def appliedToType(targ: Type)(using Ctx, CState): Tree =
      appliedToTypes(targ :: Nil)

    /** The current tree applied to given type arguments: `tree[targ0, ..., targN]` */
    def appliedToTypes(targs: List[Type])(using Ctx, CState): Tree =
      appliedToTypeTrees(targs map (TypeTree(_)))

    /** The current tree applied to given type argument: `tree[targ]` */
    def appliedToTypeTree(targ: Tree)(using Ctx, CState): Tree =
      appliedToTypeTrees(targ :: Nil)

    /** The current tree applied to given type argument list: `tree[targs(0), ..., targs(targs.length - 1)]` */
    def appliedToTypeTrees(targs: List[Tree])(using Ctx, CState): Tree =
      if (targs.isEmpty) tree else TypeApply(tree, targs)

    /** Apply to `()` unless tree's widened type is parameterless */
    def ensureApplied(using Ctx, CState): Tree =
      if (tree.tpe.widen.isParameterless) tree else tree.appliedToNone

    /** `tree == that` */
    def equal(that: Tree)(using Ctx, CState): Tree =
    ...
    def isInstance(tp: Type)(using Ctx, CState): Tree = tp.dealias match {
    ...
    def asInstance(tp: Type)(using Ctx, CState): Tree = {
    ...
    def cast(tp: Type)(using Ctx, CState): Tree = {
      assert(tp.isValueType, i"bad cast: $tree.asInstanceOf[$tp]")
     ...
     */
    def ensureConforms(tp: Type)(using Ctx, CState): Tree =
      if (tree.tpe <:< tp) tree
    ...
    def testNotNull(using Ctx, CState): Tree = {
     ...
     */
    def annotated(annot: Tree)(using Ctx, CState): Tree =
      if (tree.isTerm)
     ...
     */
    def outerSelect(levels: Int, tp: Type)(using Ctx, CState): Tree =
      untpd.Select(tree, OuterSelectName(EmptyTermName, levels)).withType(SkolemType(tp))

    /** Replace Inlined nodes and InlineProxy references to underlying arguments */
    def underlyingArgument(using Ctx, CState): Tree = {
    ...
    def underlying(using Ctx, CState): Tree = MapToUnderlying().transform(tree)

    // --- Higher order traversal methods -------------------------------

    /** Apply `f` to each subtree of this tree */
    def foreachSubTree(f: Tree => Unit)(using Ctx, CState): Unit = {
      val traverser = new TreeTraverser {
        def traverse(tree: Tree)(using Ctx, CState) = foldOver(f(tree), tree)
      }
      traverser.traverse(tree)
    }

    /** Is there a subtree of this tree that satisfies predicate `p`? */
    def existsSubTree(p: Tree => Boolean)(using Ctx, CState): Boolean = {
    ...
    def filterSubTrees(f: Tree => Boolean)(using Ctx, CState): List[Tree] = {
    ...
    def setDefTree(using Ctx, CState): ThisTree = {
      val sym = tree.symbol
  ...
  class MapToUnderlying extends TreeMap {
    ...
    private def isBinding(sym: Symbol)(using Ctx, CState): Boolean =
  ...
  trait TreeProvider {
    ...
    def mightContain(id: String)(using Ctx, CState): Boolean = true
  }

  // convert a numeric with a toXXX method
  def primitiveConversion(tree: Tree, numericCls: Symbol)(using Ctx, CState): Tree = {
  ...
  def clsOf(tp: Type)(using Ctx, CState): Tree =
   ...
   *     ~within('tree)
   */
  def letBindUnless(level: TreeInfo.PurityLevel, tree: Tree)(within: Tree => Tree)(using Ctx, CState): Tree =
  ...
  def evalOnce(tree: Tree)(within: Tree => Tree)(using Ctx, CState): Tree =
    letBindUnless(TreeInfo.Idempotent, tree)(within)

  def runtimeCall(name: TermName, args: List[Tree])(using Ctx, CState): Tree =
   ...
   */
  def enclosingInlineds(using Ctx, CState): List[Tree] =
    ctx.property(InlinedCalls).getOrElse(Nil)

  /** Record inlined trees */
  def addInlinedTrees(n: Int)(using Ctx, CState): Unit =
    ctx.property(InlinedTrees).foreach(_.count += n)

  /** Check if the limit on the number of inlined trees has been reached */
  def reachedInlinedTreesLimit(using Ctx, CState): Boolean =
   ...
   *  is defined
   */
  def sourceFile(call: Tree)(using Ctx, CState): SourceFile = call.symbol.source

  /** Desugar identifier into a select node. Return the tree itself if not possible */
  def desugarIdent(tree: Ident)(using Ctx, CState): RefTree = {
  ...
  def desugarIdentPrefix(tree: Ident)(using Ctx, CState): Tree = tree.tpe match {
   ...
   */
  def importedSymbols(expr: Tree, name: Name)(using Ctx, CState): List[Symbol] = {
   ...
   * by `imp`.
   */
  def importSelections(imp: Import)(using Ctx, CState): List[Select] = {
  ...
  def tupleTypeTree(elems: List[Tree])(using Ctx, CState): Tree = {
  ...
  def nestedPairsTypeTree(ts: List[Tree])(using Ctx, CState): Tree =
  ...
  private def focusPositions(tree: Tree)(using Ctx, CState): Tree = {
   ...
   */
  def repeated(trees: List[Tree], tpt: Tree)(using Ctx, CState): Tree =
   ...
   *
   */
  def mkList(trees: List[Tree], tpe: Tree)(using Ctx, CState): Tree =
