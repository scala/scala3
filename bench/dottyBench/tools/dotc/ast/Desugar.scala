...
object desugar {
   ...
   */
  def isRetractableCaseClassMethodName(name: Name)(using Ctx, CState): Boolean = name match {
  ...
  def isDesugaredCaseClassMethodName(name: Name)(using Ctx, CState): Boolean =
...
// ----- DerivedTypeTrees -----------------------------------
  ...
  class SetterParamTree(implicit @constructorOnly src: SourceFile) extends DerivedTypeTree {
    def derivedTree(sym: Symbol)(using Ctx, CState): tpd.TypeTree = tpd.TypeTree(sym.info.resultType)
  }

  class TypeRefTree(implicit @constructorOnly src: SourceFile) extends DerivedTypeTree {
    def derivedTree(sym: Symbol)(using Ctx, CState): tpd.TypeTree = tpd.TypeTree(sym.typeRef)
  }

  class TermRefTree(implicit @constructorOnly src: SourceFile) extends DerivedTypeTree {
    def derivedTree(sym: Symbol)(using Ctx, CState): tpd.Tree = tpd.ref(sym)
  }
  ...
  class DerivedFromParamTree()(implicit @constructorOnly src: SourceFile) extends DerivedTypeTree {
     ...
     */
    override def ensureCompletions(using Ctx, CState): Unit = {
     ...
     *  from subclasses.
     */
    def derivedTree(sym: Symbol)(using Ctx, CState): tpd.TypeTree = {
...
// ----- Desugar methods -------------------------------------------------
   ...
   */
  def valDef(vdef0: ValDef)(using Ctx, CState): Tree = {
  ...
  def makeImplicitParameters(tpts: List[Tree], implicitFlag: FlagSet, forPrimaryConstructor: Boolean = false)(using Ctx, CState): List[ValDef] =
    for (tpt <- tpts) yield {
   ...
   */
  private def defDef(meth0: DefDef, isPrimaryConstructor: Boolean = false)(using Ctx, CState): Tree = {
   ...
   */
  def quotedPattern(tree: untpd.Tree, expectedTpt: untpd.Tree)(using Ctx, CState): untpd.Tree = {
  ...
  private def addEvidenceParams(meth: DefDef, params: List[ValDef])(using Ctx, CState): DefDef =
    params match {
      case Nil =>
        meth
      case evidenceParams =>
  ...
  private def evidenceParams(meth: DefDef)(using Ctx, CState): List[ValDef] =
  ...
  def classDef(cdef: TypeDef)(using Ctx, CState): Tree = {
   ...
   *    }
   */
  def packageModuleDef(mdef: ModuleDef)(using Ctx, CState): Tree =
    val impl = mdef.impl
    val mods = mdef.mods
   ...
   *  TODO: drop this part
   */
  def moduleDef(mdef: ModuleDef)(using Ctx, CState): Tree = {
    val impl = mdef.impl
    val mods = mdef.mods
    impl.constr match {
  ...
  def extMethods(ext: ExtMethods)(using Ctx, CState): Tree = flatTree {
  ...
  def collectiveExtensionBody(stats: List[Tree],
      tparams: List[TypeDef], vparamss: List[List[ValDef]])(using Ctx, CState): List[Tree] =
   ...
   */
  def quotedPatternTypeDef(tree: TypeDef)(using Ctx, CState): TypeDef = {
   ...
   */
  def normalizeName(mdef: MemberDef, impl: Tree)(using Ctx, CState): Name = {
    var name = mdef.name
  ...
  def inventGivenOrExtensionName(impl: Tree)(using Ctx, CState): SimpleName =
    val str = impl match
      case impl: Template =>
  ...
  private class NameExtractor(followArgs: Boolean) extends UntypedTreeAccumulator[String] {
    private def extractArgs(args: List[Tree])(using Ctx, CState): String =
      args.map(argNameExtractor.apply("", _)).mkString("_")
    override def apply(x: String, tree: Tree)(using Ctx, CState): String =
      if (x.isEmpty)
        tree match {
  ...
  private def inventTypeName(tree: Tree)(using Ctx, CState): String = typeNameExtractor("", tree)
   ...
   */
  def patDef(pdef: PatDef)(using Ctx, CState): Tree = flatTree {
   ...
   */
  def makeSelector(sel: Tree, checkMode: MatchCheck)(using Ctx, CState): Tree =
   ...
   *  ValDef or DefDef.
   */
  def makePatDef(original: Tree, mods: Modifiers, pat: Tree, rhs: Tree)(using Ctx, CState): Tree = pat match {
  ...
  def patternVar(tree: Tree)(using Ctx, CState): Bind = {
   ...
   */
  def checkModifiers(tree: Tree)(using Ctx, CState): Tree = tree match {
    case tree: MemberDef =>
  ...
  def defTree(tree: Tree)(using Ctx, CState): Tree =
   ...
   *      { stats; () }
   */
  def block(tree: Block)(using Ctx, CState): Block = tree.expr match {
    case EmptyTree =>
  ...
  /** Translate infix operation expression
    ...
    */
  def binop(left: Tree, op: Ident, right: Tree)(using Ctx, CState): Apply = {
   ...
   *     (t1, ..., tN)  ==>   TupleN(t1, ..., tN)
   */
  def smallTuple(tree: Tuple)(using Ctx, CState): Tree = {
    val ts = tree.trees
    val arity = ts.length
  ...
  private def isTopLevelDef(stat: Tree)(using Ctx, CState): Boolean = stat match
   ...
   */
  def packageDef(pdef: PackageDef)(using Ctx, CState): PackageDef = {
   ...
   *      Closure($anonfun)
   */
  def makeClosure(params: List[ValDef], body: Tree, tpt: Tree = null, isContextual: Boolean)(using Ctx, CState): Block =
    Block(
   ...
   */
  def makeCaseLambda(cases: List[CaseDef], checkMode: MatchCheck, nparams: Int = 1)(using Ctx, CState): Function = {
   ...
   *  in the selector.
   */
  def makeTupledFunction(params: List[ValDef], body: Tree, isGenericTuple: Boolean)(using Ctx, CState): Tree = {
  ...
  def makeContextualFunction(formals: List[Tree], body: Tree, isErased: Boolean)(using Ctx, CState): Function = {
   ...
   */
  def makeAnnotated(fullName: String, tree: Tree)(using Ctx, CState): Annotated = {
  ...
  private def derivedValDef(original: Tree, named: NameTree, tpt: Tree, rhs: Tree, mods: Modifiers)(using Ctx, CState) = {
  ...
  def apply(tree: Tree)(using Ctx, CState): Tree = {
   ...
   */
  def makeTryCase(handlerFun: tpd.Tree)(using Ctx, CState): CaseDef =
   ...
   */
  def refinedTypeToClass(parent: tpd.Tree, refinements: List[Tree])(using Ctx, CState): TypeDef = {
   ...
   *  without duplicates
   */
  private def getVariables(tree: Tree)(using Ctx, CState): List[VarInfo] = {
    ...
    def collect(tree: Tree): Unit = tree match {
      ...
      case Quote(expr) =>
        new UntypedTreeTraverser {
          def traverse(tree: untpd.Tree)(using Ctx, CState): Unit = tree match {
