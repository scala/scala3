...
import dottyBench.tools.dotc.typer.ProtoTypes
...
object untpd extends Trees.Instance[Untyped] with UntypedTreeInfo {
  ...
  object TypedSplice {
    def apply(tree: tpd.Tree)(using Ctx, CState): TypedSplice =
  ...
  case class ModuleDef(name: TermName, impl: Template)(implicit @constructorOnly src: SourceFile)
    ...
    def withName(name: Name)(using Ctx, CState): ModuleDef = cpy.ModuleDef(this)(name.toTermName, impl)
  }
  ...
  case class Modifiers (
     ...
     *  given in the source.
     */
    def withAddedFlags(flags: FlagSet, span: Span)(using Ctx, CState): Modifiers =
  ...
  abstract class DerivedTypeTree(implicit @constructorOnly src: SourceFile) extends TypeTree {
     ...
     */
    def ensureCompletions(using Ctx, CState): Unit = ()

    /** The method that computes the tree with the derived type */
    def derivedTree(originalSym: Symbol)(using Ctx, CState): tpd.Tree
  }
   ...
   *  PrepareInlineable.
   */
  def New(tpt: Tree, argss: List[List[Tree]])(using Ctx, CState): Tree =
   ...
   */
  def makeNew(tpt: Tree)(using Ctx, CState): Tree = {
  ...
  def TypeTree(tpe: Type)(using Ctx, CState): TypedSplice = TypedSplice(TypeTree().withTypeUnchecked(tpe))

  def unitLiteral(implicit src: SourceFile): Literal = Literal(Constant(()))

  def ref(tp: NamedType)(using Ctx, CState): Tree =
  ...
  def makeConstructor(tparams: List[TypeDef], vparamss: List[List[ValDef]], rhs: Tree = EmptyTree)(using Ctx, CState): DefDef =
    DefDef(nme.CONSTRUCTOR, tparams, vparamss, TypeTree(), rhs)

  def emptyConstructor(using Ctx, CState): DefDef =
    makeConstructor(Nil, Nil)

  def makeSelfDef(name: TermName, tpt: Tree)(using Ctx, CState): ValDef =
    ValDef(name, tpt, EmptyTree).withFlags(PrivateLocal)

  def makeTupleOrParens(ts: List[Tree])(using Ctx, CState): Tree = ts match {
    case t :: Nil => Parens(t)
    case _ => Tuple(ts)
  }

  def makeTuple(ts: List[Tree])(using Ctx, CState): Tree = ts match {
    case t :: Nil => t
    case _ => Tuple(ts)
  }

  def makeAndType(left: Tree, right: Tree)(using Ctx, CState): AppliedTypeTree =
    AppliedTypeTree(ref(defn.andType.typeRef), left :: right :: Nil)

  def makeParameter(pname: TermName, tpe: Tree, mods: Modifiers, isBackquoted: Boolean = false)(using Ctx, CState): ValDef = {
    val vdef = ValDef(pname, tpe, EmptyTree)
    if (isBackquoted) vdef.pushAttachment(Backquoted, ())
    vdef.withMods(mods | Param)
  }

  def makeSyntheticParameter(n: Int = 1, tpt: Tree = null, flags: FlagSet = SyntheticTermParam)(using Ctx, CState): ValDef =
    ValDef(nme.syntheticParamName(n), if (tpt == null) TypeTree() else tpt, EmptyTree)
      .withFlags(flags)

  def lambdaAbstract(params: List[ValDef] | List[TypeDef], tpt: Tree)(using Ctx, CState): Tree =
    params match
      case Nil               => tpt
  ...
  def lambdaAbstractAll(paramss: List[List[ValDef] | List[TypeDef]], tpt: Tree)(using Ctx, CState): Tree =
   ...
   */
  def refOfDef(tree: MemberDef)(using Ctx, CState): Tree = tree match {
  ...
  def repeated(arg: Tree)(using Ctx, CState): Typed = Typed(arg, Ident(tpnme.WILDCARD_STAR))
...
// --------- Copier/Transformer/Accumulator classes for untyped trees -----
  ...
  class UntypedTreeCopier extends TreeCopier {
    ...
    def ModuleDef(tree: Tree)(name: TermName, impl: Template)(using Ctx, CState): ModuleDef = tree match {
    ...
    def ParsedTry(tree: Tree)(expr: Tree, handler: Tree, finalizer: Tree)(using Ctx, CState): TermTree = tree match {
    ...
    def SymbolLit(tree: Tree)(str: String)(using Ctx, CState): TermTree = tree match {
      case tree: SymbolLit if str == tree.str => tree
      case _ => finalize(tree, untpd.SymbolLit(str)(tree.source))
    }
    def InterpolatedString(tree: Tree)(id: TermName, segments: List[Tree])(using Ctx, CState): TermTree = tree match {
    ...
    def Function(tree: Tree)(args: List[Tree], body: Tree)(using Ctx, CState): Tree = tree match {
      case tree: Function if (args eq tree.args) && (body eq tree.body) => tree
      case _ => finalize(tree, untpd.Function(args, body)(tree.source))
    }
    def PolyFunction(tree: Tree)(targs: List[Tree], body: Tree)(using Ctx, CState): Tree = tree match {
    ...
    def InfixOp(tree: Tree)(left: Tree, op: Ident, right: Tree)(using Ctx, CState): Tree = tree match {
    ...
    def ForDo(tree: Tree)(enums: List[Tree], body: Tree)(using Ctx, CState): TermTree = tree match {
      case tree: ForDo if (enums eq tree.enums) && (body eq tree.body) => tree
      case _ => finalize(tree, untpd.ForDo(enums, body)(tree.source))
    }
    def GenFrom(tree: Tree)(pat: Tree, expr: Tree, checkMode: GenCheckMode)(using Ctx, CState): Tree = tree match {
    ...
    def GenAlias(tree: Tree)(pat: Tree, expr: Tree)(using Ctx, CState): Tree = tree match {
      case tree: GenAlias if (pat eq tree.pat) && (expr eq tree.expr) => tree
      case _ => finalize(tree, untpd.GenAlias(pat, expr)(tree.source))
    }
    def ContextBounds(tree: Tree)(bounds: TypeBoundsTree, cxBounds: List[Tree])(using Ctx, CState): TypTree = tree match {
    ...
    def PatDef(tree: Tree)(mods: Modifiers, pats: List[Tree], tpt: Tree, rhs: Tree)(using Ctx, CState): Tree = tree match {
      case tree: PatDef if (mods eq tree.mods) && (pats eq tree.pats) && (tpt eq tree.tpt) && (rhs eq tree.rhs) => tree
      case _ => finalize(tree, untpd.PatDef(mods, pats, tpt, rhs)(tree.source))
    }
    def Export(tree: Tree)(expr: Tree, selectors: List[ImportSelector])(using Ctx, CState): Tree = tree match {
    ...
    def ExtMethods(tree: Tree)(tparams: List[TypeDef], vparamss: List[List[ValDef]], methods: List[DefDef])(using Ctx, CState): Tree = tree match
    ...
    def ImportSelector(tree: Tree)(imported: Ident, renamed: Tree, bound: Tree)(using Ctx, CState): Tree = tree match {
    ...
    def Number(tree: Tree)(digits: String, kind: NumberKind)(using Ctx, CState): Tree = tree match {
      case tree: Number if (digits == tree.digits) && (kind == tree.kind) => tree
      case _ => finalize(tree, untpd.Number(digits, kind))
    }
    def TypedSplice(tree: Tree)(splice: tpd.Tree)(using Ctx, CState): ProxyTree = tree match {
      case tree: TypedSplice if splice `eq` tree.splice => tree
      case _ => finalize(tree, untpd.TypedSplice(splice)(using ctx))
    }
    def MacroTree(tree: Tree)(expr: Tree)(using Ctx, CState): Tree = tree match {
  ...
  abstract class UntypedTreeMap(cpy: UntypedTreeCopier = untpd.cpy) extends TreeMap(cpy) {
    override def transformMoreCases(tree: Tree)(using Ctx, CState): Tree = tree match {
  ...
  abstract class UntypedTreeAccumulator[X] extends TreeAccumulator[X] { self =>
    override def foldMoreCases(x: X, tree: Tree)(using Ctx, CState): X = tree match {
      case ModuleDef(name, impl) =>
        this(x, impl)
      case tree: DerivingTemplate =>
  ...
  abstract class UntypedTreeTraverser extends UntypedTreeAccumulator[Unit] {
    ...
    def apply(x: Unit, tree: Tree)(using Ctx, CState): Unit = traverse(tree)
    protected def traverseChildren(tree: Tree)(using Ctx, CState): Unit = foldOver((), tree)
  }

  /** Fold `f` over all tree nodes, in depth-first, prefix order */
  class UntypedDeepFolder[X](f: (X, Tree) => X) extends UntypedTreeAccumulator[X] {
    def apply(x: X, tree: Tree)(using Ctx, CState): X = foldOver(f(x, tree), tree)
  }

  /** Is there a subtree of this tree that satisfies predicate `p`? */
  extension (tree: Tree) def existsSubTree(p: Tree => Boolean)(using Ctx, CState): Boolean = {
    val acc = new UntypedTreeAccumulator[Boolean] {
      def apply(x: Boolean, t: Tree)(using Ctx, CState) = x || p(t) || foldOver(x, t)
    }
    acc(false, tree)
  }
