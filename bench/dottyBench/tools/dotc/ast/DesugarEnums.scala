...
object DesugarEnums {
   ...
   *  companion object.
   */
  def enumClass(using Ctx, CState): Symbol = {
    val cls = ctx.owner
    if (cls.is(Module)) cls.linkedClass else cls
  }

  /** Is `tree` an (untyped) enum case? */
  def isEnumCase(tree: Tree)(using Ctx, CState): Boolean = tree match {
   ...
   */
  def interpolatedEnumParent(span: Span)(using Ctx, CState): Tree = {
  ...
  def enumClassRef(using Ctx, CState): Tree =
    if (enumClass.exists) TypeTree(enumClass.typeRef) else TypeTree()

  /** Add implied flags to an enum class or an enum case */
  def addEnumFlags(cdef: TypeDef)(using Ctx, CState): TypeDef =
  ...
  private def registerCall(using Ctx, CState): List[Tree] =
   ...
   *       }
   */
  private def enumScaffolding(using Ctx, CState): List[Tree] = {
    val valuesDef =
   ...
   *   }
   */
  private def enumValueCreator(using Ctx, CState) = {
  ...
  def enumApplyResult(
      ...
      appliedEnumRef: Tree)(using Ctx, CState): (Tree, List[DefDef]) = {
  ...
  def typeParamIsReferenced(
    ...
    parents: List[Tree])(using Ctx, CState): Boolean = {
    ...
    object searchRef extends UntypedTreeAccumulator[Boolean] {
      ...
      def apply(x: Boolean, tree: Tree)(using Ctx, CState): Boolean = x || {
        tree match {
          case Ident(name) =>
      ...
      def apply(tree: Tree)(using Ctx, CState): Boolean =
   ...
   */
  def nextOrdinal(kind: CaseKind.Value)(using Ctx, CState): (Int, List[Tree]) = {
  ...
  def param(name: TermName, typ: Type)(using Ctx, CState) =
    ValDef(name, TypeTree(typ), EmptyTree).withFlags(Param)

  def ordinalMeth(body: Tree)(using Ctx, CState): DefDef =
    DefDef(nme.ordinalDollar, Nil, Nil, TypeTree(defn.IntType), body)

  def toStringMeth(body: Tree)(using Ctx, CState): DefDef =
    DefDef(nme.toString_, Nil, Nil, TypeTree(defn.StringType), body).withFlags(Override)

  def ordinalMethLit(ord: Int)(using Ctx, CState): DefDef =
    ordinalMeth(Literal(Constant(ord)))

  def toStringMethLit(name: String)(using Ctx, CState): DefDef =
    toStringMeth(Literal(Constant(name)))

  /** Expand a module definition representing a parameterless enum case */
  def expandEnumModule(name: TermName, impl: Template, mods: Modifiers, span: Span)(using Ctx, CState): Tree = {
  ...
  def expandSimpleEnumCase(name: TermName, mods: Modifiers, span: Span)(using Ctx, CState): Tree =
    if (!enumClass.exists) EmptyTree
