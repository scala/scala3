...
private[repl] class Rendering(parentClassLoader: Option[ClassLoader] = None) {
  ...
  private val messageRenderer = new MessageRendering {
    override def posStr(pos: SourcePosition, diagnosticLevel: String, message: Message)(using Ctx, CState): String = ""
  }
  ...
  private[repl] def classLoader()(using Ctx, CState) =
  ...
  private[repl] def replStringOf(value: Object)(using Ctx, CState): String = {
   ...
   */
  private def valueOf(sym: Symbol)(using Ctx, CState): Option[String] = {
  ...
  def renderTypeDef(d: Denotation)(using Ctx, CState): Diagnostic =
    infoDiagnostic("// defined " ++ d.symbol.showUser, d)

  def renderTypeAlias(d: Denotation)(using Ctx, CState): Diagnostic =
    infoDiagnostic("// defined alias " ++ d.symbol.showUser, d)

  /** Render method definition result */
  def renderMethod(d: Denotation)(using Ctx, CState): Diagnostic =
    infoDiagnostic(d.symbol.showUser, d)

  /** Render value definition result */
  def renderVal(d: Denotation)(using Ctx, CState): Option[Diagnostic] = {
  ...
  private def infoDiagnostic(msg: String, d: Denotation)(using Ctx, CState): Diagnostic =
...
object Rendering {
  ...
  implicit class ShowUser(val s: Symbol) extends AnyVal {
    def showUser(using Ctx, CState): String = {
      val printer = new ReplPrinter(combinedContext)
