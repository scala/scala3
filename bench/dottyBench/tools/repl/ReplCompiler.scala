...
import dottyBench.tools.dotc.core.Comments.CommentsContext
...
import dottyBench.tools.dotc.reporting.Diagnostic
import dottyBench.tools.dotc.transform.{PostTyper, Staging}
import dottyBench.tools.dotc.typer.ImportInfo
import dottyBench.tools.dotc.util.Spans._
import dottyBench.tools.dotc.util.{ParsedComment, SourceFile}
import dottyBench.tools.dotc.{CompilationUnit, Compiler, Run}
import dottyBench.tools.repl.results._
...
class ReplCompiler extends Compiler {
  ...
  def newRun(initCtx: Context, state: State): Run = new Run(this, initCtx) {
    ...
    override protected def rootContext(using Context): Context = {
      ...
      def importContext(imp: tpd.Import)(using Ctx, CState): Context =
        ctx.importContext(imp, imp.symbol).toContext

      def importPreviousRun(id: Int)(using Ctx, CState) = {
  ...
  private def createUnit(defs: Definitions, span: Span)(using Ctx, CState): CompilationUnit = {
  ...
  final def compile(parsed: Parsed)(implicit state: State): Result[(CompilationUnit, State)] = {
    ...
    val unit = createUnit(defs, Span(0, parsed.trees.last.span.end))(using state.context, state.context.cstate)
  ...
  final def typeCheck(expr: String, errorsAllowed: Boolean = false)(implicit state: State): Result[tpd.ValDef] = {
    ...
    def wrapped(expr: String, sourceFile: SourceFile, state: State)(using Ctx, CState): Result[untpd.PackageDef] = {
    ...
    def unwrapped(tree: tpd.Tree, sourceFile: SourceFile)(using Ctx, CState): Result[tpd.ValDef] = {
    ...
    inContext(state.context.fresh
      ...
      wrapped(expr, src, state).flatMap { pkg =>
        ...
        unit.untpdTree = pkg
        ctx.run.compileUnits(unit :: Nil, combinedContext)
