...
import dottyBench.tools.dotc.parsing.Scanners.Scanner
import dottyBench.tools.dotc.parsing.Tokens._
import dottyBench.tools.dotc.printing.SyntaxHighlighting
import dottyBench.tools.dotc.reporting.Reporter
...
final class JLineTerminal extends java.io.Closeable {
  ...
  private def blue(str: String)(using Ctx, CState) =
    if (ctx.settings.color.value != "never") Console.BLUE + str + Console.RESET
    else str
  private def prompt(using Ctx, CState)        = blue("scala> ")
  private def newLinePrompt(using Ctx, CState) = blue("     | ")
  ...
  private class Highlighter(using Ctx, CState) extends reader.Highlighter {
  ...
  private class Parser(using Ctx, CState) extends reader.Parser {
