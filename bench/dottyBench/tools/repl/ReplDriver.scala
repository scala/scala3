...
import dottyBench.tools.dotc.core.Phases.{unfusedPhases, typerPhase}
import dottyBench.tools.dotc.core.Denotations.Denotation
import dottyBench.tools.dotc.core.Flags._
import dottyBench.tools.dotc.core.Mode
import dottyBench.tools.dotc.core.NameKinds.SimpleNameKind
...
import dottyBench.tools.dotc.core.Symbols.{Symbol, defn}
import dottyBench.tools.dotc.interactive.Completion
import dottyBench.tools.dotc.printing.SyntaxHighlighting
import dottyBench.tools.dotc.reporting.MessageRendering
import dottyBench.tools.dotc.reporting.{Message, Diagnostic}
import dottyBench.tools.dotc.util.Spans.Span
import dottyBench.tools.dotc.util.{SourceFile, SourcePosition}
import dottyBench.tools.dotc.{CompilationUnit, Driver}
import dottyBench.tools.io._
...
class ReplDriver(settings: Array[String],
  ...
  final def runUntilQuit(initialState: State = initialState): State = {
    ...
    def readLine(state: State): ParseResult = {
      ...
      given CState = state.context.cstate
      try {
  ...
  protected final def completions(cursor: Int, expr: String, state0: State): List[Candidate] = {
    ...
    compiler
      ...
      .map { tree =>
        ...
        val unit = CompilationUnit(file)(using state.context, state.context.cstate)
        unit.tpdTree = tree
        given ctx as Context = state.context.fresh.setCompilationUnit(unit)
  ...
  private def compile(parsed: Parsed, istate: State): State = {
    ...
    implicit val state: State = {
      ...
      state0.copy(context = state0.context.withSource(parsed.source)
        .toContext(using state0.context.cstate))
    }
    compiler
      .compile(parsed)
      .fold(
  ...
  private def interpretCommand(cmd: Command)(implicit state: State): State = cmd match {
    ...
    case Imports =>
      ...
      } out.println(imp.show(using state.context, state.context.cstate))
      state
    ...
    case TypeOf(expr) =>
      compiler.typeOf(expr)(newRun(state)).fold(
        displayErrors,
        res => out.println(SyntaxHighlighting.highlight(res)(using state.context, state.context.cstate))
      )
      state
