...
import dottyBench.tools.dotc.ast.Trees.{PackageDef, ValDef}
...
import dottyBench.tools.dotc.sbt.ExtractDependencies
import Contexts._
import Phases._
import Symbols._
import Decorators._
...
class GenBCodePipeline(val int: DottyBackendInterface)(using Ctx, CState) extends BCodeSyncAndTry {
  ...
  /** Convert a `dottyBench.tools.io.AbstractFile` into a
   *  `dottyBench.tools.dotc.interfaces.AbstractFile`.
   */
  private def convertAbstractFile(absfile: dottyBench.tools.io.AbstractFile): interfaces.AbstractFile =
...
//  class BCodePhase() {
  ...
  case class SubItem2(classNode: asm.tree.ClassNode,
                      file:      dottyBench.tools.io.AbstractFile)
  ...
  case class SubItem3(
                        ...
                        jclassFile:  dottyBench.tools.io.AbstractFile
                        )
