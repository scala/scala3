...
class JUnitBootstrappers extends MiniPhase {
  ...
  override def transformPackageDef(tree: PackageDef)(using Ctx, CState): Tree = {
  ...
  private def genBootstrapper(testClass: ClassSymbol)(using Ctx, CState): TypeDef = {
    val junitdefn = jsdefn.junit
  ...
  private def genConstructor(owner: ClassSymbol)(using Ctx, CState): DefDef = {
  ...
  private def genCallOnModule(owner: ClassSymbol, name: TermName, module: Symbol, annot: Symbol)(using Ctx, CState): DefDef = {
  ...
  private def genCallOnParam(owner: ClassSymbol, name: TermName, testClass: ClassSymbol, annot: Symbol)(using Ctx, CState): DefDef = {
    val sym = newSymbol(owner, name, Synthetic | Method,
  ...
  private def genTests(owner: ClassSymbol, tests: List[Symbol])(using Ctx, CState): DefDef = {
    val junitdefn = jsdefn.junit
  ...
  private def genInvokeTest(owner: ClassSymbol, testClass: ClassSymbol, tests: List[Symbol])(using Ctx, CState): DefDef = {
    val junitdefn = jsdefn.junit
  ...
  private def genTestInvocation(testClass: ClassSymbol, testMethod: Symbol, instance: Tree)(using Ctx, CState): Tree = {
    val junitdefn = jsdefn.junit
  ...
  private def genNewInstance(owner: ClassSymbol, testClass: ClassSymbol)(using Ctx, CState): DefDef = {
  ...
  private def castParam(param: Symbol, clazz: Symbol)(using Ctx, CState): Tree =
    ref(param).cast(clazz.typeRef)

  private def annotatedMethods(owner: ClassSymbol, annot: Symbol)(using Ctx, CState): List[Symbol] =
