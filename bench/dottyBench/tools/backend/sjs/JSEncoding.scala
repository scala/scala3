...
object JSEncoding {
  ...
  class LocalNameGenerator {
    ...
    def localSymbolName(sym: Symbol)(using Ctx, CState): LocalName = {
    ...
    def labelSymbolName(sym: Symbol)(using Ctx, CState): LabelName =
  ...
  def encodeLabelSym(sym: Symbol)(
      implicit ctx: Ctx, cs: CState, pos: ir.Position, localNames: LocalNameGenerator): js.LabelIdent = {
  ...
  def encodeFieldSym(sym: Symbol)(
      implicit ctx: Ctx, cs: CState, pos: ir.Position): js.FieldIdent = {
  ...
  def encodeMethodSym(sym: Symbol, reflProxy: Boolean = false)(
      implicit ctx: Ctx, cs: CState, pos: ir.Position): js.MethodIdent = {
  ...
  private def paramOrResultTypeRef(tpe: Type)(using Ctx, CState): jstpe.TypeRef = {
    toTypeRef(tpe) match {
  ...
  def encodeLocalSym(sym: Symbol)(
      implicit ctx: Ctx, cs: CState, pos: ir.Position, localNames: LocalNameGenerator): js.LocalIdent = {
    require(!sym.owner.isClass && sym.isTerm && !sym.is(Flags.Method) && !sym.is(Flags.Module),
        "encodeLocalSym called with non-local symbol: " + sym)
    js.LocalIdent(localNames.localSymbolName(sym))
  }

  def encodeClassType(sym: Symbol)(using Ctx, CState): jstpe.Type = {
  ...
  def encodeClassRef(sym: Symbol)(using Ctx, CState): jstpe.ClassRef =
    jstpe.ClassRef(encodeClassName(sym))

  def encodeClassNameIdent(sym: Symbol)(
      implicit ctx: Ctx, cs: CState, pos: ir.Position): js.ClassIdent =
    js.ClassIdent(encodeClassName(sym))

  def encodeClassName(sym: Symbol)(using Ctx, CState): ClassName = {
    val sym1 =
  ...
  def toIRType(tp: Type)(using Ctx, CState): jstpe.Type = {
  ...
  def toTypeRef(tp: Type)(using Ctx, CState): jstpe.TypeRef =
    toTypeRefInternal(tp)._1

  private def toTypeRefInternal(tp: Type)(using Ctx, CState): (jstpe.TypeRef, Symbol) = {
   ...
   *  `sym.info.resultType`.
   */
  def patchedResultType(sym: Symbol)(using Ctx, CState): Type =
    if (sym.isConstructor) defn.UnitType
    else sym.info.resultType

  def originalNameOfLocal(sym: Symbol)(
      implicit ctx: Ctx, cs: CState, localNames: LocalNameGenerator): OriginalName = {
  ...
  def originalNameOfField(sym: Symbol)(using Ctx, CState): OriginalName =
    originalNameOf(sym.name)

  def originalNameOfMethod(sym: Symbol)(using Ctx, CState): OriginalName =
    originalNameOf(sym.name)

  def originalNameOfClass(sym: Symbol)(using Ctx, CState): OriginalName =
