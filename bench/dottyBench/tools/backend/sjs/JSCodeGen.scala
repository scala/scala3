...
class JSCodeGen()(using genCtx: Ctx, cs: CState) {
  ...
  private val positionConversions = new JSPositions()(using genCtx, cs)
  ...
  private def getFileFor(cunit: CompilationUnit, className: ClassName,
      suffix: String): dottyBench.tools.io.AbstractFile = {
      ...
      withNewLocalNameScope {
        ...
        if (existingPublicStaticMethodNames.contains(methodIdent.name)) {
          report.error(
              ...
              "Please report this as a bug in the Scala.js support in dottyBench.",
              pos)
        }
