...
import dottyBench.tools.backend.jvm.DottyPrimitives
...
class JSPrimitives(ictx: Ctx, cs: CState) extends DottyPrimitives(ictx, cs) {
  import JSPrimitives._
  import dottyBench.tools.backend.ScalaPrimitivesOps._

  private lazy val jsPrimitives: Map[Symbol, Int] = initJSPrimitives(using ictx, cs)

  override def getPrimitive(sym: Symbol): Int =
    jsPrimitives.getOrElse(sym, super.getPrimitive(sym))

  override def getPrimitive(app: Apply, tpe: Type)(using Ctx, CState): Int =
    jsPrimitives.getOrElse(app.fun.symbol, super.getPrimitive(app, tpe))

  override def isPrimitive(fun: Tree): Boolean =
    jsPrimitives.contains(fun.symbol(using ictx, cs)) || super.isPrimitive(fun)

  /** Initialize the primitive map */
  private def initJSPrimitives(using Ctx, CState): Map[Symbol, Int] = {
