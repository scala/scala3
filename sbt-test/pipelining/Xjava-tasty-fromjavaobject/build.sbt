lazy val a = project.in(file("a"))
  .settings(
    ...
    scalacOptions ++= Seq("-Xearly-tasty-output", ((ThisBuild / baseDirectory).value / "a-enum-java-tasty.jar").toString),
...
lazy val aCheck = project.in(file("a-check"))
  .settings(
    ...
    compileOrder := CompileOrder.Mixed, // ensure we send java sources to Scala compiler
    scalacOptions += "-Xjava-tasty", // enable pickling of java signatures
    scalacOptions ++= Seq("-Xearly-tasty-output", ((ThisBuild / baseDirectory).value / "a-enum-java-tasty-2.jar").toString),
