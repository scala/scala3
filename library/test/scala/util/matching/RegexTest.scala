...
import tools.AssertUtil.{assertCond, assertThrows}
...
class RegexTest {
  ...
  @Test def groupsMatching: Unit = {
    val Date = """(\d+)/(\d+)/(\d+)""".r
    for case Regex.Groups(a, b, c) <- Date.findFirstMatchIn("1/1/2001 marks the start of the millennium. 31/12/2000 doesn't.") do
      assertEquals("1", a)
      assertEquals("1", b)
      assertEquals("2001", c)
    for case Regex.Groups(a, b, c) <- Date.findAllIn("1/1/2001 marks the start of the millennium. 31/12/2000 doesn't.").matchData do
  ...
  @Test def `t6406 no longer unapply any`: Unit = {
    ...
    val is = ns collect { case u(x) => x.nn } map { case r(x) => x }
    ...
    val y1: Option[String] = for case p1(year, month, day) <- p1.findFirstIn(t) yield year.nn
    assertEquals(Some("2011"), y1)
    val y2: Option[String] = for case p1(year, month, day) <- p1.findFirstMatchIn(t) yield year.nn
