...
class StepperConversionTest {
  ...
  def yay(x: => Stepper[?])(implicit correctSpec: SpecCheck): Unit = {
    correctSpec.assert(x)
    assertTrue(isAcc(x))
  }

  def nay[X](x: => Stepper[?])(implicit correctSpec: SpecCheck): Unit = {
    correctSpec.assert(x)
    assertFalse(isAcc(x))
  }
  ...
  def comprehensivelyGeneric(): Unit = {
    implicit val spec = SpecCheck(_.isInstanceOf[AnyStepper[?]])
