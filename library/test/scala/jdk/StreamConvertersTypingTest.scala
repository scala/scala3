...
class StreamConvertersTypingTest {
  ...
  def keyValueSteppers(): Unit = {
    ...
    (m2ks: IntStepper & EfficientSplit).nextStep()
    val m2vs = m2.valueStepper
    (m2vs: DoubleStepper & EfficientSplit).nextStep()
...
//    val bla2 = bl.to(IntAccumulator) // Could implement a WideningAccumulator to support that.
  ...
  def stepperSeqStreamParStreamMethods(): Unit = {
    ...
    locally {
      ...
      val ivs = iv.stepper
      (ivs: IntStepper & EfficientSplit).nextStep(): Int
      val ias = ia.stepper
      (ias: IntStepper & EfficientSplit).nextStep(): Int
      val bvs = bv.stepper
      (bvs: IntStepper & EfficientSplit).nextStep(): Int
      val bas = ba.stepper
      (bas: IntStepper & EfficientSplit).nextStep(): Int
      val sls = sl.stepper
      (sls: AnyStepper[String] /*with EfficientSubstep*/).nextStep(): String
      val svs = sv.stepper
      (svs: AnyStepper[String] & EfficientSplit).nextStep(): String
      val sas = sa.stepper
      (sas: AnyStepper[String] & EfficientSplit).nextStep(): String
    }
  ...
  def bitSetStepper(): Unit = {
    locally {
      ...
      @unused val s3: IntStepper & EfficientSplit = ibs.stepper
      @unused val s4: Stepper[Int] = ibs.stepper
      @unused val s5: Stepper[Int] & EfficientSplit = ibs.stepper
      @unused val s6: AnyStepper[Int] = ibs.stepper[AnyStepper[Int]]
      @unused val s7: AnyStepper[Int] & EfficientSplit = ibs.stepper[AnyStepper[Int]]
    ...
    locally {
      ...
      @unused val s3: IntStepper & EfficientSplit = mbs.stepper
      @unused val s4: Stepper[Int] = mbs.stepper
      @unused val s5: Stepper[Int] & EfficientSplit = mbs.stepper
      @unused val s6: AnyStepper[Int] = mbs.stepper(using StepperShape.anyStepperShape[Int])
      @unused val s7: AnyStepper[Int] & EfficientSplit = mbs.stepper(using StepperShape.anyStepperShape[Int])
  ...
  def rangeStepper(): Unit = {
    ...
    @unused val s3: IntStepper & EfficientSplit = r.stepper
    @unused val s4: Stepper[Int] = r.stepper
    @unused val s5: Stepper[Int] & EfficientSplit = r.stepper
    @unused val s6: AnyStepper[Int] = r.stepper[AnyStepper[Int]]
    @unused val s7: AnyStepper[Int] & EfficientSplit = r.stepper[AnyStepper[Int]]
  ...
  def stringStepper(): Unit = {
    ...
    def s3: IntStepper & EfficientSplit = r.stepper
    @unused def s4: Stepper[Int] = r.stepper
    def s5: Stepper[Int] & EfficientSplit = r.stepper

    def cs1: IntStepper & EfficientSplit = r.charStepper
    def ps1: IntStepper & EfficientSplit = r.codePointStepper
  ...
  def accumulatorStepper(): Unit = {
    ...
    locally {
      ...
      val ias = ia.stepper
      (ias: IntStepper & EfficientSplit).asJavaParStream.count()
      ...
      val sas = sa.stepper
      (sas: Stepper[String] & EfficientSplit).asJavaParStream.count()
  ...
  def arrayBufferStepper(): Unit = {
    ...
    val sbS = sb.stepper
    (sbS: AnyStepper[String] & EfficientSplit).asJavaParStream.count()
    val sbAS = anyStepper(sb)
    (sbAS: AnyStepper[String]).asJavaSeqStream.count()

    val ibS = ib.stepper
    (ibS: IntStepper & EfficientSplit).asJavaParStream.count()
    val ibAS = anyStepper(ib)
    (ibAS: AnyStepper[Int]).asJavaSeqStream.count()

    val bbS = bb.stepper
    (bbS: AnyStepper[Boolean] & EfficientSplit).asJavaParStream.count()
