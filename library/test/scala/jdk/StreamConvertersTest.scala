...
class StreamConvertersTest {
  ...
  def newStream(n: Int): java.util.stream.Stream[String] = java.util.Arrays.stream(arrayO(n))
  def newDoubleStream(n: Int): DoubleStream = java.util.Arrays.stream(arrayD(n))
  def newIntStream(n: Int): IntStream = java.util.Arrays.stream(arrayI(n))
  def newLongStream(n: Int): LongStream = java.util.Arrays.stream(arrayL(n))
  ...
  def streamAccumulate(): Unit = {
    for (n <- ns) {
      ...
      assert(accO.isInstanceOf[AnyAccumulator[?]], s"stream $n to generic accumulator")
