...
class StepperTest {
  ...
  def sameElems(l: List[?], o: IterableOnce[?], testElemOrder: Boolean): Unit =
    if (testElemOrder)
  ...
  class MyIterableOnce extends IterableOnce[Int] {
    ...
    override def stepper[S <: Stepper[?]](implicit shape: StepperShape[Int, S]): S = data.stepper[S]
  }
}
