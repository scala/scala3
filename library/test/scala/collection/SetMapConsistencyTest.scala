...
class SetMapConsistencyTest {
  ...
  def boxMarm[A <: AnyRef] = new BoxMutableMap[A, cm.AnyRefMap[A, Int]](new cm.AnyRefMap[A, Int](_ => -1), "mutable.AnyRefMap") {
    ...
    override def get(n: Int, a: A) = n match {
      ...
      case 1 => m(a)
      case 2 => m.getOrElse(a, -1)
      case 3 => val x = arm.getOrNull(a); if !arm.contains(a) then -1 else x.nn
      case _ => oor("get", n)
    }
  ...
  def boxMjm = new BoxMutableMap[Long, cm.LongMap[Int]](new cm.LongMap[Int](_ => -1), "mutable.LongMap") {
    ...
    override def get(n: Int, a: Long) = n match {
      ...
      case 1 => m(a)
      case 2 => m.getOrElse(a, -1)
      case 3 => val x = lm.getOrNull(a); if !lm.contains(a) then -1 else x.nn
      case _ => oor("get", n)
    }
    override def fiddlers = 2
    override def fiddle(n: Int): Unit = { n match {
  ...
  def churn[A](map1: MapBox[A], map2: MapBox[A], keys: Array[A], n: Int = 1000, seed: Int = 42, valuer: Int => Int = identity) = {
    ...
    for (i <- 0 until n) {
      if (!check) {
        val temp = map2 match {
          case b: BoxImmutableMap[?, ?] => b.m match {
            case hx: ci.HashMap[?, ?] =>
  ...
  val stringKeys = intKeys.map(_.toString) ++ Array("")
  val anyKeys    = (stringKeys ++ Array(0L): Array[Any]) ++ Array(true) ++ Array(math.Pi)
  ...
  def extraMutableAnyRefMapTests(): Unit = {
    ...
    assert {
      val arm2 = new AnyRefMap[String, String](x => x)
      arm2.+=("cod" -> "fish", "Rarity" -> "unicorn")
      val hm2 = (new HashMap[String,String]) ++= arm2
      List("cod", "sparrow", "Rarity").forall { i =>
