...
class VectorTest {
  ...
  def hasCorrectDropAndTakeMethods(): Unit = {
    val v = Vector(0) ++ Vector(1 to 64*)

    assertEquals(Vector(0, 1), v take 2)
    assertEquals(Vector(63, 64), v takeRight 2)
    assertEquals(Vector(2 to 64*), v drop 2)
    assertEquals(Vector(0 to 62*), v dropRight 2)
...
//      println((i, size))
  ...
  def vectorIteratorDrop(): Unit = {
    val underlying = Vector(0 to 10010*)
  ...
  def vectorIteratorRepeated(): Unit = {
    val underlying = Vector(1 to 10001*)
...
object VectorUtils {
  ...
  def validate[T](v: Vector[T]): Unit = {
  ...
  def sliceName[T](v: Vector[T], i: Int): String = {
    val count = v.vectorSliceCount
    val level = (count+1)/2
    if(i+1 == level) "data"
  ...
  def toDebugString[T](v: Vector[T]): String = {
    ...
    for(i <- 0 until v.vectorSliceCount)
      logArray[T](sb, v.vectorSlice(i).asInstanceOf[Array[T | Null]], prefix = s"${sliceName(v, i)}: ")
    sb.result()
  }
  ...
  @unused private def toDebugString(v: VectorBuilder[?]): String = {
    val sb = new StringBuilder()
    sb.append(s"$v\n")
    val d = v.getData.asInstanceOf[Array[Array[AnyRef]]]
    logArray(sb, d(5).asInstanceOf[Array[AnyRef | Null]], prefix = "a6: ")
    logArray(sb, d(4).asInstanceOf[Array[AnyRef | Null]], "a5: ", "  ", d(5).asInstanceOf[Array[Array[AnyRef]]], "a6")
    logArray(sb, d(3).asInstanceOf[Array[AnyRef | Null]], "a4: ", "  ", d(4).asInstanceOf[Array[Array[AnyRef]]], "a5")
    logArray(sb, d(2).asInstanceOf[Array[AnyRef | Null]], "a3: ", "  ", d(3).asInstanceOf[Array[Array[AnyRef]]], "a4")
    logArray(sb, d(1).asInstanceOf[Array[AnyRef | Null]], "a2: ", "  ", d(2).asInstanceOf[Array[Array[AnyRef]]], "a3")
    logArray(sb, d(0).asInstanceOf[Array[AnyRef | Null]], "a1: ", "  ", d(1).asInstanceOf[Array[Array[AnyRef]]], "a2")
    sb.result()
  }

  private def validateArrays[T](a: Array[Array[T]], name: String): Int = {
    var i = 0
    var total = 0
    while(i < a.length) {
  ...
  private def logArray[T](sb: StringBuilder, a: Array[T | Null], prefix: String, indent: String = "  ", findIn: Array[Array[T]] | Null = null, findInName: String | Null = "<array>"): StringBuilder = {
    def classifier(x: T | Null): String =
      if(x == null) "-"
      else if(x.isInstanceOf[Array[AnyRef]]) "A"
      else if((x: Any).isInstanceOf[Int]) x.toString
      else "o"
    def atos(a: Array[? <: T | Null]): String =
      if(a eq null) "-"
      else {
        var i = 0
      ...
      if(idx >= 0) {
        ...
        while(i < a.length) {
          if(a(i).isInstanceOf[Array[AnyRef]]) {
            logArray(sb, a(i).asInstanceOf[Array[T | Null]], s"$i. ", indent = indent + "  ", null, null)
          }
          i += 1
        }
      }
