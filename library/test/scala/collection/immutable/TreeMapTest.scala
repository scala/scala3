...
class TreeMapTest extends AllocationTest {
  ...
  @Test def equalFastPath(): Unit = {
    ...
    val b1 = TreeMap.newBuilder[K, V2]
    for ( i <- 10 to 1000) {
      b1 += new K(i.toString) -> new V2(s"$i value")
    }
    val tree1 = b1.result()
    compareCount = 0
    ...
    exp += new K("XXX") -> new V2("YYY")
    act += new K("XXX") -> new V2("YYY")
  ...
  @Test def consistentEquals(): Unit = {
    ...
    val b1_1 = TreeMap.newBuilder[K, V](using o1)
    val b1_2 = TreeMap.newBuilder[K, V](using o1)

    val b2_1 = TreeMap.newBuilder[K, V](using o2_1)
    val b2_2 = TreeMap.newBuilder[K, V](using o2_2)
  ...
  @Test def caseIndependent1(): Unit = {
    val m = scala.collection.immutable.TreeMap.from[String, String](Nil)(using _.compareToIgnoreCase(_))
    val r = m ++ Seq("a" -> "1", "A" -> "2")
  ...
  private def assertMapSameElements[K, V](expected: Map[K, V], map: Map[K, V]): Unit = {
    def withoutOrdering(m: Map[K, V]): Map[K, V] = collection.immutable.Map.apply(m.iterator.toSeq*)
    assertEquals(withoutOrdering(expected), withoutOrdering(map))
  }

  @Test def removeNonContent(): Unit = {
    val src: Map[Int, String] = TreeMap(Range(0, 100, 2).map((_, ""))*)
    for (i <- Range(-1, 101, 2)) {
      src - i
