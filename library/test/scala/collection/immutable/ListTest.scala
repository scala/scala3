...
class ListTest extends AllocationTest {
  ...
  def testIteratorGC(): Unit = {
    ...
    while {
      ...
      num+=1
      emptyIterators.forall(_._2.get.isDefined) && num<1000
    } do ()
  ...
  def factoryReuse(): Unit = {
    val ls = List("a")
    assertSame(ls, List.apply(ls*))
    assertSame(ls, List.from(ls))
  }

  @Test def checkSearch(): Unit = SeqTests.checkSearch(List(0 to 1000*), 15,  implicitly[Ordering[Int]])
