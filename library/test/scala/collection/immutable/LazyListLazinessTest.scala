...
class LazyListLazinessTest {
  ...
  def sizeCompare_properlyLazy(): Unit =
    for factory <- List[SeqFactory[Seq]](LazyList, Vector) do
  ...
  def corresponds_properlyLazy(): Unit = {
    ...
    def check(lazyList: LazyList[Int], withChecker: LazinessChecker => Unit): Unit = {
      val checker = fresh()
  ...
  def apply_companion_properlyLazy(): Unit = {
    genericLazyOp_properlyLazy(LazyList(_*))
  }
...
private object LazyListLazinessTest {
  ...
  sealed abstract class LazinessChecker extends Serializable {
    ...
    protected def internalCheckIndex(index: Int): Unit =
    ...
    private def assertUnchecked(evaluated: Boolean,
                                      index: Int,
    ...
    private def assertCheckedInternal(evaluated: Boolean,
                                            index: Int,
                                            ex: ExceptionProvider): Unit = {
    ...
    private def assertChecked(evaluated: Boolean,
                                    index: Int,
                                    ex: ExceptionProvider): Unit = {
      checkIndex(index)
      assertUnchecked(evaluated, index, ex)
    ...
    private def assertAllImpl(evaluated: Boolean,
  ...
  final class FactoryLazinessChecker extends LazinessChecker {
    private var ll: LazyList[Int] = uninitialized
