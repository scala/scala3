...
class HashSetTest {
  @Test def t7326(): Unit = {
    def testCorrectness(): Unit = {
      ...
      subsetTest(HashSet.empty[Collision], HashSet.empty[Collision], Collision.apply, 100)

      // test the HashSet/other set case for Collision keys
      subsetTest(HashSet.empty[Collision], ListSet.empty[Collision], Collision.apply, 100)
    }
    ...
    def testNoHashCodeInvocationsDuringSubsetOf() = {
      ...
      val a = HashSet.empty ++ (0 until 100).map(HashCodeCounter.apply)
      val b = HashSet.empty ++ (0 until 50).map(HashCodeCounter.apply)
      val count0 = count
