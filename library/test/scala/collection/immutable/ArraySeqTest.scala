...
class ArraySeqTest {
  ...
  @Test def checkSearch(): Unit = SeqTests.checkSearch(ArraySeq(0 to 1000*), 15,  implicitly[Ordering[Int]])
  ...
  private def assertArraySeqAndType[A](actual: ArraySeq[A], expect: ArraySeq[A], expectedArrayType: Class[?]): Unit = {
  ...
  def from(): Unit = {
    ...
    assertSame(as, ArraySeq(as*))
  }

  private def assertConcat[A](lhs: ArraySeq[A], rhs: ArraySeq[A], expect: ArraySeq[A]): Array[?] = {
