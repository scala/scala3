...
class TreeSetTest extends AllocationTest {
  ...
  def min(): Unit = {
    ...
    assertEquals(3, TreeSet(1, 2, 3).min(using implicitly[Ordering[Int]].reverse))
  ...
  def max(): Unit = {
    ...
    assertEquals(1, TreeSet(1, 2, 3).max(using implicitly[Ordering[Int]].reverse))
  ...
  def t11637(): Unit = {
    ...
    val instrs = List[RB.Tree[Int, Null] | Null => RB.Tree[Int, Null] | Null](
    ...
    var t: RB.Tree[Int, Null] | Null = null
  ...
  @Test def consistentEquals(): Unit = {
    ...
    val b1_1 = TreeSet.newBuilder[K](using o1)
    val b1_2 = TreeSet.newBuilder[K](using o1)

    val b2_1 = TreeSet.newBuilder[K](using o2_1)
    val b2_2 = TreeSet.newBuilder[K](using o2_2)
  ...
  @Test def diff(): Unit = {
    ...
    for (set <- scala.List[IterableOnce[Int]](removeList, removeVec, removeVec.iterator, removeISS, removeMSS, removeIBS, removeMBS)) {
      assertEquals(expected, src -- set)
    }
  }
