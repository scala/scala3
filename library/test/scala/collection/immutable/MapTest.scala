...
class MapTest {
  ...
  @Test def builderCompare2(): Unit = {
    for (size <- 0 to 100;
      ...
      val sortMap  = SortedMap(data*)
      val expected = HashMap(data*)
  ...
  @Test def addition(): Unit = {
    for (size <- 0 to 100;
      ...
      val addSortSame    = SortedMap(addList*)(using Ordering[String])
      val addSortReverse = SortedMap(addList*)(using Ordering[String].reverse)
      val addTreeSame    = TreeMap(addList*)(using Ordering[String])
      val addTreeReverse = TreeMap(addList*)(using Ordering[String].reverse)
  ...
  def t12699(): Unit = {
    ...
    assertEquals(7, m1.+(elem1 = 2 -> 2, elem2 = 3 -> 3, elems = List(4 -> 4, 5 -> 5, 6 -> 6, 7 -> 7)*).size)
