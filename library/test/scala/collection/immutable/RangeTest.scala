...
class RangeTest {
  ...
  def sorted(): Unit = {
    ...
    assertEquals(List(9, 7, 5, 3, 1), x1.sorted(using reverseOrd))

    val x2 = 10 to 1 by -3
    assertEquals(List(1, 4, 7, 10), x2.sorted)
    assertEquals(List(10, 7, 4, 1), x2.sorted(using reverseOrd))
  }
  ...
  def rangeMinMax(): Unit = {
    ...
    assertEquals(20, (1 to 20).min(using Ordering.Int.reverse))
    assertEquals(1, (1 to 20).max(using Ordering.Int.reverse))

    assertEquals(20, (20 to 1 by -1).min(using Ordering.Int.reverse))
    assertEquals(1, (20 to 1 by -1).max(using Ordering.Int.reverse))
  }
