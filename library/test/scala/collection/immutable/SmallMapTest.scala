...
class SmallMapTest extends AllocationTest {
  def iterator(m: Map[?, ?])       = m.iterator
  def keysIterator(m: Map[?, ?])   = m.keysIterator
  def valuesIterator(m: Map[?, ?]) = m.valuesIterator
  ...
  def assertResult(reportedSize: Int, expected: AnyRef *): Unit = {
    assertTrue(nonAllocationResult(0).isInstanceOf[Iterator[?]])
