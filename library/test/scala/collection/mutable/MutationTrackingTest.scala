...
abstract class MutationTrackingTest[+C <: Iterable[?]](factory: Factory[Int, C]) {
  private type VoI = C => IterableOnceOps[?, AnyConstr, ?]
  // if you do bad things with this by returning a different builder, it WILL bite you
  protected type BuildSequence = Builder[Int, C @uV] => Builder[Int, C @uV]
  protected val defaultBuildSequence: BuildSequence = _ += 1 += 2 += 3 += 4

  private def runOp(op: C => Any, bs: BuildSequence, viewOrIterator: VoI): Unit = {
  ...
  private def runOpMaybeThrowing(op: C => Any, bs: BuildSequence, throws: Boolean, viewOrIterator: VoI): Unit = {
  ...
  private def runOpForViewAndIterator(op: C => Any, bs: BuildSequence, throws: Boolean): Unit = {
  ...
  protected def checkFine(op: C => Any, buildSequence: BuildSequence = defaultBuildSequence): Unit =
    runOpForViewAndIterator(op, buildSequence, throws = false)

  /** Checks that an exception is thrown by an operation. */
  protected def checkThrows(op: C => Any, buildSequence: BuildSequence = defaultBuildSequence): Unit =
...
object MutationTrackingTest {
  ...
  trait ClearableTest { self: MutationTrackingTest[Clearable & I] =>
    @Test
    def clear(): Unit = checkThrows { _.clear() }
  }

  trait GrowableTest extends ClearableTest { self: MutationTrackingTest[Growable[Int] & I] =>
    @Test
  ...
  trait ShrinkableTest { self: MutationTrackingTest[Shrinkable[Int] & I] =>
    @Test
