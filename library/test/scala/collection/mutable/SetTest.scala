...
class SetTest {
  ...
  class MySet(self: Set[String]) extends Set[String] with SetOps[String, Set, MySet] {
    override protected def fromSpecific(coll: IterableOnce[String]): MySet = new MySet(iterableFactory.from(coll))
    override protected def newSpecificBuilder: Builder[String, MySet] = iterableFactory.newBuilder[String].mapResult(new MySet(_))
