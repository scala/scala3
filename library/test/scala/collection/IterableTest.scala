...
class IterableTest {
  ...
  def emptyToTraversable(): Unit = {
    assert(Iterable.empty == Array.empty[Any].toIterable)
    assert(Iterable.empty == Array.empty[Any].toTraversable)
    ...
    assert(Iterable.empty == mutable.ArraySeq.empty[Any].toTraversable)
    ...
    assert(Iterable.empty == mutable.Traversable.empty.toTraversable)
    assert(Iterable.empty == mutable.UnrolledBuffer.empty[Any].toTraversable)
    assert(Map.empty      == mutable.WeakHashMap.empty.toTraversable)
    assert(Iterable.empty == mutable.WrappedArray.empty[Any].toTraversable)
  }
  ...
  object SingleUseIterable {
    def apply[A](xs: A*): SingleUseIterable[A] = new SingleUseIterable(xs*)
  }
