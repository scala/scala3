...
class BuildFromTest {
  ...
  def optionSequence1[CC[X] <: IterableOps[X, CC, ?], A](xs: CC[Option[A]]): Option[CC[A]] =
    xs.foldLeft[Option[Builder[A, CC[A]]]](Some(xs.iterableFactory.newBuilder[A])) {
      case (Some(builder), Some(a)) => Some(builder += a)
      case _ => None
    }.map(_.result())
  def optionSequence1[CC[X] <: SortedSet[X] & SortedSetOps[X, CC, CC[X]], A : Ordering](xs: CC[Option[A]]): Option[CC[A]] =
  ...
  def optionSequence2Test(): Unit = {
    ...
    val xs4 = List[Option[(Int, String)]](Some((1 -> "a")), Some((2 -> "b")))
    val o4 = optionSequence2(xs4)(using immutable.TreeMap) // same syntax as in `.to`
  ...
  def optionSequence3Test(): Unit = {
    ...
    val o4 = optionSequence3(xs4)(using immutable.TreeMap) // same syntax as in `.to`
    val o4t: Option[immutable.TreeMap[Int, String]] = o4
    assertEquals(Some(immutable.TreeMap(1 -> "a", 2 -> "b")), o4t)
  }
  ...
  implicitly[BuildFrom[immutable.IntMap[?], (Int, String), immutable.IntMap[String]]]
  implicitly[BuildFrom[mutable.LongMap[?], (Long, String), mutable.LongMap[String]]]
  implicitly[BuildFrom[immutable.LongMap[?], (Long, String), immutable.LongMap[String]]]
  implicitly[BuildFrom[mutable.AnyRefMap[? <: AnyRef, ?], (String, String), mutable.AnyRefMap[String, String]]]
  //implicitly[BuildFrom[mutable.AnyRefMap[String, String], (String, String), ?]]

  // Check that collection companions can implicitly be converted to a `BuildFrom` instance
  Iterable: BuildFrom[?, Int, Iterable[Int]]
  Map: BuildFrom[?, (Int, String), Map[Int, String]]
  SortedSet: BuildFrom[?, Int, SortedSet[Int]]
  SortedMap: BuildFrom[?, (Int, String), SortedMap[Int, String]]
  immutable.IntMap: BuildFrom[?, (Int, String), immutable.IntMap[String]]
  immutable.LongMap: BuildFrom[?, (Long, String), immutable.LongMap[String]]
  mutable.LongMap: BuildFrom[?, (Long, String), mutable.LongMap[String]]
  mutable.AnyRefMap: BuildFrom[?, (String, String), mutable.AnyRefMap[String, String]]
