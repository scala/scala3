...
class CollectionConvertersTest {
  ...
  @Test def `t11894 Map wrapper respects put contract`(): Unit = {
    val sut = new JMap[String, String | Null].asScala
    assertTrue(sut.isInstanceOf[JavaCollectionWrappers.JMapWrapper[?, ?]])
  ...
  @Test def `t11894 Dictionary wrapper disallows nulls`(): Unit = {
    val tbl = new JTable[String, String | Null]
    val sut = tbl.asInstanceOf[Dictionary[String, String | Null]].asScala
    assertTrue(sut.isInstanceOf[JavaCollectionWrappers.JDictionaryWrapper[?, ?]])
  ...
  @Test def `t11894 Properties wrapper enforces Strings`(): Unit = {
    ...
    // no longer possible with explicit nulls
    // assertThrows[NullPointerException](sut.put("any", null))
  ...
  @Test def `t11894 JDK concurrent Map impl disallows nulls`(): Unit = {
    val sut = new JCMap[String, String | Null].asScala
    assertTrue(sut.isInstanceOf[JavaCollectionWrappers.JConcurrentMapWrapper[?, ?]])
    assertThrows[NullPointerException](sut.put("any", null))
  }
}
