...
class MapWrapperTest {
  ...
  def testContains(): Unit = {
    ...
    // in this case.
    class MyScalaMap extends scala.collection.mutable.HashMap[String | Null, String] {
      ...
      override def contains(key: String | Null): Boolean = {
        containsCounter += 1
        super.contains(key)
      }
    }
    
    val scalaMap = new MyScalaMap
  ...
  @Test def `updateWith and getOrElseUpdate should reflect null policy of update`: Unit = {
    val jmap = new jutil.concurrent.ConcurrentHashMap[String, String | Null]()
    val wrapped = jmap.asScala
  ...
  @Test def `more updateWith and getOrElseUpdate should reflect null policy of update`: Unit = {
    val jmap = new jutil.HashMap[String, String | Null]()
    val wrapped = jmap.asScala
    wrapped.put("K", null)
  ...
  @Test def `getOrElseUpdate / updateWith support should insert null`: Unit = {
    val jmap = new jutil.HashMap[String | Null, String | Null]()
    val wrapped = jmap.asScala
