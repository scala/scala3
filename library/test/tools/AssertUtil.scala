...
object AssertUtil {
  ...
  private implicit class `class helper`(val clazz: Class[?]) extends AnyVal {
    def allFields: List[Field] = {
      def loop(k: Class[?]): List[Field] =
        if (k == null) Nil
  ...
  def assertZeroNetThreads[A](group: ThreadGroup)(body: => A): Try[A] = {
    ...
    val thread = new Thread(group, () => (test(): Unit))
    def abort(): Try[A] = {
      group.interrupt()
    ...
    try {
      thread.start()
      waitForIt(testDone.getCount == 0, Progress.Fast, label = "test result")
   ...
   */
  def waitForIt(terminated: => Boolean, progress: Progress = Progress.Fast, label: => String = "test"): Unit = {
   ...
   */
  def waitFor[A](value: => Option[A], progress: Progress = Progress.Fast, label: => String = "test"): A = {
    val limit = 5
    var n = 1
    var (dormancy, factor) = progress match {
      case Progress.Slow => (10000L, 5)
      case Progress.Fast => (250L, 4)
    }
    var period = 0L
  ...
  def readyOrNot(awaitable: Awaitable[?]): Boolean = Try(Await.ready(awaitable, TestDuration.Standard)).isSuccess
...
class NoTrace[A](body: => A) extends Runnable {
  ...
  @volatile private[tools] var result: Option[A] = None
  ...
  private[tools] lazy val errors: List[(Thread, Throwable)] = synchronized(uncaught.toList)
