...
with AnyStepper[A] {
  def nextStep(): A = if (hasStep) {
    ...
    i0 += 1
    leaves.nn(index).asInstanceOf[A]
...
with DoubleStepper {
  def nextStep(): Double = if (hasStep) {
    ...
    if (index >= 32) advanceData(i0)
    i0 += 1
    leaves.nn(index).asInstanceOf[Double]
  } else Stepper.throwNSEE()
...
with IntStepper {
  def nextStep(): Int = if (hasStep) {
    ...
    if (index >= 32) advanceData(i0)
    i0 += 1
    leaves.nn(index).asInstanceOf[Int]
  } else Stepper.throwNSEE()
...
with LongStepper {
  def nextStep(): Long = if (hasStep) {
    ...
    if (index >= 32) advanceData(i0)
    i0 += 1
    leaves.nn(index).asInstanceOf[Long]
  } else Stepper.throwNSEE()
