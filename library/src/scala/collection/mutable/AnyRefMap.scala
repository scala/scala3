...
class AnyRefMap[K <: AnyRef, V] private[collection] (defaultEntry: K => V, initialBufferSize: Int, initBlank: Boolean)
  ...
  private[this] var _hashes: Array[Int] | Null = null
  private[this] var _keys: Array[AnyRef] | Null = null
  ...
  private abstract class AnyRefMapIterator[A] extends AbstractIterator[A] {
    ...
    def hasNext: Boolean = index < hz.nn.length && {
      var h = hz(index)
      while (h+h == 0) {
        index += 1
        if (index >= hz.nn.length) return false
        h = hz(index)
      }
      true
    }
  ...
  override def foreach[U](f: ((K,V)) => U): Unit = {
    ...
    var e = _size
    while (e > 0) {
      while(i < _hashes.nn.length && { val h = _hashes(i); h+h == 0 && i < _hashes.nn.length}) i += 1
  ...
  override def clone(): AnyRefMap[K, V] = {
    val hz = java.util.Arrays.copyOf(_hashes, _hashes.nn.length)
    val kz = java.util.Arrays.copyOf(_keys, _keys.nn.length)
    val vz = java.util.Arrays.copyOf(_values,  _values.nn.length)
  ...
  def foreachKey[A](f: K => A): Unit = foreachElement[K,A](_keys.nn, f)

  /** Applies a function to all values of this map. */
  def foreachValue[A](f: V => A): Unit = foreachElement[V,A](_values.nn, f)
  ...
  def mapValuesNow[V1](f: V => V1): AnyRefMap[K, V1] = {
    ...
    val hz = java.util.Arrays.copyOf(_hashes, _hashes.nn.length)
    val kz = java.util.Arrays.copyOf(_keys, _keys.nn.length)
    val vz = new Array[AnyRef](_values.nn.length)
    var i,j = 0
    while (i < _hashes.nn.length & j < _size) {
      val h = _hashes(i)
      if (h+h != 0) {
  ...
  def transformValuesInPlace(f: V => V): this.type = {
    ...
    while (i < _hashes.nn.length & j < _size) {
      ...
      if (h+h != 0) {
        j += 1
        _values.nn.update(i, f(_values(i).asInstanceOf[V]).asInstanceOf[AnyRef])
      }
      i += 1
    }
    this
