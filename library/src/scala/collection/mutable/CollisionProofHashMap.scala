...
final class CollisionProofHashMap[K, V](initialCapacity: Int, loadFactor: Double)(implicit ordering: Ordering[K])
    ...
    val res = table(idx) match {
      ...
      case _old =>
        ...
        if(old eq null) {
          ...
          else prev.nn.next = new LLNode(key, hash, value, prev.nn.next)
        }
        true
    }
  ...
  private[this] def treeify(old: LLNode, idx: Int): Unit = {
    ...
    while(n ne null) {
      ...
      insertIntoExisting(root, idx, n.nn.key, n.nn.hash, n.nn.value, root)
      n = n.nn.next
    }
  }
  ...
  private[this] def growTable(newlen: Int) = {
    ...
    while(oldlen < newlen) {
      ...
      while (i < oldlen) {
        val old = table(i)
        if(old ne null) splitBucket(old.nn, i, i + oldlen, oldlen)
        i += 1
      }
      oldlen *= 2
    }
  ...
  private[this] def splitBucket(list: LLNode, lowBucket: Int, highBucket: Int, mask: Int): Unit = {
    ...
    //preLow.next = null
    var lastLow: LLNode | Null = preLow
    var lastHigh: LLNode | Null = preHigh
    var n: LLNode | Null = list
    while(n ne null) {
      val next = n.nn.next
      if((n.nn.hash & mask) == 0) { // keep low
        lastLow.nn.next = n
        lastLow = n
      } else { // move to high
        lastHigh.nn.next = n
    ...
    while (isRed(z.parent)) {
      if (z.parent eq z.parent.nn.parent.nn.left) {
        ...
        if (isRed(y)) {
          ...
          z.parent.nn.red = false
          z.parent.nn.parent.nn.red = true
          root = rotateRight(root, z.parent.nn.parent)
        }
        ...
        if (isRed(y)) {
          ...
          z.parent.nn.red = false
          z.parent.nn.parent.nn.red = true
          root = rotateLeft(root, z.parent.nn.parent)
        }
      }
    }
    root.red = false
  ...
  /**
    ...
    */
  private[this] def transplant(_root: RBNode, to: RBNode, from: RBNode | Null): RBNode = {
    var root = _root
    if (to.parent eq null) root = from.nn
    else if (to eq to.parent.left) to.parent.left = from.nn
    else to.parent.right = from.nn
...
object CollisionProofHashMap extends SortedMapFactory[CollisionProofHashMap] {
  ...
  private final class RBNodesIterator[A, B](tree: RBNode[A, B] | Null)(implicit @unused ord: Ordering[A]) extends AbstractIterator[RBNode[A, B]] {
    private[this] var nextNode: RBNode[A, B] | Null = if(tree ne null) minNodeNonNull(tree.nn) else null
  ...
  private final class LLNode[K, V](var key: K, var hash: Int, var value: V, var next: LLNode[K, V] | Null) extends Node {
