...
package object collection {
  ...
  object +: {
    /** Splits a sequence into head +: tail.
      ...
      */
    def unapply[A, CC[_] <: Seq[_], C <: SeqOps[A, CC, C]](t: (C with SeqOps[A, CC, C])^): Option[(A, C^{t})] =
      if(t.isEmpty) None
  ...
  object :+ {
    /** Splits a sequence into init :+ last.
      ...
      */
    def unapply[A, CC[_] <: Seq[_], C <: SeqOps[A, CC, C]](t: (C with SeqOps[A, CC, C])^): Option[(C^{t}, A)] =
      if(t.isEmpty) None
