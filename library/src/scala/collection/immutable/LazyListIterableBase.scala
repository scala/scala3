...
private[immutable] object LazyListIterableBase {
  ...
  /** Atomic accessor for the `_tail` field of [[LazyListIterableBase]] instances.
   *
   *  [[LazyListIterable]] uses a single shared instance (created through `makeTailUpdater`) to
   *  perform the atomic state transitions of `_tail` during lazy state evaluation. On Scala.js
   *  this class is replaced by a variant that uses plain, non-atomic field accesses.
   *
   *  @param u the field updater for `_tail`, created by `LazyListIterableBase.makeTailUpdater`
   */
   ...
   *
   *  [[LazyListIterable]] calls this on a `Thread` (or an `InRace` owner) found in the `_tail`
   *  field to detect a lazy state whose evaluation re-enters itself: if the thread recorded as
   *  evaluating the state is the current thread, the state depends on its own result and
   *  evaluation fails instead of deadlocking. On Scala.js this method is constant `true`, which
   *  lets dead code elimination drop the thread-coordination branches in [[LazyListIterable]].
   *
   ...
   *
   *  Called by [[LazyListIterable]] when a thread finds another thread's marker in the `_tail`
   *  field, meaning that thread is currently evaluating the lazy state. The returned latch is
   *  then installed in `_tail` so that all threads losing the race can wait for the owner to
   *  finish.
   *
   ...
   *  [[LazyListIterable]]'s state, it installs an `InRace` in the `_tail` field and blocks in
   *  `await` until the owner completes the evaluation and calls `countDown`.
   *  [[LazyListIterable]] also consults `owner` to detect self-referential evaluation: if the
   *  thread that finds the `InRace` is its owner, the lazy state depends on its own result.
   */
