...
trait Reference[+T <: AnyRef] extends Function0[T] {
  ...
  def get: Option[T]
  /** Returns the string representation of the referenced value, or `"<deleted>"` if it has been cleared or collected. */
  ...
  def clear(): Unit
  /** Adds this reference to the reference queue with which it was registered, if any. Returns `true` if it was successfully enqueued, or `false` if it was already enqueued or was not registered with a queue. */
  def enqueue(): Boolean
