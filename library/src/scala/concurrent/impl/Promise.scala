...
private[impl] final class CompletionLatch[T] extends AbstractQueuedSynchronizer with (Try[T] => Unit) {
  ...
  /*@volatile*/ private[this] var _result: Try[T] | Null = null
...
private[concurrent] object Promise {
  ...
  private[this] final val Noop = new Transformation[Nothing, Nothing](Xform_noop, null.asInstanceOf[Any => Any], ExecutionContext.parasitic)
  ...
  final class Transformation[-F, T] private[this] (
    private[this] final var _fun: (Any => Any) | Null,
    private[this] final var _ec: ExecutionContext | Null,
    private[this] final var _arg: Try[F] | Null,
    ...
    final def this(xform: Int, f: _ => _, ec: ExecutionContext) =
      this(f.asInstanceOf[Any => Any], ec.prepare(): @nowarn("cat=deprecation"), null.asInstanceOf[Try[F]], xform)
        ...
        val resolvedResult: Try[_] | Null =
          (_xform: @switch) match {
            case Xform_noop          =>
              null
            case Xform_map           =>
              if (v.isInstanceOf[Success[F]]) Success(fun.nn(v.nn.get)) else v // Faster than `resolve(v map fun)`
            case Xform_flatMap       =>
            ...
            case Xform_transform     =>
              resolve(fun.nn(v).asInstanceOf[Try[T]])
            ...
            case Xform_onComplete    =>
              ...
              null
            case Xform_recover       =>
              if (v.isInstanceOf[Failure[_]]) resolve(v.nn.recover(fun.nn.asInstanceOf[PartialFunction[Throwable, F]])) else v //recover F=:=T
            case Xform_recoverWith   =>
              if (v.isInstanceOf[Failure[F]]) {
                val f = fun.nn.asInstanceOf[PartialFunction[Throwable, Future[T]]].applyOrElse(v.asInstanceOf[Failure[F]].exception, Future.recoverWithFailed)
            ...
            case Xform_filter        =>
              if (v.isInstanceOf[Failure[F]] || fun.nn.asInstanceOf[F => Boolean](v.nn.get)) v else Future.filterFailure
            case Xform_collect       =>
              if (v.isInstanceOf[Success[F]]) Success(fun.nn.asInstanceOf[PartialFunction[F, T]].applyOrElse(v.nn.get, Future.collectFailed)) else v
            case _                   =>
