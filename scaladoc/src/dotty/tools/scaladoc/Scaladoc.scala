  ...
  def extract(args: Array[String], rootCtx: CompilerContext): (Option[Scaladoc.Args], CompilerContext) =
    ...
    argumentFilesOrNone.fold((None, newContext)) { argumentFiles =>
      ...
      val docArgs = Args(
        ...
        syntax.get.toList,
        sourceLinks.get.toList ++ legacySourceLinkList,
        revision.nonDefault,
        (externalMappings ++ legacyExternalMappings).toList,
        socialLinksParsed.toList,
        (skipById.get ++ deprecatedSkipPackages.get).toList,
        skipByRegex.get.toList,
        ...
        quickLinksParsed.toList,
        dynamicSideMenu.get,
        suppressCC.get,
        noSnippetNamesFor.get.toList,
        generateApi.get,
      )
