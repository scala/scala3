...
trait PatternFactorization extends MiniPhaseTransform {
    ...
    if (factoredCases.nonEmpty) {
      ...
      val fallbackDefDefOpt = {
        if (fallbackCases.nonEmpty) {
          ...
          val fallbackSym = {
            ctx.newSymbol(ctx.owner, fallbackName, Flags.Synthetic | Flags.Label,
                MethodType(Nil, Nil)(x => fallbackMatch.tpe))
          }
  ...
  protected def reorderedCases(cases: List[CaseDef])(
      implicit ctx: Context, info: TransformerInfo): List[CaseDef] = {
