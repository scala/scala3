  ...
  class LazyValsTransform extends TreeTransform with DenotTransformer {
    ...
    def transformFieldValDefNonVolatile(x: ValDef)(implicit ctx: Context) = x match {
      case x@ValDef(mods, name, tpt, rhs) if (mods is Flags.Lazy) =>
        ...
        val containerSymbol = ctx.newSymbol(claz, containerName, (mods &~ Flags.Lazy).flags, tpe, coord = x.symbol.coord)
    ...
    def transformFieldValDefVolatile(x: ValDef)(implicit ctx: Context) = x match {
      case x@ValDef(mods, name, tpt, rhs) if (mods is Flags.Lazy) =>
        ...
        val getOffset = Select(Ident(helperModule.termRef), "getOffset".toTermName)
        ...
        val getFlag = Select(Ident(helperModule.termRef), "get".toTermName)
        val setFlag = Select(Ident(helperModule.termRef), "setFlag".toTermName)
        val wait = Select(Ident(helperModule.termRef), "wait4Notification".toTermName)
        val state = Select(Ident(helperModule.termRef), "STATE".toTermName)
        val cas = Select(Ident(helperModule.termRef), "CAS".toTermName)
