...
import dotty.tools.dotc.typer.{Mode, ErrorReporting, Typer}
...
class Reim extends Typer {
  ...
  override def adapt(tree: tpd.Tree, pt: Type, original: untpd.Tree)(implicit ctx: Context): tpd.Tree = {
    val tree1 = super.adapt(tree, pt, original) match {
      ...
      /*
      TODO: move this to a separate transformer pass
    // forbid assignments to fields of @readonly refs
    case tree: tpd.Assign =>
      if(tree.tpe == ErrorType) tree
      else tree.lhs match {
