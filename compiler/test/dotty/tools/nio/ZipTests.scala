...
class ZipTests:
  ...
  def `Creating a non-empty archive`(): Unit =
    val archive = open()
    archive.getOrCreateFile("X.class").writeText("Hello, World!", Codec.UTF8)
    val sub = archive.getOrCreateContainer("D").getOrCreateFile("Y.tasty")
  ...
  def `Modifying files in an existing archive`(): Unit =
    ...
    archive.getContainer("X").get.getFile("B.tasty").get.writeText("R", Codec.UTF8)
    archive.close()
    val onDisk = listFiles()
  ...
  def `Deleting files from an existing archive`(): Unit =
    ...
    val archive = open()
    archive.getFile("A.class").get.delete()
  ...
  def `Reading from an existing archive`(): Unit =
    ...
    val c = archive.getFile("X.Y.C", FileExtension("txt"), separator = '.')
