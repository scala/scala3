  ...
  def canReplaceConstant: Unit = checkCompile("frontend", "class A{ val d = 1}") {
    (tree, context) =>
      ...
      class ConstantTransform extends MiniPhase {
        ...
        override def transformLiteral(tree: tpd.Literal)(implicit ctx: ContextRenamed): tpd.Tree = tpd.Literal(Constant(2))
  ...
  def canOverwrite: Unit = checkCompile("frontend", "class A{ val d = 1}") {
    (tree, context) =>
      ...
      class Transformation extends MiniPhase {
        ...
        override def transformLiteral(tree: tpd.Literal)(implicit ctx: ContextRenamed): tpd.Tree = tpd.Literal(Constant(-1))
        override def phaseName: String = "canOverwrite"

        override def transformValDef(tree: tpd.ValDef)(implicit ctx: ContextRenamed): tpd.ValDef = {
  ...
  def transformationOrder: Unit = checkCompile("frontend", "class A{ val d = 1}") {
    (tree, context) =>
      ...
      class Transformation1 extends MiniPhase {
        ...
        override def transformLiteral(tree: tpd.Literal)(implicit ctx: ContextRenamed): tpd.Tree = {
  ...
  def invocationCount: Unit = checkCompile("frontend", "class A{ val d = 1}") {
    (tree, context) =>
      ...
      class Transformation1 extends MiniPhase {
        ...
        override def transformValDef(tree: tpd.ValDef)(implicit ctx: ContextRenamed) = {
          transformed1 += 1
          Assert.assertTrue("transformation of children succeeded",
