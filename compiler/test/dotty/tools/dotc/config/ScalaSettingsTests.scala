...
class ScalaSettingsTests:
  ...
  @Test def `WConf src filter silences warnings from a matching path for real file`: Unit =
    val file = File.createTemporaryOnDisk("myfile.scala")
    try
      val result =
        wconfSrcFilterTest(
          ...
          warning = reporting.Diagnostic.Warning(
            "A warning".toMessage,
            util.SourcePosition(
              source = util.SourceFile(new dotty.tools.io.PlainFile(dotty.tools.io.Path(file.path)), new dotty.tools.io.PlainDirectory(dotty.tools.io.Directory(".")), Codec.UTF8),
              span = util.Spans.Span(1L)
            )
          )
        )
