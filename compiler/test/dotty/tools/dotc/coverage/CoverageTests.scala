...
class CoverageTests:
    ...
    dir.entries.filter(runOnFileOrDir).foreach(path => {
      ...
      val (targetDir, expectFile, expectMeasurementFile) = path match
        case d: FileContainer =>
          assert(d.recursiveEntries.collect { case f: File if f.extension.isScala => f }.nonEmpty, s"No scala files found in test directory: ${path.path}")
        ...
        case f: File =>
          ...
          (targetDir, f.getOrCreateSiblingWithExtension("scoverage.check"), f.getSiblingWithExtension("measurement.check"))

      val targetFile = targetDir.getFile("scoverage.coverage").get
  ...
  def checkCoverageWarningsIn(dir: FileContainer)(using TestGroup): Unit =
    ...
    dir.entries.collect { case f: File if runOnFile(f) => f }.foreach { path =>
      ...
      val options = defaultOptions.and("-Ycheck:instrumentCoverage", "-Ylog:instrumentCoverage", "-coverage-out", target.path, "-sourceroot", rootSrc.path)
  ...
  def checkIncrementalCoverage(): Unit =
    ...
    val sourceFile1 = sourceRoot.getOrCreateFile("file1.scala")
    ...
    locally {
      val coverage = Serializer.deserialize(java.nio.file.Path.of(scoverageFile.get.path))
      val filesWithCoverage = coverage.statements.map(s => java.nio.file.Path.of(s.location.sourcePath).getFileName.toString).toSet
      assertEquals(Set("file1.scala"), filesWithCoverage)
    }

    val sourceFile2 = sourceRoot.getOrCreateFile("file2.scala")
  ...
  def `deleted source files should not be kept in incremental coverage`(): Unit =
    ...
    locally {
      val coverage = Serializer.deserialize(java.nio.file.Path.of(scoverageFile.get.path))
      val filesWithCoverage = coverage.statements.map(s => java.nio.file.Path.of(s.location.sourcePath).toString).toSet
      assertEquals(Set("file1.scala"), filesWithCoverage)
    }
