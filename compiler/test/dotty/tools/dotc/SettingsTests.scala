...
class SettingsTests:
  ...
  @Test def `dir option also warns`: Unit =
    ...
    try
      object Settings extends SettingGroup:
        val option = FileContainerSetting(RootSetting, "option", false, "A file", io.PlainFile(io.File("a/b/c")))
  ...
  @Test def `Output setting is overriding existing jar`: Unit =
    val file = File.createTemporaryOnDisk("myfile.jar")
    try
      object Settings extends SettingGroup:
  ...
  @Test def `Output setting respects previous setting`: Unit =
    val file1 = File.createTemporaryOnDisk("myfile.jar")
    val file2 = File.createTemporaryOnDisk("myfile2.jar")
    try
      object Settings extends SettingGroup:
        val defaultDir = new PlainDirectory(Directory("."))
  ...
  @Test def `Ignored setting shifts args`: Unit =
    object Settings extends SettingGroup:
      ...
      val baz = FileContainerSetting(RootSetting, "out", false, "A file", default = io.PlainFile(io.File("out/baz")),
