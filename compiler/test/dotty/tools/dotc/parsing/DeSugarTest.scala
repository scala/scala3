...
class DeSugarTest extends ParserTest {
  ...
  object DeSugar extends UntypedTreeMap {
    ...
    def transform(tree: Tree, mode: Mode)(implicit ctx: ContextRenamed): Tree = withMode(mode) { transform(tree) }
    def transform(trees: List[Tree], mode: Mode)(implicit ctx: ContextRenamed): List[Tree] = withMode(mode) { transform(trees) }
