...
class PrintingTest {
  ...
  private def compileFile(file: File, phase: String): Boolean = {
    ...
    val checkFile = file.getOrCreateSiblingWithExtension("check")
    val flagsFile = file.getSiblingWithExtension("flags")
  ...
  def testIn(testsDir: String, phase: String) =
    val res = TestSources.rootPath().getContainer(testsDir).get.entries.collect {
      case f: File if f.extension.isJava || f.extension.isScala => compileFile(f, phase)
    }.toList
