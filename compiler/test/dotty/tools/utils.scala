...
private def withFile[T](file: File)(action: Source => T)(using Codec): T =
  resource(Source.fromFile(file))(action)

def readLines(f: File)(implicit codec: Codec = Codec.UTF8): List[String] = withFile(f)(_.getLines.toList)
def readFile(f: File)(implicit codec: Codec = Codec.UTF8): String = withFile(f)(_.mkString)
...
def toolArgsFor(files: List[JPath])(using Codec): List[String] =
