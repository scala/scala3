 ...
 */
class LocalOpt(callGraph: CallGraph, inliner: Inliner,
  ...
  private val copyProp = new CopyProp(callGraph, inliner, ts, settings)
  ...
  private def methodOptimizations(method: MethodNode, ownerClassName: InternalName): Boolean = {
    ...
    def removalRound(
      ...
      val codeRemoved = if (runDCE) LocalOptImpls.removeUnreachableCodeImpl(method, ownerClassName, callGraph) else false
      traceIfChanged("dce")
    ...
    if (settings.optUnreachableCode) OptimizerUtils.setDceDone(method)
    ...
    // OptimizerUtils.clearMaxsComputed(method)
    OptimizerUtils.clearDceDone(method)
...
object LocalOptImpls {
   ...
   */
  def minimalRemoveUnreachableCode(method: MethodNode, ownerClassName: InternalName, callGraph: CallGraph): Boolean = {
    ...
    if (method.instructions.size == 0) return false  // fast path for abstract methods
    if (OptimizerUtils.isDceDone(method)) return false // we know there is no unreachable code
    ...
    def removalRound(): Boolean = {
      val insnsRemoved = removeUnreachableCodeImpl(method, ownerClassName, callGraph)
      if (insnsRemoved) {
    ...
    OptimizerUtils.setDceDone(method)
    changed
  }
   ...
   */
  def removeUnreachableCodeImpl(method: MethodNode, ownerClassName: InternalName, callGraph: CallGraph): Boolean = {
   ...
   * before, so that `OptimizerUtils.isLabelReachable` gives a correct answer.
   */
