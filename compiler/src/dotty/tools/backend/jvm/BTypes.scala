...
final case class ClassInfo(superClass: Option[ClassBType], interfaces: List[ClassBType], flags: Int,
  ...
  /**
   * Build the InlineInfo for a ClassBType.
   *
   ...
   * classfile attribute.
   */
  def inlineInfo(bCodeRepository: BCodeRepository, primitives: DottyPrimitives, ts: CoreBTypes)(using Context): InlineInfo =
    if _inlineInfo eq null then
      _inlineInfo = inlineInfoSource match {
        case InlineInfoSource.Symbol(classSym, internalName) =>
          ...
          if atPhase(picklerPhase.next) { classSym.isDefinedInCurrentRun } then buildInlineInfoFromClassSymbol(classSym, primitives, ts) // // InlineInfo required for classes being compiled, we have to create the classfile attribute
   ...
   */
  private def buildInlineInfoFromClassSymbol(classSym: ClassSymbol, primitives: DottyPrimitives, ts: CoreBTypes)(using Context): InlineInfo = {
    ...
    val methods = if classSym.is(JavaDefined) then
      ...
      val companion = atPhase(picklerPhase.next) { classSym.companionModule }
