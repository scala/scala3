...
class BackendUtils(val postProcessor: PostProcessor) {
  ...
  lazy val classfileVersion: Int = if compilerSettings.experimental then (71 + asm.Opcodes.V_PREVIEW) else BackendUtils.classfileVersionMap(compilerSettings.target.toInt)
