...
class Setup(
  ...
  checker: CheckCaptures#CaptureChecker)
  ...
  private def mapInferred(using Context) = new TypeMap:
    ...
    def apply(t: Type) =
      ...
      val tp1 = tp match
        ...
        case _ =>
          mapOver(tp)
      Setup.addVar(addCaptureRefinements(tp1), ctx.owner)
    end apply
  end mapInferred
        ...
        case cls: ClassSymbol =>
          ...
          if (selfInfo eq NoType) || cls.is(ModuleClass) && !cls.isStatic then
            ...
            val selfRefs = CaptureSet.Var(cls)
              // it's debatable what the right level owner should be. A self type should
