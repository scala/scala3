...
extends tpd.TreeTraverser:
  ...
  private def expandAbbreviations(using Context) = new TypeMap:
    ...
    def wrapCaptures(tp: Type, outerCs: CaptureSet) =
      val impliedCs = outerCs.filter(!tp.captureSet.accountsFor(_))
      if impliedCs.isAlwaysEmpty then tp
      else CapturingType(tp, impliedCs, boxed = false)
