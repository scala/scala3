...
trait MessageRendering {
  ...
  /** When inlining a method call, if there's an error we'd like to get the
    ...
    */
  def outer(pos: Position, prefix: String)(implicit ctx: Context): List[String] =
    if (pos.outer.exists) {
  ...
  /** Get the sourcelines before and after the position, as well as the offset
    ...
    */
  def sourceLines(pos: Position)(implicit ctx: Context): (List[String], List[String], Int) = {
  ...
  def columnMarker(pos: Position, offset: Int)(implicit ctx: Context): String = {
    val prefix = " " * (offset - 1)
  ...
  /** The error message (`msg`) aligned under `pos`
    ...
    */
  def errorMsg(pos: Position, msg: String, offset: Int)(implicit ctx: Context): String = {
  ...
  /** The separator between errors containing the source file and error type
    ...
    */
  def posStr(pos: Position, diagnosticLevel: String, message: Message)(implicit ctx: Context): String =
    if (pos.exists) Blue({
  ...
  def messageAndPos(msg: Message, pos: Position, diagnosticLevel: String)(implicit ctx: Context): String = {
