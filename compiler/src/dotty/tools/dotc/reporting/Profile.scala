...
class ActiveProfile(details: Int) extends Profile:
  ...
  private def curInfo(using Context): Profile.Info =
    ...
    if unit == null || unit.source.file == null then junkInfo else unitProfile(unit)
  ...
  def printSummary()(using Context): Unit =
    ...
    def printAndAggregateSourceInfos(): Profile.Info =
      ...
      for unit <- units do
        ...
        printInfo(layout, unit.source.name, info, if file == null then "<no file>" else file.parent.path)
    ...
    def printDetails(agg: Profile.Info): Unit =
      ...
      do
        ...
        printInfo(layout, header, info, if file == null then "<no file>" else file.parent.path)
