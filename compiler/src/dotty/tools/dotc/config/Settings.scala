...
object Settings:
  ...
  case class Setting[T] private[Settings] (
    ...
    def tryToSet(state: ArgsSummary): ArgsSummary =
      ...
      def doSet(argRest: String): ArgsSummary =
        if ct == BooleanTag then setBoolean(argRest, args)
          ...
          val (arg1, args1) =
            ...
            else if argAfterParam then (args.head, args.tail)
            else ("", Nil)
          def doSet(arg: String, args: List[String]) =
            ct match
            case _ if arg.isEmpty && args.isEmpty => missingArg
            case _ if preferPrevious && changed => state.skip(s"Ignoring update of option $name")
            case ListTag if arg.isEmpty => missingArg
            case ListTag => setMultivalue(arg, args)
            case StringTag if arg.nonEmpty || choices.exists(_.contains("")) => setString(arg, args)
            case StringTag =>
              args match
            ...
            case OutputTag =>
              if changed && preferPrevious then
                ...
                setOutput(arg, args)
            case IntTag if arg.nonEmpty => setInt(arg, args)
            case IntTag =>
              args match
      ...
      def argValRest: String =
        prefix match
        case Some(prefix) => arg.drop(prefix.length)
        case none => arg.dropWhile(_ != ':').drop(1)
