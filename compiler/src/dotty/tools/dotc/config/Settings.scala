...
object Settings {
  ...
  object Setting {
    implicit class SettingDecorator[T](val setting: Setting[T]) extends AnyVal {
      def value(implicit ctx: ContextRenamed): T = setting.valueIn(ctx.settingsState)
      def update(x: T)(implicit ctx: ContextRenamed): SettingsState = setting.updateIn(ctx.settingsState, x)
      def isDefault(implicit ctx: ContextRenamed): Boolean = setting.isDefaultIn(ctx.settingsState)
    }
  }
  ...
  class SettingGroup {
    ...
    def processArguments(arguments: List[String], processAll: Boolean)(implicit ctx: ContextRenamed): ArgsSummary =
