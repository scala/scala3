...
private sealed trait XSettings:
  ...
  val Xdumpclasses: Setting[AbstractFile] = OutputSetting(AdvancedSetting, "Xdump-classes", "dir", "Dump the generated bytecode to .class files (useful for reflective compilation that utilizes in-memory classloaders).", NoAbstractFile)
