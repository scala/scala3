...
class ScalaSettings extends Settings.SettingGroup {
  ...
  val XsplitCompilation: Setting[String] = StringSetting("-XsplitCompilation", "", "Compile in two steps. The first step will compile the specified source files and the second will compile the rest with the first set in the classpath", "")
