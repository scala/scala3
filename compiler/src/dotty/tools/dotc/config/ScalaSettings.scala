...
private sealed trait XSettings:
  ...
  val Xdumpclasses: Setting[Option[FileContainer]] = OptionalFileContainerSetting(AdvancedSetting, "Xdump-classes", false, "Dump the generated bytecode to .class files (useful for reflective compilation that utilizes in-memory classloaders).")
