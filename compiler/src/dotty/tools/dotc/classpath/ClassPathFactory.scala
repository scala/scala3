...
class ClassPathFactory {
  /**
    ...
    */
  def newClassPath(file: AbstractFile)(implicit ctx: ContextRenamed): ClassPath = ClassPathFactory.newClassPath(file)

  /**
    * Creators for sub classpaths which preserve this context.
    */
  def sourcesInPath(path: String)(implicit ctx: ContextRenamed): List[ClassPath] =
    for {
  ...
  def contentsOfDirsInPath(path: String)(implicit ctx: ContextRenamed): List[ClassPath] =
    for {
  ...
  def classesInExpandedPath(path: String)(implicit ctx: ContextRenamed): IndexedSeq[ClassPath] =
    classesInPathImpl(path, expand = true).toIndexedSeq

  def classesInPath(path: String)(implicit ctx: ContextRenamed): List[ClassPath] = classesInPathImpl(path, expand = false)

  def classesInManifest(useManifestClassPath: Boolean)(implicit ctx: ContextRenamed): List[ClassPath] =
    if (useManifestClassPath) dotty.tools.io.ClassPath.manifests.map(url => newClassPath(AbstractFile getResources url))
    else Nil

  // Internal
  protected def classesInPathImpl(path: String, expand: Boolean)(implicit ctx: ContextRenamed): List[ClassPath] =
    for {
  ...
  private def createSourcePath(file: AbstractFile)(implicit ctx: ContextRenamed): ClassPath =
    if (file.isJarOrZip)
...
object ClassPathFactory {
  def newClassPath(file: AbstractFile)(implicit ctx: ContextRenamed): ClassPath = file match {
