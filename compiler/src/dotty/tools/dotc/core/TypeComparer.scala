...
class TypeComparer(@constructorOnly initctx: Context) extends ConstraintHandling, PatternTypeConstrainer {
  ...
  private def compareExistentialRight(tp1: Type, boundVar: TermParamRef, tp2unpacked: Type)(using Context): Boolean =
    ...
    try
      //println(i"assoc $boundVar = $openedExistentials")
      assocExistentials = assocExistentials.updated(boundVar, openedExistentials)
      recur(tp1, tp2unpacked)
    finally
      assocExistentials = saved

  def linkOK(tp1: TermParamRef, tp2: CaptureRef)(using Context): Boolean =
    assocExistentials.get(tp1).exists(_.contains(tp2))
    && {
      assocExistentials = assocExistentials.updated(tp1, tp2.asInstanceOf[TermParamRef] :: Nil)
      true
    }
   ...
   */
  private def existentialVarsConform(tp1: Type, tp2: Type) = tp1 match
    case tp1: TermParamRef =>
      tp2 match
        case tp2: TermParamRef =>
          linkOK(tp1, tp2) || linkOK(tp2, tp1)
        case _ => false
    case _ => false
...
object TypeComparer {
  ...
  def linkOK(tp1: TermParamRef, tp2: CaptureRef)(using Context) =
    comparing(_.linkOK(tp1, tp2))
