...
object Denotations {
  ...
  abstract class PreDenotation {
    ...
    /** Keep only those inherited members M of this predenotation for which the
      ...
      *     `ownDenots`
      * Return the denotation as seen from `pre`. Called from
      * SymDenotations.computeMember. There, `ownDenots` are the denotations
      * found in the base class, which shadow any inherited denotations with the
      * same signature. `prevDenots` are the denotations that are defined in the
      * class or inherited from a base type which comes earlier in the
      * linearization.
      */
    def mapInherited(
  ...
  abstract class Denotation(
    ...
    inline final def hasAltWithInline(
        inline p: SingleDenotation => Boolean
    ): Boolean = inline this match
    ...
    /** If this denotation is overloaded, filter with given predicate. If result
      ...
      * NoDenotation).
      */
    inline def disambiguate(
        inline p: Symbol => Boolean
    )(using Context): SingleDenotation = this match {
    ...
    /** Form a denotation by conjoining with denotation `that`.
      ...
      *      not a bridge, but the other oneis. (*) 3. The symbol is concrete,
  ...
  abstract class SingleDenotation(
    ...
    def derivedSingleDenotation(
      ...
      if (
        (symbol eq this.symbol) && (info eq this.info) && (pre eq this.prefix) && (isRefinedMethod == this.isRefinedMethod)
      ) this
    ...
    def accessibleFrom(pre: Type, superAccess: Boolean)(using
      ...
      if (!symbol.exists || symbol.isAccessibleFrom(pre, superAccess)) this
      else NoDenotation
    ...
    def initial: SingleDenotation =
      if (validFor.firstPhaseId <= 1) this
        ...
        while (current.validFor.code > this.validFor.code)
    ...
    /** Move validity period of this denotation to a new run. Throw a
      ...
      *   - If the symbol was first defined in one of the transform phases
      *     (after pickling), it should not be visible in new runs, so also
      *     return a NoDenotation.
      */
    private def bringForward()(using Context): SingleDenotation = {
      ...
      if (coveredInterval.firstPhaseId >= Phases.firstTransformPhase.id)
        return NoDenotation
    ...
    protected def transformAfter(
      ...
      while (
        current.validFor.firstPhaseId < phase.id && (current.nextInRun.validFor.code > current.validFor.code)
      )
        current = current.nextInRun
      var hasNext = true
    ...
    protected def computeAsSeenFrom(
        pre: Type
    )(using Context): SingleDenotation = {
  ...
  abstract class NonSymSingleDenotation(
      ...
      override val prefix: Type
        ...
        symbol,
        initInfo,
        initInfo.isInstanceOf[TypeType]
      ) {
  ...
  class UniqueRefDenotation(
    ...
    protected def newLikeThis(
      ...
      else new UniqueRefDenotation(s, i, currentStablePeriod, pre)
  }
  ...
  def staticRef(
    ...
    def select(prefix: Denotation, selector: Name): Denotation = {
      ...
      if (owner.exists) {
        val result =
          if (isPackage) owner.info.decl(selector)
          else owner.info.member(selector)
        if (result.exists) result
        else if (isPackageFromCoreLibMissing)
  ...
  /** An exception that is at the same type a StaleSymbol and a TypeError. Sine
    * it is a TypeError it can be reported as a nroaml error instead of crashing
    * the compiler.
    */
  class StaleSymbolTypeError(symbol: Symbol)(using Context)
      extends TypeError,
        StaleSymbol:
