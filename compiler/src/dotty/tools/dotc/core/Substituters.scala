...
object Substituters:
  ...
  final def subst(tp: Type, from: BindingType, to: BindingType, theMap: SubstBindingMap): Ctx[Type] =
    tp match {
      case tp: BoundType =>
  ...
  final def subst1(tp: Type, from: Symbol, to: Type, theMap: Subst1Map): Ctx[Type] =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
  ...
  final def subst2(tp: Type, from1: Symbol, to1: Type, from2: Symbol, to2: Type, theMap: Subst2Map): Ctx[Type] =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
  ...
  final def subst(tp: Type, from: List[Symbol], to: List[Type], theMap: SubstMap): Ctx[Type] =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
        var fs = from
  ...
  final def substSym(tp: Type, from: List[Symbol], to: List[Symbol], theMap: SubstSymMap): Ctx[Type] =
    tp match {
      case tp: NamedType =>
        val sym = tp.symbol
        var fs = from
  ...
  final def substThis(tp: Type, from: ClassSymbol, to: Type, theMap: SubstThisMap): Ctx[Type] =
    tp match {
      case tp: ThisType =>
  ...
  final def substRecThis(tp: Type, from: Type, to: Type, theMap: SubstRecThisMap): Ctx[Type] =
    tp match {
  ...
  final def substParam(tp: Type, from: ParamRef, to: Type, theMap: SubstParamMap): Ctx[Type] =
    tp match {
      case tp: BoundType =>
  ...
  final def substParams(tp: Type, from: BindingType, to: List[Type], theMap: SubstParamsMap): Ctx[Type] =
    tp match {
      case tp: ParamRef =>
