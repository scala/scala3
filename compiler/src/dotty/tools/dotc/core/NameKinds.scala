...
abstract class NumberedNameKind(tag: Int, val infoString: String) extends NameKind(tag) { self =>
  ...
  case class NumberedInfo(val num: Int) extends Info with core.NumberedInfo {
...
class FieldNameKind extends SuffixNameKind(FIELD, "$$local") {
  override def mkString(underlying: TermName, info: ThisInfo) = underlying.toString
}

class ExpandedNameKind extends QualifiedNameKind(EXPANDED, str.EXPAND_SEPARATOR) {
...
class UniqueUniqueNameKind extends UniqueNameKind("$") {
...
class DefaultGetterNameKind extends NumberedNameKind(DEFAULTGETTER, "DefaultGetter") {
...
class UniqueExtMethNameKind extends UniqueNameKind("$extension") {
  override def unmangle(name: SimpleName): TermName = {
...
class OuterSelectNameKind extends NumberedNameKind(OUTERSELECT, "OuterSelect") {
  def mkString(underlying: TermName, info: ThisInfo) = {
...
class AdaptedClosureNameKind extends SuffixNameKind(ADAPTEDCLOSURE, "$adapted") { override def definesNewName = true }
...
object NameKinds {
  ...
  def add(nk: ClassifiedNameKind) = simpleNameKinds(nk.tag) = nk
  def add(nk: QualifiedNameKind) = qualifiedNameKinds(nk.tag) = nk
  def add(nk: NumberedNameKind) = numberedNameKinds(nk.tag) = nk
  def add(nk: UniqueNameKind) = uniqueNameKinds(nk.separator) = nk
  ...
  val ExpandedName: QualifiedNameKind = new ExpandedNameKind
  ...
  val UniqueName: UniqueNameKind = new UniqueUniqueNameKind
   ...
   *  unmangled.
   */
  val UniqueExtMethName: UniqueNameKind = new UniqueExtMethNameKind
  ...
  val DefaultGetterName: NumberedNameKind = new DefaultGetterNameKind

  /** Names of the form N_<outer>. Emitted by inliner, replaced by outer path
   *  in ExplicitOuter.
   */
  val OuterSelectName: NumberedNameKind = new OuterSelectNameKind
  ...
  val FieldName: SuffixNameKind = new FieldNameKind
  ...
  val AdaptedClosureName: SuffixNameKind = new AdaptedClosureNameKind
