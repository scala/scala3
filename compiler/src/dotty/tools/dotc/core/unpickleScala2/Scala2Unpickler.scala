...
object Scala2Unpickler {
  ...
  def translateTempPoly(tp: Type)(implicit ctx: ContextRenamed): Type = tp match {
  ...
  def addConstructorTypeParams(denot: SymDenotation)(implicit ctx: ContextRenamed): Unit = {
   ...
   */
  def arrayToRepeated(tp: Type)(implicit ctx: ContextRenamed): Type = tp match {
    case tp: MethodType =>
  ...
  def ensureConstructor(cls: ClassSymbol, scope: Scope)(implicit ctx: ContextRenamed): Unit =
  ...
  def setClassInfo(denot: ClassDenotation, info: Type, selfInfo: Type = NoType)(implicit ctx: ContextRenamed): Unit = {
 ...
 */
class Scala2Unpickler(bytes: Array[Byte], classRoot: ClassDenotation, moduleClassRoot: ClassDenotation)(ictx: ContextRenamed)
  ...
  protected def errorBadSignature(msg: String, original: Option[RuntimeException] = None)(implicit ctx: ContextRenamed): Nothing = {
  ...
  protected def handleRuntimeException(ex: RuntimeException)(implicit ctx: ContextRenamed): Nothing = ex match {
  ...
  def source(implicit ctx: ContextRenamed): AbstractFile = {
    val f = classRoot.symbol.associatedFile
    if (f != null) f else moduleClassRoot.symbol.associatedFile
  }

  private def checkVersion(implicit ctx: ContextRenamed): Unit = {
    val major = readNat()
    val minor = readNat()
  ...
  protected def isSymbolEntry(i: Int)(implicit ctx: ContextRenamed): Boolean = {
   ...
   */
  protected def isRefinementSymbolEntry(i: Int)(implicit ctx: ContextRenamed): Boolean = {
    val savedIndex = readIndex
    readIndex = index(i)
  ...
  protected def isRefinementClass(sym: Symbol)(implicit ctx: ContextRenamed): Boolean =
    sym.name == tpnme.REFINE_CLASS

  protected def isLocal(sym: Symbol)(implicit ctx: ContextRenamed): Boolean = isUnpickleRoot(sym.topLevelClass)

  protected def isUnpickleRoot(sym: Symbol)(implicit ctx: ContextRenamed): Boolean = {
    val d = sym.denot
  ...
  /** Read a name */
  protected def readName()(implicit ctx: ContextRenamed): Name = {
    val tag = readByte()
    val len = readNat()
    tag match {
  ...
  protected def readTermName()(implicit ctx: ContextRenamed): TermName = readName().toTermName
  protected def readTypeName()(implicit ctx: ContextRenamed): TypeName = readName().toTypeName

  /** Read a symbol */
  protected def readSymbol()(implicit ctx: ContextRenamed): Symbol = readDisambiguatedSymbol(alwaysTrue)()

  /** Read a symbol, with possible disambiguation */
  protected def readDisambiguatedSymbol(p: Symbol => Boolean)()(implicit ctx: ContextRenamed): Symbol = {
  ...
  class LocalUnpickler extends LazyType {
    ...
    def complete(denot: SymDenotation)(implicit ctx: ContextRenamed): Unit = try {
      def parseToCompletion(denot: SymDenotation)(implicit ctx: ContextRenamed) = {
        val tag = readByte()
        val end = readNat() + readIndex
        def atEnd = readIndex == end
  ...
  class ClassUnpickler(infoRef: Int) extends LocalUnpickler with TypeParamsCompleter {
    private def readTypeParams()(implicit ctx: ContextRenamed): List[TypeSymbol] = {
      val tag = readByte()
      val end = readNat() + readIndex
      if (tag == POLYtpe) {
    ...
    private def loadTypeParams(implicit ctx: ContextRenamed) =
      atReadPos(index(infoRef), () => readTypeParams()(ctx))

    /** Force reading type params early, we need them in setClassInfo of subclasses. */
    def init()(implicit ctx: ContextRenamed): List[TypeSymbol] = loadTypeParams
   ...
   *    tp { name: T }
   */
  def elimExistentials(boundSyms: List[Symbol], tp: Type)(implicit ctx: ContextRenamed): Type = {
   ...
   */
  protected def readType()(implicit ctx: ContextRenamed): Type = {
    val tag = readByte()
    val end = readNat() + readIndex
    (tag: @switch) match {
      case NOtpe =>
  ...
  def readTypeParams()(implicit ctx: ContextRenamed): List[Symbol] = {
    val tag = readByte()
    val end = readNat() + readIndex
    if (tag == POLYtpe) {
      val unusedRestperef = readNat()
  ...
  def noSuchTypeTag(tag: Int, end: Int)(implicit ctx: ContextRenamed): Type =
    errorBadSignature("bad type tag: " + tag)

  /** Read a constant */
  protected def readConstant()(implicit ctx: ContextRenamed): Constant = {
    val tag = readByte().toInt
    val len = readNat()
    (tag: @switch) match {
  ...
  def noSuchConstantTag(tag: Int, len: Int)(implicit ctx: ContextRenamed): Constant =
    errorBadSignature("bad constant tag: " + tag)

  /** Read children and store them into the corresponding symbol.
   */
  protected def readChildren()(implicit ctx: ContextRenamed): Unit = {
    val tag = readByte()
    assert(tag == CHILDREN)
    val end = readNat() + readIndex
  ...
  protected def readSymbolRef()(implicit ctx: ContextRenamed): Symbol = { //OPT inlined from: at(readNat(), readSymbol) to save on closure creation
    val i = readNat()
    var r = entries(i)
    if (r eq null) {
      val savedIndex = readIndex
  ...
  protected def readDisambiguatedSymbolRef(p: Symbol => Boolean)(implicit ctx: ContextRenamed): Symbol =
    at(readNat(), () => readDisambiguatedSymbol(p)())

  protected def readNameRef()(implicit ctx: ContextRenamed): Name = at(readNat(), () => readName())
  protected def readTypeRef()(implicit ctx: ContextRenamed): Type = at(readNat(), () => readType()) // after the NMT_TRANSITION period, we can leave off the () => ... ()
  protected def readConstantRef()(implicit ctx: ContextRenamed): Constant = at(readNat(), () => readConstant())

  protected def readTypeNameRef()(implicit ctx: ContextRenamed): TypeName = readNameRef().toTypeName
  protected def readTermNameRef()(implicit ctx: ContextRenamed): TermName = readNameRef().toTermName

  protected def readAnnotationRef()(implicit ctx: ContextRenamed): Annotation = at(readNat(), () => readAnnotation())

  protected def readModifiersRef(isType: Boolean)(implicit ctx: ContextRenamed): Modifiers = at(readNat(), () => readModifiers(isType))
  protected def readTreeRef()(implicit ctx: ContextRenamed): Tree = at(readNat(), () => readTree())

  /** Read an annotation argument, which is pickled either
   *  as a Constant or a Tree.
   */
  protected def readAnnotArg(i: Int)(implicit ctx: ContextRenamed): Tree = bytes(index(i)) match {
   ...
   */
  private def readArrayAnnotArg()(implicit ctx: ContextRenamed): Tree = {
  ...
  private def readAnnotInfoArg()(implicit ctx: ContextRenamed): Tree = {
    readByte() // skip the `annotinfo` tag
    val end = readNat() + readIndex
    readAnnotationContents(end)
  }

  protected def readClassfileAnnotArg(i: Int)(implicit ctx: ContextRenamed): Tree = bytes(index(i)) match {
   ...
   */
  protected def readAnnotationContents(end: Int)(implicit ctx: ContextRenamed): Tree = {
    val atp = readTypeRef()
    val args = {
   ...
   */
  protected def readSymbolAnnotation()(implicit ctx: ContextRenamed): Unit = {
    val tag = readByte()
    if (tag != SYMANNOT)
   ...
   */
  protected def readAnnotation()(implicit ctx: ContextRenamed): Annotation = {
    val tag = readByte()
    if (tag != ANNOTINFO)
   ...
   */
  protected def deferredAnnot(end: Int)(implicit ctx: ContextRenamed): Annotation = {
    val start = readIndex
    val atp = readTypeRef()
    val phase = ctx.phase
    Annotation.deferred(
  ...
  protected def readTree()(implicit ctx: ContextRenamed): Tree = {
...
*/
  ...
  def noSuchTreeTag(tag: Int, end: Int)(implicit ctx: ContextRenamed): Nothing =
    errorBadSignature("unknown tree type (" + tag + ")")

  def unimplementedTree(what: String)(implicit ctx: ContextRenamed): Nothing =
    errorBadSignature(s"cannot read $what trees from Scala 2.x signatures")

  def readModifiers(isType: Boolean)(implicit ctx: ContextRenamed): Modifiers = {
    val tag = readNat()
    if (tag != MODIFIERS)
  ...
  protected def readTemplateRef()(implicit ctx: ContextRenamed): Template =
    readTreeRef() match {
      case templ: Template => templ
      case other =>
        errorBadSignature("expected a template (" + other + ")")
    }
  protected def readCaseDefRef()(implicit ctx: ContextRenamed): CaseDef =
    readTreeRef() match {
      case tree: CaseDef => tree
      case other =>
        errorBadSignature("expected a case def (" + other + ")")
    }
  protected def readValDefRef()(implicit ctx: ContextRenamed): ValDef =
    readTreeRef() match {
      case tree: ValDef => tree
      case other =>
        errorBadSignature("expected a ValDef (" + other + ")")
    }
  protected def readIdentRef()(implicit ctx: ContextRenamed): Ident =
    readTreeRef() match {
      case tree: Ident => tree
      case other =>
        errorBadSignature("expected an Ident (" + other + ")")
    }
  protected def readTypeDefRef()(implicit ctx: ContextRenamed): TypeDef =
    readTreeRef() match {
