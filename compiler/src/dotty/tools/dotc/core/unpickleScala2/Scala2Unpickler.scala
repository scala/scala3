...
class Scala2Unpickler(bytes: Array[Byte], classRoot: ClassDenotation, moduleClassRoot: ClassDenotation)(ictx: Context)
  ...
  private def readArrayAnnotArg()(using Context): Tree = {
    ...
    SeqLiteral(
      until(end, () => readClassfileAnnotArg(readNat())).toLst,
  ...
  protected def readTree()(using Context): Tree = {
    ...
    tag match {
      ...
      case UNAPPLYtree =>
        ...
        UnApply(fun, NIL, args.toLst, defn.AnyType) // !!! this is wrong in general

      case ARRAYVALUEtree =>
        val elemtpt = readTreeRef()
        val trees = until(end, () => readTreeRef())
        SeqLiteral(trees.toLst, elemtpt)
...
*/
      ...
      case APPLIEDTYPEtree =>
        val tpt = readTreeRef()
        val args = until(end, () => readTreeRef())
        AppliedTypeTree(tpt, args.toLst)
