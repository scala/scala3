...
sealed abstract class GadtConstraintHandling extends Showable {
   ...
   */
  def fullBounds(sym: Symbol): TypeBounds
   ...
   */
  def addToConstraint(syms: List[Symbol]): Boolean
  def addToConstraint(sym: Symbol): Boolean = addToConstraint(sym :: Nil)

  /** Further constrain a symbol already present in the constraint. */
  def addBound(sym: Symbol, bound: Type, isUpper: Boolean): Boolean
...
final class GadtConstraintHandlingImpl(
  ...
  override def addToConstraint(params: List[Symbol]): Boolean = {
    ...
    val poly1 = PolyType(params.map { sym => DepParamName.fresh(sym.name.toTypeName) })(
      pt => params.map { param =>
        ...
        def substDependentSyms(tp: Type, isUpper: Boolean): Type = {
  ...
  override def addBound(sym: Symbol, bound: Type, isUpper: Boolean): Boolean = {
  ...
  override def isLess(sym1: Symbol, sym2: Symbol): Boolean =
    constraint.isLess(tvarOrError(sym1).origin, tvarOrError(sym2).origin)

  override def fullBounds(sym: Symbol): TypeBounds =
    mapping(sym) match {
      case null => null
      case tv =>
  ...
  override def bounds(sym: Symbol): TypeBounds =
    mapping(sym) match {
      case null => null
      case tv =>
  ...
  override def contains(sym: Symbol): Boolean = mapping(sym) ne null

  override def approximation(sym: Symbol, fromBelow: Boolean): Type = {
  ...
  override def isSameType(tp1: Type, tp2: Type): Boolean = ctx.typeComparer.isSameType(tp1, tp2)
   ...
   override def nonParamBounds(param: TypeParamRef): TypeBounds =
     constraint.nonParamBounds(param) match {
       case TypeAlias(tpr: TypeParamRef) => TypeAlias(externalize(tpr))
       case tb => tb
     }

   override def fullLowerBound(param: TypeParamRef): Type =
     constraint.minLower(param).foldLeft(nonParamBounds(param).lo) {
       (t, u) => t | externalize(u)
     }

   override def fullUpperBound(param: TypeParamRef): Type =
  ...
  private def externalize(param: TypeParamRef): Type =
    reverseMapping(param) match {
      case sym: Symbol => sym.typeRef
      case null => param
    }

  private def tvarOrError(sym: Symbol): TypeVar =
  ...
  override def debugBoundsDescription: String = {
