...
object TypeTree {
  ...
  def apply(tree: Tree)(implicit ctx: Context): trees.TypeTree = Impl(tree, ctx)
  ...
  def unapplyTypeIdent(tree: scala.tasty.Tree): Option[trees.TypeIdent.Data] = tree match {
    case Impl(id@Trees.Ident(name: Names.TypeName), _) if id.isType => Some(TypeName(name))
    case _ => None
  }

  def unapplyTypeSelect(tree: scala.tasty.Tree): Option[trees.TypeSelect.Data] = tree match {
    case Impl(id@Trees.Select(qual, name: Names.TypeName), ctx) if id.isType => Some(Term(qual)(ctx), TypeName(name))
    case _ => None
  }

  def unapplySingleton(tree: scala.tasty.Tree): Option[trees.Singleton.Data] = tree match {
    case Impl(Trees.SingletonTypeTree(ref), ctx) => Some(Term(ref)(ctx))
    case _ => None
  }

  def unapplyRefined(tree: scala.tasty.Tree): Option[trees.Refined.Data] = tree match {
    case Impl(Trees.RefinedTypeTree(tpt, refinements), ctx) => Some(TypeTree(tpt)(ctx), refinements.map(Definition(_)(ctx)))
    case _ => None
  }

  def unapplyApplied(tree: scala.tasty.Tree): Option[trees.Applied.Data] = tree match {
    case Impl(Trees.AppliedTypeTree(tycon, args), ctx) => Some(TypeTree(tycon)(ctx), args.map(TypeTree(_)(ctx)))
    case _ => None
  }

  def unapplyAnnotated(tree: scala.tasty.Tree): Option[trees.Annotated.Data] = tree match {
    case Impl(Trees.Annotated(arg, annot), ctx) => Some(TypeTree(arg)(ctx), Term(annot)(ctx))
    case _ => None
  }

  def unapplyAnd(tree: scala.tasty.Tree): Option[trees.And.Data] = tree match {
    case Impl(Trees.AndTypeTree(left, right), ctx) => Some(TypeTree(left)(ctx), TypeTree(right)(ctx))
    case _ => None
  }

  def unapplyOr(tree: scala.tasty.Tree): Option[trees.Or.Data] = tree match {
    case Impl(Trees.OrTypeTree(left, right), ctx) => Some(TypeTree(left)(ctx), TypeTree(right)(ctx))
    case _ => None
  }

  def unapplyByName(tree: scala.tasty.Tree): Option[trees.ByName.Data] = tree match {
    case Impl(Trees.ByNameTypeTree(tpt), ctx) => Some(TypeTree(tpt)(ctx))
    case _ => None
  }

  private case class Impl(tree: Tree, ctx: Context) extends trees.TypeTree with Positioned {
