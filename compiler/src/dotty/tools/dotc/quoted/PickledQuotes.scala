            ...
            if isTerm then
              val quotedExpr = termHole match
                case ExprHole.V1(evalHole) =>
                  evalHole.nn.apply(idx, reifyExprHoleV1Args(args.toList), QuotesImpl())
                case ExprHole.V2(evalHole) =>
                  evalHole.nn.apply(idx, reifyExprHoleV2Args(args.toList), QuotesImpl())
              ...
              val quotedType = evalHole.nn.apply(idx, reifyTypeHoleArgs(args.toList))
