...
object ScopeImpl {
  ...
  def apply()(using Context): Scope =
    new ScopeImpl(ctx)

  def showTree(tree: tpd.Tree)(using Context): String = {
    val s = ScopeImpl()(using MacroExpansion.context(tree))
    val syntaxHighlight =
  ...
  private def show(using s: Scope)(tree: s.tasty.Tree, syntaxHighlight: SyntaxHighlight)(using s.tasty.Context) =
...
class ScopeImpl private (ctx: Context) extends Scope {
  ...
  object tasty extends scala.tasty.Reflection, scala.internal.tasty.CompilerInterface:
    ...
    object TreeMethodsImpl extends TreeMethods:
      ...
      extension [T](tree: Tree)
        def asExprOf(using s: Scope)(using s.Type[T]): s.Expr[T] =
          if tree.isExpr then
            if tree.tpe <:< Type.of[T] then
              tree.asInstanceOf[s.Expr[T]]
            else
              throw new scala.tasty.reflect.ExprCastError(
                s"""Expr: ${tree.show}
                  |of type: ${tree.tpe.show}
                  |did not conform to type: ${Type.of[T].show}
                  |""".stripMargin
              )
          else tree match
    ...
    object TermMethodsImpl extends TermMethods:
      extension (self: Term):
        def seal(using s: Scope): s.Expr[Any] =
          if self.isExpr then  self.asInstanceOf[s.Expr[Any]]
        ...
        def sealOpt(using s: Scope): Option[s.Expr[Any]] =
          if self.isExpr then Some(self.asInstanceOf[s.Expr[Any]])
          else None
    ...
    object Type extends TypeModule:
      def of[T <: AnyKind](using s: Scope)(using qtype: s.Type[T]): Type = qtype.asInstanceOf[TypeTree].tpe
    ...
    def Constraints_context[T]: scala.quoted.Scope =
      ...
      dotty.tools.dotc.quoted.ScopeImpl()(using ctx1)
