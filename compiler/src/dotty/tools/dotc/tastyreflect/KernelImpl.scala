...
class KernelImpl(val rootContext: core.Contexts.Context, val rootPosition: util.SourcePosition) extends Kernel {
  ...
  type TypeOf = Types.TypeOf

  def matchTypeOf(tpe: TypeOrBounds)(implicit ctx: Context): Option[TypeOf] = tpe match {
    case tpe: Types.TypeOf => Some(tpe)
    case _ => None
  }

  def typeOf_underlying(self: TypeOf)(implicit ctx: Context): Type = self.underlyingTp
