...
class KernelImpl(val rootContext: core.Contexts.Context, val rootPosition: util.SourcePosition) extends Kernel {
  ...
  def Tree_pos(self: Tree) given (ctx: Context): Position = self.sourcePos
  def Tree_symbol(self: Tree) given (ctx: Context): Symbol = self.symbol

  type PackageClause = tpd.PackageDef

  def matchPackageClause(tree: Tree) given (ctx: Context): Option[PackageClause] = tree match {
    case x: tpd.PackageDef => Some(x)
    case _ => None
  }

  def PackageClause_pid(self: PackageClause) given (ctx: Context): Ref = self.pid
  ...
  def matchStatement(tree: Tree) given (ctx: Context): Option[Statement] = tree match {
  ...
  def matchImport(tree: Tree) given (ctx: Context): Option[Import] = tree match {
    case tree: tpd.Import => Some(tree)
    case _ => None
  }

  def Import_implied(self: Import): Boolean = self.importGiven
  def Import_expr(self: Import) given (ctx: Context): Tree = self.expr
  ...
  def matchDefinition(tree: Tree) given (ctx: Context): Option[Definition] = tree match {
    case tree: tpd.MemberDef => Some(tree)
    case tree: PackageDefinition => Some(tree)
    case _ => None
  }

  def Definition_name(self: Definition) given (ctx: Context): String = self match {
  ...
  def matchPackageDef(tree: Tree) given (ctx: Context): Option[PackageDef] = tree match {
    case x: PackageDefinition => Some(x)
    case _ => None
  }

  def PackageDef_owner(self: PackageDef) given (ctx: Context): PackageDef = packageDefFromSym(self.symbol.owner)
  ...
  def PackageDef_symbol(self: PackageDef) given (ctx: Context): PackageDefSymbol = self.symbol

  type ClassDef = tpd.TypeDef

  def matchClassDef(tree: Tree) given (ctx: Context): Option[ClassDef] = tree match {
    case x: tpd.TypeDef if x.isClassDef => Some(x)
    case _ => None
  }

  def ClassDef_constructor(self: ClassDef) given (ctx: Context): DefDef = ClassDef_rhs(self).constr
  def ClassDef_parents(self: ClassDef) given (ctx: Context): List[Term | TypeTree] = ClassDef_rhs(self).parents
  def ClassDef_derived(self: ClassDef) given (ctx: Context): List[TypeTree] = ClassDef_rhs(self).derived.asInstanceOf[List[TypeTree]]
  def ClassDef_self(self: ClassDef) given (ctx: Context): Option[ValDef] = optional(ClassDef_rhs(self).self)
  def ClassDef_body(self: ClassDef) given (ctx: Context): List[Statement] = ClassDef_rhs(self).body
  def ClassDef_symbol(self: ClassDef) given (ctx: Context): ClassDefSymbol = self.symbol.asClass
  private def ClassDef_rhs(self: ClassDef) = self.rhs.asInstanceOf[tpd.Template]

  def ClassDef_copy(original: ClassDef)(name: String, constr: DefDef, parents: List[Term | TypeTree], derived: List[TypeTree], selfOpt: Option[ValDef], body: List[Statement]) given (ctx: Context): ClassDef = {
  ...
  def matchTypeDef(tree: Tree) given (ctx: Context): Option[TypeDef] = tree match {
  ...
  def matchDefDef(tree: Tree) given (ctx: Context): Option[DefDef] = tree match {
    case x: tpd.DefDef => Some(x)
    case _ => None
  }

  def DefDef_typeParams(self: DefDef) given (ctx: Context): List[TypeDef] = self.tparams
  def DefDef_paramss(self: DefDef) given (ctx: Context): List[List[ValDef]] = self.vparamss
  def DefDef_returnTpt(self: DefDef) given (ctx: Context): TypeTree = self.tpt
  def DefDef_rhs(self: DefDef) given (ctx: Context): Option[Tree] = optional(self.rhs)
  ...
  def matchValDef(tree: Tree) given (ctx: Context): Option[ValDef] = tree match {
    case x: tpd.ValDef => Some(x)
    case _ => None
  }

  def ValDef_tpt(self: ValDef) given (ctx: Context): TypeTree = self.tpt
  def ValDef_rhs(self: ValDef) given (ctx: Context): Option[Tree] = optional(self.rhs)
  def ValDef_symbol(self: ValDef) given (ctx: Context): ValDefSymbol = self.symbol.asTerm
  ...
  def matchTerm(tree: Tree) given (ctx: Context): Option[Term] = tree match {
    case x: tpd.SeqLiteral => Some(tree)
    case _ if tree.isTerm => Some(tree)
    case _ => None
  }

  def Term_pos(self: Term) given (ctx: Context): Position = self.sourcePos
  def Term_tpe(self: Term) given (ctx: Context): Type = self.tpe
  def Term_underlyingArgument(self: Term) given (ctx: Context): Term = self.underlyingArgument
  def Term_underlying(self: Term) given (ctx: Context): Term = self.underlying

  type Ref = tpd.RefTree

  def matchRef(tree: Tree) given (ctx: Context): Option[Ref] = tree match {
  ...
  def matchIdent(x: Term) given (ctx: Context): Option[Ident] = x match {
  ...
  def matchSelect(x: Term) given (ctx: Context): Option[Select] = x match {
    case x: tpd.Select if x.isTerm => Some(x)
    case _ => None
  }

  def Select_qualifier(self: Select) given (ctx: Context): Term = self.qualifier
  def Select_name(self: Select) given (ctx: Context): String = self.name.toString
  ...
  def matchLiteral(x: Term) given (ctx: Context): Option[Literal] = x match {
  ...
  def matchThis(x: Term) given (ctx: Context): Option[This] = x match {
  ...
  def matchNew(x: Term) given (ctx: Context): Option[New] = x match {
  ...
  def matchNamedArg(x: Term) given (ctx: Context): Option[NamedArg] = x match {
    case x: tpd.NamedArg if x.name.isInstanceOf[core.Names.TermName] => Some(x) // TODO: Now, the name should alwas be a term name
    case _ => None
  }

  def NamedArg_name(self: NamedArg) given (ctx: Context): String = self.name.toString
  ...
  def matchApply(x: Term) given (ctx: Context): Option[Apply] = x match {
    case x: tpd.Apply => Some(x)
    case _ => None
  }

  def Apply_fun(self: Apply) given (ctx: Context): Term = self.fun
  def Apply_args(self: Apply) given (ctx: Context): List[Term] = self.args
  ...
  def matchTypeApply(x: Term) given (ctx: Context): Option[TypeApply] = x match {
    case x: tpd.TypeApply => Some(x)
    case _ => None
  }

  def TypeApply_fun(self: TypeApply) given (ctx: Context): Term = self.fun
  ...
  def matchSuper(x: Term) given (ctx: Context): Option[Super] = x match {
    case x: tpd.Super => Some(x)
    case _ => None
  }

  def Super_qualifier(self: Super) given (ctx: Context): Term = self.qual
  ...
  def matchTyped(x: Term) given (ctx: Context): Option[Typed] = x match {
    case x: tpd.Typed => Some(x)
    case _ => None
  }

  def Typed_expr(self: Typed) given (ctx: Context): Term = self.expr
  ...
  def matchAssign(x: Term) given (ctx: Context): Option[Assign] = x match {
    case x: tpd.Assign => Some(x)
    case _ => None
  }

  def Assign_lhs(self: Assign) given (ctx: Context): Term = self.lhs
  ...
  def matchBlock(x: Term) given (ctx: Context): Option[Block] = normalizedLoops(x) match {
   ...
   */
  private def normalizedLoops(tree: tpd.Tree) given (ctx: Context): tpd.Tree = tree match {
  ...
  private def needsNormalization(tree: tpd.Tree) given (ctx: Context): Boolean = tree match {
    case _: tpd.Closure => true
    case _ => false
  }

  def Block_statements(self: Block) given (ctx: Context): List[Statement] = self.stats
  ...
  def matchInlined(x: Term) given (ctx: Context): Option[Inlined] = x match {
    case x: tpd.Inlined => Some(x)
    case _ => None
  }

  def Inlined_call(self: Inlined) given (ctx: Context): Option[Term | TypeTree] = optional(self.call)
  def Inlined_bindings(self: Inlined) given (ctx: Context): List[Definition] = self.bindings
  ...
  def matchClosure(x: Term) given (ctx: Context): Option[Closure] = x match {
    case x: tpd.Closure => Some(x)
    case _ => None
  }

  def Closure_meth(self: Closure) given (ctx: Context): Term = self.meth
  ...
  def matchIf(x: Term) given (ctx: Context): Option[If] = x match {
    case x: tpd.If => Some(x)
    case _ => None
  }

  def If_cond(self: If) given (ctx: Context): Term = self.cond
  def If_thenp(self: If) given (ctx: Context): Term = self.thenp
  ...
  def matchMatch(x: Term) given (ctx: Context): Option[Match] = x match {
    case x: tpd.Match if !x.selector.isEmpty => Some(x)
    case _ => None
  }

  def Match_scrutinee(self: Match) given (ctx: Context): Term = self.selector
  ...
  def matchImplicitMatch(x: Term) given (ctx: Context): Option[Match] = x match {
    case x: tpd.Match if x.selector.isEmpty => Some(x)
    case _ => None
  }
  ...
  def matchTry(x: Term) given (ctx: Context): Option[Try] = x match {
    case x: tpd.Try => Some(x)
    case _ => None
  }

  def Try_body(self: Try) given (ctx: Context): Term = self.expr
  def Try_cases(self: Try) given (ctx: Context): List[CaseDef] = self.cases
  ...
  def matchReturn(x: Term) given (ctx: Context): Option[Return] = x match {
  ...
  def matchRepeated(x: Term) given (ctx: Context): Option[Repeated] = x match {
    case x: tpd.SeqLiteral => Some(x)
    case _ => None
  }

  def Repeated_elems(self: Repeated) given (ctx: Context): List[Term] = self.elems
  ...
  def matchSelectOuter(x: Term) given (ctx: Context): Option[SelectOuter] = x match {
    case x: tpd.Select =>
      x.name match {
  ...
  def SelectOuter_qualifier(self: SelectOuter) given (ctx: Context): Term = self.qualifier
  ...
  def matchWhile(x: Term) given (ctx: Context): Option[While] = x match {
    case x: tpd.WhileDo => Some(x)
    case _ => None
  }

  def While_cond(self: While) given (ctx: Context): Term = self.cond
  ...
  def matchTypeTree(x: TypeTree | TypeBoundsTree) given (ctx: Context): Option[TypeTree] = x match {
    case x: tpd.TypeBoundsTree => None
    case _ => if (x.isType) Some(x) else None
  }

  def TypeTree_pos(self: TypeTree) given (ctx: Context): Position = self.sourcePos
  def TypeTree_symbol(self: TypeTree) given (ctx: Context): Symbol = self.symbol
  def TypeTree_tpe(self: TypeTree) given (ctx: Context): Type = self.tpe.stripTypeVar

  type Inferred = tpd.TypeTree

  def matchInferred(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[Inferred] = tpt match {
  ...
  def matchTypeIdent(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[TypeIdent] = tpt match {
    case tpt: tpd.Ident if tpt.isType => Some(tpt)
    case _ => None
  }

  def TypeIdent_name(self: TypeIdent) given (ctx: Context): String = self.name.toString
  ...
  def matchTypeSelect(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[TypeSelect] = tpt match {
    case tpt: tpd.Select if tpt.isType && tpt.qualifier.isTerm  => Some(tpt)
    case _ => None
  }

  def TypeSelect_qualifier(self: TypeSelect) given (ctx: Context): Term = self.qualifier
  ...
  def matchProjection(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[Projection] = tpt match {
    case tpt: tpd.Select if tpt.isType && tpt.qualifier.isType => Some(tpt)
    case _ => None
  }

  def Projection_qualifier(self: Projection) given (ctx: Context): TypeTree = self.qualifier
  def Projection_name(self: Projection) given (ctx: Context): String = self.name.toString
  ...
  def matchSingleton(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[Singleton] = tpt match {
  ...
  def matchRefined(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[Refined] = tpt match {
    case tpt: tpd.RefinedTypeTree => Some(tpt)
    case _ => None
  }

  def Refined_tpt(self: Refined) given (ctx: Context): TypeTree = self.tpt
  def Refined_refinements(self: Refined) given (ctx: Context): List[Definition] = self.refinements
  ...
  def matchApplied(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[Applied] = tpt match {
    case tpt: tpd.AppliedTypeTree => Some(tpt)
    case _ => None
  }

  def Applied_tpt(self: Applied) given (ctx: Context): TypeTree = self.tpt
  ...
  def matchAnnotated(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[Annotated] = tpt match {
    case tpt: tpd.Annotated => Some(tpt)
    case _ => None
  }

  def Annotated_arg(self: Annotated) given (ctx: Context): TypeTree = self.arg
  ...
  def matchMatchTypeTree(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[MatchTypeTree] = tpt match {
    case tpt: tpd.MatchTypeTree => Some(tpt)
    case _ => None
  }

  def MatchTypeTree_bound(self: MatchTypeTree) given (ctx: Context): Option[TypeTree] = if (self.bound == tpd.EmptyTree) None else Some(self.bound)
  def MatchTypeTree_selector(self: MatchTypeTree) given (ctx: Context): TypeTree = self.selector
  ...
  def matchByName(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[ByName] = tpt match {
  ...
  def matchLambdaTypeTree(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[LambdaTypeTree] = tpt match {
    case tpt: tpd.LambdaTypeTree => Some(tpt)
    case _ => None
  }

  def Lambdatparams(self: LambdaTypeTree) given (ctx: Context): List[TypeDef] = self.tparams
  ...
  def matchTypeBind(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[TypeBind] = tpt match {
    case tpt: tpd.Bind if tpt.name.isTypeName => Some(tpt)
    case _ => None
  }

  def TypeBind_name(self: TypeBind) given (ctx: Context): String = self.name.toString
  def TypeBind_body(self: TypeBind) given (ctx: Context): TypeTree | TypeBoundsTree = self.body

  def TypeBind_copy(original: TypeBind)(name: String, tpt: TypeTree | TypeBoundsTree) given (ctx: Context): TypeBind =
    tpd.cpy.Bind(original)(name.toTypeName, tpt)

  type TypeBlock = tpd.Block

  def matchTypeBlock(tpt: TypeTree | TypeBoundsTree) given (ctx: Context): Option[TypeBlock] = tpt match {
    case tpt: tpd.Block => Some(tpt)
    case _ => None
  }

  def TypeBlock_aliases(self: TypeBlock) given (ctx: Context): List[TypeDef] = self.stats.map { case alias: TypeDef => alias }
  ...
  def matchTypeBoundsTree(x: TypeTree | TypeBoundsTree) given (ctx: Context): Option[TypeBoundsTree] = x match {
  ...
  def TypeBoundsTree_tpe(self: TypeBoundsTree) given (ctx: Context): TypeBounds = self.tpe.asInstanceOf[Types.TypeBounds]
  def TypeBoundsTree_low(self: TypeBoundsTree) given (ctx: Context): TypeTree = self.lo
  def TypeBoundsTree_hi(self: TypeBoundsTree) given (ctx: Context): TypeTree = self.hi

  type WildcardTypeTree = tpd.Ident

  def matchWildcardTypeTree(x: TypeTree | TypeBoundsTree) given (ctx: Context): Option[WildcardTypeTree] = x match {
    case x @ Trees.Ident(nme.WILDCARD) => Some(x)
    case _ => None
  }

  def WildcardTypeTree_tpe(self: WildcardTypeTree) given (ctx: Context): TypeOrBounds = self.tpe.stripTypeVar

  type CaseDef = tpd.CaseDef

  def matchCaseDef(tree: Tree) given (ctx: Context): Option[CaseDef] = tree match {
    case tree: tpd.CaseDef if tree.body.isTerm => Some(tree)
    case _ => None
  }

  def CaseDef_pattern(self: CaseDef) given (ctx: Context): Pattern = self.pat
  def CaseDef_guard(self: CaseDef) given (ctx: Context): Option[Term] = optional(self.guard)
  def CaseDef_rhs(self: CaseDef) given (ctx: Context): Term = self.body
  ...
  def matchTypeCaseDef(tree: Tree) given (ctx: Context): Option[TypeCaseDef] = tree match {
    case tree: tpd.CaseDef if tree.body.isType => Some(tree)
    case _ => None
  }

  def TypeCaseDef_pattern(self: TypeCaseDef) given (ctx: Context): TypeTree = self.pat
  def TypeCaseDef_rhs(self: TypeCaseDef) given (ctx: Context): TypeTree = self.body
  ...
  def Pattern_pos(self: Pattern) given (ctx: Context): Position = self.sourcePos
  def Pattern_tpe(self: Pattern) given (ctx: Context): Type = self.tpe.stripTypeVar
  def Pattern_symbol(self: Pattern) given (ctx: Context): Symbol = self.symbol
  ...
  def Pattern_Value_value(self: Value) given (ctx: Context): Term = self

  def Pattern_Value_module_apply(term: Term) given (ctx: Context): Value = term match {
    case lit: tpd.Literal => lit
    case ref: tpd.RefTree if ref.isTerm => ref
    case ths: tpd.This => ths
  }
  def Pattern_Value_module_copy(original: Value)(term: Term) given (ctx: Context): Value = term match {
  ...
  def matchPattern_Bind(x: Pattern) given (ctx: Context): Option[Bind] = x match {
    case x: tpd.Bind if x.name.isTermName => Some(x)
    case _ => None
  }

  def Pattern_Bind_name(self: Bind) given (ctx: Context): String = self.name.toString
  ...
  def matchPattern_Unapply(pattern: Pattern) given (ctx: Context): Option[Unapply] = pattern match {
    case pattern @ Trees.UnApply(_, _, _) => Some(pattern)
    case Trees.Typed(pattern @ Trees.UnApply(_, _, _), _) => Some(pattern)
    case _ => None
  }

  def Pattern_Unapply_fun(self: Unapply) given (ctx: Context): Term = self.fun
  def Pattern_Unapply_implicits(self: Unapply) given (ctx: Context): List[Term] = self.implicits
  ...
  def matchPattern_Alternatives(pattern: Pattern) given (ctx: Context): Option[Alternatives] = pattern match {
  ...
  def matchPattern_TypeTest(pattern: Pattern) given (ctx: Context): Option[TypeTest] = pattern match {
  ...
  def matchTypeBounds(x: TypeOrBounds) given (ctx: Context): Option[TypeBounds] = x match {
    case x: Types.TypeBounds => Some(x)
    case _ => None
  }

  def TypeBounds_low(self: TypeBounds) given (ctx: Context): Type = self.lo
  def TypeBounds_hi(self: TypeBounds) given (ctx: Context): Type = self.hi

  type Type = Types.Type

  def matchType(x: TypeOrBounds) given (ctx: Context): Option[Type] = x match {
  ...
  def `Type_=:=`(self: Type)(that: Type) given (ctx: Context): Boolean = self =:= that
  ...
  /** Widen from singleton type to its underlying non-singleton
    ...
    *  <o.x.type>.widen = o.C
    */
  def Type_widen(self: Type) given (ctx: Context): Type = self.widen

  def Type_dealias(self: Type) given (ctx: Context): Type = self.dealias
  ...
  def Type_typeSymbol(self: Type) given (ctx: Context): Symbol = self.typeSymbol

  def Type_isSingleton(self: Type) given (ctx: Context): Boolean = self.isSingleton
  ...
  def matchConstantType(tpe: TypeOrBounds) given (ctx: Context): Option[ConstantType] = tpe match {
    case tpe: Types.ConstantType => Some(tpe)
    case _ => None
  }

  def ConstantType_constant(self: ConstantType) given (ctx: Context): Constant = self.value

  type SymRef = Types.NamedType

  def matchSymRef(tpe: TypeOrBounds) given (ctx: Context): Option[SymRef] = tpe match {
  ...
  def SymRef_qualifier(self: SymRef) given (ctx: Context): TypeOrBounds = self.prefix

  // TODO remove this method. May require splitting SymRef into TypeSymRef and TermSymRef
  def matchSymRef_unapply(tpe: TypeOrBounds) given (ctx: Context): Option[(Symbol, Type | NoPrefix)] = tpe match {
  ...
  def matchTermRef(tpe: TypeOrBounds) given (ctx: Context): Option[TermRef] = tpe match {
    case tpe: Types.NamedType =>
      tpe.designator match {
  ...
  def TermRef_name(self: TermRef) given (ctx: Context): String = self.name.toString
  def TermRef_qualifier(self: TermRef) given (ctx: Context): TypeOrBounds = self.prefix
  ...
  def matchTypeRef(tpe: TypeOrBounds) given (ctx: Context): Option[TypeRef] = tpe match {
    case tpe: Types.NamedType =>
      tpe.designator match {
  ...
  def TypeRef_name(self: TypeRef) given (ctx: Context): String = self.name.toString
  def TypeRef_qualifier(self: TypeRef) given (ctx: Context): TypeOrBounds = self.prefix

  type SuperType = Types.SuperType

  def matchSuperType(tpe: TypeOrBounds) given (ctx: Context): Option[SuperType] = tpe match {
    case tpe: Types.SuperType => Some(tpe)
    case _ => None
  }

  def SuperType_thistpe(self: SuperType) given (ctx: Context): Type = self.thistpe
  def SuperType_supertpe(self: SuperType) given (ctx: Context): Type = self.supertpe

  type Refinement = Types.RefinedType

  def matchRefinement(tpe: TypeOrBounds) given (ctx: Context): Option[Refinement] = tpe match {
    case tpe: Types.RefinedType => Some(tpe)
    case _ => None
  }

  def Refinement_parent(self: Refinement) given (ctx: Context): Type = self.parent
  def Refinement_name(self: Refinement) given (ctx: Context): String = self.refinedName.toString
  def Refinement_info(self: Refinement) given (ctx: Context): TypeOrBounds = self.refinedInfo

  type AppliedType = Types.AppliedType

  def matchAppliedType(tpe: TypeOrBounds) given (ctx: Context): Option[AppliedType] = tpe match {
    case tpe: Types.AppliedType => Some(tpe)
    case _ => None
  }

  def AppliedType_tycon(self: AppliedType) given (ctx: Context): Type = self.tycon
  def AppliedType_args(self: AppliedType) given (ctx: Context): List[TypeOrBounds] = self.args

  type AnnotatedType = Types.AnnotatedType

  def matchAnnotatedType(tpe: TypeOrBounds) given (ctx: Context): Option[AnnotatedType] = tpe match {
    case tpe: Types.AnnotatedType => Some(tpe)
    case _ => None
  }

  def AnnotatedType_underlying(self: AnnotatedType) given (ctx: Context): Type = self.underlying.stripTypeVar
  def AnnotatedType_annot(self: AnnotatedType) given (ctx: Context): Term = self.annot.tree

  type AndType = Types.AndType

  def matchAndType(tpe: TypeOrBounds) given (ctx: Context): Option[AndType] = tpe match {
    case tpe: Types.AndType => Some(tpe)
    case _ => None
  }

  def AndType_left(self: AndType) given (ctx: Context): Type = self.tp1.stripTypeVar
  def AndType_right(self: AndType) given (ctx: Context): Type = self.tp2.stripTypeVar

  type OrType = Types.OrType

  def matchOrType(tpe: TypeOrBounds) given (ctx: Context): Option[OrType] = tpe match {
    case tpe: Types.OrType => Some(tpe)
    case _ => None
  }

  def OrType_left(self: OrType) given (ctx: Context): Type = self.tp1.stripTypeVar
  def OrType_right(self: OrType) given (ctx: Context): Type = self.tp2.stripTypeVar

  type MatchType = Types.MatchType

  def matchMatchType(tpe: TypeOrBounds) given (ctx: Context): Option[MatchType] = tpe match {
    case tpe: Types.MatchType => Some(tpe)
    case _ => None
  }

  def MatchType_bound(self: MatchType) given (ctx: Context): Type = self.bound
  def MatchType_scrutinee(self: MatchType) given (ctx: Context): Type = self.scrutinee
  def MatchType_cases(self: MatchType) given (ctx: Context): List[Type] = self.cases

  type ByNameType = Types.ExprType

  def matchByNameType(tpe: TypeOrBounds) given (ctx: Context): Option[ByNameType] = tpe match {
    case tpe: Types.ExprType => Some(tpe)
    case _ => None
  }

  def ByNameType_underlying(self: ByNameType) given (ctx: Context): Type = self.resType.stripTypeVar

  type ParamRef = Types.ParamRef

  def matchParamRef(tpe: TypeOrBounds) given (ctx: Context): Option[ParamRef] = tpe match {
  ...
  def ParamRef_paramNum(self: ParamRef) given (ctx: Context): Int = self.paramNum

  type ThisType = Types.ThisType

  def matchThisType(tpe: TypeOrBounds) given (ctx: Context): Option[ThisType] = tpe match {
    case tpe: Types.ThisType => Some(tpe)
    case _ => None
  }

  def ThisType_tref(self: ThisType) given (ctx: Context): Type = self.tref

  type RecursiveThis = Types.RecThis

  def matchRecursiveThis(tpe: TypeOrBounds) given (ctx: Context): Option[RecursiveThis] = tpe match {
  ...
  def matchRecursiveType(tpe: TypeOrBounds) given (ctx: Context): Option[RecursiveType] = tpe match {
  ...
  def matchMethodType(tpe: TypeOrBounds) given (ctx: Context): Option[MethodType] = tpe match {
  ...
  def MethodType_paramNames(self: MethodType) given (ctx: Context): List[String] = self.paramNames.map(_.toString)
  def MethodType_paramTypes(self: MethodType) given (ctx: Context): List[Type] = self.paramInfos
  def MethodType_resType(self: MethodType) given (ctx: Context): Type = self.resType

  type PolyType = Types.PolyType

  def matchPolyType(tpe: TypeOrBounds) given (ctx: Context): Option[PolyType] = tpe match {
    case tpe: Types.PolyType => Some(tpe)
    case _ => None
  }

  def PolyType_paramNames(self: PolyType) given (ctx: Context): List[String] = self.paramNames.map(_.toString)
  def PolyType_paramBounds(self: PolyType) given (ctx: Context): List[TypeBounds] = self.paramInfos
  def PolyType_resType(self: PolyType) given (ctx: Context): Type = self.resType

  type TypeLambda = Types.TypeLambda

  def matchTypeLambda(tpe: TypeOrBounds) given (ctx: Context): Option[TypeLambda] = tpe match {
    case tpe: Types.TypeLambda => Some(tpe)
    case _ => None
  }

  def TypeLambda_paramNames(self: TypeLambda) given (ctx: Context): List[String] = self.paramNames.map(_.toString)
  def TypeLambda_paramBounds(self: TypeLambda) given (ctx: Context): List[TypeBounds] = self.paramInfos
  def TypeLambda_resType(self: TypeLambda) given (ctx: Context): Type = self.resType
  ...
  def matchSimpleSelector(self: ImportSelector) given (ctx: Context): Option[SimpleSelector] = self match {
    case self: untpd.Ident => Some(self)
    case _ => None
  }

  def SimpleSelector_selection(self: SimpleSelector) given (ctx: Context): Id = self

  type RenameSelector = untpd.Thicket

  def matchRenameSelector(self: ImportSelector) given (ctx: Context): Option[RenameSelector] = self match {
  ...
  def matchOmitSelector(self: ImportSelector) given (ctx: Context): Option[OmitSelector] = self match {
  ...
  def Id_pos(self: Id) given (ctx: Context): Position = self.sourcePos

  def Id_name(self: Id) given (ctx: Context): String = self.name.toString
  ...
  def Symbol_owner(self: Symbol) given (ctx: Context): Symbol = self.owner

  def Symbol_flags(self: Symbol) given (ctx: Context): Flags = self.flags
  ...
  def Symbol_name(self: Symbol) given (ctx: Context): String = self.name.toString

  def Symbol_fullName(self: Symbol) given (ctx: Context): String = self.fullName.toString

  def Symbol_pos(self: Symbol) given (ctx: Context): Position = self.sourcePos
  ...
  def Symbol_isLocalDummy(self: Symbol) given (ctx: Context): Boolean = self.isLocalDummy
  def Symbol_isRefinementClass(self: Symbol) given (ctx: Context): Boolean = self.isRefinementClass
  def Symbol_isAliasType(self: Symbol) given (ctx: Context): Boolean = self.isAliasType
  def Symbol_isAnonymousClass(self: Symbol) given (ctx: Context): Boolean = self.isAnonymousClass
  def Symbol_isAnonymousFunction(self: Symbol) given (ctx: Context): Boolean = self.isAnonymousFunction
  def Symbol_isAbstractType(self: Symbol) given (ctx: Context): Boolean = self.isAbstractType
  def Symbol_isClassConstructor(self: Symbol) given (ctx: Context): Boolean = self.isClassConstructor
  ...
  private def isMethod(sym: Symbol) given (ctx: Context): Boolean =
  ...
  private def isField(sym: Symbol) given (ctx: Context): Boolean = sym.isTerm && !sym.is(Flags.Method)
  ...
  def matchNoSymbol(symbol: Symbol) given (ctx: Context): Boolean = symbol eq core.Symbols.NoSymbol
  ...
  def matchImplicitSearchSuccess(isr: ImplicitSearchResult) given (ctx: Context): Option[ImplicitSearchSuccess] = isr.tpe match {
    case _: SearchFailureType => None
    case _ => Some(isr)
  }
  def ImplicitSearchSuccess_tree(self: ImplicitSearchSuccess) given (ctx: Context): Term = self

  type ImplicitSearchFailure = Tree
  def matchImplicitSearchFailure(isr: ImplicitSearchResult) given (ctx: Context): Option[ImplicitSearchFailure] = isr.tpe match {
  ...
  def matchDivergingImplicit(isr: ImplicitSearchResult) given (ctx: Context): Option[DivergingImplicit] = isr.tpe match {
    case _: Implicits.DivergingImplicit => Some(isr)
    case _ => None
  }

  type NoMatchingImplicits = Tree
  def matchNoMatchingImplicits(isr: ImplicitSearchResult) given (ctx: Context): Option[NoMatchingImplicits] = isr.tpe match {
    case _: Implicits.NoMatchingImplicits => Some(isr)
    case _ => None
  }

  type AmbiguousImplicits = Tree
  def matchAmbiguousImplicits(isr: ImplicitSearchResult) given (ctx: Context): Option[AmbiguousImplicits] = isr.tpe match {
