...
trait Dynamic {
  ...
  def typedDynamicApply(tree: untpd.Apply, isInsertedApply: Boolean, pt: Type)(
    ...
    def typedDynamicApply(
      ...
      val args = tree.args
      val dynName =
        if (args.exists(isNamedArg)) nme.applyDynamicNamed else nme.applyDynamic
  ...
  def typedDynamicAssign(tree: untpd.Assign, pt: Type)(using Context): Tree = {
    ...
    def reassignmentError =
      val isDef = tree.lhs.symbol.is(Method)
      val err =
        if isDef then ReassignmentToDef(tree.lhs.symbol.name)
        else ReassignmentToVal(tree.lhs.symbol.name)
      errorTree(tree, err)
  ...
  def handleStructural(tree: Tree)(using Context): Tree = {
    ...
    extension (tree: Tree)
      ...
      def maybeBoxingCast(tpe: Type) =
        val maybeBoxed =
          if tpe.classSymbol.isDerivedValueClass && qual.tpe <:< defn.ReflectSelectableTypeRef
          then
    ...
    fun.tpe.widen match {
      case tpe: ValueType =>
        structuralCall(nme.selectDynamic, Nil).maybeBoxingCast(
          fun.tpe.widenExpr
        )
