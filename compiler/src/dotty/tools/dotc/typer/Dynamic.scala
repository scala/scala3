...
object DynamicUnapply {
  def unapply(tree: tpd.Tree): Option[Vector[tpd.Tree]] = tree match
...
trait Dynamic {
  ...
  def typedDynamicApply(tree: untpd.Apply, isInsertedApply: Boolean, pt: Type)(using Context): Tree = {
    def typedDynamicApply(qual: untpd.Tree, name: Name, selSpan: Span, targs: Vector[untpd.Tree]): Tree = {
      ...
      if (dynName == nme.applyDynamicNamed && untpd.isWildcardStarArgList(args))
        ...
        def namedArgTuple(name: String, arg: untpd.Tree) = untpd.Tuple(Vector(Literal(Constant(name)), arg))
    ...
    if (isInsertedApply) {
      tree.fun match {
        ...
        case fun =>
          typedDynamicApply(fun, nme.apply, fun.span, Vector())
      }
    } else {
      tree.fun match {
        case sel @ Select(qual, name) if !isDynamicMethod(name) =>
          typedDynamicApply(qual, name, sel.span, Vector())
   ...
   */
  def typedDynamicSelect(tree: untpd.Select, targs: Vector[untpd.Tree], pt: Type)(using Context): Tree =
  ...
  def typedDynamicAssign(tree: untpd.Assign, pt: Type)(using Context): Tree = {
    def typedDynamicAssign(qual: untpd.Tree, name: Name, selSpan: Span, targs: Vector[untpd.Tree]): Tree =
      typedApply(untpd.Apply(coreDynamic(qual, nme.updateDynamic, name, selSpan, targs), tree.rhs), pt)
    tree.lhs match {
      case sel @ Select(qual, name) if !isDynamicMethod(name) =>
        typedDynamicAssign(qual, name, sel.span, Vector())
      case TypeApply(sel @ Select(qual, name), targs) if !isDynamicMethod(name) =>
  ...
  private def coreDynamic(qual: untpd.Tree, dynName: Name, name: Name, selSpan: Span, targs: Vector[untpd.Tree])(using Context): untpd.Apply = {
  ...
  def handleStructural(tree: Tree)(using Context): Tree = {
    ...
    def structuralCall(selectorName: TermName, classOfs: => Vector[Tree]) = {
      ...
      val base =
        untpd.Apply(
          ...
          (Literal(Constant(name.encode.toString)) +: Vector()).map(untpd.TypedSplice(_)))
      ...
      def addClassOfs(tree: Tree): Tree = tree match
        ...
        case Apply(fn @ Select(_, nme.applyDynamic), nameArg +: _ +: Vector()) =>
          fn.tpe.widen match
            case mt: MethodType => mt.paramInfos match
              case _ +: classOfsParam +: Vector()
              if classOfsParam.isRepeatedParam
                 && classOfsParam.argInfos.head.isRef(defn.ClassClass) =>
                  ...
                  cpy.Apply(tree)(fn,
                    nameArg +: seqToRepeated(SeqLiteral(classOfs, TypeTree(jlClassType))) +: Vector())
              case _ => tree
            case other => tree
        case _ => tree
      ...
      def maybeBoxingCast(tpe: Type) =
        val maybeBoxed =
          if tpe.classSymbol.isDerivedValueClass && qual.tpe <:< defn.ReflectSelectableTypeRef then
            ...
            New(tpe.widen.dealias, tree.cast(underlying) +: Vector())
          else
            tree
        maybeBoxed.cast(tpe)
    ...
    fun.tpe.widen match {
      case tpe: ValueType =>
        structuralCall(nme.selectDynamic, Vector()).maybeBoxingCast(fun.tpe.widenExpr)
        ...
        if (isDependentMethod(tpe))
          ...
          def classOfs =
            if tpe.paramInfoss.nestedExists(!TypeErasure.hasStableErasure(_)) then
              fail(i"has a parameter type with an unstable erasure") +: Vector()
            else
