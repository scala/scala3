...
object CrossVersionChecks:
  ...
  def checkUndesiredProperties(sym: Symbol, pos: SrcPos)(using Context): Unit =
   ...
   */
  def checkDeprecated(sym: Symbol, pos: SrcPos)(using Context): Unit = checkDeprecatedImpl(sym, pos, deferred = false)
  /** Defer check to avoid cycling. */
  def checkDeprecatedDeferred(sym: Symbol, pos: SrcPos)(using Context): Unit = checkDeprecatedImpl(sym, pos, deferred = true)

  private def checkDeprecatedImpl(sym: Symbol, pos: SrcPos, deferred: Boolean)(using Context): Unit =

    // pre: sym is an enumcase
    def isDeprecatedOrEnum(owner: Symbol)(using Context): Boolean =
      /** is the owner an enum or its companion and also the owner of sym */
      def isEnumOwner: Boolean =
      ...
      owner.isDeprecated || isEnumOwner
     ...
     */
    def skipWarningAt(owner: Symbol)(using Context) = owner.ownersIterator.exists(if sym.isEnumCase then isDeprecatedOrEnum else _.isDeprecated)
    ...
    for annot <- sym.getAnnotation(defn.DeprecatedAnnot) do
      val text =
        ...
        s"${sym.showLocated} is deprecated${since}${msg}"
      val owner = ctx.owner
      if deferred then
        ctx.deferredChecks.add(name) {
          if !skipWarningAt(owner) then report.deprecationWarning(text, pos)
        }
      else if !skipWarningAt(owner) then report.deprecationWarning(text, pos)
  end checkDeprecatedImpl

  def checkExperimentalSignature(sym: Symbol, pos: SrcPos)(using Context): Unit =
  ...
  def checkExperimentalAnnots(sym: Symbol)(using Context): Unit =
    if !sym.isInExperimentalScope then
   ...
   *  emit a warning.
   */
  def checkMigration(sym: Symbol, pos: SrcPos, xMigrationValue: ScalaVersion)(using Context): Unit =
   ...
   */
  def checkDeprecatedOvers(tree: Tree)(using Context): Unit =
    val symbol = tree.symbol
    if symbol.isDeprecated then
      val concrOvers = symbol.allOverriddenSymbols.filter(sym => !sym.isDeprecated && !sym.is(Deferred))
      if !concrOvers.isEmpty then
        report.deprecationWarning(
          s"$symbol overrides concrete, non-deprecated symbol(s):${
  ...
  def checkExperimentalInheritance(cls: ClassSymbol)(using Context): Unit =
