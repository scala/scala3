...
object Applications {
  ...
  def indexOfNames(tp: Type)(using Context): Map[String, Int] = {
    // TODO: Everything here
    val r = tp.typeMembers
      .filter(tpe => tpe.name.startsWith("_"))
      .map(tpe => tpe.name.show -> tpe.info.asInstanceOf[TypeAlias].toString)
      .collect { case (s"_$i", s"TypeBounds(ConstantType(Constant($nameAndMore") =>
        nameAndMore.takeWhile(_.isLetter) -> (i.toInt - 1)
      }
      .toMap
        ...
        if (bunchedArgs != Nil && argTypes != Nil) {
          ...
          val positionOfName: Map[String, Int] =
              ...
              if unapplyFn.tpe.widen.paramInfoss.head.head.typeSymbol.is(CaseClass) then
                ...
                indexOfNames(unapplyFn.tpe.widen.asInstanceOf[MethodType].resType)
          ...
          val namedArgs = bunchedArgs
            ...
            .groupBy { case NamedArg(name, _) => name.toString }
            .map {
              ...
              case (unkownName, pattern @ NamedArg(_, term) :: Nil) =>
                ...
                report.error(s"${unkownName.show} is unknown", tree.srcPos)
