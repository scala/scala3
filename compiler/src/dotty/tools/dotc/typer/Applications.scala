...
object Applications {
  ...
  def indexOfNames(tp: Type)(using Context): Map[String, Int] = {
    ...
    val ExprType(AndType(_, RefinedType(_, names, TypeBounds(lower, _)))) = tp.member(nme.get).info
    assert(names.toString == "Names")

    val reg = "Constant\\(([^\\)]*)".r

    reg.findAllMatchIn(lower.toString)
      .map(_.group(1))
      .zipWithIndex
      .toMap
  }
      ...
      case mt: MethodType if mt.paramInfos.length == 1 =>
        ...
        if (bunchedArgs != Nil && argTypes != Nil) {
          ...
          val positionOfName: Map[String, Int] =
              // TODO: Prefere explicitly declared names, check that type member 'Names' exsists
