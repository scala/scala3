...
trait TypeAssigner {
  ...
  def avoidingType(expr: Tree, bindings: Vector[Tree])(using Context): Type =
  ...
  def selectionType(tree: untpd.RefTree, qual1: Tree)(using Context): Type =
    ...
    val p = nme.primitive
    name match
      case p.arrayApply  => MethodType(defn.IntType +: Vector(), arrayElemType)
      case p.arrayUpdate => MethodType(defn.IntType +: arrayElemType +: Vector(), defn.UnitType)
      case p.arrayLength => MethodType(Vector(), defn.IntType)
  ...
  def superType(qualType: Type, mix: untpd.Ident, mixinClass: Symbol, pos: SrcPos)(using Context) =
    qualType match
      ...
      case qtype @ ThisType(_) =>
        val cls = qtype.cls
        def findMixinSuper(site: Type): Type = (site.parents filter (_.typeSymbol.name == mix.name): @unchecked) match {
          case p +: Vector() =>
            p.typeConstructor
          case Vector() =>
   ...
   */
  private def safeSubstParams(tp: Type, params: Vector[ParamRef],
      args: Vector[Tree], skolems: SkolemBuffer)(using Context): Type = (args: @unchecked) match
    case arg +: args1 =>
  ...
  def assignType(tree: untpd.Apply, fn: Tree, args: Vector[Tree])(using Context): Apply = {
  ...
  def assignType(tree: untpd.TypeApply, fn: Tree, args: Vector[Tree])(using Context): TypeApply = {
      ...
      case pt: TypeLambda =>
        tree.withType {
          ...
          if (hasNamedArg(args)) {
            ...
            val normArgs: Vector[Type] = paramNames.zipWithIndex.map { case (pname, idx) =>
  ...
  def assignType(tree: untpd.Block, stats: Vector[Tree], expr: Tree)(using Context): Block =
    tree.withType(avoidingType(expr, stats))

  def assignType(tree: untpd.Inlined, bindings: Vector[Tree], expansion: Tree)(using Context): Inlined =
  ...
  def assignType(tree: untpd.Match, scrutinee: Tree, cases: Vector[CaseDef])(using Context): Match =
  ...
  def assignType(tree: untpd.Try, expr: Tree, cases: Vector[CaseDef])(using Context): Try =
    if (cases.isEmpty) tree.withType(expr.tpe)
    else tree.withType(TypeComparer.lub(expr.tpe +: cases.tpes))

  def assignType(tree: untpd.SeqLiteral, elems: Vector[Tree], elemtpt: Tree)(using Context): SeqLiteral =
   ...
   */
  def assignType(tree: untpd.RefinedTypeTree, parent: Tree, refinements: Vector[Tree], refineCls: ClassSymbol)(using Context): RefinedTypeTree = {
  ...
  def assignType(tree: untpd.AppliedTypeTree, tycon: Tree, args: Vector[Tree])(using Context): AppliedTypeTree = {
  ...
  def assignType(tree: untpd.LambdaTypeTree, tparamDefs: Vector[TypeDef], body: Tree)(using Context): LambdaTypeTree =
  ...
  def assignType(tree: untpd.MatchTypeTree, bound: Tree, scrutinee: Tree, cases: Vector[CaseDef])(using Context): MatchTypeTree = {
  ...
  def assignType(tree: untpd.Alternative, trees: Vector[Tree])(using Context): Alternative =
