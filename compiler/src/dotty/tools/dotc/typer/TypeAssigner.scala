...
trait TypeAssigner {
   ...
   */
  private def safeSubstParams(tp: Type, params: Lst[ParamRef],
      args: Lst[Tree], skolems: SkolemBuffer)(using Context): Type =
    (0 until args.length).foldLeft(tp): (tp, i) =>
      safeSubstParam(tp, params(i), args(i).tpe, sk => skolems += ((args(i), sk)))

  def assignType(tree: untpd.Apply, fn: Tree, args: Lst[Tree])(using Context): Apply = {
  ...
  def assignType(tree: untpd.TypeApply, fn: Tree, args: Lst[Tree])(using Context): TypeApply = {
  ...
  def assignType(tree: untpd.SeqLiteral, elems: Lst[Tree], elemtpt: Tree)(using Context): SeqLiteral =
