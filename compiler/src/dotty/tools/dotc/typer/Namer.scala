...
class Namer { typer: Typer =>
  ...
  def wrapMethodType(restpe: Type, paramSymss: List[List[Symbol]], isJava: Boolean)(using Context): Type =
    instantiateDependent(restpe, paramSymss)
    methodType(paramSymss, restpe, isJava)
  ...
  def defDefSig(ddef: DefDef, sym: Symbol)(using Context): Type = {
    ...
    def wrapMethType(restpe: Type): Type =
      wrapMethodType(restpe, paramSymss, ddef.mods.is(JavaDefined))
    if (isConstructor) {
