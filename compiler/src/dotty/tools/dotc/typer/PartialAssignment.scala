 ...
 */
private[typer] final class PartialAssignment[+T <: PartialAssignment.LValue](val lhs: T)(
...
private[typer] object PartialAssignment:
   ...
   */
  final class PossiblyHoistedValue private (representation: tpd.Tree):
  ...
  /** A simple expression, typically valid on left-hand side of an `Assign` tree.
    ...
    */
  final case class SimpleLValue(expression: tpd.Tree) extends LValue:
    def locals: List[tpd.ValDef] =
      List()
    def genAssignment(rhs: untpd.Tree)(using Context): untpd.Tree =
