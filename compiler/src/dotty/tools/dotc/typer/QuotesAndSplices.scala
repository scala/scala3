...
trait QuotesAndSplices {
  ...
  def typedQuote(tree: untpd.Quote, pt: Type)(using Context): Tree = {
    ...
    val tree1 =
      if ctx.mode.is(Mode.Pattern) then
        ...
        typedTypeApply(untpd.TypeApply(untpd.ref(defn.QuotedTypeModule_apply.termRef), Lst(tree.quoted)), pt)(using quoteContext).select(nme.apply).appliedTo(qctx)
      else
  ...
  private def splitQuotePattern(quoted: Tree)(using Context): (Map[Symbol, Bind], Tree, List[Tree]) = {
    ...
    object splitter extends tpd.TreeMap {
      ...
      override def transform(tree: Tree)(using Context) = tree match {
        ...
        case Apply(fn, Lst(pat)) if fn.symbol.isExprSplice =>
  ...
  private def typedQuotePattern(tree: untpd.Quote, pt: Type, qctx: Tree)(using Context): Tree = {
    ...
    val splicePat =
      ...
      else typed(untpd.Tuple(splices.toLst.map(x => untpd.TypedSplice(replaceBindingsInTree.transform(x)))).withSpan(quoted.span), patType)
