...
object Texts {
  ...
  sealed abstract class Text {
    ...
    def remaining(width: Int): Int = (this: @unchecked) match
      case Str(s) =>
    ...
    def ~ (that: Text): Text =
      if this.isEmpty then that
        ...
        case _             => that match
          ...
          case _                                => Fluid(that +: this +: Vector())

    def ~~ (that: Text): Text =
      if this.isEmpty then that
      else if that.isEmpty then this
      else Fluid(that +: Str(" ") +: this +: Vector())

    def over (that: Text): Vertical =
      if this.isVertical then Vertical(that +: this.relems)
      else Vertical(that +: this +: Vector())
  }
  ...
  object Text {
    ...
    def lines(xs: Iterable[Text]): Vertical = Vertical(xs.toVector.reverse)
  ...
  case class Str(s: String) extends Text:
    override def relems: Vector[Text] = Vector(this)
    override def toString = s"Str($s)"

  case class Vertical(relems: Vector[Text]) extends Text
  case class Fluid(relems: Vector[Text]) extends Text

  class Closed(relems: Vector[Text]) extends Fluid(relems):
