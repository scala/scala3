...
import util.{Attachment, DotClass, Property, Stats}
...
object Trees {
  ...
  abstract class Tree[-T >: Untyped] extends Positioned
    ...
    override def clone: Tree[T] = {
      ...
      AllocationStats.registerAllocation(tree)(null)
      tree
    }
  }
  ...
  case class Thicket[-T >: Untyped](trees: List[Tree[T]])
    ...
    override def withPos(pos: Position): this.type = {
      ...
      if (trees eq newTrees)
        ...
        AllocationStats.registerAllocation(new Thicket[T](newTrees))(null).asInstanceOf[this.type]
    }
  ...
  @sharable val theEmptyValDef = AllocationStats.registerAllocation(new EmptyValDef[Type])(null)
  ...
  abstract class Instance[T >: Untyped <: Type] extends DotClass { inst =>
    ...
    def Thicket(trees: List[Tree])(implicit ctx: Context): Thicket = AllocationStats.registerAllocation(new Thicket(trees))
    ...
    abstract class TreeCopier {
      ...
      def Ident(tree: Tree)(name: Name)(implicit ctx: Context): Ident = tree match {
        case tree: BackquotedIdent =>
          ...
          else finalize(tree, AllocationStats.registerAllocation(new BackquotedIdent(name)))
      ...
      def Select(tree: Tree)(qualifier: Tree, name: Name)(implicit ctx: Context): Select = tree match {
        case tree: SelectWithSig =>
          ...
          else finalize(tree, AllocationStats.registerAllocation(new SelectWithSig(qualifier, name, tree.sig)))
      ...
      def SeqLiteral(tree: Tree)(elems: List[Tree], elemtpt: Tree)(implicit ctx: Context): SeqLiteral = tree match {
        case tree: JavaSeqLiteral =>
          ...
          else finalize(tree, AllocationStats.registerAllocation(new JavaSeqLiteral(elems, elemtpt)))
