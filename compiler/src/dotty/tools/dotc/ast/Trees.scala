...
object Trees {
  ...
  extends Positioned, SrcPos, Product, Attachment.Container, printing.Showable {
     ...
     */
    private[dotc] def setType(tpe: T): Unit =
      myTpe = tpe
  ...
  case class Select[-T >: Untyped] private[ast] (
      private var _qualifier: Tree[T],
      name: Name)(implicit @constructorOnly src: SourceFile)
    extends RefTree[T] {
    type ThisTree[-T >: Untyped] = Select[T]
    def qualifier: Tree[T] = _qualifier
    private[ast] def update(qualifier: Tree[T] @uncheckedVariance): Unit =
      _qualifier = qualifier
  }

  class SelectWithSig[-T >: Untyped] private[ast] (
      _qualifier: Tree[T], _name: Name, val sig: Signature)(implicit @constructorOnly src: SourceFile)
    extends Select[T](_qualifier, _name) {
    override def toString: String = s"SelectWithSig($qualifier, $name, $sig)"
  }

  /** qual.this */
  case class This[-T >: Untyped] private[ast] (
      qual: untpd.Ident)(implicit @constructorOnly src: SourceFile)
  ...
  case class Super[-T >: Untyped] private[ast] (
      private var _qual: Tree[T],
      mix: untpd.Ident)(implicit @constructorOnly src: SourceFile)
    extends ProxyTree[T] with TermTree[T] {
    type ThisTree[-T >: Untyped] = Super[T]
    def qual: Tree[T] = _qual
    private[ast] def update(qual: Tree[T] @uncheckedVariance): Unit =
      _qual = qual
  ...
  case class Apply[-T >: Untyped] private[ast] (
      private var _fun: Tree[T],
      private var _args: List[Tree[T]])(implicit @constructorOnly src: SourceFile)
    extends GenericApply[T] {
    type ThisTree[-T >: Untyped] = Apply[T]
    def fun: Tree[T] = _fun
    def args: List[Tree[T]] = _args
    private[ast] def update(fun: Tree[T] @uncheckedVariance, args: List[Tree[T]] @uncheckedVariance): Unit =
      _fun = fun
      _args = args
  ...
  case class Literal[-T >: Untyped] private[ast] (
      const: Constant)(implicit @constructorOnly src: SourceFile)
    extends Tree[T] with TermTree[T] {
    type ThisTree[-T >: Untyped] = Literal[T]
  }

  /** new tpt, but no constructor call */
  case class New[-T >: Untyped] private[ast] (
      private var _tpt: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends Tree[T] with TermTree[T] {
    type ThisTree[-T >: Untyped] = New[T]
    def tpt = _tpt
    private[ast] def update(tpt: Tree[T] @uncheckedVariance): Unit =
      _tpt = tpt
  }

  /** expr : tpt */
  case class Typed[-T >: Untyped] private[ast] (
      private var _expr: Tree[T],
      private var _tpt: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends ProxyTree[T] with TermTree[T] {
    type ThisTree[-T >: Untyped] = Typed[T]
    def expr: Tree[T] = _expr
    def tpt: Tree[T] = _tpt
    private[ast] def update(expr: Tree[T] @uncheckedVariance, tpt: Tree[T] @uncheckedVariance): Unit =
      _expr = expr
      _tpt = tpt
  ...
  case class NamedArg[-T >: Untyped] private[ast] (
      name: Name,
      private var _arg: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends Tree[T] {
    type ThisTree[-T >: Untyped] = NamedArg[T]
    def arg: Tree[T] = _arg
    private[ast] def update(arg: Tree[T] @uncheckedVariance): Unit =
      _arg = arg
  }

  /** name = arg, outside a parameter list */
  case class Assign[-T >: Untyped] private[ast] (
      private var _lhs: Tree[T],
      private var _rhs: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TermTree[T] {
    type ThisTree[-T >: Untyped] = Assign[T]
    def lhs: Tree[T] = _lhs
    def rhs: Tree[T] = _rhs
    private[ast] def update(lhs: Tree[T] @uncheckedVariance, rhs: Tree[T] @uncheckedVariance): Unit =
      _lhs = lhs
      _rhs = rhs
  }
  ...
  case class Block[-T >: Untyped] private[ast] (
      private var _stats: List[Tree[T]],
      private var _expr: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends Tree[T] {
    type ThisTree[-T >: Untyped] = Block[T]
    def stats: List[Tree[T]] = _stats
    def expr: Tree[T] = _expr
    private[ast] def update(stats: List[Tree[T]] @uncheckedVariance, expr: Tree[T] @uncheckedVariance): Unit =
      _stats = stats
      _expr = expr
  ...
  case class If[-T >: Untyped] private[ast] (
      private var _cond: Tree[T],
      private var _thenp: Tree[T],
      private var _elsep: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TermTree[T] {
    type ThisTree[-T >: Untyped] = If[T]
    def cond: Tree[T] = _cond
    def thenp: Tree[T] = _thenp
    def elsep: Tree[T] = _elsep
    private[ast] def update(cond: Tree[T] @uncheckedVariance, thenp: Tree[T] @uncheckedVariance, elsep: Tree[T] @uncheckedVariance): Unit =
      _cond = cond
      _thenp = thenp
      _elsep = elsep
    def isInline = false
  }
  class InlineIf[-T >: Untyped] private[ast] (_cond: Tree[T], _thenp: Tree[T], _elsep: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends If(_cond, _thenp, _elsep) {
  ...
  case class Closure[-T >: Untyped] private[ast] (
      private var _env: List[Tree[T]],
      private var _meth: Tree[T],
      private var _tpt: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TermTree[T] {
    type ThisTree[-T >: Untyped] = Closure[T]
    def env: List[Tree[T]] = _env
    def meth: Tree[T] = _meth
    def tpt: Tree[T] = _tpt
    private[ast] def update(env: List[Tree[T]] @uncheckedVariance, meth: Tree[T] @uncheckedVariance, tpt: Tree[T] @uncheckedVariance): Unit =
      _env = env
      _meth = meth
      _tpt = tpt
  }
  ...
  case class Match[-T >: Untyped] private[ast] (
      private var _selector: Tree[T],
      private var _cases: List[CaseDef[T]])(implicit @constructorOnly src: SourceFile)
    extends TermTree[T] {
    type ThisTree[-T >: Untyped] = Match[T]
    def selector: Tree[T] = _selector
    def cases: List[CaseDef[T]] = _cases
    private[ast] def update(selecor: Tree[T] @uncheckedVariance, cases: List[CaseDef[T]] @uncheckedVariance): Unit =
      _selector = selector
      _cases = cases
    def isInline = false
  }
  class InlineMatch[-T >: Untyped] private[ast] (_selector: Tree[T], _cases: List[CaseDef[T]])(implicit @constructorOnly src: SourceFile)
    extends Match(_selector, _cases) {
    override def isInline = true
  ...
  case class CaseDef[-T >: Untyped] private[ast] (
      private var _pat: Tree[T],
      private var _guard: Tree[T],
      private var _body: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends Tree[T] {
    type ThisTree[-T >: Untyped] = CaseDef[T]
    def pat: Tree[T] = _pat
    def guard: Tree[T] = _guard
    def body: Tree[T] = _body
    private[ast] def update(pat: Tree[T] @uncheckedVariance, guard: Tree[T] @uncheckedVariance, body: Tree[T] @uncheckedVariance): Unit =
      _pat = pat
      _guard = guard
      _body = body
  }
  ...
  /** label[tpt]: { expr } */
  case class Labeled[-T >: Untyped] private[ast] (
      private var _bind: Bind[T],
      private var _expr: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends NameTree[T] {
    type ThisTree[-T >: Untyped] = Labeled[T]
    def bind: Bind[T] = _bind
    def expr: Tree[T] = _expr
    private[ast] def update(bind: Bind[T] @uncheckedVariance, expr: Tree[T] @uncheckedVariance): Unit =
      _bind = bind
      _expr = expr
  ...
  case class Return[-T >: Untyped] private[ast] (
      private var _expr: Tree[T],
      private var _from: Tree[T] = genericEmptyTree)(implicit @constructorOnly src: SourceFile)
    extends TermTree[T] {
    type ThisTree[-T >: Untyped] = Return[T]
    def expr: Tree[T] = _expr
    def from: Tree[T] = _from
    private[ast] def update(expr: Tree[T] @uncheckedVariance, from: Tree[T] @uncheckedVariance): Unit =
      _expr = expr
      _from = from
  }

  /** while (cond) { body } */
  case class WhileDo[-T >: Untyped] private[ast] (
      private var _cond: Tree[T],
      private var _body: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TermTree[T] {
    ...
    def cond: Tree[T] = _cond
    def body: Tree[T] = _body
    private[ast] def update(cond: Tree[T] @uncheckedVariance, body: Tree[T] @uncheckedVariance): Unit =
      _cond = cond
      _body = body
  }
  ...
  case class Try[-T >: Untyped] private[ast] (
      private var _expr: Tree[T],
      private var _cases: List[CaseDef[T]],
      private var _finalizer: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TermTree[T] {
    type ThisTree[-T >: Untyped] = Try[T]
    def expr: Tree[T] = _expr
    def cases: List[CaseDef[T]] = _cases
    def finalizer: Tree[T] = _finalizer
    private[ast] def update(expr: Tree[T] @uncheckedVariance, cases: List[CaseDef[T]] @uncheckedVariance, finalizer: Tree[T] @uncheckedVariance): Unit =
      _expr = expr
      _cases = cases
  ...
  case class SeqLiteral[-T >: Untyped] private[ast] (
      private var _elems: List[Tree[T]],
      private var _elemtpt: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends Tree[T] {
    type ThisTree[-T >: Untyped] = SeqLiteral[T]
    def elems: List[Tree[T]] = _elems
    def elemtpt: Tree[T] = _elemtpt
    private[ast] def update(elems: List[Tree[T]] @uncheckedVariance, elemtpt: Tree[T] @uncheckedVariance): Unit =
      _elems = elems
      _elemtpt = elemtpt
  }

  /** Array(elems) */
  class JavaSeqLiteral[-T >: Untyped] private[ast] (_elems: List[Tree[T]], _elemtpt: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends SeqLiteral(_elems, _elemtpt) {
  ...
  case class Inlined[-T >: Untyped] private[ast] (
      private var _call: tpd.Tree,
      private var _bindings: List[MemberDef[T]],
      private var _expansion: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends Tree[T] {
    type ThisTree[-T >: Untyped] = Inlined[T]
    def call: tpd.Tree = _call
    def bindings: List[MemberDef[T]] = _bindings
    def expansion: Tree[T] = _expansion
    private[ast] def update(call: tpd.Tree, bindings: List[MemberDef[T]] @uncheckedVariance, expansion: Tree[T] @uncheckedVariance): Unit =
      _call = call
      _bindings = bindings
  ...
  /** ref.type */
  case class SingletonTypeTree[-T >: Untyped] private[ast] (
      private var _ref: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends DenotingTree[T] with TypTree[T] {
    type ThisTree[-T >: Untyped] = SingletonTypeTree[T]
    def ref: Tree[T] = _ref
    private[ast] def update(ref: Tree[T] @uncheckedVariance): Unit =
      _ref = ref
  }

  /** tpt { refinements } */
  case class RefinedTypeTree[-T >: Untyped] private[ast] (
      private var _tpt: Tree[T],
      private var _refinements: List[Tree[T]])(implicit @constructorOnly src: SourceFile)
    extends ProxyTree[T] with TypTree[T] {
    type ThisTree[-T >: Untyped] = RefinedTypeTree[T]
    def tpt: Tree[T] = _tpt
    def refinements: List[Tree[T]] = _refinements
    private[ast] def update(tpt: Tree[T] @uncheckedVariance, refinements: List[Tree[T]] @uncheckedVariance): Unit =
      _tpt = tpt
  ...
  case class AppliedTypeTree[-T >: Untyped] private[ast] (
    ...
    def tpt: Tree[T] = _tpt
    def args: List[Tree[T]] = _args
    private[ast] def update(tpt: Tree[T] @uncheckedVariance, args: List[Tree[T]] @uncheckedVariance): Unit =
      _tpt = tpt
      _args = args
    def forwardTo: Tree[T] = tpt
  }
  ...
  case class LambdaTypeTree[-T >: Untyped] private[ast] (
      private var _tparams: List[TypeDef[T]],
      private var _body: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TypTree[T] {
    type ThisTree[-T >: Untyped] = LambdaTypeTree[T]
    def tparams: List[TypeDef[T]] = _tparams
    def body: Tree[T] = _body
    private[ast] def update(tparams: List[TypeDef[T]] @uncheckedVariance, body: Tree[T] @uncheckedVariance): Unit =
      _tparams = tparams
      _body = body
  }

  case class TermLambdaTypeTree[-T >: Untyped] private[ast] (
      private var _params: List[ValDef[T]],
      private var _body: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TypTree[T] {
    type ThisTree[-T >: Untyped] = TermLambdaTypeTree[T]
    def params: List[ValDef[T]] = _params
    def body: Tree[T] = _body
    private[ast] def update(params: List[ValDef[T]] @uncheckedVariance, body: Tree[T] @uncheckedVariance): Unit =
      _params = params
      _body = body
  }
  ...
  case class MatchTypeTree[-T >: Untyped] private[ast] (
      private var _bound: Tree[T],
      private var _selector: Tree[T],
      private var _cases: List[CaseDef[T]])(implicit @constructorOnly src: SourceFile)
    extends TypTree[T] {
    type ThisTree[-T >: Untyped] = MatchTypeTree[T]
    def bound: Tree[T] = _bound
    def selector: Tree[T] = _selector
    def cases: List[CaseDef[T]] = _cases
    private[ast] def update(bound: Tree[T] @uncheckedVariance, selector: Tree[T] @uncheckedVariance, cases: List[CaseDef[T]] @uncheckedVariance): Unit =
      _bound = bound
      _selector = selector
      _cases = cases
  }
  ...
  /** => T */
  case class ByNameTypeTree[-T >: Untyped] private[ast] (
      private var _result: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TypTree[T] {
    type ThisTree[-T >: Untyped] = ByNameTypeTree[T]
    def result: Tree[T] = _result
    private[ast] def update(result: Tree[T] @uncheckedVariance): Unit =
      _result = result
  }
  ...
  case class TypeBoundsTree[-T >: Untyped] private[ast] (
      private var _lo: Tree[T],
      private var _hi: Tree[T],
      private var _alias: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends TypTree[T] {
    type ThisTree[-T >: Untyped] = TypeBoundsTree[T]
    def lo: Tree[T] = _lo
    def hi: Tree[T] = _hi
    def alias: Tree[T] = _alias
    private[ast] def update(lo: Tree[T] @uncheckedVariance, hi: Tree[T] @uncheckedVariance, alias: Tree[T] @uncheckedVariance): Unit =
      _lo = lo
      _hi = hi
      _alias = alias
  }
  ...
  /** name @ body */
  case class Bind[-T >: Untyped] private[ast] (name: Name,
      private var _body: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends NamedDefTree[T] with PatternTree[T] {
    type ThisTree[-T >: Untyped] = Bind[T]
    def body: Tree[T] = _body
    private[ast] def update(body: Tree[T] @uncheckedVariance): Unit =
      _body = body
  ...
  /** tree_1 | ... | tree_n */
  case class Alternative[-T >: Untyped] private[ast] (
      private var _trees: List[Tree[T]])(implicit @constructorOnly src: SourceFile)
    extends PatternTree[T] {
    type ThisTree[-T >: Untyped] = Alternative[T]
    def trees: List[Tree[T]] = _trees
    private[ast] def update(trees: List[Tree[T]] @uncheckedVariance): Unit =
      _trees = trees
  }
  ...
  case class UnApply[-T >: Untyped] private[ast] (
      private var _fun: Tree[T],
      private var _implicits: List[Tree[T]],
      private var _patterns: List[Tree[T]])(implicit @constructorOnly src: SourceFile)
    extends ProxyTree[T] with PatternTree[T] {
    type ThisTree[-T >: Untyped] = UnApply[T]
    def fun: Tree[T] = _fun
    def implicits: List[Tree[T]] = _implicits
    def patterns: List[Tree[T]] = _patterns
    private[ast] def update(fun: Tree[T] @uncheckedVariance, implicits: List[Tree[T]] @uncheckedVariance, patterns: List[Tree[T]] @uncheckedVariance): Unit =
      _fun = fun
  ...
  case class ValDef[-T >: Untyped] private[ast] (name: TermName,
      private var _tpt: Tree[T],
      private var _rhs: LazyTree[T @uncheckedVariance])(implicit @constructorOnly src: SourceFile)
    extends ValOrDefDef[T] {
    type ThisTree[-T >: Untyped] = ValDef[T]
    def tpt: Tree[T] = _tpt
    private[ast] def update(tpt: Tree[T] @uncheckedVariance, rhs: LazyTree[T] @uncheckedVariance): Unit =
      _tpt = tpt
      _rhs = rhs
    assert(isEmpty || tpt != genericEmptyTree)

    def unforced: LazyTree[T] = _rhs
    protected def force(x: Tree[T @uncheckedVariance]): Unit = _rhs = x
  }
  ...
  case class DefDef[-T >: Untyped] private[ast] (
      name: TermName,
      private var _tparams: List[TypeDef[T]],
      private var _vparamss: List[List[ValDef[T]]],
      private var _tpt: Tree[T],
      private var _rhs: LazyTree[T @uncheckedVariance])(implicit @constructorOnly src: SourceFile)
    extends ValOrDefDef[T] {
    type ThisTree[-T >: Untyped] = DefDef[T]
    def tparams: List[TypeDef[T]] = _tparams
    def vparamss: List[List[ValDef[T]]] = _vparamss
    def tpt: Tree[T] = _tpt
    private[ast] def update(
        tparams: List[TypeDef[T]] @uncheckedVariance,
        vparamss: List[List[ValDef[T]]] @uncheckedVariance,
        tpt: Tree[T] @uncheckedVariance,
        rhs: LazyTree[T] @uncheckedVariance): Unit =
      _tparams = tparams
      _vparamss = vparamss
      _tpt = tpt
      _rhs = rhs
   ...
   */
  case class TypeDef[-T >: Untyped] private[ast] (
      name: TypeName,
      private var _rhs: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends MemberDef[T] {
    type ThisTree[-T >: Untyped] = TypeDef[T]
    def rhs: Tree[T] = _rhs
    private[ast] def update(rhs: Tree[T] @uncheckedVariance): Unit =
      _rhs = rhs
  ...
  case class Template[-T >: Untyped] private[ast] (
      private var _constr: DefDef[T],
      private var _parentsOrDerived: List[Tree[T]],
      private var _self: ValDef[T],
      private var _body: LazyTreeList[T @uncheckedVariance])(implicit @constructorOnly src: SourceFile)
    extends DefTree[T] with WithLazyField[List[Tree[T]]] {
    type ThisTree[-T >: Untyped] = Template[T]
    def constr: DefDef[T] = _constr
    def parentsOrDerived: List[Tree[T]] = _parentsOrDerived
    def self: ValDef[T] = _self
    private[ast] def update(
        constr: DefDef[T] @uncheckedVariance,
        parentsOrDerived: List[Tree[T]] @uncheckedVariance,
        self: ValDef[T] @uncheckedVariance,
        body: LazyTreeList[T @uncheckedVariance]): Unit =
      _constr = constr
      _parentsOrDerived = parentsOrDerived
      _self = self
      _body = body

    def unforcedBody: LazyTreeList[T] = unforced
    def unforced: LazyTreeList[T] = _body
    protected def force(x: List[Tree[T @uncheckedVariance]]): Unit = _body = x
  ...
  case class Import[-T >: Untyped] private[ast] (
      private var _expr: Tree[T],
      private var _selectors: List[untpd.ImportSelector])(implicit @constructorOnly src: SourceFile)
    extends DenotingTree[T] {
    type ThisTree[-T >: Untyped] = Import[T]
    def expr: Tree[T] = _expr
    def selectors: List[untpd.ImportSelector] = _selectors
    private[ast] def update(expr: Tree[T] @uncheckedVariance, selectors: List[untpd.ImportSelector]): Unit =
      _expr = expr
  ...
  case class PackageDef[-T >: Untyped] private[ast] (
      private var _pid: RefTree[T],
      private var _stats: List[Tree[T]])(implicit @constructorOnly src: SourceFile)
    extends ProxyTree[T] {
    type ThisTree[-T >: Untyped] = PackageDef[T]
    def pid: RefTree[T] = _pid
    def stats: List[Tree[T]] = _stats
    private[ast] def update(pid: RefTree[T] @uncheckedVariance, stats: List[Tree[T]] @uncheckedVariance): Unit =
      _pid = pid
      _stats = stats
  ...
  case class Annotated[-T >: Untyped] private[ast] (
      private var _arg: Tree[T],
      private var _annot: Tree[T])(implicit @constructorOnly src: SourceFile)
    extends ProxyTree[T] {
    type ThisTree[-T >: Untyped] = Annotated[T]
    def arg: Tree[T] = _arg
    def annot: Tree[T] = _annot
    private[ast] def update(arg: Tree[T] @uncheckedVariance, annot: Tree[T] @uncheckedVariance): Unit =
      _arg = arg
      _annot = annot
  ...
  abstract class Instance[T >: Untyped <: Type] { inst =>
    ...
    abstract class TreeCopier {
      ...
      protected def keep(tree: Select)(qualifier: Tree): Boolean =
        qualifier eq tree.qualifier
      protected def keep(tree: Super)(qual: Tree): Boolean =
        qual eq tree.qual
      protected def keep(tree: Apply)(fun: Tree, args: List[Tree]): Boolean =
        (fun eq tree.fun) && (args eq tree.args)
      protected def keep(tree: TypeApply)(fun: Tree, args: List[Tree]): Boolean =
        (fun eq tree.fun) && (args eq tree.args)
      protected def keep(tree: New)(tpt: Tree): Boolean =
        tpt eq tree.tpt
      protected def keep(tree: Typed)(expr: Tree, tpt: Tree): Boolean =
        (expr eq tree.expr) && (tpt eq tree.tpt)
      protected def keep(tree: NamedArg)(arg: Tree): Boolean =
        arg eq tree.arg
      protected def keep(tree: Assign)(lhs: Tree, rhs: Tree): Boolean =
        (lhs eq tree.lhs) && (rhs eq tree.rhs)
      protected def keep(tree: Block)(stats: List[Tree], expr: Tree): Boolean =
        (stats eq tree.stats) && (expr eq tree.expr)
      protected def keep(tree: If)(cond: Tree, thenp: Tree, elsep: Tree): Boolean =
        (cond eq tree.cond) && (thenp eq tree.thenp) && (elsep eq tree.elsep)
      protected def keep(tree: Closure)(env: List[Tree], meth: Tree, tpt: Tree): Boolean =
        (env eq tree.env) && (meth eq tree.meth) && (tpt eq tree.tpt)
      protected def keep(tree: Match)(selector: Tree, cases: List[CaseDef]): Boolean =
        (selector eq tree.selector) && (cases eq tree.cases)
      protected def keep(tree: CaseDef)(pat: Tree, guard: Tree, body: Tree): Boolean =
        (pat eq tree.pat) && (guard eq tree.guard) && (body eq tree.body)
      protected def keep(tree: Labeled)(bind: Bind, expr: Tree): Boolean =
        (bind eq tree.bind) && (expr eq tree.expr)
      protected def keep(tree: Return)(expr: Tree, from: Tree): Boolean =
        (expr eq tree.expr) && (from eq tree.from)
      protected def keep(tree: WhileDo)(cond: Tree, body: Tree): Boolean =
        (cond eq tree.cond) && (body eq tree.body)
      protected def keep(tree: Try)(expr: Tree, cases: List[CaseDef], finalizer: Tree): Boolean =
        (expr eq tree.expr) && (cases eq tree.cases) && (finalizer eq tree.finalizer)
      protected def keep(tree: SeqLiteral)(elems: List[Tree], elemtpt: Tree): Boolean =
        (elems eq tree.elems) && (elemtpt eq tree.elemtpt)
      protected def keep(tree: Inlined)(call: tpd.Tree, bindings: List[MemberDef], expansion: Tree): Boolean =
        (call eq tree.call) && (bindings eq tree.bindings) && (expansion eq tree.expansion)
      protected def keep(tree: SingletonTypeTree)(ref: Tree): Boolean =
        ref eq tree.ref
      protected def keep(tree: RefinedTypeTree)(tpt: Tree, refinements: List[Tree]): Boolean =
        (tpt eq tree.tpt) && (refinements eq tree.refinements)
      protected def keep(tree: AppliedTypeTree)(tpt: Tree, args: List[Tree]): Boolean =
        (tpt eq tree.tpt) && (args eq tree.args)
      protected def keep(tree: LambdaTypeTree)(tparams: List[TypeDef], body: Tree): Boolean =
        (tparams eq tree.tparams) && (body eq tree.body)
      protected def keep(tree: TermLambdaTypeTree)(params: List[ValDef], body: Tree): Boolean =
        (params eq tree.params) && (body eq tree.body)
      protected def keep(tree: MatchTypeTree)(bound: Tree, selector: Tree, cases: List[CaseDef]): Boolean =
        (bound eq tree.bound) && (selector eq tree.selector) && (cases eq tree.cases)
      protected def keep(tree: ByNameTypeTree)(result: Tree): Boolean =
        result eq tree.result
      protected def keep(tree: TypeBoundsTree)(lo: Tree, hi: Tree, alias: Tree): Boolean =
        (lo eq tree.lo) && (hi eq tree.hi) && (alias eq tree.alias)
      protected def keep(tree: Bind)(body: Tree): Boolean =
        body eq tree.body
      protected def keep(tree: Alternative)(trees: List[Tree]): Boolean =
        trees eq tree.trees
      protected def keep(tree: UnApply)(fun: Tree, implicits: List[Tree], patterns: List[Tree]): Boolean =
        (fun eq tree.fun) && (implicits eq tree.implicits) && (patterns eq tree.patterns)
      protected def keep(tree: ValDef)(tpt: Tree, rhs: LazyTree): Boolean =
        (tpt eq tree.tpt) && (rhs eq tree.unforcedRhs)
      protected def keep(tree: DefDef)(tparams: List[TypeDef], vparamss: List[List[ValDef]], tpt: Tree, rhs: LazyTree): Boolean =
        (tparams eq tree.tparams) && (vparamss eq tree.vparamss) && (tpt eq tree.tpt) && (rhs eq tree.unforcedRhs)
      protected def keep(tree: TypeDef)(rhs: Tree): Boolean =
        rhs eq tree.rhs
      protected def keep(tree: Template)(constr: DefDef, parents: List[Tree], derived: List[untpd.Tree], self: ValDef, body: LazyTreeList): Boolean =
        (constr eq tree.constr) && (parents eq tree.parents) && (derived eq tree.derived) && (self eq tree.self) && (body eq tree.unforcedBody)
      protected def keep(tree: Import)(expr: Tree, selectors: List[untpd.ImportSelector]): Boolean =
        (expr eq tree.expr) && (selectors eq tree.selectors)
      protected def keep(tree: PackageDef)(pid: RefTree, stats: List[Tree]): Boolean =
        (pid eq tree.pid) && (stats eq tree.stats)
      protected def keep(tree: Annotated)(arg: Tree, annot: Tree): Boolean =
        (arg eq tree.arg) && (annot eq tree.annot)
      protected def keep(tree: Thicket)(trees: List[Tree]): Boolean =
        trees eq tree.trees
      ...
      def Select(tree: Tree)(qualifier: Tree, name: Name)(using Context): Select = tree match {
        case tree: SelectWithSig =>
          if keep(tree)(qualifier) && name == tree.name then tree
          else finalize(tree, SelectWithSig(qualifier, name, tree.sig)(sourceFile(tree)))
        case tree: Select if keep(tree)(qualifier) && name == tree.name => tree
      ...
      def Super(tree: Tree)(qual: Tree, mix: untpd.Ident)(using Context): Super = tree match {
        case tree: Super if keep(tree)(qual) && (mix eq tree.mix) => tree
      ...
      def Apply(tree: Tree)(fun: Tree, args: List[Tree])(using Context): Apply = tree match {
        case tree: Apply if keep(tree)(fun, args) => tree
      ...
      def TypeApply(tree: Tree)(fun: Tree, args: List[Tree])(using Context): TypeApply = tree match {
        case tree: TypeApply if keep(tree)(fun, args) => tree
      ...
      def NamedArg(tree: Tree)(name: Name, arg: Tree)(using Context): NamedArg = tree match {
        case tree: NamedArg if name == tree.name && keep(tree)(arg) => tree
      ...
      def Assign(tree: Tree)(lhs: Tree, rhs: Tree)(using Context): Assign = tree match {
        case tree: Assign if keep(tree)(lhs, rhs) => tree
      ...
      def Block(tree: Tree)(stats: List[Tree], expr: Tree)(using Context): Block = tree match {
        case tree: Block if keep(tree)(stats, expr) => tree
      ...
      def If(tree: Tree)(cond: Tree, thenp: Tree, elsep: Tree)(using Context): If = tree match {
        case tree: If if keep(tree)(cond, thenp, elsep) => tree
      ...
      def Closure(tree: Tree)(env: List[Tree], meth: Tree, tpt: Tree)(using Context): Closure = tree match {
        case tree: Closure if keep(tree)(env, meth, tpt) => tree
      ...
      def Match(tree: Tree)(selector: Tree, cases: List[CaseDef])(using Context): Match = tree match {
        case tree: Match if keep(tree)(selector, cases) => tree
      ...
      def CaseDef(tree: Tree)(pat: Tree, guard: Tree, body: Tree)(using Context): CaseDef = tree match {
        case tree: CaseDef if keep(tree)(pat, guard, body) => tree
      ...
      def Labeled(tree: Tree)(bind: Bind, expr: Tree)(using Context): Labeled = tree match {
        case tree: Labeled if keep(tree)(bind, expr) => tree
      ...
      def Return(tree: Tree)(expr: Tree, from: Tree)(using Context): Return = tree match {
        case tree: Return if keep(tree)(expr, from) => tree
      ...
      def WhileDo(tree: Tree)(cond: Tree, body: Tree)(using Context): WhileDo = tree match {
        case tree: WhileDo if keep(tree)(cond, body) => tree
      ...
      def Try(tree: Tree)(expr: Tree, cases: List[CaseDef], finalizer: Tree)(using Context): Try = tree match {
        case tree: Try if keep(tree)(expr, cases, finalizer) => tree
      ...
      def SeqLiteral(tree: Tree)(elems: List[Tree], elemtpt: Tree)(using Context): SeqLiteral = tree match {
        case tree: JavaSeqLiteral =>
          if keep(tree)(elems, elemtpt) then tree
          else finalize(tree, untpd.JavaSeqLiteral(elems, elemtpt))
        case tree: SeqLiteral if keep(tree)(elems, elemtpt) => tree
      ...
      def Inlined(tree: Tree)(call: tpd.Tree, bindings: List[MemberDef], expansion: Tree)(using Context): Inlined = tree match {
        case tree: Inlined if keep(tree)(call, bindings, expansion) => tree
      ...
      def SingletonTypeTree(tree: Tree)(ref: Tree)(using Context): SingletonTypeTree = tree match {
        case tree: SingletonTypeTree if keep(tree)(ref) => tree
      ...
      def RefinedTypeTree(tree: Tree)(tpt: Tree, refinements: List[Tree])(using Context): RefinedTypeTree = tree match {
        case tree: RefinedTypeTree if keep(tree)(tpt, refinements) => tree
      ...
      def AppliedTypeTree(tree: Tree)(tpt: Tree, args: List[Tree])(using Context): AppliedTypeTree = tree match {
        case tree: AppliedTypeTree if keep(tree)(tpt, args) => tree
      ...
      def LambdaTypeTree(tree: Tree)(tparams: List[TypeDef], body: Tree)(using Context): LambdaTypeTree = tree match {
        case tree: LambdaTypeTree if keep(tree)(tparams, body) => tree
      ...
      def TermLambdaTypeTree(tree: Tree)(params: List[ValDef], body: Tree)(using Context): TermLambdaTypeTree = tree match {
        case tree: TermLambdaTypeTree if keep(tree)(params, body) => tree
      ...
      def MatchTypeTree(tree: Tree)(bound: Tree, selector: Tree, cases: List[CaseDef])(using Context): MatchTypeTree = tree match {
        case tree: MatchTypeTree if keep(tree)(bound, selector, cases) => tree
      ...
      def ByNameTypeTree(tree: Tree)(result: Tree)(using Context): ByNameTypeTree = tree match {
        case tree: ByNameTypeTree if keep(tree)(result) => tree
      ...
      def TypeBoundsTree(tree: Tree)(lo: Tree, hi: Tree, alias: Tree)(using Context): TypeBoundsTree = tree match {
        case tree: TypeBoundsTree if keep(tree)(lo, hi, alias) => tree
      ...
      def Bind(tree: Tree)(name: Name, body: Tree)(using Context): Bind = tree match {
        case tree: Bind if name == tree.name && keep(tree)(body) => tree
      ...
      def Alternative(tree: Tree)(trees: List[Tree])(using Context): Alternative = tree match {
        case tree: Alternative if keep(tree)(trees) => tree
      ...
      def UnApply(tree: Tree)(fun: Tree, implicits: List[Tree], patterns: List[Tree])(using Context): UnApply = tree match {
        case tree: UnApply if keep(tree)(fun, implicits, patterns) => tree
      ...
      def ValDef(tree: Tree)(name: TermName, tpt: Tree, rhs: LazyTree)(using Context): ValDef = tree match {
        case tree: ValDef if name == tree.name && keep(tree)(tpt, rhs) => tree
      ...
      def DefDef(tree: Tree)(name: TermName, tparams: List[TypeDef], vparamss: List[List[ValDef]], tpt: Tree, rhs: LazyTree)(using Context): DefDef = tree match {
        case tree: DefDef if name == tree.name && keep(tree)(tparams, vparamss, tpt, rhs) => tree
      ...
      def TypeDef(tree: Tree)(name: TypeName, rhs: Tree)(using Context): TypeDef = tree match {
        case tree: TypeDef if name == tree.name && keep(tree)(rhs) => tree
      ...
      def Template(tree: Tree)(constr: DefDef, parents: List[Tree], derived: List[untpd.Tree], self: ValDef, body: LazyTreeList)(using Context): Template = tree match {
        case tree: Template if keep(tree)(constr, parents, derived, self, body) => tree
      ...
      def Import(tree: Tree)(expr: Tree, selectors: List[untpd.ImportSelector])(using Context): Import = tree match {
        case tree: Import if keep(tree)(expr, selectors) => tree
      ...
      def PackageDef(tree: Tree)(pid: RefTree, stats: List[Tree])(using Context): PackageDef = tree match {
        case tree: PackageDef if keep(tree)(pid, stats) => tree
      ...
      def Annotated(tree: Tree)(arg: Tree, annot: Tree)(using Context): Annotated = tree match {
        case tree: Annotated if keep(tree)(arg, annot) => tree
      ...
      def Thicket(tree: Tree)(trees: List[Tree])(using Context): Thicket = tree match {
        case tree: Thicket if keep(tree)(trees) => tree
