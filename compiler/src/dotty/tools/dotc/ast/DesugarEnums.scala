...
object DesugarEnums {
   ...
   *  companion object.
   */
  def enumClass(implicit ctx: ContextRenamed): Symbol = {
    val cls = ctx.owner
    if (cls.is(Module)) cls.linkedClass else cls
  }

  /** Is `tree` an (untyped) enum case? */
  def isEnumCase(tree: Tree)(implicit ctx: ContextRenamed): Boolean = tree match {
   ...
   */
  def interpolatedEnumParent(pos: Position)(implicit ctx: ContextRenamed): Tree = {
  ...
  def enumClassRef(implicit ctx: ContextRenamed): Tree =
    if (enumClass.exists) TypeTree(enumClass.typeRef) else TypeTree()

  /** Add implied flags to an enum class or an enum case */
  def addEnumFlags(cdef: TypeDef)(implicit ctx: ContextRenamed): TypeDef =
  ...
  private def registerCall(implicit ctx: ContextRenamed): List[Tree] =
   ...
   */
  private def enumScaffolding(implicit ctx: ContextRenamed): List[Tree] = {
   ...
   *   }
   */
  private def enumValueCreator(implicit ctx: ContextRenamed) = {
  ...
  def enumApplyResult(
      ...
      appliedEnumRef: Tree)(implicit ctx: ContextRenamed): (Tree, List[DefDef]) = {
   ...
   */
  def nextEnumTag(kind: CaseKind.Value)(implicit ctx: ContextRenamed): (Int, List[Tree]) = {
   ...
   */
  def enumTagMeth(kind: CaseKind.Value)(implicit ctx: ContextRenamed): (DefDef, List[Tree]) = {
    val (tag, scaffolding) = nextEnumTag(kind)
    (DefDef(nme.enumTag, Nil, Nil, TypeTree(), Literal(Constant(tag))), scaffolding)
  }

  /** Expand a module definition representing a parameterless enum case */
  def expandEnumModule(name: TermName, impl: Template, mods: Modifiers, pos: Position)(implicit ctx: ContextRenamed): Tree = {
  ...
  def expandSimpleEnumCase(name: TermName, mods: Modifiers, pos: Position)(implicit ctx: ContextRenamed): Tree =
    if (!enumClass.exists) EmptyTree
