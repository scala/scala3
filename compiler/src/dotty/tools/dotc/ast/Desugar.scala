...
// ----- Desugar methods -------------------------------------------------
  ...
  private def defDef(meth0: DefDef, isPrimaryConstructor: Boolean = false)(using Context): Tree = {
    ...
    def takeUpTo(vparamss: Lst[Lst[ValDef]], n: Int): Lst[Lst[ValDef]] = vparamss match {
    ...
    def normalizedVparamss = meth1.vparamss.nestedMap(vparam =>
      if vparam.rhs.isEmpty then vparam
      else cpy.ValDef(vparam)(rhs = EmptyTree).withMods(vparam.mods | HasDefault)
    )

    def defaultGetters(vparamss: Lst[Lst[ValDef]], n: Int): Lst[DefDef] = vparamss match {
      case NIL +: vparamss1 =>
  ...
  def classDef(cdef: TypeDef)(using Context): Tree = {
    ...
    val cdef1 = addEnumFlags {
      ...
      val vparamAccessors: Lst[Tree] = {
        val originalVparamsIt = originalVparamss.flatten.iterator
  ...
  def makeClosure(params: Lst[ValDef], body: Tree, tpt: Tree = null, isContextual: Boolean)(using Context): Block =
    Block(
      DefDef(nme.ANON_FUN, NIL, Lst(params), if (tpt == null) TypeTree() else tpt, body)
      ...
      case Function(vargs, res) =>
        ...
        if (ctx.mode.is(Mode.Type)) {
          ...
          RefinedTypeTree(polyFunctionTpt, Lst(
            DefDef(nme.apply, applyTParams, Lst(applyVParams), res, EmptyTree)
          ))
        }
        else {
          ...
          New(Template(emptyConstructor, List(polyFunctionTpt), Nil, EmptyValDef,
            Lst(DefDef(nme.apply, applyTParams, Lst(applyVParams), TypeTree(), res))
            ))
        }
      case _ =>
