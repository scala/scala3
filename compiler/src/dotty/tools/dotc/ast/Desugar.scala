        ...
        def apply(tp: Type) = tp match {
          case tp: NamedType if tp.symbol.exists && (tp.symbol.owner eq originalOwner) =>
            ...
            if (local.exists) (defctx.owner.thisType select local).dealiasKeepAnnots
              ...
              def msg =
                s"no matching symbol for ${tp.symbol.showLocated} in ${defctx.owner} / ${defctx.effectiveScope.toLst}"
...
// ----- Desugar methods -------------------------------------------------
  ...
  private def defDef(meth0: DefDef, isPrimaryConstructor: Boolean = false)(using Context): Tree = {
    ...
    val meth1 =
      rhs match
        ...
        case _ => addEvidenceParams(cpy.DefDef(meth)(name = methName, tparams = tparams1), epbuf.toLst)

    /** The longest prefix of parameter lists in vparamss whose total length does not exceed `n` */
    def takeUpTo(vparamss: List[Lst[ValDef]], n: Int): List[Lst[ValDef]] = vparamss match {
    ...
    def normalizedVparamss = meth1.vparamss.nestedMapConserveLst(vparam =>
      if vparam.rhs.isEmpty then vparam
      else cpy.ValDef(vparam)(rhs = EmptyTree).withMods(vparam.mods | HasDefault)
    )

    def defaultGetters(vparamss: List[Lst[ValDef]], n: Int): Lst[DefDef] = vparamss match {
      case NIL :: vparamss1 =>
      ...
      case (vparam +: vparams) :: vparamss1 =>
        def defaultGetter: DefDef =
          DefDef(
            ...
            vparamss = takeUpTo(normalizedVparamss.nestedMapLst(toDefParam(_, keepAnnotations = true, keepDefault = false)), n),
            tpt = TypeTree(),
            rhs = vparam.rhs
          )
  ...
  def classDef(cdef: TypeDef)(using Context): Tree = {
    ...
    lazy val derivedEnumParams = enumClass.typeParams.toLst.map(derivedTypeParamWithVariance)
    val impliedTparams =
      if (isEnumCase) {
    ...
    val constrVparamss =
      ...
      else originalVparamss.nestedMapLst(toDefParam(_, keepAnnotations = true, keepDefault = true))
    val derivedTparams =
      constrTparams.zipWith(impliedTparams)((tparam, impliedParam) =>
      ...
      if (isCaseClass) {
        ...
        val applyMeths =
          if (mods.is(Abstract)) Nil
            ...
            val appParamss =
              derivedVparamss.nestedZipWith(constrVparamss)((ap, cp) =>
    ...
    val cdef1 = addEnumFlags {
      ...
      val vparamAccessors: Lst[Tree] = {
        val originalVparamsIt = originalVparamss.flattenLst.iterator
        derivedVparamss match {
          case first :: rest =>
            ...
            rest.flattenLst.map(_.withMods(mods(originalVparamsIt.next())))
          case _ =>
            NIL
        }
      }
   ...
   *      Closure($anonfun)
   */
  def makeClosure(params: Lst[ValDef], body: Tree, tpt: Tree = null, isContextual: Boolean)(using Context): Block =
    Block(
  ...
  def apply(tree: Tree)(using Context): Tree = {
    ...
    def makePolyFunction(targs: Lst[Tree], body: Tree): Tree = body match {
      ...
      case Function(vargs, res) =>
        ...
        val applyTParams = targs.asInstanceOf[Lst[TypeDef]]
