...
object desugar {
   ...
   */
  def isRetractableCaseClassMethodName(name: Name)(implicit ctx: ContextRenamed): Boolean = name match {
  ...
  def isDesugaredCaseClassMethodName(name: Name)(implicit ctx: ContextRenamed): Boolean =
    isRetractableCaseClassMethodName(name) || name.isSelectorName

// ----- DerivedTypeTrees -----------------------------------

  class SetterParamTree extends DerivedTypeTree {
    def derivedTree(sym: Symbol)(implicit ctx: ContextRenamed): tpd.TypeTree = tpd.TypeTree(sym.info.resultType)
  }

  class TypeRefTree extends DerivedTypeTree {
    def derivedTree(sym: Symbol)(implicit ctx: ContextRenamed): tpd.TypeTree = tpd.TypeTree(sym.typeRef)
  }

  class TermRefTree extends DerivedTypeTree {
    def derivedTree(sym: Symbol)(implicit ctx: ContextRenamed): tpd.Tree = tpd.ref(sym)
  }
  ...
  class DerivedFromParamTree(suffix: String) extends DerivedTypeTree {
     ...
     */
    override def ensureCompletions(implicit ctx: ContextRenamed): Unit =
     ...
     */
    def derivedTree(sym: Symbol)(implicit ctx: ContextRenamed): tpd.TypeTree = {
  ...
  def derivedTypeParam(sym: TypeSymbol)(implicit ctx: ContextRenamed): TypeDef =
...
// ----- Desugar methods -------------------------------------------------
   ...
   */
  def valDef(vdef: ValDef)(implicit ctx: ContextRenamed): Tree = {
  ...
  def makeImplicitParameters(tpts: List[Tree], forPrimaryConstructor: Boolean = false)(implicit ctx: ContextRenamed): List[ValDef] =
    for (tpt <- tpts) yield {
   ...
   */
  private def defDef(meth: DefDef, isPrimaryConstructor: Boolean = false)(implicit ctx: ContextRenamed): Tree = {
  ...
  private def addEvidenceParams(meth: DefDef, params: List[ValDef])(implicit ctx: ContextRenamed): DefDef =
    params match {
      case Nil =>
        meth
      case evidenceParams =>
  ...
  private def evidenceParams(meth: DefDef)(implicit ctx: ContextRenamed): List[ValDef] =
  ...
  def classDef(cdef: TypeDef)(implicit ctx: ContextRenamed): Tree = {
   ...
   */
  def moduleDef(mdef: ModuleDef)(implicit ctx: ContextRenamed): Tree = {
   ...
   */
  def checkNotReservedName(mdef: MemberDef)(implicit ctx: ContextRenamed): Name = {
    val name = mdef.name
   ...
   */
  def patDef(pdef: PatDef)(implicit ctx: ContextRenamed): Tree = flatTree {
   ...
   *  ValDef or DefDef.
   */
  def makePatDef(original: Tree, mods: Modifiers, pat: Tree, rhs: Tree)(implicit ctx: ContextRenamed): Tree = pat match {
  ...
  def patternVar(tree: Tree)(implicit ctx: ContextRenamed): Bind = {
    val Ident(name) = tree
    Bind(name, Ident(nme.WILDCARD)).withPos(tree.pos)
  }

  def defTree(tree: Tree)(implicit ctx: ContextRenamed): Tree = tree match {
   ...
   *      { stats; () }
   */
  def block(tree: Block)(implicit ctx: ContextRenamed): Block = tree.expr match {
    case EmptyTree =>
  ...
  /** Translate infix operation expression
    ...
    */
  def binop(left: Tree, op: Ident, right: Tree)(implicit ctx: ContextRenamed): Apply = {
   ...
   *     (t1, ..., tN)  ==>   TupleN(t1, ..., tN)
   */
  def smallTuple(tree: Tuple)(implicit ctx: ContextRenamed): Tree = {
    val ts = tree.trees
    val arity = ts.length
   ...
   *      Closure($anonfun)
   */
  def makeClosure(params: List[ValDef], body: Tree, tpt: Tree = TypeTree(), isImplicit: Boolean)(implicit ctx: ContextRenamed): Block =
    Block(
   ...
   */
  def makeCaseLambda(cases: List[CaseDef], nparams: Int = 1, unchecked: Boolean = true)(implicit ctx: ContextRenamed): Function = {
   ...
   *    }
   */
  def makeTupledFunction(params: List[ValDef], body: Tree, isGenericTuple: Boolean)(implicit ctx: ContextRenamed): Tree = {
  ...
  def makeImplicitFunction(formals: List[Type], body: Tree)(implicit ctx: ContextRenamed): Tree = {
   ...
   */
  def makeAnnotated(fullName: String, tree: Tree)(implicit ctx: ContextRenamed): Annotated = {
  ...
  private def derivedValDef(original: Tree, named: NameTree, tpt: Tree, rhs: Tree, mods: Modifiers)(implicit ctx: ContextRenamed) = {
  ...
  def apply(tree: Tree)(implicit ctx: ContextRenamed): Tree = {
   ...
   */
  def refinedTypeToClass(parent: tpd.Tree, refinements: List[Tree])(implicit ctx: ContextRenamed): TypeDef = {
 ...
 /** If tree is of the form `id` or `id: T`, return its name and type, otherwise return None.
  ...
  private object IdPattern {
    def unapply(tree: Tree)(implicit ctx: ContextRenamed): Option[VarInfo] = tree match {
   ...
   *  without duplicates
   */
  private def getVariables(tree: Tree)(implicit ctx: ContextRenamed): List[VarInfo] = {
