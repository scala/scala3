 ...
 */
// NOTE: `inline` is disabled here because to be useful you really need to inline
//       the function argument of an inlined method like `foreach` or `map` as well.
//       But scalac does not allow the `inline` modifier and `dotc` does not ipnterpret
//       `@inline` annotation on function arguments as `inline`. Once we have a bootstrap
//       we can drop all comment brackets around `inline`.
