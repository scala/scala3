...
class Mixin extends MiniPhase with SymTransformer { thisPhase =>
  ...
  override def transformTemplate(impl: Template)(implicit ctx: Context) = {
    ...
    def superCallOpt(baseCls: Symbol): List[Tree] = superCalls.get(baseCls) match {
      case Some(call) =>
        if (defn.NotRuntimeClasses.contains(baseCls) || baseCls.isBoth(NoInits, and = Trait)) Nil
        else call :: Nil
      case None =>
        if (baseCls.isBoth(NoInits, and = Trait) || defn.NoInitClasses.contains(baseCls) || defn.isFunctionClass(baseCls)) Nil
        else {
