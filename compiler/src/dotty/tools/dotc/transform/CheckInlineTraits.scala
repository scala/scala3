...
class CheckInlineTraits extends MiniPhase:
    ...
    override def transformIdent(tree: Ident)(using Context): Tree = {
      ...
      if tree.symbol == defn.SpecializedModule_apply then 
    ...
    override def transformDefDef(tree: DefDef)(using Context): Tree = {
      ...
      val isConstructorOfNonInlineType = tree.symbol.isConstructor && !tree.symbol.owner.is(Inline)
      val isRegularDefDef = !tree.symbol.isConstructor && !tree.symbol.is(Inline) 
    ...
    override def prepareForOther(tree: Tree)(using Context): Context = tree match {
      case tree: AppliedTypeTree =>
        if tree.tpt.symbol == defn.SpecializedClass && 
