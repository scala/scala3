...
class FunctionXXLForwarders extends MiniPhase with IdentityDenotTransformer {
  ...
  override def transformTemplate(impl: Template)(using Context): Template = {
    ...
    def forwarderRhs(receiver: Tree, xsTree: Tree): Tree = {
      ...
      var idx = -1
      val argss = receiver.tpe.widenDealias.paramInfoss.toLst.map(_.toLst.map { param =>
        idx += 1
        argsApply.appliedToArgs(Lst(Literal(Constant(idx)))).cast(param)
      })
    ...
    val forwarders =
      for
        tree <- if impl.symbol.owner.is(Trait) then NIL else impl.body
