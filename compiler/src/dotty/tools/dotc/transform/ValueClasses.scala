...
object ValueClasses {
  ...
  def isDerivedValueClass(sym: Symbol)(implicit ctx: ContextRenamed): Boolean = {
    val d = sym.denot
  ...
  def isMethodWithExtension(sym: Symbol)(implicit ctx: ContextRenamed): Boolean =
  ...
  def valueClassUnbox(cls: ClassSymbol)(implicit ctx: ContextRenamed): Symbol =
   ...
   */
  def u2evt(cls: ClassSymbol)(implicit ctx: ContextRenamed): Symbol =
   ...
   *  and further described in [[ExtensionMethods]].
   */
  def evt2u(cls: ClassSymbol)(implicit ctx: ContextRenamed): Symbol =
    cls.linkedClass.info.decl(nme.EVT2U).symbol

  /** The unboxed type that underlies a derived value class */
  def underlyingOfValueClass(sym: ClassSymbol)(implicit ctx: ContextRenamed): Type =
    valueClassUnbox(sym).info.resultType

  /** Whether a value class wraps itself */
  def isCyclic(cls: ClassSymbol)(implicit ctx: ContextRenamed): Boolean = {
    def recur(seen: Set[Symbol], clazz: ClassSymbol)(implicit ctx: ContextRenamed): Boolean =
      (seen contains clazz) || {
