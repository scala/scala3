...
trait FullParameterization {
   ...
   *  otherwise NoSymbol.
   */
  protected def rewiredTarget(referenced: Symbol, derived: Symbol)(implicit ctx: ContextRenamed): Symbol
   ...
   */
  protected def rewiredTarget(tree: Tree, derived: Symbol)(implicit ctx: ContextRenamed): Symbol =
    rewiredTarget(tree.symbol, derived)
   ...
   */
  def fullyParameterizedType(info: Type, clazz: ClassSymbol, abstractOverClass: Boolean = true, liftThisType: Boolean = false)(implicit ctx: ContextRenamed): Type = {
   ...
   */
  private def allInstanceTypeParams(originalDef: DefDef, abstractOverClass: Boolean)(implicit ctx: ContextRenamed): List[Symbol] =
   ...
   */
  def fullyParameterizedDef(derived: TermSymbol, originalDef: DefDef, abstractOverClass: Boolean = true)(implicit ctx: ContextRenamed): Tree =
    polyDefDef(derived, trefs => vrefss => {
       ...
       */
      def rewireTree(tree: Tree, targs: List[Tree])(implicit ctx: ContextRenamed): Tree = {
   ...
   */
  def forwarder(derived: TermSymbol, originalDef: DefDef, abstractOverClass: Boolean = true, liftThisType: Boolean = false)(implicit ctx: ContextRenamed): Tree = {
    val fun =
      ref(derived.termRef)
...
object FullParameterization {
   ...
   */
  def memberSignature(info: Type)(implicit ctx: ContextRenamed): Signature = info match {
    case info: PolyType =>
