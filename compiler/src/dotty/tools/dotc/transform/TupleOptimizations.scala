...
class TupleOptimizations extends MiniPhase with IdentityDenotTransformer {
  ...
  private def transformTupleCons(tree: tpd.Apply)(using Context): Tree = {
    val head +: tail +: Vector() = tree.args: @unchecked
    tree.tpe.widenTermRefExpr.tupleElementTypes match {
      case Some(tpes) =>
        ...
        if (size <= 5)
          ...
          val fullIterator = ref(defn.RuntimeTuples_consIterator).appliedToTermArgs(head +: tail +: Vector())
  ...
  private def transformTupleTail(tree: tpd.Apply)(using Context): Tree = {
    val Apply(_, tup +: Vector()) = tree: @unchecked
      ...
      case Some(tpes) =>
        ...
        if (size == 1)
          ...
          evalOnce(tup.asInstance(defn.ProductClass.typeRef).select(nme.productIterator)) { it =>
            Block(
              it.select(nme.next).ensureApplied +: Vector(),
  ...
  private def transformTupleConcat(tree: tpd.Apply)(using Context): Tree = {
    val Apply(_, self +: that +: Vector()) = tree: @unchecked
  ...
  private def transformTupleApply(tree: tpd.Apply)(using Context): Tree = {
    val Apply(_, tup +: nTree +: Vector()) = tree: @unchecked
  ...
  private def knownTupleFromElements(tpes: Vector[Type], elements: Vector[Tree])(using Context) = {
  ...
  private def knownTupleFromIterator(size: Int, it: Tree)(using Context): Tree =
    if (size == 0)
      ...
      val tpes = Vector.fill(size)(defn.AnyType)
      val elements = (0 until size).map(_ => it.select(nme.next)).toVector
      knownTupleFromElements(tpes, elements)
    }
    else
      // No optimization, keep:
  ...
  private def tupleSelectors(tup: Tree, size: Int)(using Context): Vector[Tree] =
    (0 until size).map(i => tup.select(nme.selectorName(i))).toVector
}
