...
class DesugarSpecializedTraits extends MiniPhase, IdentityDenotTransformer:
  ...
  private def buildImplementationClassTree(specialization: Specialization, interface: Option[ClassSymbol], cls: ClassSymbol)(using Context) = {
    ...
    val typeMap = new TypeMap: // TODO: As per tm in newInterfaceTrait
  ...
  private def transformStatements(stats1: List[Tree], cache: SpecializationCache)(using Context): (List[Tree], SpecializationCache) = {
    ...
    // TODO: Since the only change we make to stats1 => stats is inlining we could potentially arguably "undo" 
  ...
  private def checkSpecializedTraitRules(tree: Tree)(using Context) =
    ...
    // we can fix this now we are using a miniphase as we can just have this run inside e.g. transformDefDef
    // and transformBlock
    tree.foreachSubTree { 
  ...
  private var specializedTraitCache = SpecializationCache(genInterfaceSymbol = newInterfaceTrait, genImplementationSymbol = newImplementationClass)
...
class Specialization(val symbol: Symbol, val args: List[Type], val span: Span)(using Context):
  ...
  // and we need therefore to avoid name conflicts. 
  ...
  // That is probably worth doing from compression. Alternatively to avoid the conflict we can create our own 
  // NameKind for scala 3 specialized names and then use the presence of that to determine if a symbol comes
