...
class TryCatchPatterns extends MiniPhase {
  ...
  override def checkPostCondition(tree: Tree)(implicit ctx: ContextRenamed): Unit = tree match {
    case Try(_, cases, _) =>
      cases.foreach {
  ...
  private def isCatchCase(cdef: CaseDef)(implicit ctx: ContextRenamed): Boolean = cdef match {
  ...
  private def isSimpleThrowable(tp: Type)(implicit ctx: ContextRenamed): Boolean = tp.stripAnnots match {
  ...
  private def mkFallbackPatterMatchCase(patternMatchCases: List[CaseDef], pos: Position)(
      implicit ctx: ContextRenamed): Option[CaseDef] = {
