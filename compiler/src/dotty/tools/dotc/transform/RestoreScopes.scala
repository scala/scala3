...
class RestoreScopes extends MiniPhase with IdentityDenotTransformer { thisPhase =>
   ...
   */
  override def transformPackageDef(pdef: PackageDef)(implicit ctx: ContextRenamed): PackageDef = {
    pdef.stats.foreach(restoreScope)
    pdef
  }

  private def restoreScope(tree: Tree)(implicit ctx: ContextRenamed) = tree match {
