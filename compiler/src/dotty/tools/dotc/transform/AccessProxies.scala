...
abstract class AccessProxies {
   ...
   *  as a parameterer?
   */
  protected def passReceiverAsArg(accessorName: Name)(implicit ctx: ContextRenamed): Boolean = false
   ...
   */
  private def accessorDefs(cls: Symbol)(implicit ctx: ContextRenamed): Iterator[DefDef] =
  ...
  def addAccessorDefs(cls: Symbol, body: List[Tree])(implicit ctx: ContextRenamed): List[Tree] = {
  ...
  trait Insert {
    ...
    def ifNoHost(reference: RefTree)(implicit ctx: ContextRenamed): Tree = {
      assert(false, "no host found for $reference with ${reference.symbol.showLocated} from ${ctx.owner}")
      reference
    }

    /** A fresh accessor symbol */
    private def newAccessorSymbol(owner: Symbol, name: TermName, info: Type, pos: Position)(implicit ctx: ContextRenamed): TermSymbol = {
    ...
    protected def accessorSymbol(owner: Symbol, accessorName: TermName, accessorInfo: Type, accessed: Symbol)(implicit ctx: ContextRenamed): Symbol = {
    ...
    private def rewire(reference: RefTree, accessor: Symbol)(implicit ctx: ContextRenamed): Tree = {
      reference match {
    ...
    def useSetter(getterRef: Tree)(implicit ctx: ContextRenamed): Tree = getterRef match {
    ...
    /** Create an accessor unless one exists already, and replace the original
      ...
      */
    def useAccessor(reference: RefTree)(implicit ctx: ContextRenamed): Tree = {
    ...
    def accessorIfNeeded(tree: Tree)(implicit ctx: ContextRenamed): Tree = tree match {
...
object AccessProxies {
   ...
   */
  def hostForAccessorOf(accessed: Symbol)(implicit ctx: ContextRenamed): Symbol = {
