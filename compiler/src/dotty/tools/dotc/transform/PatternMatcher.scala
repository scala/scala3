...
class PatternMatcher extends MiniPhase {
  ...
  override def transformMatch(tree: Match)(implicit ctx: ContextRenamed): Tree = {
...
object PatternMatcher {
  ...
  def isPatmatGenerated(sym: Symbol)(implicit ctx: ContextRenamed): Boolean =
   ...
   */
  class Translator(resultType: Type, thisPhase: MiniPhase)(implicit ctx: ContextRenamed) {
    ...
    private def varRefCount(plan: Plan): collection.Map[Symbol, Int] = {
      object refCounter extends RefCounter {
        override val treeMap = new TreeMap {
          override def transform(tree: Tree)(implicit ctx: ContextRenamed) = tree match {
            case tree: Ident =>
