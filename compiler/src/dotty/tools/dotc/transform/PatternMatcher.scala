...
object PatternMatcher {
  ...
  class Translator(resultType: Type, thisPhase: MiniPhase)(using Context) {
    ...
    private def targetVersion(using Context): Int =
      ...
      val target = Option(ctx.settings.XuncheckedJavaOutputVersion.value).filter(_.nonEmpty)
      release.orElse(target).flatMap(_.toIntOption).getOrElse(17)
    ...
    private def isTypeSwitchSelectorType(tpe: Type): Boolean =
      ...
      !(widened.isPrimitiveValueType || widened =:= defn.NullType) && widened <:< defn.ObjectType
    ...
    private def isEnumValuePat(pat: Tree): Boolean =
      val sym = pat.symbol
      sym.isAllOf(Flags.EnumValue) || (sym.is(Flags.Enum) && sym.is(Flags.JavaStatic))
    ...
    private def collectTypeSwitchCases(scrutinee: Tree, plan: SeqPlan): List[(Tree, Plan)] = {
      ...
      if isTypeSwitchSelectorType(scrutinee.tpe) then recur(plan)
      else Nil
    }
    ...
    private def isTriggerCase(caze: (Tree, Plan)): Boolean =
      val (pat, ons) = caze
      !pat.isEmpty && (
        isNullPat(pat) ||
        isEnumValuePat(pat) ||
        typeSwitchConstLabel(pat).fold(true)(_ => planHasGuardTest(ons))
      )
    ...
    private def typeSwitchLabelKey(label: TypeSwitchLabel): Option[(String, Any)] = label match
      ...
      case StringLabel(value) => Some(("string", value))
      case IntegerLabel(value) => Some(("int", value))
    ...
    private def isJavaEnumSelector(tpe: Type): Boolean =
      val cls = tpe.widen.classSymbol
      cls.exists && cls.derivesFrom(defn.JavaEnumClass)

    private def isUnsupportedScalaEnumSelector(tpe: Type): Boolean =
      val cls = tpe.widen.classSymbol
      cls.exists && cls.is(Flags.Enum) && !cls.derivesFrom(defn.JavaEnumClass)
    ...
    private def isTypeSwitchEligible(scrutinee: Tree, plan: SeqPlan): (Boolean, List[(Tree, Plan)]) = {
      ...
      if isUnsupportedScalaEnumSelector(scrutinee.tpe) then return (false, Nil)
      ...
      if !cases.exists(isTriggerCase) then return (false, Nil)
    ...
    private def computeTypeSwitchInfo(selectorTpe: Type, cases: List[(Tree, Plan)]): TypeSwitchInfo =
      TypeSwitchInfo(
        isEnumBootstrap = isJavaEnumSelector(selectorTpe),
        selectorTpe = selectorTpe.widen,
        labels = cases.flatMap { case (pat, _) => patToLabel(pat) },
        hasNullCase = cases.exists { case (pat, _) => isNullPat(pat) }
      )

    private def typeSwitchCaseDef(pat: Tree, ons: Plan, scrutineeTpe: Type): CaseDef = pat match
      case EmptyTree => CaseDef(Underscore(scrutineeTpe.widen), EmptyTree, emit(ons))
      case Literal(Constant(null)) => CaseDef(pat, EmptyTree, emit(ons))
      case _ if typeSwitchConstLabel(pat).isDefined => CaseDef(pat, EmptyTree, emit(ons))
      case tpt => CaseDef(Typed(Underscore(tpt.tpe), tpt), EmptyTree, emit(ons))
    ...
    private def emitTypeSwitchMatch(scrutinee: Tree, cases: List[(Tree, Plan)]): Match = {
      ...
      val matchTree = Match(scrutinee, ordered.map { case (pat, ons) => typeSwitchCaseDef(pat, ons, scrutinee.tpe) })
      ...
      plan match {
        ...
        case plan: SeqPlan =>
          ...
          def maybeEmitTypeSwitch(scrutinee: Tree): Tree = {
            val (isEligible, typeSwitchCases) = isTypeSwitchEligible(scrutinee, plan)
            if isEligible then emitTypeSwitchMatch(scrutinee, typeSwitchCases)
            else maybeEmitSwitch(scrutinee)
          }
          plan.head match {
