...
class CollectNullableFields extends MiniPhase {
  ...
  private def recordUse(tree: Tree)(implicit ctx: ContextRenamed): Tree = {
    val sym = tree.symbol
  ...
  def lazyValNullables(implicit ctx: ContextRenamed): IdentityHashMap[Symbol, mutable.ListBuffer[Symbol]] = {
