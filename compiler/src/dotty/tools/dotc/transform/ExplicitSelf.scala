...
class ExplicitSelf extends MiniPhase {
  ...
  override def transformIdent(tree: Ident)(implicit ctx: ContextRenamed): Tree = tree.tpe match {
    case tp: ThisType =>
  ...
  override def transformSelect(tree: Select)(implicit ctx: ContextRenamed): Tree = tree match {
