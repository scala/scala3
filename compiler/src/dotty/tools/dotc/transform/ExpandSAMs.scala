...
class ExpandSAMs extends MiniPhase {
  ...
  private def toPartialFunction(tree: Block, tpe: Type)(using Context): Tree = {
    ...
    val closureDef(anon @ DefDef(_, _, Lst(Lst(param)), _, _)) = tree
    anon.rhs match {
      case PartialFunctionRHS(pf) =>
        ...
        def isDefinedAtRhs(paramRefss: Lst[Lst[Tree]])(using Context) = {
        ...
        def applyOrElseRhs(paramRefss: Lst[Lst[Tree]])(using Context) = {
