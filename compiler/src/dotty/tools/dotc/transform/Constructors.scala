...
class Constructors extends MiniPhase with IdentityDenotTransformer { thisPhase =>
  ...
  private def markUsedPrivateSymbols(tree: RefTree)(implicit ctx: ContextRenamed): Unit = {
  ...
  override def transformIdent(tree: tpd.Ident)(implicit ctx: ContextRenamed): tpd.Tree = {
   ...
   */
  private def noDirectRefsFrom(tree: Tree)(implicit ctx: ContextRenamed) =
    tree.isDef && tree.symbol.isClass

  /** Class members that can be eliminated if referenced only from their own
   *  constructor.
   */
  private def mightBeDropped(sym: Symbol)(implicit ctx: ContextRenamed) =
  ...
  override def transformTemplate(tree: Template)(implicit ctx: ContextRenamed): Tree = {
    ...
    object intoConstr extends TreeMap {
      ...
      def apply(tree: Tree, prevOwner: Symbol)(implicit ctx: ContextRenamed): Tree = {
