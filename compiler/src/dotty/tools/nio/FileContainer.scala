...
object FileContainer:
  ...
  def canGetFromFile(file: File): Boolean =
    file.extension.isJar || file.extension.isZip
  ...
  private[nio] def getPath(path: String, extension: FileExtension | Null, separator: Char): String =
    if separator == FileSystemEntry.separator then
      getPath(path, extension)
    else
      getPath(path.replace(separator, FileSystemEntry.separator), extension)

  /** Utility for implementers. Gets the real path given path+ext, avoiding unnecessary work and allocations if possible. */
  private[nio] def getPath(path: String, extension: FileExtension | Null): String =
    if extension == null then path
...
abstract class FileContainer extends FileSystemEntry:
  ...
  def getFile(path: String, extension: FileExtension | Null = null, separator: Char = FileSystemEntry.separator): Option[File]
  ...
  def getOrCreateFile(path: String, extension: FileExtension | Null = null, separator: Char = FileSystemEntry.separator): File
