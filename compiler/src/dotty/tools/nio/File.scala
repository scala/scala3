...
object File:
  ...
  def createThenReplaceOnDisk(path: String)(action: File => Unit): Unit =
    val temp = createTemporaryOnDisk("partial", FileExtension.Empty)
    action(temp)
...
abstract class File extends FileSystemEntry:
  ...
  def getSiblingWithExtension(ext: FileExtension): Option[File] =
    parent.getFile(nameWithoutExtension, ext)

  /** Gets or creates a sibling to this file with the same name but a different extension. */
  def getOrCreateSiblingWithExtension(ext: FileExtension): File =
    parent.getOrCreateFile(nameWithoutExtension, ext)
