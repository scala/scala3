...
private final class ZipContainer private(val underlying: File, version: String, compressionLevel: Int, manifest: Map[String, String]) extends FileContainer, ZipEntryContainerBase(""):
  ...
  lazy val archive: Option[ZipFile] = Option.when(underlying.size() > 0)(
    if underlying.extension.isJar then
      new JarFile(
  ...
  override def close(): Boolean = {
    if modified then
      val outputManifest = archive match
        ...
        case Some(jf) => None
        case None if underlying.extension.isJar =>
...
private trait ZipEntryContainerBase(entryPath: String) extends FileContainer:
  ...
  override def getFile(path: String, extension: FileExtension | Null = null, separator: Char = FileSystemEntry.separator): Option[File] =
    container.rawEntries.get(entryPath + FileContainer.getPath(path, extension, separator)) match
  ...
  override def getContainer(path: String, separator: Char = FileSystemEntry.separator): Option[FileContainer] =
    container.rawEntries.get(entryPath + FileContainer.getPath(path, null, separator) + "/") match
  ...
  override def getOrCreateFile(path: String, extension: FileExtension | Null = null, separator: Char = FileSystemEntry.separator): File = {
    val entryName = entryPath + FileContainer.getPath(path, extension, separator)
    container.rawEntries.get(entryName) match
      case Some(f: File) => f
      case _ =>
        container.modified = true
        val entry =
          if container.underlying.extension.isJar
        ...
        container.rawEntries(entryName) = result
        result
  }

  override def getOrCreateContainer(path: String, separator: Char = FileSystemEntry.separator): FileContainer = {
    val entryName = entryPath + FileContainer.getPath(path, null, separator) + "/"
    container.rawEntries.get(entryName) match
      case Some(c: FileContainer) => c
      case _ =>
        container.modified = true
        val entry =
          if container.underlying.extension.isJar
