...
private final class DiskDirectory(private val underlying: JPath) extends FileContainer:
  ...
  override def entries: Iterable[FileSystemEntry] =
    ...
    val files =
      try stream.toArray(n => new Array[JPath](n))
      finally stream.close()
    files.map(p => if JFiles.isDirectory(p) then new DiskDirectory(p) else new DiskFile(p))
  ...
  override def getContainer(path: String, separator: Char = FileSystemEntry.separator): Option[FileContainer] =
    val jpath = underlying.resolve(FileContainer.getPath(path, null, separator)).normalize()
    Option.when(JFiles.isDirectory(jpath))(new DiskDirectory(jpath))
