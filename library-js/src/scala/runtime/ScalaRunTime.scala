...
object ScalaRunTime {
  ...
  /** Returns a hash code for a case class via [[scala.util.hashing.MurmurHash3.productHash]],
   *  mixing the hash of its `productPrefix` with those of its elements.
   *
   ...
   *  @param xs the array to wrap
   *  @return an `ArraySeq.ofRef` backed by `xs`, the shared empty `ArraySeq` if `xs` is empty,
   *          or `null` if `xs` is `null`
   */
