...
object BoxesRunTime {
   ...
   *
   *  Compares for equality by first trying a cheap fast path and, if that
   *  does not settle the question, falling back to the full dispatch of
   *  [[equals2]], which handles `Long`s, characters, `ScalaNumber`s and
   *  user-defined `equals` methods.
   *
   *  Which fast path is used is chosen at link time by module kind. For Wasm
   *  modules it is a reference comparison, with a `Double` additionally
   *  required to equal itself, which rejects `NaN`. Otherwise it is
   *  JavaScript's strict equality operator (`===`); because boxed numbers are
   *  primitive JavaScript numbers on Scala.js, that path also equates
   *  numerically equal values of different boxed types (a boxed `Int` `1`
   *  equals a boxed `Double` `1.0`), which the reference comparison instead
   *  leaves to [[equals2]]. Identical references and two `null`s are settled
   *  by either. The two paths are deliberately aligned and give the same
   *  answers; they differ only in how much they settle before [[equals2]] is
   *  consulted. `NaN` fails both, and is also unequal to itself in
   *  [[equals2]], preserving `Double.NaN != Double.NaN`.
   *
